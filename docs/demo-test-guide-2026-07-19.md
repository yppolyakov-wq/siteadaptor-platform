...
**После пересева (2026-08-19):** демо-тенант открывается сразу кабинетом —
кит помечает мастер настройки пройденным, плашки готовности на главной нет.
Сам мастер остаётся доступен по `/dashboard/setup/` (в т.ч. `?step=<шаг>`).
