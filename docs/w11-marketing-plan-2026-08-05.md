...
- **W11-4 ✅** (2026-08-05): акция на услугу/номер/комбо из UI. PromotionForm
  += FK-цели service/stay_unit/combo (лейблы Leistung/Zimmer/Kombi; гейт по
  модулям — неактивный модуль убирает поле; tenant=None → fail-open как прочие
  гейты формы); help-тексты честно описывают семантику PL (авто-цена услуги /
  прайс-кандидат номера). promotion_create префиллит цель из GET (?service= и
  т.д.). Карточки Angebote несут «% Aktion» (ManagedSellable.promo_param по
  kind; event без цели). Движок PL не тронут. 3 msgid × 5 .po; 4 замка
  test_w11_targets.
- Дальше: W11-5 (Website-свод — разведка site.html консумеров ДО кода).
