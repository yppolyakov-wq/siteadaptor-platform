...
## 7. Открытые под-решения (для владельца)
- **S-1 — язык кабинета/админки:** (a) кабинет владельца тоже мультиязычный (L4 покрывает и его хром) *(реком.,
  раз «языки админки»)* · (b) только витрина сейчас, кабинет DE, админка-язык позже.
- **S-2 — правовое i18n (L5):** (a) i18n-JSONField на `Tenant` (`{locale:text}`, дёшево, как контент) *(реком.)* ·
  (b) отдельная модель `LegalDoc(tenant, kind, locale, text)` (гибче для версий/аудита, крупнее).
- **S-3 — стартовый набор языков:** сейчас DE+EN; какие следующие в реестр (`settings.LANGUAGES`) планировать
  (FR/IT/TR/PL/RU…)? Влияет на объём засева L3/L5 и `.po` L4 (технически — «добавить локаль» дёшево).
