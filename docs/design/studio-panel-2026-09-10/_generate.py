...
<style>
  ...
  .hd .k { font-size:10px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; color:%(IND)s; }
  .hd h2 { margin:3px 0 0; font-size:17px; color:%(G900)s; letter-spacing:-.01em; }
  .hd p { margin:4px 0 0; font-size:11.5px; color:%(G500)s; line-height:1.5; max-width:62ch; }

  /* ── панель ── */
  .pane { width:380px; background:#fff; border:1px solid %(LINE)s; border-radius:14px;
  ...
  .pane-hd .t { font-size:13px; font-weight:700; color:%(G700)s; }
  .pane-hd .sp { flex:1; }
  .pane-hd .x { color:%(G400)s; font-size:13px; }
  .pane-hd .lang { border:1px solid #d1d5db; border-radius:7px; padding:1px 6px;
                   font-size:10.5px; color:%(G500)s; }
  .pane-bd { padding:12px; }

  /* ── СЕГОДНЯ: карточка-фиксет как сейчас ── */
  .fs { border:1px solid %(LINE)s; border-radius:16px; padding:14px; background:#fff; margin-bottom:12px; }
  .fs > .lg { font-size:14px; font-weight:700; color:%(G900)s; margin-bottom:10px; }
  .old-lbl { font-size:13px; font-weight:600; color:%(G900)s; margin:0 0 6px; }
  .old-sub { font-size:11px; color:%(G500)s; margin:0 0 6px; }
  .old-sel { border:1px solid #d1d5db; border-radius:8px; padding:6px 8px; font-size:12.5px;
             color:%(G500)s; background:#fff; display:flex; justify-content:space-between; }
  .clip { display:block; width:64px; font-size:10px; line-height:1.2; color:%(G500)s;
          height:24px; overflow:hidden; margin-top:4px; }
  .tiles-flex { display:flex; flex-wrap:wrap; gap:8px; }
  .tile { border:1px solid %(LINE)s; border-radius:8px; padding:6px; background:#fff; }
  .tile.on { border-color:%(IND)s; box-shadow:0 0 0 2px #c7d2fe; }
  .thumb { width:56px; height:40px; border-radius:4px; background:#f3f4f6;
           box-shadow:inset 0 0 0 1px %(LINE)s; display:block; }
  .ax2 { display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:8px; }
  .ax2 .f { font-size:11px; color:%(G500)s; }
  .ax2 .c { margin-top:2px; border:1px solid #d1d5db; border-radius:5px; padding:4px 6px;
            font-size:11px; color:%(G500)s; background:#fff; }
  .ax2 .ph { color:#b6bcc6; }

  /* ── ПРЕДЛОЖЕНИЕ: группа оси ── */
  .grp { border:1px solid %(LINE)s; border-radius:10px; background:#fff; margin-bottom:8px; }
  .grp-hd { display:flex; align-items:center; gap:6px; padding:8px 10px; }
  .grp-hd .ch { color:%(G400)s; font-size:10px; width:9px; }
  .grp-hd .ax { font-size:9px; font-weight:800; letter-spacing:.07em; color:%(IND)s;
                border:1px solid #dfe3ff; background:%(IND_BG)s; border-radius:4px; padding:1px 4px; }
  .grp-hd .nm { font-size:12.5px; font-weight:700; color:%(G900)s; }
  .grp-hd .val { margin-left:auto; font-size:11px; color:%(G400)s; max-width:44%%;
  ...
  .row .lb { display:block; font-size:11.5px; font-weight:600; color:%(G700)s; margin-bottom:4px; }
  .row .hint { display:block; font-size:10.5px; color:%(G400)s; margin:-2px 0 4px; line-height:1.35; }
  .ctrl { width:100%%; box-sizing:border-box; height:32px; border:1px solid #d1d5db;
          border-radius:8px; padding:0 9px; font-size:12.5px; color:%(G900)s; background:#fff;
          display:flex; align-items:center; }
  .ctrl .cv { margin-left:auto; color:%(G400)s; font-size:11px; }
  ...
  .seg span { flex:1; display:flex; align-items:center; justify-content:center;
              font-size:12px; color:%(G500)s; }
  .seg span.on { background:%(IND_BG)s; color:%(IND_TX)s; font-weight:700; }

  /* плитки — GRID, подпись переносится */
  .tiles { display:grid; grid-template-columns:repeat(auto-fill, minmax(88px,1fr)); gap:8px; }
  .tl { border:1px solid %(LINE)s; border-radius:9px; padding:6px; background:#fff; }
  .tl.on { border-color:%(IND)s; box-shadow:0 0 0 2px #c7d2fe; }
  .tl.off { opacity:.45; }
  .tl .th { width:100%%; height:46px; border-radius:5px; background:#f3f4f6;
            box-shadow:inset 0 0 0 1px %(LINE)s; display:block; }
  .tl .cap { display:block; margin-top:5px; font-size:10.5px; line-height:1.25; color:%(G500)s;
             overflow-wrap:anywhere; hyphens:auto; }
  .tl.on .cap { color:%(IND_TX)s; font-weight:600; }

  .scope { display:flex; gap:4px; }
  .scope span { border:1px solid #d1d5db; border-radius:999px; padding:1px 8px;
                font-size:10.5px; color:%(G500)s; }
  .scope span.on { border-color:%(IND)s; background:%(IND_BG)s; color:%(IND_TX)s; font-weight:600; }

  .note { margin-top:8px; border:1px solid #e8eaee; background:#fafbfc; border-radius:8px;
          padding:8px 9px; font-size:10.5px; color:%(G500)s; line-height:1.45; }
  .note b { color:%(G700)s; }
  .note a { color:%(IND)s; }
  ...
  .foot .tx { font-size:11.5px; color:%(G700)s; }
  .foot .tx small { display:block; color:%(G400)s; font-size:10.5px; }
  .foot .go { margin-left:auto; font-size:11px; color:%(IND)s; }
  ...
  .cal .tx { font-size:11.5px; color:%(G700)s; line-height:1.45; }
  .cal .tx b { color:%(G900)s; }
  ...
  .mtx th { text-align:left; font-size:10px; font-weight:700; color:%(G500)s;
            padding:5px 6px; border-bottom:1px solid %(LINE)s; }
  .mtx td { padding:4px 6px; border-bottom:1px solid #f2f3f5; color:%(G700)s; }
  .mtx td.y { color:#15803d; font-weight:700; }
  .mtx td.n { color:%(G400)s; }
  .mtx td.w { color:#b45309; font-weight:700; }
  .mtx td.x { color:#b91c1c; font-weight:700; }
  .mtx tr.sec td { background:#f8f9fb; font-weight:700; color:%(G900)s; font-size:10.5px; }
  .lgnd { font-size:10.5px; color:%(G500)s; margin-top:8px; line-height:1.6; }

  .spec { display:flex; gap:14px; align-items:flex-start; margin-bottom:14px; }
  .spec .demo { flex:0 0 200px; }
  .spec .txt { font-size:11.5px; color:%(G700)s; line-height:1.5; }
  .spec .txt b { color:%(G900)s; display:block; margin-bottom:2px; font-size:12px; }
  .spec .txt code { font-size:10.5px; background:#f3f4f6; border-radius:3px; padding:0 3px; color:%(G500)s; }
</style>
""" % dict(IND=IND, IND_BG=IND_BG, IND_TX=IND_TX, LINE=LINE, G400=G400, G500=G500, G700=G700, G900=G900)
...
def head(kicker: str, title: str, sub: str) -> str:
    return (
        f'<div class="hd"><div class="k">{kicker}</div><h2>{title}</h2><p>{sub}</p></div>'
    )
...
def _old_tile(cap: str, on: bool = False, clip: bool = False) -> str:
    ...
    return f'<div class="{cls}"><span class="thumb"></span><span class="{capcls}">{cap}</span></div>'


def heute_aktionen() -> str:
    comp = "".join(
        _old_tile(c, on=(c == "С обложкой"), clip=True)
        for c in ["Стандарт (сетка)", "С обложкой", "Прайс-лист",
                  "Полки (подкатегориями лентами)", "Вкладки (подкатегориями вкладками)", "Витрина",
                  "Навигатор", "Журнал", "Компактно"]
    )
    grid = "".join(
        _old_tile(c, on=(c == "3 в строке"), clip=True)
        for c in ["Стандартный", "Список", "2 в строке", "3 в строке",
                  "4 в строке", "5 в ряд", "6 в ряд", "Галерея"]
    )
    panel = f"""
    <div class="pane">
      ...
      <div class="pane-bd">
        ...
        <div class="fs">
          ...
          <div class="ax2">
            <div><div class="f">Вывод</div><div class="c">Слайдер ▾</div></div>
    ...
    calls = (
        cal("1", "<b>Заголовок карточки дублирует настройку.</b> «Шаблон страницы» → и сразу «Страница акций: шаблон». Рамка не несёт смысла.")
        + cal("2", "<b>Подписи обрезаны.</b> «Полки (подкатегор…», «Вкладки (подкатегор…» — у плитки жёсткая ширина подписи <code>w-16</code> = 64 px и <code>line-clamp-2</code>. По-немецки влезает, по-русски нет.")
        + cal("3", "<b>Два набора плиток одного вида означают разное.</b> Сверху — композиция страницы, снизу — пресет сетки. Мини-макеты нарисованы одинаково.")
        + cal("4", "<b>Одна ось разорвана надвое.</b> Плитки «Акции: сетка» и пять полей ниже — это ОДНА ось «сетка», но выглядят как два раздела: крупные плитки и мелкие серые поля в две колонки.")
        + cal("5", "<b>Значение по умолчанию выглядит выключенным.</b> «все», «Стандартный», «выкл» — это <code>placeholder</code> пустых числовых полей. Серый текст читается как «недоступно».")
        + cal("6", "<b>Замер:</b> 4 настройки = 973 px. Плитки композиции 289 px · сетка 296 px · форма карточки 332 px. Почти три экрана прокрутки.")
    )
    return doc(
        f"""<div class="bd">
      {head("Сегодня · замер на стенде", "Панель на /aktionen/ — как есть",
...
def heute_katalog() -> str:
    forms = "".join(
        ...
        for c in ["Стандартный", "Текст на фото", "Компактная строка",
                  "Ценник", "Полочный ценник", "Лукбук", "Плитка предложения"]
    )
    panel = f"""
    <div class="pane">
    ...
    calls = (
        cal("1", "<b>Заголовок карточки обманывает.</b> Карточка называется «Шаблон страницы», а внутри — <b>форма карточки</b>. Настоящий шаблон страницы (<code>catalog_page_style</code>) лежит НИЖЕ, во второй карточке «Целевые страницы».")
        + cal("2", "<b>Три оси разложены по двум карточкам с чужими именами.</b> Композиция и сетка — в «Целевых страницах», форма карточки — в «Шаблоне страницы». Ни одна карточка не называет ось.")
        + cal("3", "<b>Ещё один набор одинаковых плиток.</b> Владелец только что видел такие же выше — там они значили форму карточки, здесь плотность сетки.")
        + cal("4", "<b>Перелив 30 px по горизонтали</b> — в русской локали; в немецкой 0. Отсюда полоса прокрутки внизу панели.")
        + cal("5", "<b>Ритм рваный.</b> Замер строк: форма карточки 319 px · сетка 405 px · шаблон 302 px — и между ними «Сортировка» 24 px и «Показать фильтры» 16 px. Самые частые настройки выглядят как случайные строчки.")
        + cal("6", "<b>Пять шаблонов строки в одной панели:</b> подпись сверху + широкий контрол · флекс «подпись—селект» · мелкая инлайн-подпись · чекбокс · плитки. Три разных размера поля.")
    )
    return doc(
        f"""<div class="bd">
      {head("Сегодня · замер на стенде", "Панель на /sortiment/ — как есть",
...
def tiles(items, cols_hint="") -> str:
    ...
    for cap, state in items:
        ...
        out.append(f'<div class="{cls}"><span class="th"></span><span class="cap">{cap}</span></div>')
...
FOOT = (
    ...
    '<span class="tx">Оформление сайта<small>Look «Проспект» · Barlow Condensed</small></span>'
...
def main_board() -> str:
    grid_body = (
        row("Тип вывода", '<div class="seg"><span class="on">Сетка</span><span>Слайдер</span></div>')
        + row("Плотность", tiles([("3 в строке", "on"), ("4 в строке", ""), ("2 в строке", ""),
                                  ("Список", ""), ("Прайс-лист", ""), ("Галерея", "")]))
    ...
    body = (
        ...
        + grp("—", "Содержание страницы", "Группировка · 1", open_=False)
    )
    return doc(
        f"""<div class="bd">
      {head("Предложение · основной макет", "Панель: страница со списком",
            "Одна структура на всех 30 типах: <b>A Композиция · B Сетка · C Карточка · Содержание</b>. "
            "Группы свёрнуты, значение — в заголовке; открыта та, что правят. "
            "Четыре настройки помещаются в один экран вместо 973 px прокрутки.")}
      {pane("Эта страница: Акции", body, "")}
      <div style="width:380px; margin-top:14px">
        {cal("✓", "<b>Ось названа.</b> Бейдж A/B/C и имя группы говорят, что именно настраивается — вместо «Шаблон страницы», под которым лежит форма карточки.", True)}
        {cal("✓", "<b>Сетка снова одна.</b> Плотность и параметры — в одной группе, одним размером контролов.", True)}
        {cal("✓", "<b>Значение видно свёрнутым.</b> «Сетка · 3 в строке» в заголовке; листать, чтобы вспомнить выбор, не нужно.", True)}
        {cal("✓", "<b>«все» и «Стандартный» — это значения,</b> а не серые плейсхолдеры пустого поля: обычный селект с явной опцией.", True)}
      </div>
    </div>"""
    )
...
def kategorie() -> str:
    comp = tiles([("Стандарт (сетка)", ""), ("С обложкой", "on"), ("Сначала сеты и меню", "off"),
                  ("Полки (подкатегориями лентами)", "off"), ("Вкладки (подкатегориями вкладками)", "off"),
                  ("Витрина", ""), ("Навигатор", ""), ("Журнал", ""), ("Мозаика", ""), ("Компактно", "")])
    body = (
        grp("A", "Шаблон страницы", "С обложкой · только здесь", open_=True, scope="own",
            body=comp + '<div class="note"><b>Полки</b>, <b>Вкладки</b> и <b>Сначала сеты</b> '
        ...
        + grp("C", "Карточка", "Наследует сайт · Лукбук", open_=False)
        + grp("—", "Содержание страницы", "Сортировка · Фильтры · Подкатегории", open_=False)
    )
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Панель: категория — охват и гейт",
            "Пилюля «Для всех / Только здесь» переезжает <b>в заголовок группы</b>, к значению, "
            "а не висит под плитками. Недоступные композиции показаны погашенными с причиной — "
...
def detail() -> str:
    body = (
        grp("A", "Шаблон страницы", "Вкладки", open_=True,
            body=row("Аufbau страницы", sel("Вкладки"),
                     "Как разложены секции детали: подряд · вкладками · во всю ширину."))
        + grp("B", "Вывод", "Похожие: 4 в строке", open_=True,
              body=row("Похожие товары", sel("4 в строке"),
                       "У детали одна поверхность вывода — полоса «Похожие». "
                       "Остальные параметры сетки для неё сегодня не хранятся (Д: частичная ось).")
              )
        + grp("C", "Карточка", "Полочный ценник · только этот товар", open_=False)
        + grp("—", "Содержание страницы", "Секции · 6 из 8", open_=False)
    )
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Панель: страница товара",
      ...
      {pane("Эта страница: Страница товара", body)}
    </div>"""
    )
...
def textseite() -> str:
    body = (
        grp("—", "Содержание страницы", "Ширина текста · Обычная", open_=True,
    ...
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Панель: текстовая страница",
...
def startseite() -> str:
    body = (
        grp("A", "Первый экран", "Сплит с фото", open_=False)
        + grp("—", "Блоки главной", "9 включено · порядок", open_=True,
    ...
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Панель: главная",
...
def telefon() -> str:
    body = (
        grp("A", "Шаблон страницы", "С обложкой", open_=False)
        + grp("B", "Вывод", "Сетка · 3 в строке", open_=True,
              body=row("Тип вывода", '<div class="seg"><span class="on">Сетка</span><span>Слайдер</span></div>')
              + row("Плотность", tiles([("3 в строке", "on"), ("4 в строке", ""), ("Список", "")])))
        + grp("C", "Карточка", "Полочный ценник", open_=False)
    ...
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Телефон: тот же порядок групп",
...
def gestaltung() -> str:
    left = f"""<div class="pane" style="width:340px">
      ...
      <div class="pane-bd">
        ...
        <div class="foot"><span class="sw"></span>
          <span class="tx">Оформление сайта<small>Look «Проспект» · Barlow Condensed · зелёный</small></span>
          <span class="go">Открыть →</span></div>
      </div></div>"""
    right = f"""<div style="flex:1">
      {cal("1", "<b>Шрифт, цвет, тема, скругления — остаются на «Оформлении сайта».</b> Это глобальные настройки; смешивать их с постраничными и значит вернуть кашу. Их вынесли туда осознанно (STU-12g, решение владельца 1B) и закрепили замком.", True)}
      {cal("2", "<b>Что добавляем:</b> живая сводка внизу панели — имя Look, шрифт, кружок акцента — и переход. Владелец видит, чем оформлен сайт, не уходя со страницы.", True)}
      {cal("3", "<b>Плитки рисуются палитрой сайта.</b> Сегодня мини-макеты жёстко серо-янтарно-красные (<code>bg-amber-200</code>, <code>bg-red-500</code>): плитка «Проспект» красная, даже если акцент сайта зелёный. Предлагаю рисовать их переменными витрины и подписывать шрифтом сайта.")}
      {cal("4", "<b>Развилка Р-4:</b> «Стартовые пакеты» на экране оформления одним кликом переписывают композицию, сетку и форму карточки — то есть настройки, которые владелец только что выставил в панели. Предупреждать? Или запретить пакету трогать то, что тронуто руками (правило LAY-5)?")}
    ...
    return doc(
        f"""<div class="bd">
      {head("Предложение · согласование с палитрой", "Где живут шрифты и палитра",
...
def komponenten() -> str:
    ...
    demo_tiles = f'<div style="width:200px">{tiles([("Полки (подкатегориями лентами)", "on"), ("Витрина", ""), ("Журнал", "")])}</div>'
    demo_scope = ('<div style="width:200px"><div class="scope"><span class="on">Для всех</span>'
    ...
    items = [
        ...
        (demo_tiles, "Плитки",
         ...
         "Подпись переносится (<code>overflow-wrap:anywhere</code>) вместо обрезки по <code>w-16</code>: "
         "это и чинит перелив 30 px в русской локали."),
        (demo_seg, "Тип вывода",
        ...
        (demo_scope, "Охват",
         "Переезжает в заголовок группы, к значению («С обложкой · только здесь»). "
         "Точка на «Только здесь» = у объекта своё значение."),
    ]
    specs = "".join(
        f'<div class="spec"><div class="demo">{d}</div>'
        f'<div class="txt"><b>{t}</b>{x}</div></div>'
        for d, t, x in items
    )
    return doc(
        f"""<div class="bd">
      {head("Предложение", "Компоненты панели",
            "Пять компонентов закрывают все 40 строк реестра. Значения сверены с кабинетом "
            "после редизайна B: карточки <code>rounded-2xl</code>, индиго #4f46e5, панель 380 px.")}
      {specs}
...
def matrix() -> str:
    rows = [
        ...
        ("", "Категория", "y", "y", "y", "сорт · фильтры · подкатегории"),
        ...
        ("", "Акция", "y", "n", "y", "—"),
        ("sec", "Прочее", "", "", "", ""),
        ("", "Главная", "y", "n", "n", "состав блоков"),
        ("", "Finder", "n", "x", "y", "ширина текста"),
        ("", "Корзина", "n", "n", "y", "«Passt dazu»"),
        ("", "Команда · Галерея", "n", "n", "n", "сетка — из блока главной"),
    ...
    for kind, name, a, b, c, d in rows:
        if kind == "sec":
            ...
            continue
        def cell(v):
            return {"y": '<td class="y">✓</td>', "n": '<td class="n">—</td>',
                    "w": '<td class="w">частично</td>', "x": '<td class="x">НЕТ</td>'}[v]
        body += f"<tr><td>{name}</td>{cell(a)}{cell(b)}{cell(c)}<td>{d}</td></tr>"
    return doc(
        f"""<div class="bd">
      {head("Разведка · на согласование", "Какая ось действует на каком типе страницы",
      ...
      <table class="mtx">
        <tr><th style="width:30%">Тип страницы</th><th>A Композиция</th><th>B Сетка</th>
...
ARTBOARDS = {
    "HeuteAktionen": (heute_aktionen, 980, 1110, "Сегодня: /aktionen/"),
    "HeuteKatalog": (heute_katalog, 980, 1060, "Сегодня: /sortiment/"),
    ...
    "Gestaltung": (gestaltung, 1020, 460, "Шрифты и палитра"),
    "Komponenten": (komponenten, 780, 880, "Компоненты"),
    "Matrix": (matrix, 900, 880, "Матрица осей"),
}

PAGE_OF = {
    "HeuteAktionen": "heute", "HeuteKatalog": "heute",
    "Main": "vorschlag", "Kategorie": "vorschlag", "Detail": "vorschlag",
    "TextSeite": "vorschlag", "Startseite": "vorschlag", "Telefon": "vorschlag",
    "Gestaltung": "vorschlag",
    "Komponenten": "system", "Matrix": "system",
}

LAYOUT = {
    "HeuteAktionen": (0, 0), "HeuteKatalog": (1080, 0),
    "Main": (0, 0), "Kategorie": (560, 0), "Detail": (1120, 0), "TextSeite": (1680, 0),
    "Startseite": (0, 1290), "Telefon": (560, 1290), "Gestaltung": (1120, 1290),
    "Komponenten": (0, 0), "Matrix": (880, 0),
}

NOTES = [
    {"id": "note-heute", "x": 0, "y": -150, "w": 700, "page": "heute",
    ...
    {"id": "note-vorschlag", "x": 0, "y": -170, "w": 760, "page": "vorschlag",
    ...
    {"id": "note-system", "x": 0, "y": -150, "w": 700, "page": "system",
...
def main() -> None:
    ...
    for name, (fn, w, h, title) in ARTBOARDS.items():
        ...
        x, y = LAYOUT[name]
        boards.append({"file": f"{name}.dc.html", "x": x, "y": y, "w": w, "h": h,
                       "title": title, "page": PAGE_OF[name]})
