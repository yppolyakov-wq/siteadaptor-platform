...
<style>
  ...
  .grp-hd .val { margin-left:auto; font-size:11px; color:%(G400)s; max-width:54%%;
  ...
  .spec .txt > b:first-child { color:%(G900)s; display:block; margin-bottom:2px; font-size:12px; }
  .spec .txt b { color:%(G900)s; }
...
def heute_aktionen() -> str:
    comp = "".join(
        ...
        for c in ["Стандарт (сетка)", "С обложкой", "Прайс-лист",
                  "Полки (подкатегории лентами)", "Вкладки (подкатегории вкладками)", "Витрина",
                  "Навигатор", "Журнал", "Компактно"]
    )
    promo_forms = "".join(
        _old_tile(c, on=(c == "Цена сверху"), clip=True)
        for c in ["Стандартный", "Цена сверху", "Полочный ценник",
                  "Лукбук", "Плитка предложения", "Купон", "Кольцо отсчёта"]
    )
    grid = "".join(
    ...
    calls = (
        ...
        + cal("2", "<b>Подписи обрезаны.</b> «Полки (подкатегори…», «Вкладки (подкатегор…» — у плитки жёсткая ширина подписи <code>w-16</code> = 64 px и <code>line-clamp-2</code>. По-немецки влезает, по-русски нет.")
...
def kategorie() -> str:
    comp = tiles([("Стандарт (сетка)", ""), ("С обложкой", "on"), ("Сначала сеты и меню", "off"),
                  ("Полки (подкатегории лентами)", "off"), ("Вкладки (подкатегории вкладками)", "off"),
                  ("Витрина", ""), ("Навигатор", ""), ("Журнал", ""), ("Мозаика", ""), ("Компактно", "")])
    body = (
        grp("A", "Шаблон страницы", "С обложкой · здесь", open_=True, scope="own",
            body=comp + '<div class="note"><b>Полки</b>, <b>Вкладки</b> и <b>Сначала сеты и меню</b> '
...
def detail() -> str:
    body = (
        grp("A", "Шаблон страницы", "Вкладки", open_=True,
            body=row("Раскладка страницы", sel("Вкладки"),
...
def textseite() -> str:
    body = (
        grp("—", "Содержание страницы", "Обычная ширина", open_=True,
...
def startseite() -> str:
    body = (
        ...
        + grp("—", "Блоки главной", "Состав и порядок", open_=True,
    ...
    for kind, name, a, b, c, d in rows:
        ...
        def cell(v):
            return {"y": '<td class="y">✓</td>', "n": '<td class="n">—</td>',
                    "w": '<td class="w">частично</td>', "x": '<td class="x">НЕТ</td>',
                    "b": '<td style="color:#6b7280">в блоке</td>'}[v]
