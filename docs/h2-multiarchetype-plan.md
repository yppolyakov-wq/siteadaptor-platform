...
| **H2-4** | Схема слайдера: `archetypes[key].slides:[{image,title?,link?}]` в `normalize` (НЕ терять на save!) | нет | нет (CI) |
| **H2-5** | Редактор слайдера (расширить cover-форму `sections_view`, реюз `_upload_cover_gallery`) | нет | **да** |
| **H2-6** | Единая страница архетипа: `_archetype_cover` (описание) + `_archetype_slider` (карусель) над ITEMS; вставить в stays/events/booking лендинги (products — эталон) | нет | **да** |
