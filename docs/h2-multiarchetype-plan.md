...
> **СТАТУС 2026-06-30: ФУНКЦИОНАЛЬНОЕ ЯДРО H2 ГОТОВО.** D-HOME — агрегация главной (H2-2) + контроль в билдере
> (секции архетипов в списке, гейтинг H0). D-MENU — авто-меню из архетипов (фолбэк, работает) + богатый лендинг
> архетипа = описание+слайдер+товары (H2-6, браузерно). Остаток (H2-3 бейдж/тумблер, H2-7 rich-карточки) —
> КОСМЕТИКА, отложено. Дальше по плану — H3 (демоданные) / H4 (онбординг).
...
| **H2-3** ⏸ | (полировка, отложено) Билдер УЖЕ даёт контроль: секции архетипов в списке с чекбоксом/порядком (гейтинг H0). Осталось косметика: архетип-бейдж + тумблер «по реестру/вручную». | нет | да |
...
| **H2-7** ⏸ | Авто-меню из активных архетипов УЖЕ работает (`_normalize_menus`→nav фолбэк; тест `test_auto_menu`). Осталось косметика: rich-карточки в дропдауне (link уже работает). | нет | да |
