...
## 0d. Решения владельца 2026-07-13 (границы AB6.2) + разбивка

**Решения (AskUserQuestion):** (1) пикеры вида в v1 — **Главная + Товар/услуга +
Категория + Тексты** (корзина — отдельным шагом позже, это функц. страница оплаты/
доставки, не шаблон). (2) стиль выбора — **и то, и то**: «весь образ архетипа одним
кликом» (галерея `sitetemplates`, 8 шаблонов, рекомендованные типу первыми) как
базовый пресет + до­настройка ВАРИАНТАМИ конкретного блока на каждом слайде
(вид карточек товара, раскладка категории, стиль секций, вид шапки).

**Переработанная карта слайдов AB6.2** (business убран из потока — тип уже задан
при регистрации; escape-hatch «Andere Branche / erweitern» для смежных архетипов):
1. `company` — лого + контакты + название/город (§0b.1)
2. `stil` — «весь образ архетипа одним кликом»: галерея `sitetemplates` (базовый пресет)
3. `menu` — вид шапки (`NAV_STYLES` мокапы) + пункты меню чипами
4. `offer` — вид страницы товара/услуги (стиль карточек `site_defaults` + секции
   `detail_sections`) + мини-форма первого товара (AB6.5) + правка демо (§0b.4)
5. `category` — вид страницы категории (раскладка `LAYOUT_PRESETS`/listing) ← НОВЫЙ
6. `home` — главная: блоки/секции (refinement поверх `stil`-пресета) + hero
7. `payment` — оплата/доставка (гейт по чекаут-модулям, реюз W4-3)
8. `texts` — About + Impressum + правовые реквизиты (§0b.2)
9. `done` — сводка ✓/⏭ + «Andere Branche / erweitern» + «Funktionen»

**Инкременты AB6.2** (каждый = ветка/батч → CI → чекпоинт):
- **AB6.2a (структурный backbone):** новый реестр `SETUP_STEPS` (карта выше) +
  легаси-remap (v1-int И v2-AB6.1-слаги → новые ключи, дедуп) + `SetupStep.check`
  (авто-галочка по реальному контенту) + `SetupStep.gate` (payment по чекаут-модулям;
  business скрыт когда тип задан) + `completeness()` фасад (контракт 5 ключей цел) +
  рельса по новой карте + `stil`/`category` — сначала стабы, остальные слайды
  переназначены на новые ключи. Замки: remap обеих схем, гейт payment/business,
  паритет completeness, рельса.
- **AB6.2b:** `stil` — галерея архетип-шаблонов (реюз `templates_for`/`apply_template`,
  мокапы `site.html:80-108`), one-click.
- **AB6.2c:** `offer` — пикер вида товара (стиль карточек + секции) [мини-форма — AB6.5].
- **AB6.2d:** `category` — пикер раскладки категории.
- **AB6.2e:** `menu` — мокапы шапки в партиал (реюз канва+мастер) + чипы пунктов.
- **AB6.2f:** `company` — лого/контакты/название.
- **AB6.2g:** `payment` (партиализация W4-3) + `texts` (about+Impressum+реквизиты).
- Далее AB6.5 (мини-форма первого товара), AB6.8 (демо kit-lite), AB7 (блочная главная).

## 0e. Фидбэк владельца 2026-07-13 (после демонстрации AB6.2a) → AB6.9 first-run

«Сделай это [мастер] первым этапом после регистрации, прям ПОВЕРХ всего функционала,
и там логически встроить добавление демоданных КАК НА ДЕМКЕ, и далее пошаговые настройки.»

Требования:
1. **Мастер = первый экран после регистрации, полноэкранно focused** («поверх всего
   функционала») — БЕЗ сайдбара/хрома кабинета. Сейчас `setup.html` extends
   `_base_dashboard.html` (сайдбар виден). Нужен focused-лейаут (мини-шапка: лого/город/
   «Später fertigstellen»/Sign out; без nav_groups). AB5-редирект усилить (первый вход
   ведёт в мастер).
2. **Rich-demo первым логическим шагом**: «добавление демоданных КАК НА ДЕМКЕ» = БОГАТОЕ
   демо (фото/меню/hero/отзывы/категории — уровень `demo_kits.apply_kit`/демо-сайтов),
   НЕ тонкий `demo.load_demo`. Слайд business (где был demo_start) скрыт гейтом → demo-seed
   некуда → добавить НОВЫЙ первый видимый слайд «start/welcome» (Mit Beispielen [rich] /
   Leer starten) ПЕРЕД company. ⚠️ Ключевой риск — безопасность `apply_kit` на РЕАЛЬНОМ
   тенанте (не демо-тенанте): не перезаписать настроенный site_config, идемпотентность,
   обратимость (clear). Разведка wfrgcwm2j → выбор: apply_kit напрямую / безопасный субсет /
   обогащение load_demo из kit-спеки.
3. Далее — пошаговые слайды (AB6.2 карта §0d).

Инкремент **AB6.9** (перед наполнением слайдов AB6.2b-g): focused-лейаут + слайд `start` +
rich-demo seed (по вердикту разведки) + усиление AB5. SOURCE — этот §0e + разведка.
