...
## 7. Что уточнилось при исполнении (2026-09-03)

1. **Затенение вместо переименования.** `{% card_form p "product" as storefront_card_style %}`
   переопределяет контекстную переменную ВНУТРИ партиала карточки. Иначе пришлось бы
   переписывать ветвление и три замка, которые ищут в файле строку
   `storefront_card_style == "overlay"`.
2. **Фильтр не годился.** `{% with X=p|filter:X %}` падает, когда контекстной переменной нет
   вовсе (Django не глушит `VariableDoesNotExist` в АРГУМЕНТЕ фильтра) — а карточки рендерятся
   и голым `render_to_string({"p": promo})` в паритет-замках. Поэтому simple_tag с
   `takes_context`, читающий дефолт сайта сам.
3. **Купон — вертикальный.** Первая версия была горизонтальной строкой и в ячейке сетки
   (~290 px) ломалась: заголовок выдавливался, CTA `hidden sm:inline-flex` вылезал за карточку
   (медиа-запрос смотрит на ОКНО, а не на ячейку). Стенд показал это сразу; форма собрана
   стопкой — работает в любой ширине. Вырезы-«дырки» по бокам убраны: их заливка совпадает
   с фоном страницы, а он у Look-семейств разный.
4. **Кольцо: своя короткая подпись.** `timeuntil|truncatewords:1` давало «1 …» — добавлено
   свойство `Promotion.time_left_label` («2 T.», «5 Std.», «12 Min.»).
5. **Сборка бьёт кит.** `_apply_bundle_axes` применяется ПОСЛЕ полей кита, поэтому
   `fokus_lookbook` (у неё была ось `card_style: overlay`) перебивала выбор кита моды —
   ось сборки переведена на `lookbook`, что отвечает её собственному названию.
6. **Замок `test_grid_view`** режет overlay-ветку по `rindex("{% else %}")` — новые ветки
   добавлены ВЫШЕ `compact`, иначе он проверял бы чужую разметку.
7. **Плитки не должны сидеть под «Экспертом».** Стенд показал, что блок «Kartenstil
   (ganze Website)» помечен `data-expert="1"`, а режим редактора по умолчанию «Простой» —
   и переключатель Простой/Эксперт живёт в контексте ВЫБРАННОГО блока. То есть новая
   настройка была бы недостижима на свежем входе. Пикеры вынесены из expert-блока
   (прецедент W11-5 с `quick_add`), замок `test_card_form_picker_is_not_hidden_behind_expert_mode`.
8. **Кнопка корзины у «Lookbook»** сначала стояла на фиксированной высоте от низа карточки
   и при длинной подписи наезжала бы на текст — теперь она в углу подписи, а подпись
   резервирует под неё место (`pr-12`), как в прежних формах.
