...
## 6. Открытые решения U-B (для владельца)
1. **Группировка услуг/номеров (UB3-2):** плоское поле `category` (events-style, миграция) *(реком.)* ·
   generic-коллекция (M2M, крупнее) · оставить flat (без группировки).
2. **Бэкенд поиска (UB2-2):** icontains v1 *(реком., дёшево)* · сразу Postgres FTS/trigram (крупнее).
3. **Охват карточки (UB1-2):** листинг + home-секции сразу *(реком.)* · только листинг, секции позже.
