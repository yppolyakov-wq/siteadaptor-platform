...
**H8b ✅ — живой поиск по датам:** форма Anreise/Abreise/Gäste на портале; при датах
показываются только отели со свободными номерами, цена за выбранный период и диплинк
в прямое бронирование с предзаполненными датами. Доступность считается в схеме каждого
отеля (`aggregator/hotel_search.py`) с кешем (Redis, TTL 300 с).
