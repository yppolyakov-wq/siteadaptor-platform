...
⚠️ **Грабля (укусила 2×): НИКОГДА не писать многострочные `{# … #}`** — Django их НЕ сворачивает
(однострочные), текст течёт на страницу видимым мусором. Многострочный комментарий — ТОЛЬКО
`{% comment %}…{% endcomment %}`. Гард-тест `apps/core/tests/test_template_comments.py` это ловит
(прогоняй ЕГО перед push — он быстрый, без БД). Локально гонять ВСЮ суиту дорого, но этот гард + ruff
обязательны до push, иначе CI краснеет на ерунде после 10-минутного прогона.
...
## АКТИВНЫЙ ЭТАП (2026-06-30): «архетипы как сущности» — план `docs/archetype-entities-plan.md`
...
- **H1 Templates inline-add (2026-06-30, фидбэк «кнопки неактивны»)** — добавленный C-блок реально появляется в
  live-preview. 3 причины (браузерная диагностика на стенде): `site_preview_draft` фильтровал C-блоки (только
  фикс-секции, дедуп по ключу) → ветка `REPEATABLE_BLOCKS` по id; `collect()` не сериализовал C-блоки → ветка cb_;
  пустой блок рендерился как ничего → плейсхолдер `_block_placeholder.html` под `?preview=1` (+`is_preview` в
  контекст-процессоре). **БРАУЗЕРНО ПРОВЕРЕНО** (стенд baeckerei: «+ Text»→блок на канве, правка→контент вживую,
  public чисто). 919 тестов. `d53a6d0`. **Адверсариал-ревью диффа — в фоне (workflow).**
...
не связано с правками; на чистом контейнере CI проходит). Гейт: `ruff check/format` (вся репа) + **таргетный
pytest по blast-radius** затронутых модулей + браузер. Для возврата видимости CI — переавторизовать GitHub-коннектор.
...
## Фидбэк по слайд-менеджеру (3 пункта) — СДЕЛАНО, проверено в браузере
По скрину Pranasy (менеджер баннер-слайдов): (1) **контент влезает в панель** (карточка в стопку, поля
во всю ширину `w-full min-w-0`, без бокового скролла — `scrollWidth−clientWidth=1px`); (2) **возврат к
области после сохранения** (`localStorage['sf_cur_area']`, init-restore на широком экране); (3) **SE-3e
ширина контейнера секции** (пер-секционный `width` contained/full, селект «Width» в билдере, full-bleed
обёртка `w-screen` в `storefront/home.html` при full — как hero, без гориз. скролла; общий размер — в
шаблоне `_base.html` max-w-7xl). Тесты: нормализация (+C-блок)/POST/GET-рендер. Без миграций.
