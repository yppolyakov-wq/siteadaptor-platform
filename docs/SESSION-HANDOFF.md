...
## Точное состояние git (обновлено 2026-06-29, сессия 2)
Баг D + ТЗ #2/#3/#4/#6 — СДЕЛАНЫ и проверены в браузере. Баг D уже в **main** (`bfecd8a`). Батч #2/#3/#4/#6
лежит на ветке `claude/session-handoff-docs-tc1qws` (коммиты `9ccfe46`/`9037a01`/`6806f48` + докстрока) —
после зелёного CI FF-merge в main. Миграций НЕ добавляли.
...
7. ⏳ **Inline-content Фаза 2/3 (НА СОГЛАСОВАНИИ — не делал автономно):** цена (поповер+валидация Decimal),
   фото-замена; «✎» на карточке → полная форма. Паттерн — `category_inline_edit`/`product_inline_edit`; вайтлист
   полей. План — `docs/storefront-inline-content-plan.md`. Согласовать с владельцем перед кодом.
