...
## 7. Открытые решения U-E (для владельца)
- **D1 (головное) — свободная канва x/y ИЛИ composable-slots?** (a) *slots-first: промо-блок с пресет-позициями
  бейджа + overlay-текст поверх фона + кнопка + богатые стиль-контролы, на существующем стек+visual+`collect()`*
  **(реком.** — переиспользует весь движок, purge-safe, a11y/мобайл-дружелюбно, быстрый ROI) · (b) настоящая
  свободная канва x/y/z/w/h — координатная схема + absolute-рендер + drag/resize (UE1-4, вне критпути,
  расходится со всей архитектурой §0.1). **Реком. (a) сейчас, (b) опциональным поздним под-треком.**
- **D2 — промо-блок несёт LIVE-данные акции ИЛИ статический клон?** (a) *LIVE (`promo_pk`, рендер тянет свойства
  из БД)* — цена/остаток/countdown актуальны, JSON-LD валиден; риск фантома при удалённой акции (фолбэк — скрыть) ·
  (b) *статический дизайн-клон* — независим от БД, но **риск устаревшей цены** + JSON-LD нельзя. **Реком. (a) LIVE
  с fail-safe скрытием** (единый источник правды цены).
- **D3 — насколько расширять `promotion_inline_edit` и один ли диспетчер?** (a) *свой view, вайтлист
  +`discount_percent`/`compare_at_price`/`ends_at` + промо-фото-edit* **(реком.** — де-риск, не ждёт U-C) ·
  (b) слить в общий `sellable-inline-edit` (UC2-4) — чище, но связывает волны. **Реком. (a) сейчас, (b) позже
  как тонкий алиас.** В любом варианте — **НЕ добавлять `status`/`available_quantity`**.
- **D4 — промо-шаблоны как `block_templates` (реюз) ИЛИ новый реестр?** (a) *`block_templates` с `key='promo'`*
  **(реком.** — реюз `normalize_block_templates`/SE-4a-UI/инсертера, 0 нового кода реестра, `_MAX=50`) ·
  (b) отдельный `promo_templates`-реестр — изолированнее, но дублирует машинерию. **Реком. (a).**
