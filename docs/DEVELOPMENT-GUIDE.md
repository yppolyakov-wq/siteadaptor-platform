...
### 5.1 Топология (`hetzner-claude-code-setup.md`)
- **App-сервер** CPX21 (~€7/мес): Django (gunicorn) + Celery worker/beat + Redis +
  Caddy. Здесь репозиторий и `deploy.sh`.
- **DB-сервер** CCX13 (~€17/мес): PostgreSQL 16, приватная сеть, `DB_HOST` в `.env.prod`.
- Claude Code на прод **не ставится**.

### 5.2 Разовая подготовка app-сервера
```bash
sudo mkdir -p /opt && cd /opt
git clone <repo-url> siteadaptor-platform && cd siteadaptor-platform
cp .env.prod.example .env.prod        # заполни все CHANGE-ME
...
```
**Или на сервере:** `cd /opt/siteadaptor-platform && ./scripts/deploy.sh`

`scripts/deploy.sh`: `git pull` (main) → `build` → `migrate_schemas --shared` →
`migrate_schemas` → `collectstatic` → `up -d` → `check --deploy` → проверка
`/health/ready/`. Идемпотентно.
