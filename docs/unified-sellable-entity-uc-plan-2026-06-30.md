...
## 8. Открытые решения U-C (для владельца)
- **D1 — насколько обобщать 6 инлайн-эндпоинтов (UC2-4)?** (a) *один диспетчер за адаптером, 6 URL как
  алиасы* **(реком.** — де-риск, JS не трогаем) · (b) полная замена, старые URL убрать (чище, рискованнее) ·
  (c) оставить 6, добавить лишь service/stay-паритет (мельче, не унифицирует). i18n-асимметрия склоняет к (a).
- **D2 — generic `Review` в U-C или раньше в U-A4?** UA-план (§0.0.3) уже решил: **generic `Review`+дата-
  миграция в UA4-4a (U-A)** → тогда UC4-1 = чистая проводка (без миграции в U-C). Подтвердить, что остаётся там.
- **D3 — `legal`/`info` — first-class редактируемые page-type сейчас (UC1-2/UC2-3) или позже?** Сейчас
  `group='text'` + fallback. (a) *first-class сейчас* — нужно для E-2 (AGB/§312j/PAngV как унифиц. инфо-
  страницы), тянет E-2 вперёд **(реком.)** · (b) оставить fallback, поднять в E-2.
- **D4 — галерея Service: шим single-dict→список (без миграции) или поле-список (UC4-3, миграция)?**
  (a) *адаптер-шим dict→[dict]* (без миграции, replace-only, текущий UX) · (b) *миграция `Service.image`→
  JSONField-список* (полный add/remove/reorder-паритет; единственная возможная миграция в U-C). Реком. (b)
  для настоящего паритета — гейт на аппетит владельца.
