...
1. **UC1-1′** (M, было L) — `PAGE_SECTION_REGISTRY (page_type, section)` ПОВЕРХ готового
   `detail_sections` + home `SECTIONS`; normalize home/event/product — байт-в-байт (жёсткий гейт).
2. **UC1-2′** (M) — page-types `listing` и first-class `info`/`legal` (D3; тянет **E-2** в волну).
3. **UC1-3** (M) — ICONS/LABELS в реестр; `page_inspector()` вместо ручных списков.
4. **UC2-1** (L, ⚠️ горячее) — page-scoped draft `config["pages"][page_type]` + шимы; мерж по diff.
5. **UC2-2** (L) — on-canvas add/move (`+`/drag) на деталь/листинг/инфо.
...
9. **UC4-3** (M) — галерея Service: шим dict→[dict] (D4a, БЕЗ миграции) → `_media_gallery` на 5 kind.
10. **UC4-2-доводка** (S) — Offer+BreadcrumbList; Event-поля.
11. **UC5-1** (S) — реестр метит buy-box `configurable="form"`.
...
**Втянуто из U-E (анализ 2026-07-02, предложение владельцу):** две ветки U-E НЕ зависят от
...
13. **Пакет «Стили скидки» (U-E2):** UE2-1 единый `_discount_display.html` (консолидация 8
...
14. **Пакет «Промо на канве» (U-E3):** UE3-1 инлайн-правка `discount_percent`/`compare_at_price`/
