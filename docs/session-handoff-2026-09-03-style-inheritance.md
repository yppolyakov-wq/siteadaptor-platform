...
## 13. Ребейз на main и локальная среда (2026-09-10)

**Ребейз.** Пока шли ревью, параллельная сессия ушла на 38 коммитов вперёд
(волна STU-14/15). Ветка отребейзена: конфликт только в пяти `.po` и одного
класса — обе стороны дописывали записи В ХВОСТ. Свод = оба блока подряд
(сначала main, затем ветка); проверено по существу: 5367 записей в каждом
каталоге, дублей ноль (`polib`). Ветка снова FF-мержится в main.

**Локальные прогоны врали всю сессию.** В контейнере нет `gettext`, поэтому
`.mo` не собираются и десятки чужих тестов падают на немецких ассертах — я
каждый раз доказывал «пред-существующее» через `git stash`, вместо того чтобы
один раз собрать каталоги. Правильный первый шаг новой сессии:

    uv run python -c "import polib,pathlib;[polib.pofile(str(p)).save_as_mofile(str(p.with_suffix('.mo'))) for p in pathlib.Path('locale').glob('*/LC_MESSAGES/django.po')]"

После этого 41 падение на широком прогоне превращается в 2, и оба —
пред-существующие (`test_public.py::test_reserve_*`, воспроизводятся на чистом
дереве; на CI зелены). `.mo` в `.gitignore`, в коммит не попадут.

**`scripts/i18n_gap.py` локально не запускается вовсе** — ему нужен
`makemessages`, то есть тот же `gettext`. Это гейт CI; локальная замена —
`scripts/i18n_quickcheck.py` (он и стоит в pre-commit).
