...
владельца «мержи как зелёный будет»). Ops-шаги владельцу — §10, без изменений,
плюс новый: после деплоя один раз прогнать `manage.py cleanup_import_files`
(dry-run → `--apply`), чтобы убрать легаси-загрузки импорта из /media/.
