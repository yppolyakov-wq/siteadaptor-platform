...
- **2026-07-06 — UC6-7c-2 + ревью-фиксы 7c.** 7c-2: вставка C-блока БЕЗ перезагрузки
  билдера для хостов страниц (fetch add_block → сервер сохраняет + отдаёт HTML строки
  `_cb_row.html` → вставка в форму + schedule перерисует ТОЛЬКО канву; главная и первый
  блок пустого хоста — прежний форм-POST). **Адверсариальный ревью (5 измерений × verify)
  нашёл 3 реальных дефекта — все исправлены:** (1) вставка затирала несохранённый
  drag-порядок (renumber по DOM вместо значений order_cb) → теперь сортируем хост по
  текущему order_cb, ставим новый блок после add_after в этом порядке значений, нумеруем
  1..N; (2) `.catch` любой ошибки fetch делал форм-POST → ВТОРОЙ блок (сервер коммитит
  до ответа) → теперь идемпотентный `reloadBuilderPage` (показывает истинное состояние
  без дубля); (3) гейт `|| curPbHost` вешал drag-ручки/«+» на ФИКС-секции витрины
  (листинг/отзывы/похожие — у них тоже data-sf-section) → цели drag/«+» на странице
  сужены до `[data-pb-host] [data-sf-section]` (реальные page_blocks), клик-правка на
  фикс-секциях осталась. + guard: fetch с невалидным page_key не сохраняет на главную.
  Проверено на стенде (verify_7c_fixes: drag→insert сохраняет порядок + X после A, 0 JS-
  ошибок; verify_scope3: фикс-грид /sortiment/ без ручки/«+»). БЕЗ миграций. Остаток: 7d.

- **2026-07-06 — UC6-7d: настройки меню + библиотека блоков в тулбар (фидбэк владельца
  «добавь для меню настройки и примеры в тулбар»).** Две кнопки в верхнем тулбаре (стиль
  существующих Template/Share/Save): **«☰ Menu»** — шорткат к области настроек шапки/меню
  (`toggleRail(true)`+`showArea("menu")`, повторный клик сворачивает); **«🧱 Blocks»** —
  открывает библиотеку блоков (инсертер с миниатюрами-примерами) для вставки в КОНЕЦ
  текущей страницы/главной (`showInserter("")` → `addBlockInsert` c add_after="" +
  curPbHost/previewPath; stopPropagation, чтобы outside-click не закрыл сразу). «⚙️ Template»
  остаётся глобальными настройками шаблона. Реюз механизмов — БЕЗ бэкенда/миграций. e2e
...
- **2026-07-06 — UC6-9: оптимизация пространства ленты + настройки-«поля» (фидбэк
  владельца со скриншотом).** План `docs/uc6-9-ribbon-fields-plan-2026-07-06.md`.
  **Часть A**: Undo/Redo + статус + подсказка правки перенесены из отдельной полосы над
  канвой в ВЕРХНЮЮ строку тулбара; полоса убрана → канва выше (минус строка). Те же id →
  JS не тронут. **Часть B**: настройки ленты = «поля»: каждый параметр — отдельный
  визуальный блок в строчку, мелкий ЗАГОЛОВОК сверху, контрол снизу (select/slider);
  тоглы-чекбоксы/цвет — компактный чип (подпись в строку, `:has()`); блоки разделены
  рамкой+фоном+радиусом. CSS-селектор — ДЕСКРИПТОРНЫЙ (`.home-block-settings label`,
  `.cb-row label`), т.к. реальные лейблы лежат внутри `div.w-full` (display:contents
  «расплющивает» их визуально, но `>` их не видит — первая версия не срабатывала).
  C-блоки: голые селекты (`_cb_row.html`: align/size/color/width/pos/rounded/side/badge)
  обёрнуты в `<label>Заголовок <select></label>` → поля и в ленте, и в форме. Проверено
  на стенде (скриншоты Simple/Expert секции + C-блок): поля-блоки с заголовками, 0
  JS-ошибок. Имена контролов те же → collect/Save не тронуты. Без миграций. Тесты
  157/157 (имена полей не изменились). Открытый пункт: «менб» в конце сообщения владельца
  (вероятно оборвано) — уточнить.
...
- **2026-07-06 — UC6-7c-2: вставка C-блока БЕЗ перезагрузки билдера (хосты страниц).**
  ...
  row_html — тот же партиал `_cb_row.html`, что в форме). Клиент `addBlockInsert`
  вставляет строку в `.page-block[data-page-key=host] > .space-y-3`, переписывает
  order_cb хоста 1..N и `schedule()` → перерисовывается ТОЛЬКО канва (iframe), не весь
  билдер. Главная (page_key пуст) — прежний форм-POST (order-пространство общее с
  фикс-секциями в двух DOM-контейнерах — просто не сводится). Первый блок пустого
  хоста / любая ошибка → фолбэк на форм-POST (одна перезагрузка). Закрывает пункт
  владельца «применять автоматом без обновления страницы». Грабля CSS-свежести:
  переменная `container` + токен `!container` в JS/комментарии подхватываются
  Tailwind-сканером → лишний `.\!container` в app.css (переименовал в hostBox +
  переформулировал комментарий без литерала). БЕЗ миграций. Замки: test_cblocks_builder
...
- **2026-07-06 — AB3 (Sprint G): демо-дефолты в мастере онбординга («начать с примеров»).**
  Аудит: мастер v2 в основном готов (прогресс-бар, live-превью 2–6, автосейв 2–5, карточки
  архетипа), НО демо-контент только на шаге 6 → шаги 1–5 просят заполнять ПУСТЫЕ поля.
  По принципу плана «дефолты вместо пустых полей / ребёнок соберёт»: на шаге 1 добавлен
  рекомендуемый путь **«🎁 Mit Beispielen starten»** (action `demo_start`): сохраняет тип
  бизнеса (общий хелпер `apply_business_type`) + СРАЗУ заливает демо-кит архетипа
  (`demo.load_demo`) + шагает дальше → весь мастер заполнен примерами, владелец правит, а
  не создаёт с нуля. Вторичная «Ohne Beispiele» — пустой старт. Замки: `test_step1_demo_
...
- **2026-07-07 — UC6-10b: ещё компактнее (фидбэк «можешь ещё компактнее»).** Высота
  редактор-хрома простого блока **119px → ~81px (−32%)**: верхний тулбар `py-2→py-1`,
  кнопки девайса/Menu/Blocks/Template/Share/Save `py-1.5→py-1` (51px→39px); лента —
  тело/строка/поля тоньше (поле-настройка 46px→31px: gap `.1rem→0`, padding `.2→.05rem`,
  заголовок 10px→9px), контролы select/input `padding-top/bottom 1px`; лента `top`
  `3.35rem→2.5rem` (зазор под тулбаром 15px→1px). Стиль «заголовок сверху, контрол снизу»
  (UC6-9) сохранён. Проверено на стенде (Простой=2 узкие строки, Эксперт — все контролы в
  одной обёрнутой строке). Без миграций, `app.css` не менялся (классы уже были). main
  `a0e221b`.

- **2026-07-07 — U-D: ТЗ подготовлено (доки-до-кода), волна НЕ начата.** По просьбе
  владельца «составь план для U-D и ТЗ полноценное для начала новой сессии». Разведка —
  3 Explore-агента (карта 6 order-моделей+FSM / кабинет-UI+инвентарь / извлечение scope из
  доков) + чтение `fsm.py` и существующего `unified-sellable-entity-ud-plan-2026-06-30.md`
  (адверсариально проверен). Решения владельца: **Kanban = drag-drop** (Trello-style, реюз
  паттерна конструктора без JS-деп), **первая сессия = вся волна U-D1..U-D3** (проекция +
  доска + склад-леджер `StockMovement`, вкл. миграцию). Создан самодостаточный
  **`docs/ud-wave-tz-2026-07-07.md`** (§0 контекст+решения D1-D4, §1 область/6 kinds/
  партиция/инварианты, §2 подзадачи UD1-1…UD3-3 с файлами/критериями/тестами, §3 критпуть
  `UD1-1→UD2-1→UD2-2→UD2-3`, §4 миграции, §5 риски, §6 DoD, §7 reuse, §8 связанные).
  Актуализировано под уехавшее вперёд: E-7 уже отгрузил `Order.payment_method` (UD1-1 только
...
- **Ф1–Ф3 «per-language ввод + переводимые витринные метки товара»** (2026-07-09, ветка
  ...
  T-1 (в конце бэклога).
- **Фидбэк-батч после Ф1–Ф3 (2026-07-10, ветка `claude/admin-simplification-handoff-dfawis`):
  5 пунктов владельца.** **#1 (баг, регресс Ф1):** кнопки переключателя языка/табов не
  работали — скрипт в партиале `_i18n_switch.html` выполнялся РАНЬШЕ, чем в DOM появлялись
  табы/панели (они ниже в форме) → `querySelectorAll("[data-pf-tab]")` ловил пустой NodeList.
  Фикс: делегирование клика на `document` + живые запросы в обработчиках (+ guard от двойной
  привязки); проверено в Chromium/Playwright (клик по табу/пилюле переключает). `1e32174`.
  **#2:** Varianten/Modifiers — отдельная вкладка «Variants & extras» (режим редактирования;
  панель `data-pf-panel="varianten"` ВНЕ основной формы — вложенные `<form>` недопустимы), под
  `base_price` кнопка «Erweiterte Preise» (`data-pf-goto` → активирует вкладку); на создании —
  подсказка «сначала сохранить». Проверено в браузере. `da3e83f`. **#4:** ясность режима
  Einfach/Experte — `modules.simple_hidden_labels(tenant)` (человекочит. названия скрываемого,
  независимо от текущего режима, только активные) → контекст `ui_simple_hidden`; на «Funktionen»
  конкретный список + бейдж «Currently: Simple/Expert»; тултип тумблера в шапке — перечень. `247997a`.
  **#3/#5:** кнопки «Demo ansehen» на карточках типов бизнеса (регистрация + шаг 1 мастера) →
  живая демо-витрина архетипа. `onboarding.DEMO_KIT_HOST` (тип → поддомен ближайшего seed-демо),
  `business_type_cards(request)` строит `demo_url` из `TENANT_DOMAIN_BASE`, ГЕЙТ по реально
...
- **Демо-фото: реальный CC0-набор (2026-07-10, ветка `claude/youthful-lovelace-suk4wc`).**
  По списку `demo_photo_report --missing` (248 позиций, 232 уникальных файла) скачаны реальные
  фото: Openverse API (`license=cc0,pdm`) + Wikimedia Commons (`haswbstatement` CC0/PD),
  лицензия каждого файла проверена по метаданным API. Пайплайн: скрипт-загрузчик (скоринг
  кандидатов: блок-лист NSFW/историческое/брендинг/лица, бонус кураторским провайдерам) →
  cover-crop+webp (товары 800×600, hero 1600×900, q≈80, <150 KB) → **визуальная верификация
  КАЖДОГО фото LLM-агентами** (сюжет/стиль/лица/приличия) → reject-цикл по кэшу кандидатов
  (4 раунда: 191→+25→+21→+6). Итог: **146 файлов = 167/248 позиций (67 %)**; брак и
  не-найденное остаётся на штатном SVG-фолбэке. 20 портретов команды пропущены намеренно
  (реальные лица). Провенанс — `static/demo/photos/SOURCES.md` (файл → URL+лицензия).
  Тесты demo_images/demo_kits обновлены под «фото есть → static-URL» (5 ассертов допускают
  оба префикса; детерминизм SVG залочен на незакрытом ключе). ⚠️ ops: чтобы фото попали в
  демо — пересеять `seed_demo_tenants --kit <kit> --recreate` (URL пишется при сидинге).
- **Демо-фото: AI-набор FLUX.1-schnell → покрытие 100 %** (2026-07-10, интегрирован
  черри-пиком из `claude/demo-photos-replicate-rtcs78`). Отдельная сессия догенерила
  120 недостающих ключей через Replicate `black-forest-labs/flux-schnell` (нативный webp,
  без Pillow — pypi заблокирован на egress): 120/120, 0 сбоев, бюджет 120/150; все < 150 KB
  (2–115 KB, медиана 26 KB), пропорции по типу (товары/галереи 4:3, hero 16:9, портреты
  1:1); 22 портрета — вымышленные лица на нейтральном фоне (снимает вопрос прав личности);
  немецкие промпты, без текста/логотипов. Провенанс — 120 строк «AI-generated, FLUX.1-schnell»
  в `SOURCES.md` (маркер-секция `<!-- AI-SET -->`). `demo_photo_report` 173/298 → **298/298**.
  Ревью: magic-bytes RIFF/WEBP (0 битых), визуальный спот-чек (хлеб/портрет/запчасть/интерьер —
  по теме, качество ок), пропорции по типам, тесты demo_images+demo_kits 48 зелёных (на свежей
...
- **T1-b: мультиязычный хром — DeepL-перевод .po (en/tr/ru/uk) + компиляция всех локалей**
  (2026-07-11, ветка `claude/cabinet-de-en-deepl-zw5152`, БЕЗ миграций; выполнено удалённой
  сессией + владелец на сервере, DeepL-бюджет ~228k/1M). Пайплайн на локаль: `makemessages` →
  DeepL (защита `%(x)s`/`{var}` через XML tag_handling; авто-детект языка источника, identity
  для строк уже на целевом языке) → адресный QA коротких строк → сохранность URL-слагов
  (`/unterkunft/` и т.п.) → грамматика плюралов → `msgfmt --check-format`. **en.po** (`4d89ec6`):
  2447 записей = 383 DE→EN + 2042 identity + 22 plural; ~75 правок после adversarial-ревью
  (2 верификатора + 3 искателя пропусков): DeepL коверкает ≤4-символьные (`AGB→"All-American
  Boy"`, `Mo→In`, `Bestand→File`, `EK→I`), раздувает акронимы (FAQ/VAT/SEO), переводит слаги и
  теряет literal `%%`. **tr** (`0e5bf06`) ~2496 + 46 правок (Wochentage `Pzt..Paz`, `KDV`, `SSS`).
  **ru** (`1981a03`) / **uk** (`85ec7e4`) по ~2499 + ручные 4-форменные плюралы («ночь/ночи/
  ночей», `spot→место` — DeepL давал «пятна», «посетить» вместо «посещений»). **forms.py фикс**
  (`ed071e5`): 4 promo-хелпа с литеральным `%` («Either a % or…») — `% o`/`% v` парсились как
  %-спеки → msgfmt count-mismatch в любых переводах; переформулированы на «percentage or»
  (EN-рендер не изменился). **ci.yml+deploy.sh** (`9042540`): msgfmt-цикл по `locale/*/django.po`
  и `compilemessages` без `-l en` — иначе tr/ru/uk/de молча неактивны в тестах и ПРОДЕ (падение
  15 DE-ассертов в CI #1396 это и вскрыло). **de.po — построен, но ОТКАЧЕН** (реверты
  `5719264`+`...`; решение владельца 2026-07-11): полная активация вскрыла 54 красных теста —
  англ. ассерты в DE-рендере, golden-normalize зависит от локали, лейблы хабов требуют ревью
  DeepL-немецкого — ровно предсказание `legal-lang-package-plan §2`, из-за которого T-1 и был
  отложен; готовые de.po (2129 переводов + QA-карта) и патч 16 тестов лежат в истории ветки
  (`93e19cf`/`1c8be62`) для cherry-pick при возобновлении. **Уроки:** (1) скриптовые правки
  тестов → `ruff format --check .` целиком ДО пуша (CI #1395 красный только из-за формата);
...
## 2026-07-13 — Редактор: Kompakt-Modus инспектора + ресайз до 230px (фидбэк владельца)

- **Запрос:** «редактор можно свернуть/уменьшить ширину — дать ЕЩЁ способ уменьшить размер
  без потери функционала (зум / одно под другим); проанализируй и сделай оптимально».
- **Разведка (Explore-агент):** карта правого инспектора (`templates/tenant/site_home.html`,
  `#bld-editor-pane` fixed, ширина `--bld-panel-w` 280–640 + свёртка `bld-panel-min`; поля
  ленты уже стилизованы «полями» UC6-9; Простой/Эксперт `data-mode` уже прячет технические
  контролы; зум канвы `applyDevice`/`transform:scale` уже есть — но не на панели). 4 варианта
  оценены: (а) zoom контента — риск с fixed-лентой, (б) **dense поля — оптимально** (чистый
  CSS, функционал 1:1), (в) нижняя панель — против «беречь вертикаль», (г) мини-рейка — дорого.
- **Сделано (коммит `fcfdcff`, БЕЗ миграций):** тумблер «🔍 Compact» в тулбаре (`bld-compact-btn`,
  класс `bld-dense` на `#bld-root`, персист `sf_editor_dense`) → плотнее padding/gap/шрифт
  инспектора и ленты блока (все контролы на месте); нижняя граница ручки ресайза 280→230px
  (restore-валидация + кламп) — панель можно тянуть заметно уже, в dense остаётся удобной.
  Стенд Chromium: normal 380px → Compact+240px, канва отвоёвывает место, поля Theme читаемы.
- **Пункт 3 фидбэка (branchen-CTA `?type=` предвыбор отрасли):** разведка подтвердила —
  РАБОТАЕТ end-to-end в HEAD (CTA `/registrieren/?type=<slug>` → `views.py:69-71` читает →
  `onboarding.html:89` предвыбирает; тест `test_industry_pages.py`). Обрыв на siteadaptor.de =
...
## 2026-07-14 — Регистрация с Branchen-страницы: компактная форма вместо пикера (фидбэк)

- Фидбэк владельца: переход `/branchen/restaurant/` → «Jetzt starten» → `/registrieren/?type=
  restaurant` показывал ВЕСЬ пикер отраслей (выбранная карточка ниже сгиба, «непонятно что
  происходит»), а ожидается «просто форма регистрации». Фикс: при валидном `?type=`
  `BusinessSignupView` отдаёт `preselected_type`; шаблон `tenants/onboarding.html` показывает
  компактный баннер выбранной отрасли (иконка+label+blurb) + скрытое поле `business_type` +
  ссылка «Ändern» → `/registrieren/` (полный пикер) — вместо грид-пикера. Форма (название/
  город/субдомен/email/пароль) сразу на виду. Холодный вход `/registrieren/` (без type) и
  POST-переррендер после ошибки — пикер как прежде (характеризационный тест цел). Замки
  `test_signup_preselected_type_shows_compact_form_not_picker`, `…_invalid_type_falls_back_
  to_full_picker`. Без миграций.

## 2026-07-14 — Мастер: фикс привязки live-превью к форме слайда (+ расследование кнопок)

- Фидбэк владельца «не работают кнопки Weiter/Überspringen в мастере». **Расследование
  (headless Playwright + серверный POST):** кнопки шага — нативные `type="submit"` внутри
  формы слайда; в браузере (в т.ч. с htmx+alpine, как на проде) КЛИК даёт submit, а
  серверный POST (skip/Weiter) отвечает 302 и продвигает шаг (company→stil). Т.е. в текущем
  коде кнопки исправны — вероятная причина на проде: устаревший деплой/кэш (просьба
  передеплоить main + хард-рефреш; при повторе — консоль браузера).
- **Найдена и исправлена реальная регрессия (AB6.9):** live-превью цеплялось к ПЕРВОЙ
  `form[method=post]` страницы = хедер-форме «Später fertigstellen» (не к форме слайда) →
  превью не обновлялось по вводу, а пустой live-POST от хедер-формы мог затирать поля шага
  (city/address/…). Фикс: слайд обёрнут в `#setup-slide`, live-save скоуплен на
  `#setup-slide form[method="post"]`. Проверено: селектор теперь = multipart-форма слайда,
  кнопки по-прежнему сабмитят. Замок `test_live_preview_scoped_to_current_slide_form`. Без миграций.

## 2026-07-14 — Фидбэк-батч UI: тост в билдере + прогресс мастера вверх + zoom-out превью

- **Билдер: flash → тост с автозакрытием** (фидбэк «сделай во всплывающем окне с
  автозакрытием»): «Блок добавлен.» и прочие flash-сообщения были in-flow полосой под
  тулбаром (site_home.html:70) — сдвигали канву вниз. Теперь `#bld-flash` — fixed-тост
  по центру под тулбаром (`z-[70]`, `pointer-events-none`, `role="status"`), авто-скрытие
  2.5с (ошибки — 6с) с fade. Канву не двигает.
- **Мастер: «Шаг N из M» + прогресс-бар — компактной строкой НАД сеткой** (фидбэк
  «перенеси прогресс вверх, не получается просмотреть сайт»): строка на всю ширину,
  колонки рельса/слайда/превью выровнены по верху, вертикаль свободна.
- **Мастер v2 layout (2-й заход фидбэка «перенеси ВСЕ пункты наверх + раздели на пол
  экрана»):** рельса ВСЕХ 10 шагов теперь горизонтальным рядом СВЕРХУ (`flex flex-wrap`,
  не левой колонкой) + прогресс-бар под ней; тело — ДВЕ ПОЛОВИНЫ `lg:grid-cols-2`
  (настройки слева | превью справа). Превью-шелл выше (`min(80vh,780px)`), zoom-fit
  теперь даёт ~0.57 (колонка ½ экрана — сайт крупнее). Шаги без превью (business/texts)
  — одноколоночный центрированный слайд. Проверено headless: рельса сверху (10 чипов),
  two-pane, scale(0.565).
- **Тянущаяся граница панелей мастера (фидбэк 2026-07-16 «сдвигать влево/вправо,
  увеличивая одну из них»):** ручка `#setup-resizer` между половинами (grid-колонка
  14px, реюз паттерна bld-panel-resizer: Pointer Capture — drag не теряется над
  iframe). Доли — CSS-переменные `--setup-l/--setup-r` (fr), клампы 25–75%, персист
...
- **Фикс (фидбэк 2026-07-16 «при расширении стопорится на 50%»):** превью упиралось в
  DESK=1200px (кап scale≤1 при фикс-ширине рендера) — при панели шире справа оставалась
  пустая полоса. Теперь `frameW = max(DESK, w)`: панель шире 1200 → сайт рендерится на
...
- **Мастер: zoom-out live-превью** (фидбэк «сайт немного зумировать, чтоб всё было
  ...
  мобильная нарезка. Шелл `[data-preview-shell]` высотой `min(70vh,640px)` (было 520px);
  fit-JS на load/resize/ResizeObserver; без JS — фолбэк `w-full/h-full` (прежний режим).
- Верификация headless (Chromium): прогресс над сеткой, `scale(0.331)` на iframe при
...
## 2026-07-16 — Склад-2 E1.5: FEFO-врезка в атомик заказа/заявки + демо-партии (без миграции)

- **FEFO на продаже** (owner: «сразу полный FEFO»): `consume_fefo` врезан в
  `orders/services.py::_reserve_stock` (после декремента счётчика, в той же `@transaction.atomic`
  create_order) и `jobs/services.py::_commit_stock` (расход материалов) — гейт `has_lots(...)`, для
  товаров БЕЗ партий поведение байт-в-байт (паритет: партии не заводятся, счётчик/леджер прежние).
  Гасит партию с ближайшим MHD первой; недостачу докрывает чистый счётчик (D1).
- **Возврат/отмена** (`orders/state_machine.py::_restore_stock`): новый `restore_fefo` доливает
  партии в обратном FEFO-порядке (ранний MHD первым, до `qty_received`) рядом с F()-инкрементом
  счётчика → Σlot снова сходится со счётчиком. No-op для сущностей без партий.
- Конкуренция: партии блокируются `select_for_update` ВНУТРИ атомика заказа, за row-lock'ом
  Product/Variant (`_reserve_stock` держит его до коммита) → расход FEFO сериализован, без гонок/
  дедлоков (порядок блокировок продукт→партии стабилен).
- Замки `test_fefo_wiring.py` (5): продажа гасит ближайший-MHD-первым (Σlot==счётчик); продажа без
  партий — паритет (Lot=0); отмена доливает партии обратно; недостача (Σlot<счётчик, счётчик —
  истина); job commit гасит FEFO. Прогон `apps/orders`+`apps/jobs` 185 зелёных (паритет подтверждён).
- **Демо-партии** (`demo_kits`): флаг `DemoKit.enable_lots` (BAKERY/BUTCHER) → `cfg["lots_enabled"]`
  + `_seed_demo_lots` (первым ≤8 товарам без вариантов заводит 1-2 Chargen с MHD; товару без учёта
...
## 2026-07-18 — FD-3-lite: кабинет Finder (тумблер + превью; без миграций)

- `/dashboard/finder/` (`finder_settings`, вкладка Marketing-хаба в «Erweitert», module_key
  None): тумблер «Finder auf der Website aktivieren» (targeted-write ТОЛЬКО `enabled` —
  кастом-вопросы целы; выключение при пустом finder убирает ключ целиком, presence-minimal)
...
## 2026-07-19 — LS-4 «Слой доверия» v1 (этап C2 ТЗ; БЕЗ миграций; счётчики — за владельцем)

- **«Лицо» в касаниях:** имя сотрудника (author_user.first_name) над staff-пузырём в
  ПУБЛИЧНОМ треде клиента + живая подпись в письме-ответе (`inbox_customer`: «Mit
  freundlichen Grüßen / Maria · Business»). Фото сотрудника — v2 (team-секция не связана
  с User; без миграции нечем матчить честно).
- **Публичный бейдж времени ответа:** `avg_reaction_minutes()` (та же выборка, что SLA
  LS-6) → на странице контакта «⚡ Antwortet in der Regel in ~N Min» ТОЛЬКО при хорошем
  значении (≤ 2 ч; нет данных/медленно → без бейджа — честность важнее украшения).
- «Jetzt geöffnet» — уже был (P1b). **⚠️ Счётчики «продано N на этой неделе» — НЕ делаем
  до решения владельца** (пункт ТЗ C2; вопрос в чекпоинте вместе с ST-4b).
- Тесты: +2 в test_direct_line (имя в треде и письме; гейт бейджа 15/600/None мин) —
  14 зелёных. i18n +1 msgid → 4 .po. CSS пересобран.

## 2026-07-19 — LS-5 «Care-цикл» v1 (этап C3 ТЗ; БЕЗ миграций) — ЭТАП C ЗАКРЫТ

- Свод по карте цикла ТЗ: **до** (напоминания B2/beat — были) · **момент** (подтверждения +
  живое имя LS-4 — было) · **после** — НОВОЕ: **развилка настроения в 4 «Wie war's?»-письмах**
  (post_purchase/post_visit/post_stay/post_event): 👍 отзыв (review-ссылки были) + 👎
  «Nicht ganz zufrieden? …» → problem_url (доверенный гейт LS-6: high-тред + Telegram-пуш
  владельцу) — недовольный перехватывается ДО публичного отзыва · **долгосрочно**
  (лояльность/win-back B4 — были; день рождения — v2).
- **Управление циклом:** вкладка «Care-Zyklus» в Marketing-хабе (Erweitert) → матрица
  UD4-2 (per-событие email/Telegram — уже покрывает все касания цикла).
...
## 2026-07-20 — ST-4b влит + «продано N» влит; этап B ТЗ закрыт целиком
...
## 2026-07-20 — FD-4 «Finder агрегатора» (этап D5 ТЗ; БЕЗ миграций) — ОЧЕРЕДЬ ТЗ A–D ИСЧЕРПАНА

- План `docs/fd4-aggregator-finder-plan-2026-07-20.md`. Отдельный public-движок
  `apps/aggregator/finder.py` (core/finder не переносим — тенантный): вертикаль →
  город (пересчёт по вертикали, невалидное значение = возврат на шаг) → 3
  результата `listings_for`-порядком; пустая выдача → честный фолбэк
  (ending_soon/новые + маркер). Вьюха `platform_finder` на `/entdecken/finder/`
  (роут ВЫШЕ city catch-all — иначе «finder» матчится как город); рейтинги +
  open-status; `cache_public_page` кэширует только query-less шаг 1. Параметры
  `?typ=&stadt=` (не `?a=` — города могут содержать разделители). **UWG §5a:**
  выдача органическая (featured НЕ поднимается), платная позиция несёт «★
  Anzeige» из общего `_cards.html`, «Unser Vorschlag» не используется — всё
  замками. CTA-вход на `/entdecken/`. 4 теста (шаги/невалид/пересчёт городов/
  органический порядок+метка), aggregator 172 зелёных.
- **Попутно закрыт i18n-долг всего Finder-трека:** 32 msgid (FD-1 витрина, FD-2
  секция, FD-3 кабинет+флеш, FD-4 агрегатор) добавлены в en/tr/ru/uk .po — все
  четыре инкремента шли без .po-записей (немецкий фолбэк маскировал). Счётчик
  шагов агрегатора сведён на общий blocktrans-msgid FD-1 «Frage %(step)s von
  %(total)s» (раздельные trans "Frage"/"von" ломали бы порядок слов).
...
## 2026-07-23 (продолжение) — DL-3: демо-контент на ru/uk/tr (5 языков)

Запрос владельца: «переведи весь демо-контент так же на русский, украинский, турецкий».

- **Механизм обобщён на N локалей:** `demo_i18n.py` — по словарю на локаль
  (`demo_i18n_<loc>.json` = {de: перевод}), генерик-проходы `overlay_config(cfg,
  locales)` и `translate_tenant_content(tenant, locales)` (было хардкод-en).
  `DEMO_LOCALES=(en,ru,uk,tr)`.
- **Переводы:** те же 1119 немецких строк переведены на ru/uk/tr (7 параллельных
  переводчиков-субагентов, покрытие 1119/1119); идентичные (бренды/имена) отброшены →
  `demo_i18n_ru.json` 1040, `demo_i18n_uk.json` 1040, `demo_i18n_tr.json` 1018 записей.
- **Демо-киты:** `DemoKit.enabled_locales` дефолт → все 5 (`de,en,ru,uk,tr`) → свитчер
  в шапке показывает 5 языков, контент+тексты витрины переводятся на все. Все 5 в
  реестре LANGUAGES + CABINET_LANGUAGES.
- **Тесты:** пост-сид перевод на 4 локали + идемпотентность per-locale + непустые
  словари всех локалей; курируемые двуязычные замки ослаблены с точного равенства
...
## 2026-07-27 — Багфикс «изменить даты в календаре не получается» (репорт владельца)

Репро в реальном Chromium (Playwright, локальный стенд hotel-кита) — найдены и
исправлены ДВА дефекта под жалобу:

- **Кабинет (Belegungsplan):** drop плашки принимали ТОЛЬКО клетки строки
  счётчиков — бросок на её же дорожку (естественный жест «тащу бронь по своей
  строке») молча игнорировался. Фикс: дорожки `<tr data-unit>`, drop целится по
  ВСЕЙ строке юнита (день = колонка под курсором по клеткам счётчиков) +
  **grab-offset** (`data-arrival` на плашке): за какую ночь плашку взяли, той
  ночью она и ложится — заезд не прыгает под курсор; подсветка = клетка нового
  заезда. Проверено в браузере: drop на свою дорожку двигает даты (204→reload,
  заезд реально меняется), drop на чужую дорожку меняет номер с сохранением дат.
- **Витрина (селектор дат номера):** диапазон через границу месяца молча
  отвергался — `rangeFree` требовал, чтобы каждая ночь была ВИДИМЫМ свободным
  днём, а после перелистывания месяца дни прошлого месяца исчезали из DOM.
  Фикс: блокируют только видимые ЗАНЯТЫЕ ночи (`busyDates`), невидимые считаются
  свободными — сервер ре-валидирует наличие и честно показывает отказ. Плюс
  событие `staycal:swap` из `_stay_calendar.html`: подсветка выбранного заезда и
  min-nights-гашение восстанавливаются после перелистывания (раньше терялись).
- Замки: +2 ассерта в `test_calendar_renders_bar_with_name_code_and_drag`
  (data-unit на дорожке, data-arrival на плашке); 50 stays-тестов зелёные.
  Без миграций, без новых msgid (JS/шаблоны).

## 2026-07-27 — Ресторан: выбор даты/времени столика БЕЗ перезагрузки (фидбэк владельца)

«При выборе даты страница обновляется, так же при выборе столика» — селекторы
записи были ссылками с полным GET-реренедером. Фикс по паттерну Батча B
(fetch-своп):

- Динамическая часть страниц слотов вынесена в партиалы: `_booking_slots_box.html`
  (ресурс: ресторан «Tisch», групповые курсы — календарь/дни/времена/форма) и
  `_service_slots_box.html` (услуга: + пикер мастера/buy-box); полные страницы
  включают их в `#svc-box`. Разметка 1:1 (156 booking-тестов зелёные без правок,
  паритет buybox цел).
- `?box=1` в `termin_slots`/`service_slots` отдаёт только фрагмент (embed-контекст
  сохранён); общий скрипт `_svc_box_script.html` — делегированный перехват
  внутренних ссылок (?tag/?cal/?slot/?resource) → fetch+innerHTML-своп +
  history.replaceState; выбранное время докручивает к форме; фолбэк при ошибке —
  обычная навигация (без JS всё работает как раньше).
- Проверено в Chromium на restaurant-demo (дата/месяц/время — документ не
...
## 2026-07-27 — Багфиксы PMS-аудита (отчёт G9 + G6 Online-Checkin), БЕЗ миграций

Три адверсариальных разведчика сверили код с PMS-спекой TravelLine (WebPMS
5.1-5.16) — найденные РЕАЛЬНЫЕ дефекты исправлены сразу:

- **`occupancy_report` (G9) — три искажения формулы:** (1) мульти-номерная бронь
  G5 продавала 1 ночь вместо `rooms` → Belegung занижен, ADR завышен в rooms
  раз; (2) блокировки (ремонт/своё) не вычитались из доступных ночей → Belegung/
  RevPAR занижены у отелей с ремонтом; (3) хардкод `_COUNTED` терял кастом-
  статусы тенанта («Anzahlung erhalten» демо-отеля выпадала из отчёта) → новый
  tenant-aware `status_registry.counted_statuses_for` (встроенные ∪ кастом с
  ёмкостью/выручкой). Замки: +3 в test_reports; golden-замок реестра обновлён
  осознанно (реестр = единственный источник).
- **G6 Online-Checkin достижим:** ссылка на цифровой Meldeschein теперь в
  письмах confirmed + pre-arrival reminder (пока регкарта не подписана);
  раньше жила только на странице подтверждения — гость, закрывший вкладку,
  до чекина не добирался. 1 msgid × 4 .po; замок в test_cabinet.
- **Мёртвое свойство `GuestRegistration.kurtaxe_eur`** (обращалось к
  несуществующему полю → AttributeError при вызове) удалено.

## 2026-07-28 — Фидбэк владельца: правка брони в кабинете + карточка брони ПОД календарём

«Не нашёл где редактировать бронь (увеличить количество дней, человек)» +
«нужно открывать брони в календаре при нажатии сразу же под календарём».
Без миграций.

- **Карточка брони — общий партиал `stays/_booking_card.html`** (тело FB-11:
  статус/гость/Aufenthalt/суммы/Meldeschein/действия) — используется страницей
  `booking_detail` И инлайн-панелью календаря.
- **Форма «✏️ Buchung bearbeiten»** (pending/confirmed): даты, номер (селект),
  взрослые/дети, заметка + чекбокс «Preis neu berechnen» (по умолчанию вкл;
  выкл = цена сохраняется). Новый action `update` в `stay_action`: гости/заметка
  пишутся до `move_stay` (перерасчёт Kurtaxe видит свежих adults), даты/номер —
  через `move_stay` (те же локи/anti-oversell); walk-in-правки владельца
  Verkaufsregeln не гейтят. Редирект держит панель открытой (`?buchung=`).
- **Клик по плашке Belegungsplan** → карточка брони СРАЗУ ПОД календарём:
  `?buchung=<pk>` в `calendar` (+`&box=1` — fetch-фрагмент), делегированный JS
  свапает `#booking-panel` без перезагрузки + скролл; фолбэк — прежняя страница
  брони. Кривой pk — панель просто скрыта.
- Проверено в Chromium на hotel-стенде: клик → панель без перезагрузки, правка
  departure из панели сохраняется («Stay updated», панель остаётся открытой).
  Замки: +4 теста (update даты/гости/заметка/перерасчёт и без него, overflow-
  отказ, панель+фрагмент, форма на странице брони). Грабля стенда: /etc/hosts
  контейнера сбрасывается — записи *.siteadaptor.de надо восстанавливать.

## 2026-07-28 — Багфиксы PMS-аудита №2 (UWG-доказательство, кастом-статусы шахматки, win-back, DSGVO-purge)

Без миграций. Четыре дефекта из CRM/фронт-деск разведок:

- **UWG §7 — момент согласия:** `marketing_opt_in_at` теперь фиксируется и в ЛК
  гостя, и при галке владельца в CRM-форме (раньше согласие ставилось без
  доказательства). Замок в test_account_profile.
- **Кастом-статусы на Belegungsplan:** цвет/drag плашки — по `blocks_capacity`
  из реестра (не литералы pending/confirmed): «Anzahlung erhalten» больше не
  серая и двигается; done/cancelled — статичные. Встроенные цвета байт-в-байт.
- **Авто-win-back с top_ltv:** `segment_customers(exclude_ids=…)` применяется ДО
  слайса (раньше `.exclude()` после среза ронял TypeError из шелла/админки).
...
## 2026-07-28 — PMS-R2: назначение физического номера брони + «Ohne Zimmer»

⚠️ миграция `stays/0026` (StayBooking.room FK, SET_NULL, аддитивная).

- `services.free_rooms_for` (комнаты категории, свободные на диапазон; конфликт
  по пересечению активных броней) + `assign_room` (назначить/снять,
  `RoomConflict` на чужую категорию/занятость).
- Селект «🚪 Zimmer» в форме правки брони (виден только при заведённых
  комнатах — ленивая активация); номер на заголовке карточки и на плашке
  Belegungsplan; чип «⚠️ N Buchungen ohne Zimmer» над календарём.
- `move_stay`: смена категории сбрасывает room. **Попутно исправлен латентный
  баг:** reprice-ветка move_stay НЕ сохраняла смену юнита («unit» отсутствовал
  в update_fields; путь был недостижим до формы правки — кнопка Move юнит не
  передавала). Устаревший селект комнаты при смене категории игнорируется.
- Замки: +3 теста (assign/conflict/free_rooms; update-назначение + сброс при
  смене категории; чип и номер на плашке). 3 msgid × 4 .po. Дальше: R3
  (шахматка построчно по комнатам + drop-назначение), R4 (хаускипинг-lite).

## 2026-07-28 — PMS-R3: шахматка построчно по комнатам + drop-назначение (БЕЗ миграций)

- `availability.room_lane_rows` — у категории с комнатами дорожка на КАЖДУЮ
  комнату (🚪label; пустая комната = пустая строка) + безымянные «ohne Zimmer»
  (неназначенные + блокировки); общая укладка вынесена в `_pack_lanes`/`_clipped_seg`
  (booking_bars байт-в-байт, замки целы). Без комнат — прежний вид (адаптер).
- Drop плашки на строку комнаты назначает номер вместе с переносом (`room` в
  POST move); конфликт комнаты проверяется ДО переноса → 409/snap-back с
  целыми датами. Перенос, ломающий прежнее назначение, честно снимает номер.
...
## 2026-07-28 — PMS-B2: авто-выселение beat'ом (БЕЗ миграций)

- `tasks.auto_fulfil_departed` + beat `auto-fulfil-stays` (раз в сутки):
  confirmed-брони с прошедшим выездом → `fulfilled` ШТАТНЫМ FSM-путём (выручка
  RevenueEntry + комната dirty R4 + audit — та же цепочка, что кнопка
  владельца). pending не трогаем (no-show — решение владельца). Кастом-правила
  переходов уважаются (IllegalTransition → скип). Идемпотентно по статусу.
- Это оживляет LTV отеля и авто-win-back (CRM-аудит: «пока владелец не жмёт
  Abgereist руками, LTV структурно пуст»). Замок: полный цикл + идемпотентность.

## 2026-07-28 — CP-1 «Marktposition» (цены конкурентов — одобрено) + планы CO/CP (БЕЗ миграций)

План `docs/pms-corporate-competitor-plans-2026-07-28.md` (CP цены конкурентов +
CO корпоративный контур v1 — обе развилки «да» владельца).

- **CP-1:** карточка «📊 Market position» на `stays:reports` — публичные
  базовые цены «ab €» отелей платформы того же города (AggregatorListing
  KIND_STAY, схлопывание отель=min по своим номерам), позиция «N из M»,
  min/median/max, свой отель подсвечен. Гейт: ≥3 отелей включая свой (иначе
  карточка скрыта), город обязателен. ОСОЗНАННО без рекомендаций цены
  (картельный риск) и без живых цен по датам (v2). Замок: ранжирование/
  схлопывание/гейты. 5 msgid × 4 .po.
- Грабля: вставка хелпера над `def reports` перехватила `@login_required` —
  декоратор ушёл на хелпер (2 упавших замка поймали) → возвращён на вьюху.

## 2026-07-28 — CO-1: справочник компаний (корпоративный контур v1)

План `docs/pms-corporate-competitor-plans-2026-07-28.md` («корпоративный
контур — да» владельца). ⚠️ Миграции `crm/0002_company` + `promotions/0022`.

- TENANT-модель `crm.Company` (name/vat_id/street/postal/city/email/phone/note
  + `discount_percent` ≤50 — задел CO-3) + свойство `invoice_recipient`
  (многострочные реквизиты для счёта — потребитель CO-2).
- `Customer.company` FK SET_NULL (строковая ссылка "crm.Company" — crm
  импортирует promotions, не наоборот; удаление компании клиентов не трогает).
- Кабинет `/crm/firmen/`: список (счётчик клиентов, чип скидки) + создание/
  правка + удаление с confirm; вход кнопкой «🏢 Companies» из списка клиентов.
- Селект компании в форме клиента ТОЛЬКО при непустом справочнике (маленький
  бизнес без компаний видит прежнюю форму); чипы 🏢 в списке клиентов
  (обе раскладки, select_related — без N+1) и на карточке 360° (ссылкой).
- Тесты `test_companies.py` (5: CRUD, привязка+чипы, гейт формы, реквизиты,
  кап скидки). 13 msgid × 4 .po.

## 2026-07-28 — CO-2: счёт с реквизитами компании + фильтр CRM ?firma=

Без миграций (поверх CO-1).

- Создание счёта: корпоративный гость без явного получателя →
  `Invoice.recipient` = `company.invoice_recipient` (name+адрес+USt-IdNr);
  явный текст владельца всегда сильнее; хинт под полем Empfänger.
- CRM `?firma=<uuid>`: сужает список И CSV-экспорт (общий `_filtered_customers`),
  чип активного фильтра с ✕-сбросом, carry в пагинации/поиске/CSV-ссылке;
...
## 2026-07-28 — PMS-C: отчёты stays v2 — разрезы, LOS, CSV

Без миграций; `occupancy_report` (golden G9) не тронут — новый `breakdowns()`.

- Разрезы месяца «канал / тариф / категория» (bookings·nights·выручка,
  выручка пропорциональна ночам в окне, как G9; сортировка по выручке;
  таблица тарифов скрыта, пока тарифы не используются; пустой канал = direct).
- Ø Aufenthaltsdauer (LOS) по ЗАЕЗДАМ месяца (хвосты прошлых месяцев не
  размывают) + счётчик заездов; карточка в KPI-гриде.
...
## 2026-07-28 — PMS-B (остаток): авто-тег «stammkunde»

Без миграций. Beat `auto_tag_customers` (раз в сутки, все схемы): клиенты с
≥3 покупками (RevenueEntry — generic-деньги всех движков, как LTV) получают
тег `stammkunde` → питает сегменты купон-кампаний B4. Добавляется ТОЛЬКО этот
тег (чужие целы, ничего не удаляем); тег поддерживается автоматикой (снятый
вручную вернётся, пока клиент проходит порог). 2 теста.

## 2026-07-28 — PMS-B2: ДР-кампания «Geburtstagsgruß» (очередь «делай все», п.1)

План `docs/pms-b2-birthday-plan-2026-07-28.md`. ⚠️ Миграция `promotions/0023`
(Customer.birthday + choices kind).

- `Customer.birthday` (опционально; DSGVO-purge обезличивания обнуляет);
  ввод: CRM-форма клиента (date-инпут) + ЛК гостя (пусто = стереть, кривая
  дата — прежнее значение).
- `CouponCampaign.KIND_BIRTHDAY` + панель «🎂 Geburtstagsgruß» на
  /promotions/kampagnen/ (паттерн Auto Win-back: тумблер/Rabatt/Gültig/Betreff
  на кампании-синглтоне) + счётчик подписчиков с известной датой.
- Beat `send_birthday_coupons` (раз в сутки, все схемы): именинники из
  consented-базы [+tag], **29.02 в невисокосный год празднуем 28.02**,
  годовой дедуп окном 300 дней; коды/письма — `send_coupon_campaign`
  (идемпотентно, UWG §7 по построению).
- Тесты: 3 beat (+opt-in-гейт/дедуп/пауза/29.02) + purge birthday +
  профиль ЛК + CRM-форма. 7 msgid × 4 .po.

## 2026-07-28 — Telegram-канал в кампаниях (очередь «делай все», п.2)

План `docs/campaign-telegram-plan-2026-07-28.md`. Без миграций.

- Купон-кампании (вкл. win-back и Geburtstagsgruß — идут тем же
  send_coupon_campaign) и Newsletter-рассылки дублируются в Telegram
  привязавшим бота подписчикам: тот же consented-сегмент (DOI-опт-ин),
  one-click отписка глушит оба канала по построению, «Abmelden»-ссылка и в
  TG-тексте; идемпотентно своим `…:tg` dedupe_key; no-op без привязки/бота.
- Осознанно v2: клиенты «только Telegram» (DOI-флоу в боте) и команда /stop.
- Тесты +2 (дубль-канал/без привязки/идемпотентность; грабля: TelegramBot
  is_active default False — в тестах включать явно).

## 2026-07-28 — Фидбэк владельца: Belegungsplan первым + поиск + цветные плашки

Без миграций.

- **Отель — календарь первым:** карточка «📅 Belegungsplan» на главной кабинета
  (гейт `orders_view=calendar`, вне блока секций — видна и без броней);
  «Full view» встроенного канбана ведёт в календарь; канбан — дополнение.
- **Поиск в Belegungsplan:** `?q=` по имени гостя / e-mail / номеру брони
  (любые даты и статусы, топ-20) — панель результатов над сеткой, клик
  открывает карточку брони (`?von=…&buchung=…`).
- **Плашки без цвета — корень найден:** классы `bg-*-200/80` задаются в
  Python (bar_color) и не попадали в Tailwind-скан шаблонов → purge их
  вырезал, плашки выглядели голым текстом «под сеткой». Фикс: сплошные
  классы + safelist в tailwind.config.js (прецедент M20R-1) + пересборка
  app.css; дорожки прижаты к строке юнита (align-top) — плашки с ФИО
  читаются как часть сетки, занятость подтверждена цветом статуса.
- Замки: поиск (имя/почта/номер/пусто), solid-классы в рендере, карточка
  главной отель/не-отель. 4 msgid × 5 .po.

## 2026-07-28 — T-1 ЗАКРЫТ: массовый de.po активирован (немецкий кабинет/витрина)

План `docs/t1-de-po-revival-plan-2026-07-28.md` (очередь «делай все», п.3).
Без миграций; прод получит немецкий хром при следующей пересборке образа.

- de.po возрождён черри-пиком T1-b (`93e19cf`, 2477 записей DeepL+QA) и
  дозаполнен до 3050: identity для немецких msgid + ~85 ручных DE-переводов
  свежего англоязычного хвоста (G12/PMS-R/CO/PMS-A/C/D/B2) + QA-фиксы
  DeepL-коротышей («Board→Vorstand», «Stay→Bleib», «from→aus»,
  «%(m)s min→Ich habe», «In %(n)d days→In den Tagen von», «Kontakte→Kontakt»,
  «Messages→Beiträge», «About this service→Über diesen Dienst» и др.).
- `normalize` локале-стабилен (translation.override(None)) — хранимый
  site_config и golden-эталоны не зависят от языка запроса (влит ранее).
- ~60 англ. тест-ассертов в 25 файлах переведены на немецкий рендер
  (методика: сверка с фактическим de.po-значением); все 3 кластера локально
  зелёные под DE (~2800 тестов; .mo локально собирается polib —
  msgfmt в среде нет, это и разблокировало трек).
- Правка переводов дальше — rosetta (`/rosetta/`, прод-цикл T1-c).

## 2026-07-28 — Форма номера табами + сворачиваемый сайдбар (фидбэк владельца)

План `docs/unit-form-tabs-sidebar-collapse-plan-2026-07-28.md`. Без миграций.

- Форма StayUnit на units: «портянка» → табы Grunddaten (описание/переводы/
  кровать/площадь/удобства/фото) · Preise (цена/выходные/депозит) · Regeln &
  Kapazität (quantity/min_nights/max_guests/ручное подтверждение). ОДНА
  форма, панели скрыты только CSS (W0-замок «все поля в DOM»), Save с любого
  таба сохраняет всё; JS — делегирование на document (урок Ф1).
- Сайдбар кабинета: «⟨» сворачивает меню влево целиком, плавающая «☰»
  возвращает; localStorage без прыжка при загрузке (влит ранее, `82f0df6`).

## 2026-07-28 — «Lücken-Deal»: скидка на короткие промежутки между бронями

План `docs/gap-deal-plan-2026-07-28.md` (фидбэк владельца). ⚠️ Миграция
`stays/0029` (StaySettings.gap_max_nights + gap_discount_percent, аддитив).

- `pricing.gap_discount`: свободный отрезок ночей (free>0), зажатый
  ПОЛНОСТЬЮ занятыми ночами с обеих сторон, длиной ≤ N — весь диапазон гостя
  внутри → скидка; край «сегодня» границей не считается. Кандидат в G4
  `auto_discount` (opt-in kwargs unit/departure — без них байт-в-байт);
  действует на витрине И на стойке (семантика G4), label «Lücken-Deal −X%»
  виден в существующей разбивке цены.
- Кабинет: секция «💥 Lücken-Deal» на units (тумблер/ночи/%);
  **автопредложение** на Belegungsplan — карточка «Lücken gefunden …
  aktivieren?» с one-click (фича выключена + детектор нашёл люку ≤3 ночей в
  60 днях); **первичные настройки** — чекбокс на слайде «Angebot» мастера
  для stays-архетипов (дефолты 3 ночи / −25 %).
- 6 тестов (детектор/границы/сегодня/интеграция/кабинет+хинт/мастер).
  7 msgid × 5 .po.

### 2026-07-28 (вечер) — доводка «портянки» настроек номера (фидбэк владельца)

Фидбэк: «не вижу группировки в настройках номеров». Причина двойная — (1) на
сервере старый образ (табы `bd2da56` в main, но деплой не прогонялся), (2) сами
табы покрывали только поля формы. Замер стенда до правки: страница
`/dashboard/stays/units/` 9290 px, первый номер на 3164 px от верха.

- **Табы на весь блок номера** (5): Grunddaten (поля + фото), Preise (цены +
  Saisonpreise), Regeln & Kapazität (ёмкость + Sperrzeiten), 🚪 Rooms (счётчик
  в пилюле), 🔗 Calendar sync. `data-unit-tabs` переехал с формы на карточку;
  панель может принадлежать нескольким табам (`data-ut-panel="basis preise
  regeln"` на форме → она скрыта на Zimmer/iCal, где её Save ни при чём).
- **Глобальные настройки** (Ratenpläne, Automatik-Rabatte, Dynamische Preise,
  Lücken-Deal, Verkaufsregeln, Kurtaxe, Buchungs-Widget, Metasuche) — свёрнутые
  `<details>` со счётчиками, перенесены ПОД список номеров: страница открывается
  номерами, а не стеной опций.
- Инвариант W0 цел (скрытие только CSS/`<details>`, все поля в DOM) — замок
  `test_unit_tabs_cover_whole_card_and_settings_are_collapsed`.
- Замер после: 4844 px (−48 %), первый номер на 258 px; блок номера 613 px на
  Grunddaten, 215–369 px на остальных табах. Стенд Playwright: переключение
  табов, Save с непервого таба (Beschreibung со скрытого таба не теряется),
  открытие аккордеонов. 1 msgid × 5 .po. Без миграций.

### 2026-07-28 (вечер-2) — обложки разделов: один узкий слайдер (фидбэк владельца)

Фидбэк: «во всех архетипах/категориях главный баннер, потом текст и слайдер —
должен быть ОДИН узкий слайдер с пагинацией, кнопкой и текстом на нём». Это
S3-обложка `_archetype_cover.html` (баннер 16/6 → абзац intro → snap-галерея),
рендерится над каждым лендингом архетипа.

- `_archetype_cover.html` переписан: один слайдер h-56/h-64 (rounded-2xl) —
  слайды = hero-фото + галерея раздела (контекст собирает единый список
  `slides`, hero первым, без дублей), intro и кнопка НА слайдере, точки-
  пагинация + автопрокрутка 6с (1 слайд → без точек и JS). Без фото — прежний
  текстовый абзац. Кнопка: `button_label`/`button_url` из кабинета «Bereiche»
  (новые поля normalize `archetypes.*`), пусто → дефолт «Discover» (msgid был
  во всех 5 .po) с якорем `#cover-ende` вниз к содержимому.
- Кабинет: `/dashboard/site/sections/` — поля «Button label»/«Button link».
- ⚠️ ОСОЗНАННАЯ регенерация golden `normalize_detail_pages.json` (+2 пустых
  ключа в archetypes-оверрайде) + обновлён замок sanitize.
- Замер стенда (демо-отель /unterkunft/): шапка раздела 846 px → 256 px,
...
### 2026-07-28 (ночь-2) — каждый номер на своей странице (фидбэк «почему все на одной»)

Фидбэк: `/dashboard/stays/units/` рендерил ПОЛНУЮ карточку каждого номера
подряд — «это же разные номера, почему они настраиваются все на одной
странице?». Разнесено:

- НОВЫЙ роут `stays:unit-edit` = `/dashboard/stays/units/<pk>/` — та же вьюха
  `units(request, pk=None)`: с pk рендерится ТОЛЬКО карточка этого номера
  (5 табов + фото + Zimmer + Sperrzeiten + Saisonpreise + iCal), формы постят
  на неё же и redirect возвращает на неё; заголовок = имя номера, крошка
  «← Units». Глобальные настройки и форма нового юнита на странице номера
  не рендерятся, скрипт табов гейтится unit_page.
- Список стал КОМПАКТНЫМ: строка = превью-фото/имя/тип/цена/×кол-во/🚪 +
  «Bearbeiten →» + ★ Feature + Aktivieren; полной карточки на списке нет.
  «Neue Einheit» и глобальные аккордеоны — как были, под списком; после
  создания номера — редирект сразу на его страницу.
- Замки: test_units_list_is_compact_and_links_to_unit_page (список без
  data-unit-tabs, чужих номеров на странице номера нет, POST возвращает на
  /units/<pk>/), прежние таб-замки переведены на страницу номера.
- Стенд Playwright: 4 строки со ссылками, страница номера — 5 табов без
  соседей, Save возвращает на неё; высота списка 4844 → 2667 px. Без миграций.

### 2026-07-28 (ночь-3) — Verkäufe отеля: 3 чистых таба (фидбэк владельца)

Спецификация: основная вкладка — календарь и ТОЛЬКО календарь (клик по брони →
карточка ниже); доска — текущий канбан с настройками внутри; «＋ Buchung» —
отдельная вкладка только с формой; hub-табы «Board/Termine/Übernachtungen» не
нужны; на календаре остаются добавление брони и блокировка дат.

- **Календарь** (`stays/calendar.html`): убраны hub-табы и весь блок
  «Buchungen in diesem Zeitraum» (список + панели статусов FB-4b/FB-3 —
  дубль доски); остались шахматка, карточка брони по клику (#booking-panel),
  хаускипинг и walk-in форма («Add booking» + «Block dates» — клик по
  свободной клетке её префиллит).
- **«＋ Buchung» страницей**: walk-in форма вынесена в партиал
  `stays/_walkin_form.html`; НОВЫЙ роут `stays:stay-new` = `/dashboard/stays/neu/`
  (только форма); `create_option` сегмента ведёт туда (booking — якорь #neu
  как раньше), вкладка подсвечивается (active="create").
- **hub-табы**: в не-classic хабе «Verkäufe» скрыты 4 вкладки, покрытые
  сегментом (board/orders/booking/stays); Tickets/Aufträge остаются;
  classic_ui — полный tab-bar (замки test_hub_tabs обновлены осознанно).
- **Доска**: в ящик «⚙️ Spalten» добавлены ссылки «Status & Übergänge» по
  активным kind'ам (`status-manager`) — настройки статусов, ушедшие с
  календаря, достижимы из доски (как просил владелец).
- Замки: test_calendar_is_lean_…, обновлён test_calendar_transition_rule_…
  (через ?buchung=), create-опция → /stays/neu/. Стенд Playwright: сегмент
  «Board · Kalender · ＋ Buchung», календарь без списка, /neu/ только форма,
  2 ссылки статусов в ящике доски. Без миграций. Грабля rl:* в Redis снова
  ловилась (429 в test_public) — чистка scan_iter.

### 2026-07-28 (ночь-4) — список номеров: табы «Einheiten / Einstellungen» (фидбэк №2)

Фидбэк: «разделить табами — номера + кнопка добавить новый, и таб с настройками;
сгруппировать настройки, чтобы был порядок».

- **Табы страницы списка** (`data-pt-tab`/`data-pt-panel`, W0: панели в DOM,
  скрытие только CSS): «🛏 Einheiten» — компактный список + кнопка
  «＋ Neue Einheit» (details, форма раскрывается; пустой список — раскрыта
  сразу); «⚙️ Einstellungen» — глобальные настройки.
- **Группировка настроек** (4 блока, h3-заголовки): 💶 Preise & Rabatte
  (Ratenpläne · Automatik-Rabatte · Dynamische Preise · Lücken-Deal) →
  📅 Verkaufsregeln (окно бронирования + правила) → 🏛 Kurtaxe & Hausordnung →
  🔗 Kanäle & Export (Widget · Metasuche-Feed). Внутри — прежние свёрнутые
  details; заголовок «Settings for all units» убран (его заменил таб).
- **Save возвращает на свой таб**: настроечные действия редиректят на
  `?tab=einstellungen`; создание номера — на страницу нового номера.
  Начальное состояние табов рендерит сервер (?tab=), JS — делегированный
  переключатель + history.replaceState.
- Замок test_units_list_page_tabs_and_settings_groups (обе панели в DOM,
  порядок групп по эмодзи-маркерам, redirect'ы). 2 msgid × 5 .po. Стенд
  Playwright: дефолт список, переключение, Save → таб настроек активен,
  «＋» раскрывает форму. Без миграций.
- Доводка по адверсариальному ревью-workflow (10 агентов, все находки
  верифицированы): (1) hover:bg-gray-200 прилипал к активной пилюле после
  JS-переключения (белый текст на сером) — фикс в обоих переключателях
  (page-табы + табы формы номера, где баг жил с прошлого инкремента);
  (2) замок «создание → страница номера» усилен до точного endswith
  /units/<pk>/ (мутационная проверка показала, что старый пропускал регресс).
  + пересборка app.css (урок CI-1121 снова: first:mt-0 в заголовках групп).

### 2026-07-28 (ночь-5) — под-табы настроек номеров + Termine + мобильная адаптация кабинета

Три запроса владельца одним батчем.

**1. Настройки номеров в виде табов.** Панель «Einstellungen» списка номеров:
4 группы-заголовка → 4 ПОД-ТАБА (`data-st-tab`/`data-st-panel`): Preise &
Rabatte · Verkaufsregeln · Kurtaxe & Hausordnung · Kanäle & Export. Полоса
скроллится на мобиле; одиночные карточки в своих табах раскрыты; Save
настройки возвращает на таб И секцию (`?tab=einstellungen&sec=<key>`),
начальное состояние — с сервера. W0 цел (все панели в DOM).

**2. Страницы Termine («сдвинуто влево и пусто»).** Дефолтный грид услуг был
`max-w-3xl` — на 1280px карточки занимали левые 768px. Теперь 1/2/3 колонки на
всю ширину (характеризационный замок обновлён осознанно). Новый партиал
`storefront/_booking_info.html` — под листингом для ВСЕХ booking-архетипов:
«как проходит запись» (3 шага) + часы/адрес+карта/телефон-WhatsApp-контакт,
всё по наличию данных. Ветка ресурсов и пустое состояние (`booking_index.html`)
перестали быть голой строкой. Демо-фото: префиксный фолбэк файла
(`hair,styling` → `hair-salon.webp`, детерминированно по lock; кэш индекса по
корню каталога) + эмодзи-карта расширена сервисными темами (💇/💈/🔧/🚗/🧰/…),
гастро-«тарелка» осталась только для съедобных ключей — в карточке стрижки она
читалась как ошибка. Отелю добавлены гостевые услуги (массаж/сауна/завтрак в
номер/e-bike) — его Termine-страница была пустой. 13 msgid × 5 .po.

**3. Мобильная адаптация кабинета** (по аудиту Explore-агента, 390px):
`<main>` += `min-w-0 overflow-x-clip` (страховка каркаса); тумблер Einfach/
Experte больше не скрыт на мобиле; 9 таблиц (товары/категории/акции/
резервации/ваучеры/лояльность/закупки/уведомления) обёрнуты в
`-mx-4 px-4 overflow-x-auto` + `min-w-[…]`; `flex-wrap` в шапках и строках
полей (CRM, заказы, акции, календари stays/booking, настройки, статус-менеджер,
доска, журнал, событие); гриды без мобильного префикса → `grid-cols-1 sm:*`
(кампании, смета Handwerker, импорт, партнёры, Finder); тосты `bottom-20
md:bottom-*` (не накрывают таб-бар); колонка канбана `w-[85vw] max-w-[16rem]
sm:w-64 snap-start`. Стенд Playwright 390px: на 9 страницах кабинета
горизонтальный скролл = 0, элементов шире экрана вне скроллящихся обёрток нет.
Без миграций.
- Вторая порция ревью того же батча (итог воркфлоу: 39 вердиктов, 22
  подтверждено / 17 опровергнуто; первые 10 закрыты в 3ba4569): молчащие
  ветки настроек (toggle/delete тарифов, удаление авто-скидок/правил/
  occupancy) не давали ни сообщения, ни видимого следа в свёрнутой карточке →
  `messages.success`; pt-обработчик терял активный под-таб в URL (F5 сбрасывал
  секцию); `aria-selected` на обычных `<button>` не поддерживается (axe-
  нарушения, состояние не озвучивается) → `aria-pressed` на всех трёх уровнях
  табов; немецкие msgid «Grunddaten/Preise/Regeln & Kapazität» не переводились
  на не-DE языках → английские msgid + 3×5 записей; забытый 14-й msgid
  «Contact» добавлен в 5 каталогов. Три замка были вакуумными (проверено
  мутациями ревьюеров): сумма 8 карточек не различала распределение 4+4;
  «нет глобальных настроек на странице номера» не ловил частичную утечку;
  layout-движок листинга услуг проверялся подстрокой, попадающей и в новый
  дефолт; мой замок стабильности фото не исполнял формулу (алиас
  короткозамыкал) — все четыре усилены. Отложено в roadmap: `--accent-fg`
  (контраст CTA на светлых Look-акцентах, пред-существующее).

### 2026-07-29 — типы акций явными (фидбэк «не понимаю какие типы акций») + рынок DACH

Фидбэк владельца по aktionsmarkt: 12 акций с разными механиками рендерились
неотличимыми карточками. Сделано (без миграций):

- **Чипы механики** — новый part="flags" единого `_discount_display.html`
  (существующие ветки не тронуты — parity-замки целы): 📦 «Online reservieren,
  im Laden abholen» (promo_type=reservation) · 🔁 «Jeden Tag / Jede Woche»
  (recurrence) · 🆕 «Neu» (новое свойство `Promotion.is_new`: старт ≤ 7 дней,
  повторяющиеся не считаются) · 📅 «bis dd.mm.» (срок для не-countdown акций,
  Prospekt-привычка). На карточке (sm) и детальной (lg).
- **Группы — секциями**: /aktionen/ без фильтра рендерит секции с заголовками
  и счётчиками (порядок первого вхождения в выдачу, безгрупповые в конце под
  «Weitere Angebote»); выбранный чип-фильтр — прежняя плоская сетка.
- **Рынок DACH**: строка выгоды «Sie sparen X € (−Y %)» на детальной
  (part="savings"; % вне blocktrans — урок msgfmt); чип срока действия на
  карточке.
- **Демо aktionsmarkt**: + «Mystery-Deal der Woche» (mystery был единственным
  стилем вне showcase); сидер раскидывает starts_at (−10 дней), «Neu» — только
  у помеченных spec'ов (иначе у свежего сида все 13 были «новыми»).
- Замок стиля badge уточнён осознанно: скрыт именно БЕЙДЖ процента
  (`>−25 %</span>`), строка выгоды легитимна. 3 новых теста
  (test_promo_flags), 7 msgid × 5 .po. Проверено на пересеянном стенде
  (секции/чипы/mystery/«Sie sparen» в HTML + скриншоты владельцу).

### 2026-07-29 (№2) — деталь акции: попап-форма + галерея + стилизованный блок цены

Фидбэк владельца: «вместо открытой формы — кнопку и попап; карточку с галереей
под главным фото; блок цены стилизовать». Без миграций:

- **Попап-форма**: buy-box (reserve / sold-out→waitlist) переехал из открытой
  простыни в модал за CTA-кнопкой («Reserve now» / «🔔 Notify me» при sold-out;
  бордовая на var(--accent), подпись «Ohne Konto — in 30 Sekunden»). Внутри
  модала тот же единый `_buybox.html` — паритет-замки целы (форма всегда в
  DOM). Модал: mobile bottom-sheet / desktop по центру, закрытие фон/✕/Esc,
  body-scroll lock; при ошибках формы (ре-рендер POST'а reservation_create)
  модал открыт сразу — покупатель видит ошибку.
- **Галерея**: миниатюры под главным фото (новое свойство
  `Promotion.gallery_images` — своя галерея, иначе фото товара; лента при >1),
  клик свапает главное изображение (делегированный JS, активная миниатюра —
  ring); mystery-blur цел.
- **Блок цены**: тонированная карточка (gradient red-50→amber-50, ring) —
  цена + «Sie sparen» + чипы механики + остаток + countdown + CTA одним
  визуальным блоком.
- Сидер: `spec["images"]` списком (галерея из ≥2 фото); демо Kaffee/Bio-Kiste
  — по 3/3 фото (разные файлы: coffee-ground/coffee-cafe/espresso — токен-
  фолбэк резолвил три ключа в один coffee.webp, свап был невидим).
- 2 новых замка (модал скрыт/открыт-при-ошибках + галерея + sold-out CTA);
  2 msgid × 5 .po. Стенд: свап миниатюр, открытие/Esc, форма в модале.

## 2026-07-30 — Блок цены детали акции по референсу владельца (aktionsmarkt)
Фидбэк («Блок цены нужно ещё переработать, посмотри можно что-то такое»,
скрин e-commerce-детали): иерархия «сначала итог» + сегментный отсчёт.
- **Иерархия цены** (белая карточка ring): крупная итоговая (text-4xl, красная
  при скидке) → под ней зачёркнутая старая + пилюля скидки `data-price-badge`
  (−% или −€ при стиле badge) → «Sie sparen X € (−Y %)» → строка наличия
  (✓ Verfügbar / «Nur noch N» / Ausverkauft) → чипы механики → CTA. Стили
  UE2-2 уважены: mystery (reveal), festpreis/ab (без зачёркнутой), strikethrough
  (без пилюли); валюта через `cursym` (паритет с `_price.html`).
- **Регресс-фикс по ходу**: акция только-с-процентом (цен нет) теряла скидку
  на детальной (absolute-бейдж с фото убран) → новая ветка: крупный «−30 %»
  вместо цены (размер скидки и есть оффер).
- **Сегментный отсчёт** (референс): промо-баннер accent-фоном «🏷️ <группа>»
  + белая вложенная карточка `data-cd` «Aktion endet in: Tage|Std.|Min.|Sek.»
  (grid-cols-4, tabular-nums, page-JS тикер); без тикера — прежняя строка
  «Gültig bis». Карточки листинга не тронуты (прежний `data-countdown`).
- 3 замка обновлены ОСОЗНАННО (bейдж детали → пилюля/крупный %, деталь
...
## 2026-07-30 (вечер) — Фидбэк владельца ×4 + ревью-фиксы M3 (HIGH ×2)
- **Меню**: пункт-группа «Aktionen» была НЕкликабельной (`<span>`, только hover) →
  (1) группа, все дети которой promo_group, получает url `/aktionen/` (menu.py);
  (2) любая группа без url раскрывается КЛИКОМ (`<button data-nav-group>` +
  делегированный toggle в _base) — тач-экраны и клик мышью.
- **Шапка отеля**: убраны «технические» пункты FAQ / Über uns / Jetzt buchen
  (секции и страница About остаются, ссылки в шапке — нет).
- **Услуги отеля**: витрина писала «нет доступных» — у кита не было `resources`
  (AvailabilityRule живёт на Resource) → добавлены 2 ресурса (Wellnessbereich
  09–19, Rezeption 08–20, слот 30, ежедневно) → слоты появились.
- **Календарь услуг у отеля был недостижим** из хаба «Verkäufe» (ST-5b показывал
  только primary-календарь stays) → сегмент-контрол получил ВТОРОЙ календарь
  (Termine/Belegungsplan) при обоих активных модулях.
- **«Bearbeiten» номера открывал СПИСОК** (`sellable_manage["stay"]["edit"]` был
  не per-pk) → `stays:unit-edit` с pk; та же правка в мастере (setup_steps).
- **Корзина**: без доставки шаг «как получить» скрывался молча → явная плашка
  «🛍 Abholung im Geschäft» + адрес (запрос владельца).
- **Ревью-workflow M3 (4 измерения, адверсариальная верификация) — 2 HIGH
  подтверждены и исправлены:** (1) beat-экспайр применял cancelled по
  устаревшему снапшоту (без select_for_update/re-check/atomic — клон потерял
  guard оригинала promotions.expire): оплаченный/выданный резерв мог быть
  отменён с возвратом стока проданной вещи → per-order atomic + лок + повторная
  проверка + логирование; (2) ремап писем глушил owner-ветку — владелец НЕ
...
## 2026-07-30 (ночь) — Фидбэк отеля: сезоны/разбивка цены + Lücken-Deal per-Zimmer; M4-D
Порядок владельца («1-3» + таймер). Разведка (workflow, часть агентов срезал лимит сессии).
- **Мультисезонный расчёт — ПРОВЕРЕН, дефекта нет**: `pricing.quote_total_cents` идёт
  `nights_between` и берёт `nightly_price_cents(unit, day, seasons)` на КАЖДУЮ ночь;
  плоских `price × nights` в путях нет (витрина/walk-in/перенос/events зовут ту же
  функцию). Закреплено замками: бронь через 2 сезона (22.12→27.12 = 2×80 + 3×200),
  сезон+база (30.06→04.07), 3 периода не нужны — механика та же.
- **Прозрачность (новое)**: `pricing.price_breakdown()` — строки «дата · сезон/выходные ·
  цена», сумма СОВПАДАЕТ с quote_total (замок); в кабинете — `<details>` «Preis pro Nacht
  anzeigen» на карточке брони (FB-11 партиал, обе поверхности).
- **Lücken-Deal per-Zimmer** (⚠️ миграция `stays/0030`, аддитив): `StayUnit.gap_max_nights`
  + `gap_discount_percent` (0 = наследовать общие); `pricing.gap_discount` — приоритет
  юнита над StaySettings; поля в блоке «Preise» карточки номера (presence-сентинел
  `gap_present`, чтобы список не затирал). Замок: 0/0 → общие; свой % перебивает;
  свой лимит длины люки перебивает общий в обе стороны (лимит меряет ЛЮКУ, не бронь —
  тестовое ожидание было неверным, код корректен).
- **M4-D** (волна M4 бутика, план `m4-boutique-plan-2026-07-30.md`): «Nur noch X € bis
...
## 2026-07-30 (ночь, продолжение 2) — M4-B Lookbook + i18n-аудит (3 разведки) + I18N-1
- **M4-B Lookbook** (⚠️ миграции `catalog/0020` + `collections/0002`): `Product.collections`
  M2M + `Collection.images`; публичная `/lookbook/<slug>/` (галерея + товары образа);
  фасет `?kollektion=` в `CatalogFacets` + чипы каталога; кабинет подборок открыт для
  каталога (404-гейт booking/stays снят — catalog core), чекбоксы товаров с presence-guard,
  загрузка фото; демо CLOTHING — «Herbst-Looks»/«Business» с фото, «Basics» без.
  Побочно: `core.finder` теперь матчит товары по подборке. Осознанно обновлён замок
  `test_cabinet_renders_checkbox_rows_and_gates_modules` (пинил снятый 404).
- **i18n-аудит (3 параллельные разведки)** — план `docs/i18n-full-coverage-plan-2026-07-30.md`.
  Ключевое: `.po` формально полны (0 пустых/0 fuzzy, наборы msgid совпадают, плейсхолдеры
  целы), **de.po существует и в git**; настоящая проблема — **~1100 строк вообще не доезжают
  до `.po`** (choices моделей ~292, реестры ~290, формы ~45, messages 21, views ~60, PDF ~100,
  шаблоны кабинета 73). Плюс дефекты: `%%` в msgstr 4 локалей (рендерился бы буквально),
  de.po отставал на 37 msgid, 3 немецких хвоста в en, точечные смысловые ошибки ru/uk/tr.
- **I18N-1 (первая волна)**: 216 меток `choices` в 22 файлах обёрнуты в `gettext_lazy`
  (скриптом по AST; грабля — `ast.col_offset` в БАЙТАХ, посимвольные срезы рвали строки
...
- **I18N-4 (2026-07-31):** 25 label'ов формы мероприятия/ретрита (самые
  «инструктивные» подписи кабинета) + 36 меток колонок мастера импорта
  (PRODUCT/VARIANT/PROMOTION_FIELDS) + подпись файла; +55 msgid × 5 .po.
- **I18N-8:** 14 смысловых правок ru/uk/tr/en («Meldebestand» → мин. остаток,
  «Artikel» → товар, «geschwärzt» → окрашено в чёрный, пара Eingelöst/Einlösen,
  en-фраза про уведомление ПОКУПАТЕЛЕЙ вместо владельца).
- **Дымовой замок** `apps/core/tests/test_smoke_i18n.py`: ключевые метки реально
  резолвятся в ru/tr (статусы, типы бизнеса, стили секций, Finder, форма события).
- **I18N-7 (PDF/письма/постеры) ОТЛОЖЕН за решением владельца** — там документы,
  а не UI: DATEV-заголовки переводить нельзя, для Rechnung нужен явный выбор
  «язык клиента vs язык бизнеса» (развилка и рекомендация — в план-доке §3).

## 2026-07-31 — M4-C «Merkzettel» (список отложенного)
Третий пункт волны M4 бутика (план `m4-boutique-plan-2026-07-30 §C`), БЕЗ миграций.
- НОВЫЙ `apps/orders/wishlist.py` — хранение в СЕССИИ по образцу корзины: ничего о
  неавторизованном посетителе в БД (DSGVO-чисто), новое первым, кап 60 позиций,
  мёртвые pk молча выпадают из выдачи (товар мог быть скрыт после добавления).
- Опция витрины `wishlist` в site_config: дефолт по архетипу (clothing/retail/
  online_shop — да; гастро/услуги — нет), явный тумблер сильнее архетипа.
- Сердечко во ВСЕХ ТРЁХ раскладках карточки (compact/overlay/default) — позиции
  разные, чтобы не столкнуться с контролами канвы (✎/📷 держат правый верх);
  на детали — статичной строкой под buy-box.
- Иконка-счётчик в шапке; страница `/merkzettel/` карточками каталога; toggle
  через AJAX (сердце и бейдж перекрашиваются на месте), фолбэк — обычный POST
  (работает без JS).
...
## 2026-07-31 — M4-A: цвет × размер + фото варианта (волна M4 бутика ЗАКРЫТА)
Последний пункт волны (план `m4a-variant-axes-plan-2026-07-31.md`; порядок волны
был D→B→C→A). ⚠️ Миграция `catalog/0021` (аддитив: size/color/images на
ProductVariant).
- **Оси рядом с label, а не вместо него.** `label` — денормализованный ключ, на
  котором держатся склад-леджер, `OrderItem.title_snapshot`, PDF, товарный фид и
  CSV-импорт; замена ключа = переписывание пяти подсистем. Поэтому `size`/`color`
  необязательные, `save()` собирает label из осей ТОЛЬКО когда он пуст (старые
  варианты и ручной ввод не трогаются), constraint не менялся.
- **Фасет Größe — единый резолвер** `size_axis = Coalesce(NullIf(size,""), label)`:
  чипы и фильтр смотрят в ОДНО значение. Первая версия переключала поле глобально
  («есть хоть один вариант с осью → всё по size») — в смешанном каталоге это
  выкидывало легаси-товары из чипов и выдачи; замок `test_mixed_catalog_…` ловит.
- Витрина: оси и фото варианта — `data-`атрибутами поверх ЕДИНСТВЕННОГО
  `select name="variant"` (набор полей формы пинится `test_buybox_parity`), выбор
  подменяет главное фото галереи (`.js-media-main`); без JS — прежний селект.
- Кабинет: поля размера/цвета в строке варианта (сентинел `axes_present` —
  инвариант W0), заведение варианта осями (label соберётся сам).
- CSV-импорт: матч по `(product, size, color)` → фолбэк на label (в т.ч. на
  СОБРАННЫЙ из осей) — иначе повторный импорт задвоил бы варианты; вариант,
  заведённый до осей, дополняется, а не дублируется.
- Демо CLOTHING: платье 3 размера × 2 цвета с фото на цвет, футболка 4 × 2;
  кардиган остался на легаси-label (демонстрирует смешанный каталог + Warteliste).
- 3 msgid × 5 .po. ⚠️ ops: после деплоя `seed_demo_tenants --kit clothing --recreate`.

## 2026-07-31 — Дрейф состояния миграций после I18N-1 (aggregator/0015, NO-OP)
`makemigrations --check` показывал незакоммиченное изменение: обёртка меток
`choices` в `gettext_lazy` (I18N-1) видна автодетектору, когда активная локаль
реально переводит метку (после активации de.po — это уже так). Дрейф был один —
`AggregatorPortal.business_type`. Сгенерирована миграция `aggregator/0015`:
choices-only AlterField, **DDL не порождает** (choices живут в Python, не в БД).
Урок к конвенции «метки choices миграций не требуют»: схему они правда не меняют,
но состояние автодетектора — меняют, и следующий `makemigrations` затянул бы их
в чужую миграцию. Проверять `makemigrations --check` в конце батча.

## 2026-07-31 — I18N-9: makemessages починен + 177 непокрытых msgid + CI-гейт
Стенд (кабинет демо `mode` на ru) показал немецкие «Sichtbar»/«Bearbeiten» на
`/dashboard/angebote/`. Расследование вскрыло СИСТЕМНУЮ причину, а не опечатку:
- **`makemessages` падал** (`xgettext: msgid 'Only %(n)s left' is used without
  plural and with plural`) → `.po` велись руками → строка, обёрнутая в
  `{% trans %}` без ручной записи в `.po`, молча выводилась по-немецки.
- Причина конфликта — `_buybox.html` брал ту же строку через `blocktrans with`,
  а акции через `blocktrans count`. Побочно: singular-форма НЕ находила запись в
  `.po` вообще, т.е. «Nur noch N übrig» не переводилось нигде, включая de.
  Фикс: buybox переведён на count-форму (msgid тот же).
- **Сверка кода с `.po`:** извлечено 3891 msgid, в `.po` не было **401**.
  Из них 177 — реальный UI (склад/закупки, карточка брони отеля, статус-менеджер,
  Angebote, Finder, модули, мастер, публичные страницы платформы) → переведены
  на **все 5 локалей** (de/en — identity там, где msgid уже на этом языке).
- **224 оставшихся — маркетинговая копия Branchen-лендингов**
  (`apps/tenants/archetype_pages.py`): SEO-тексты под DACH-рынок. Перевод — не
  техдолг, а решение владельца (тон/ключевые слова), поэтому вынесен в
  allowlist гейта и в план §3, а не «сделан молча».
- **Замок от регресса:** `scripts/i18n_gap.py` (makemessages во временную локаль
  `xx` → diff с реальными `.po`; ru/tr/uk дополнительно проверяются на пустой
  msgstr) + шаг «i18n coverage (I18N-9)» в CI перед ruff.
Урок: наличие `{% trans %}` ≠ наличие перевода. Пока экстракция сломана, гейта
нет и разрыв копится молча — поэтому гейт важнее разового прохода.

## 2026-07-31 — Демо-витрины: первый экран на en/ru/uk/tr (81 строка × 4)
Продолжение I18N-9, вторая половина запроса «все языки во всех уголках кабинета
И ДЕМО-ВИТРИН». Механизм перевода демо (`demo_i18n_<loc>.json`, DL-волна) был на
месте, но словари не знали строк китов, добавленных ПОСЛЕ той волны
(baeckerei/metzgerei/cafe/mode/touren + доработки прочих).
- Замер: из 1505 строк китов без ru — 559; в подмножестве «первый экран + доверие»
  (hero/heroes/about/faq/testimonials/process/trust/cta/usp) — 157 уникальных.
  Из них реально переводимых 81: остальное — ключи фото («bakery,bag»), имена
  людей и названия заведений (Backhaus Krume, Bella Vista…), которые переводить
  нельзя, и марки доверия (Slow Food, Specialty Coffee).
- 81 строка × 4 локали влита в `demo_i18n_{en,ru,uk,tr}.json` (формат файлов
  сохранён: en — без отступа, остальные — 2 пробела; порядок ключей сортирован).
- Проверено на стенде: главная демо `mode` на ru больше не содержит немецкого
  хрома и немецких блоков первого экрана (остаются адрес и тексты отзывов —
  это контент, отзывы намеренно не переводим).
Остаток (~478 строк на локаль) — описания отдельных товаров/услуг/номеров;
делать по спросу, механизм и словари готовы.

## 2026-07-31 — I18N-7b: документы (PDF) на 5 языках — ПОЛНЫЙ вариант
Решение владельца («полный», а не гибрид) — план `docs/i18n-7b-documents-plan-2026-07-31.md`.
Переведены все пять генераторов: Rechnung (`finance/pdf`), Angebot
(`jobs/pdf`), Lieferschein (`orders/pdf`), Teilnehmer-Infoblatt (`events/memo`),
Schaufenster-Poster (`promotions/poster`).
- **Общий слой** `apps/core/documents.py`: резолвер языка (`?lang=` → активный
  язык поверхности → LANGUAGE_CODE, валидация по локалям тенанта), регистрация
  TTF, локале-зависимые `money/qty/doc_date/doc_datetime`.
- **Шрифт.** Встроенный Helvetica у reportlab — WinAnsi. Кириллицы там нет
  ВООБЩЕ, а турецкие ı/ş/ğ стенд показал как разрыв слова («Açıklama» →
  «A ç n klama»). Поэтому: регистрируем DejaVu (в образ добавлен
  `fonts-dejavu-core`), а если шрифта нет — язык документа честно понижается до
  английского вместо «квадратов».
- **Кабинет:** рядом с кнопкой скачивания — пилюли языков (`{% doc_langs %}`),
  владелец отдаёт клиенту счёт на его языке, не меняя язык кабинета. Памятка
  участника печатается на языке, выбранном гостем на витрине.
- **НЕ переводим (осознанно):** DATEV-экспорт (машинный формат, заголовки
  фиксированы спецификацией), немецкие идентификаторы `USt-IdNr.`/`Steuernummer`
  и ссылку на § 19 UStG — по ним документ опознают получатель и налоговая.
- **Три дефекта, которые нашёл стенд (а не тесты):**
  1. `gettext` внутри f-строки **не извлекается** xgettext'ом — `f"{_('Invoice')}…"`
     молча оставался непереведённым, и гейт I18N-9 его не видел (строки не было
     и в экстракции). Вынесли в переменные: `title = _("Invoice")`.
  2. Немецкие переводы-артефакты вылезли в счёте: `Net` → «Nicht» (отрицание!),
     `Quote` → «Zitat» (цитата), `Ticket` → «Fahrkarte». Исправлены на
     Netto/Angebot/Ticket.
  3. `Invoice.number_display` возвращал жёсткое «Entwurf» → теперь `_("Draft")`.
- 19 msgid × 5 локалей (включая плюрал «%(n)s place» с 4 формами у ru/uk);
  8 замков (`apps/core/tests/test_documents_i18n.py`), 1495 смежных зелёные.
⚠️ ops: образ пересобрать (появился `fonts-dejavu-core`).

## 2026-07-31 — Переводы документов: фикс омонимов в de/tr/ru/uk
Продолжение I18N-7b. Стенд (реальный текст из PDF) показал системный класс
ошибок DeepL-перевода: английский msgid переведён не в том значении.
- «Net» → de «Nicht», tr «Değil», ru «Нет», uk «Ні» — **отрицание** вместо
  «нетто» в блоке итогов счёта. → Netto / Net / Нетто / Нетто.
- «Quote» → de «Zitat», tr «Alıntı», ru/uk «Цитата» — цитата вместо сметы.
  → Angebot / Teklif / Смета / Кошторис.
- «Ticket» → de «Fahrkarte» (проездной) → Ticket.
- «Draft» → ru/uk «Проект» → Черновик / Чернетка (в счёте это черновик).
- «Amount» и «Quantity» в tr оба были «Miktar» — в накладной колонки совпадали;
  Amount → «Tutar».
Урок: короткие английские msgid — главный источник таких ошибок; проверять их
надо НА ПОВЕРХНОСТИ (стенд), потому что тесты сверяют байты PDF, а не смысл.

## 2026-07-31 — I18N-7b/2: язык финансового документа выбирается при выставлении
Фидбэк владельца: «в финансовых документах нужно выбирать, на каком языке
выставлять». Разовая ссылка `?lang=` для этого не годится: счёт с одним номером
обязан всегда давать ОДИН документ (GoBD), иначе повторное скачивание на другом
языке кабинета порождает второй вариант того же счёта.
- ⚠️ Миграции `finance/0006_invoice_language` + `jobs/0012_job_language` (аддитив).
- **Счёт:** селектор языка в форме создания черновика (дефолт — язык бизнеса
  `Tenant.default_locale`) + смена языка у ЧЕРНОВИКА на карточке; при выставлении
  язык фиксируется (если не выбран — язык бизнеса) и дальше показывается
  read-only. `?lang=` для выставленного счёта игнорируется.
- **Смета (Angebot):** тот же выбор на карточке заявки — клиент повторно скачает
  тот же документ.
- Языки выбора = всё, что мы умеем печатать (локали витрины + языки кабинета):
  язык клиента ≠ язык сайта, турецкому клиенту немецкого магазина нужен
  турецкий счёт.
- Накладная (Lieferschein) осталась на разовом `?lang=` — она не нумеруется и
  не является финансовым документом.
- Замок-урок: прежний тест сравнивал БАЙТЫ PDF, а в них есть CreationDate/ID —
  он прошёл бы и без перевода. Переписан на сравнение ТЕКСТА (ASCII85+zlib →
  строки `(…)`): «Rechnung/Netto» в de, «Invoice/Net» в en, немецкие подписи не
  протекают в EN-документ. 12 замков, 1023 смежных зелёные; 3 msgid × 5 локалей.

## 2026-07-31 — HF-0: два мобильных бага отеля (фидбэк владельца, пп. 12–13)
Волна HF (план `docs/hotel-feedback-wave-2026-07-31.md`), без миграций.

- **#12 «перешёл на номер сразу — не выбираются несколько дат».** Репро на
  мобильном вьюпорте (Playwright 390×844, `--host-resolver-rules`) показало: JS
  выбора диапазона исправен, а календарь открывался на ТЕКУЩЕМ месяце всегда.
  31-го числа это ровно один кликабельный день — гость тапает, ничего не
  собирается, и делает вывод «не работает». Новый резолвер
  `public_views._calendar_start`: без дат листаем вперёд до первого месяца, где
  свободных дней ≥ 2 (заезд + выезд), в пределах окна брони; с датой заезда —
  поведение прежнее (её месяц). Посчитанные дни прокидываются в
  `_calendar_context(days=…)` — второго запроса за тот же месяц нет.
- Дополнительно к тому же пункту: полностью занятый месяц теперь ГОВОРИТ об
  этом («Dieser Monat ist ausgebucht — … ›») вместо молчаливой серой сетки;
  стрелки месяцев ‹ › выросли 32→44 px (мишень для пальца); занятый день-выезд
  получил `cursor-pointer` — Safari на iOS не поднимает делегированный click с
  неинтерактивного элемента без курсора-указателя, из-за чего checkout-хук на
  телефоне не срабатывал вовсе.
- **#13 «поле дат вылезает за границы блока».** Причина не в конкретной форме:
  у `input[type=date]` в iOS большая intrinsic-ширина, а ячейка грида по
  умолчанию `min-width:auto` → колонка растягивается. Точечные `min-w-0` уже
  стояли в части форм, но каждая новая форма приносила баг заново — поэтому
  правило одно и глобальное в `static/src/app.css` (`min-width:0;max-width:100%`
  для date/time/datetime-local). Плюс закрыты два реальных пробела: грид
  `_stay_search.html` и поле `hh_bis` в `_hero_widget.html`.
- Замки: `test_calendar_starts_at_first_month_with_selectable_days` (в т.ч.
  паритет «с датой заезда — её месяц»), `test_fully_booked_month_says_so`;
  270 тестов stays зелёные. 2 msgid × 5 локалей, `i18n_gap` чистый.

## 2026-07-31 — HF-1: услуги/акции/«О нас»/новости в меню и на главной
Волна HF (план `docs/hotel-feedback-wave-2026-07-31.md`), пп. 7/8/9/10/14. Без миграций.

- **Акции стали полноценным пунктом меню.** У модуля `promotions` не было
  `storefront_landing`, хотя страница `/aktionen/` существовала всегда — из-за
  этого узел меню `archetype/promotions` не резолвился, и «Aktionen» в шапку
  можно было положить только сырым URL. Добавили landing (+ `storefront_teaser=
  False`: акции — пункт меню, но не «раздел» бизнеса; на главной уже есть блок).
  Побочно акции теперь попадают в `llms.txt`.
- **Дефолтная шапка выросла на три пункта** — `promotions`, `blog`, `about`.
  Первые два гейтятся своим модулем, «Über uns» доступна всегда (страница
  витрины есть у каждого бизнеса). Владелец не находил ни новостей, ни страницы
  «О нас», потому что в дефолте их не было вовсе.
- **Секция «Neuigkeiten» на главной** — новый ключ реестра `SECTIONS` (выкл по
  умолчанию), партиал `sections/_blog.html`, лимит 3, настраиваемый заголовок/
  интро/«View all», якорь `#neuigkeiten`, `SECTION_ARCHETYPE_MODULE["blog"]`.
  Двойной гейт: секция включена И есть опубликованные посты — пустого заголовка
  на главной не появится. Модуль блога существовал давно (`events.BlogPost`,
  кабинет, витрина) — не хватало только входов.
- **Демо отеля наполнено:** меню += Wellness & Extras / Angebote / Neuigkeiten /
  Über uns; три акции для НОМЕРОВ и пакетов (Frühbucher, 3 Nächte zum Preis von 2,
  Wellness-Paket) — модуль promotions у типа `hotel` лишь `suited_for`, поэтому
  включён явно; три новости с фото. Секция акций в демо раньше зависела от
  наличия каталога — теперь включается и по промо-спеке (у отеля каталога нет).
- ⚠️ **Осознанная регенерация golden-эталонов normalize** (4 файла): три новых
  пункта nav/menus + секция `blog`. Диффы проверены — других изменений нет.
...
## 2026-07-31 — HF-4: визуальный календарь доступности услуг
Волна HF, п. 1. Без миграций. `/dashboard/booking/verfuegbarkeit/`.

- Движок был готов давно (`AvailabilityRule` — недельные окна, `ClosedDate` —
  точечные выходные), но управлялся списком дат в форме: владелец не находил,
  где «закрыть продажи», и не видел картины месяца. Теперь месяц-сетка, клик по
  дню закрывает/открывает продажи (тот же `ClosedDate`, повторный клик снимает).
- Селектор «Ganzer Betrieb | конкретный ресурс»: закрытие пишется на выбранный
  ресурс. Чужой ClosedDate (всего бизнеса при выбранном ресурсе) виден, но не
  кликабелен — снять его можно только там, где он поставлен.
- Цветом видно ТРИ состояния: bookable / закрыт вручную / **нет недельных часов
  на этот день недели**. Последнее и было причиной витринного «keine freien
  Termine»: раньше причина нигде не называлась. Плюс явное предупреждение, когда
  расписания нет вовсе.
- Входы: со списка услуг, с дневного календаря записей; со страницы календаря —
  обратно к «Öffnungszeiten & Ressourcen» и к любой услуге.
...
## 2026-07-31 — HF-6a/b + доводка HF-5 по адверсариальному ревью
⚠️ Миграция `booking/0019_booking_group_code` (аддитив).

- **Ревью батча HF-1..HF-5 (workflow, 5 измерений × скептик-верификатор) нашло
  реальный дефект доверия:** страница услуги печатала БАЗОВУЮ цену, а бронь
  списывала СЕЗОННУЮ (HF-5). Для §1 PAngV и просто честности это недопустимо.
  Исправлено: вьюха отдаёт `day_price_eur`/`day_price_label`/`day_price_differs`
  тем же резолвером `pricing.price_cents_for`, шапка и липкая панель показывают
  цену выбранного дня, при отличии от базовой называется причина («Feiertage»).
  Замок `test_storefront_shows_the_price_it_will_charge` держит равенство
  «показано = списано» (проверяет ВИДИМЫЙ текст: базовая цена остаётся в
  `data-price` инлайн-редактора канвы — он правит именно базовое поле).
- **HF-6a/b (по плану `hf6-multislot-plan-2026-07-31`):** `Booking.group_code`
  (пусто = одиночная бронь) + `services.book_many(starts, …)` — N периодов в
  одной транзакции с общим кодом; занятый период откатывает ВСЮ группу (гость не
  получает половину заказа), anti-oversell не дублируется (каждый период идёт
  через обычный `book`), письмо клиенту — ОДНО на группу (`notify=False` в
  цикле). Один период = прежнее поведение байт-в-байт (паритет-замок).
  Кап `MAX_GROUP_SLOTS=6`. Витрина (мультивыбор слотов) — HF-6c, следующий шаг.
- Осознанно обновлён замок демо-отеля: секция акций у отеля теперь ВКЛЮЧЕНА
  (HF-1 дал ему акции для номеров), `products` остаётся выключенной.
- Урок теста: у услуги 60 мин при шаге сетки 30 мин соседние старты
  ПЕРЕСЕКАЮТСЯ — набор из «слот 0 + слот 1» невозможен by design; в замках
  берём каждый второй старт.

## 2026-07-31 — ещё два дефекта из адверсариального ревью (HF-4, HF-5)
Пробники ревью-агентов (удалены, вместо них — постоянные замки в
`apps/booking/tests/test_services.py`). Без миграций.

- **HF-4: день, закрытый для ВСЕГО бизнеса, был кликабелен в виде ресурса.**
  Клик создавал ВТОРУЮ, ресурс-скоупную запись `ClosedDate`; день визуально не
  менялся (он всё ещё закрыт глобально), владелец жал снова — и получал мусор в
  данных вместо результата. Теперь такой день рендерится неинтерактивной ячейкой
  с подсказкой, где именно его открывать. Замок держит отсутствие кнопки.
- **HF-5: пустое поле цены молча делало услугу в эти даты БЕСПЛАТНОЙ.**
  `_eur_to_cents("")` → 0, и ноль доезжал до брони через тот же резолвер. Теперь
  цена ≤ 0 отбивается сообщением; окно не создаётся, цена остаётся базовой.
  Замок проверяет и отсутствие записи, и что `price_cents_for` вернул базовую.
- Урок: оба дефекта — «тихие» (никаких исключений, тесты зелёные), поэтому их не
  ловил ни один существующий замок. Нашла их скептик-проверка находок ревью,
  а не сам поиск: агент писал пробник и печатал фактическое состояние БД.

## 2026-07-31 (вечер) — HF-6c/6d мультивыбор времён + попап формы + демо-акции/услуги

**HF-6c (витрина, мультивыбор времён услуги).** Времена на `/termin/leistung/<pk>/` —
переключатели, состояние в URL (`?slot=A&slot=B`): без JS, живёт в iframe-виджете и
переживает fetch-своп селектора. Найдено и закрыто на стенде три дыры сверх плана:
(1) выбор терялся смыслом при переходе на соседний день — форма пропадала, а времена
не уходили в POST → `buybox_ready` теперь по ЛЮБОМУ отмеченному времени, hidden-поля
несут весь набор (сервер `service_book` ре-валидирует каждое по СВОЕЙ дате — это уже
работало); (2) сетка идёт шагом расписания, а услуга может длиться дольше шага
(демо-прокат 480 мин при шаге 30) → пересекающиеся старты помечены `data-slot="blocked"`
и не кликабельны, раньше их можно было отметить и получить отказ на отправке;
(3) мусор/протухшие времена из URL отбрасываются, иначе невидимый гостю слот ронял
весь заказ. Заголовок формы при наборе с РАЗНЫХ дней подписывает даты.

**HF-6d.** Подтверждение `/t/<code>/` и письма (гостю created/confirmed + владельцу)
перечисляют все периоды группы; календарь кабинета помечает «часть общей брони».

**Попап формы (фидбэк «ввод данных в попап»).** `_buybox_modal.html` + общий
`_modal_script.html`; флаг `buybox_in_modal` у номера и услуги. Форма остаётся В DOM
(паритет-замки полей целы), правая колонка номера 899 px → 286. Две грабли: скрипт
внутри `#svc-box` умирал после первого fetch-свопа (innerHTML не исполняет `<script>`)
→ вынесен наружу; диалог лежит в `lg:sticky`-колонке, а sticky создаёт stacking-контекст
→ шапка сайта (z-20) рисовалась ПОВЕРХ затемнения (проверено `elementFromPoint`) →
при открытии диалог портируется в `<body>`, протухшие портированные копии выбрасываются.

**«Описание съехало».** `max-w-3xl mx-auto` центрировал тело детали внутри обёртки
`max-w-5xl` → описание начиналось на 128 px правее фотографии над ним. Убран `mx-auto`
(проверено: галерея x=238 и тело x=238 на услуге и номере).

**Демо.** `translate_tenant_content` не обходил акции — на en/ru/uk/tr они были
немецкими (переводы 39 строк лежали в словарях и не применялись); добавлен проход +
8 новых записей в 4 словаря. Трём акциям отеля — фото из CC0/AI-набора. Четырём
услугам отеля — полные описания (ru/uk/tr добавлены) + характеристики и FAQ.
Ограничение записано в roadmap: `Service.attributes`/`faq` и `Promotion.group` без
overlay → на других локалях остаются немецкими.

Тесты: +6 booking (мультивыбор/блокировка пересечений/мусор/кросс-день/подтверждение),
+2 попапа (booking+stays parity), 453 booking+stays и 626 tenants зелёные; 6 msgid × 5 .po.
⚠️ ops: после деплоя `seed_demo_tenants --kit hotel --recreate` (фото/переводы акций,
описания услуг). Миграций НЕТ.

## 2026-07-31 (ночь) — ревью-фиксы HF-6 (8 подтверждённых дефектов)

Адверсариальное ревью влитых `6de01b9b`/`07c09814` (5 измерений × скептики-
верификаторы, 21 агент): 16 находок, 14 подтверждены, 2 опровергнуты; после
дедупликации — 8 различимых дефектов, все воспроизведены и закрыты.

**Деньги в групповой брони (high).** `services.book_many` разносил kwargs по
КАЖДОМУ периоду → `voucher_code`/`extras` уезжали в каждый `book()`. Следствия:
одноразовый Gutschein гасился первой бронью, вторая падала `PromoInvalid` и
откатывала транзакцию — запись на 2+ времени с обычным промокодом была
НЕВОЗМОЖНА, а гость видел «код недействителен» при валидном коде; многоразовый
код давал N-кратную скидку; Extras оплачивались N раз. Фикс: промокод и Extras
применяются к ПЕРВОЙ брони группы (заказ один).

**Mehrfachkarte (high).** `_redeem_pass_if_code` получала только `created[0]` →
один визит за N периодов (до `MAX_GROUP_SLOTS=6`), подтверждалась лишь первая
бронь. Фикс: гасим визит на каждый период, при нехватке — сообщение со
списанным числом.

**Попап (high).** `pick()` выбрасывал портированные копии, когда свежего диалога
в потоке страницы нет → ВТОРОЕ открытие удаляло единственный диалог, кнопка
после первого закрытия была мертва. Мой стендовый тест этого не поймал: между
открытиями он всегда делал fetch-своп, который создавал свежий диалог — урок про
ложный «зелёный». Фикс: чистим копии только при наличии свежего.

**Фрагмент номера `?buybox=1` (medium).** Не получал `buybox_in_modal` → форма
рендерилась инлайном; через этот фрагмент идёт ОСНОВНОЙ флоу (выбор дат на
календаре), так что попап работал только при заходе с датами в URL.

**Прочее.** ISO без смещения в `?slot=` давал naive datetime → `sorted()` с
aware-стартами ронял страницу в 500 (`_parse_start` теперь требует aware).
Подтверждение перечисляло N времён, а цену показывало за один период → итог по
группе. Письмо «Reservierung bestätigt» перечисляло ещё не подтверждённые
периоды → только периоды того же статуса. Без JS форма была недостижима →
`<noscript>` раскрывает диалог как обычный блок. Докрутка после выбора времени
вела к скрытой форме → ведёт к кнопке.

Тесты: +6 (промокод/Extras/карта/naive/итог группы/письмо), 459 booking+stays
зелёные. 1 msgid × 5 .po; `ngettext` намеренно НЕ использован — msgid уже живёт
в .po как одиночный, а такая коллизия ломает `makemessages`.
Уроки: `Extra.scope` (не `module`), `Notification.payload["body"]` (нет `.body`),
`rl:*` в Redis переживают прогоны → уникальный IP на каждый публичный POST.

## 2026-07-31 (ночь, продолжение) — HF-7: единицы времени, календарь сразу, ярлык UA

План — `docs/hf7-service-duration-plan-2026-07-31.md`. Всё БЕЗ миграций, main `de1ca71b`.

**Попап — граница по смыслу (уточнение владельца «тарифы не нужно было прятать»).**
Прошлый заход унёс в диалог всю форму брони: тарифы, разбивку PAngV, Extras и
промокод — то есть ровно то, ради чего гость заходит на страницу номера. Теперь в
диалоге ТОЛЬКО личные данные (имя/почта/телефон/заметка/согласие + кнопка);
всё, что влияет на выбор и цену, — на странице. Общий хвост вынесен в
`_buybox_contact.html` (+`_buybox_contact_fields`), зовут обе формы; `_buybox.html`
больше не оборачивает форму целиком, `_buybox_modal.html` удалён. Поля диалога
несут `form="buybox-form"`: при открытии диалог портируется в `<body>` (sticky-
колонка накрывала его шапкой) и физически покидает `<form>` — без атрибута бронь
уходила бы пустой.

**HF-7-1 «услуга на несколько единиц времени» (фидбэк: «с 8:30 до 12:30»).**
HF-6c давал мультивыбор РАЗНЫХ времён — это не отрезок; при услуге длиннее шага
сетки непрерывный блок был невозможен в принципе. Теперь `?dauer=N` — сколько
подряд идущих единиц берёт гость: сетка считается на суммарную длительность,
бронь — ОДНА запись длиной N×услуга, цена ×N. Чипы показывают только достижимые в
этот день длительности; заголовок называет отрезок. Движок уже принимал длину —
дополнены сквозным параметром `service_slots`/`assign_resource`. `dauer=1` —
прежнее поведение байт-в-байт. Кап 8 единиц, мусор → 1.

**HF-7-2 «календарь сразу».** Деталь услуги была SEO-страницей с кнопкой на
ОТДЕЛЬНЫЙ слот-пикер (перезагрузка → и только там календарь). Теперь календарь и
времена на детали, кнопка брони появляется по выбору времени; `/termin/leistung/<pk>/`
остаётся для писем/QR. Исключение — услуги с `primary_service_cta='request'`
(A7/A9): там деталь остаётся страницей с парой CTA. Вторичная «Angebot anfragen»
переехала под пикер.

**HF-7-3 «украинский — ua».** Новый `apps/core/langs.py::badge` — ярлык `uk` → **UA**
в переключателях витрины и публичных страниц. Внутренний код остаётся `uk`: ISO
639-1 украинского ЯЗЫКА, на нём завязаны `locale/uk/`, `.po/.mo`,
`settings.LANGUAGES` и сохранённые `active_locales` тенантов; `ua` — код страны.

Стенд: на детали 16 времён + календарь без второй страницы, кнопки брони до выбора
нет; `4×` сужает сетку до 7 стартов, заголовок «26.08.2026 · 09:00–15:00», POST даёт
`/t/T-…/` с тем же отрезком; пилюля языка — UA; у номера 4 тарифа видны без диалога.
Тесты: +4 HF-7 +1 UA +2 попапа; 464 booking+stays, 627 tenants; 2 msgid × 5 .po.
Три замка старого двухстраничного флоу обновлены осознанно.
**Грабля (CI #1773/#1774 красные):** новые Tailwind-классы → обязателен
`npm run build:css` + коммит `static/css/app.css`; гейт свежести ловит это молча
(тесты зелёные, падает отдельный шаг).

## 2026-08-01 — витрина: переключатель вида, кнопка «Фильтры», наплывание на карточке

Фидбэк владельца по скриншотам с телефона. Всё БЕЗ миграций, main `cd8ef6a0`.

**Переключатель «список ↔ плитка» — на всех сетках.** Существовал в одном месте
(секция акций главной): инлайн-разметка со своим скриптом, только на мобиле,
жёстко привязан к `#promo-grid`. Вынесен в `_grid_view.html` + один делегированный
обработчик `_grid_view_script.html` (подключён в `_base.html`). Кнопки находят
сетку по `data-grid="<ключ>"`, выбор в localStorage НА КЛЮЧ («список» на акциях не
переключает каталог). Режим списка — один CSS-класс `[data-grid].is-list`, который
перебивает любые `grid-cols-*` layout-движка, поэтому работает с произвольной
раскладкой секции и на всех ширинах. Врезано: главная (акции/товары/услуги/
категории/номера/события/новости), `/aktionen/`, четыре листинга через слот
`listing_view` каркаса `listing.html`. Замок `test_grid_view` держит пару
«сетка ↔ кнопки» по 12 файлам.

**Кнопка «Фильтры» (каталог).** Панель фасетов занимала на телефоне весь первый
экран. `_filter_toggle.html` + обработчик: на узком экране свёрнута, на широком
открыта, выбор запоминается; применённый фильтр НЕ прячем (иначе выдача выглядит
сузившейся без причины), число активных фильтров — на кнопке. Стенд 390 px:
первая карточка с ~1600 px → 507 px. Поиск по датам на листинге отеля осознанно
не трогаем — это основное действие страницы, а не фильтр.

**«Контент наплывает один на один».** На overlay-карточке круглая кнопка
добавления лежала ПОВЕРХ названия и цены, сердечко «отложить» — поверх бейджа
(«♡опулярные»). Под кнопку зарезервировано место справа (`pr-16` при активном
quick-add), бейдж сдвигается при включённом Merkzettel (обе раскладки).
Проверено измерением bounding-box: пересечений нет.

⚠️ Грабля (снова): новые Tailwind-классы → `npm run build:css` + коммит
`static/css/app.css`, иначе CI падает на гейте свежести при зелёных тестах.

## 2026-08-01 — «Примеры по функциям» на /branchen/

Владелец: «У нас нет сайта с акциями на демо, если нет, добавь». Разведка: демо
**есть** (`aktionsmarkt` — все механики скидок, галереи, mystery-deal), не было
**находимости**: демо-витрины подписаны видами бизнеса («Lebensmittel»,
«Friseur»), и по названию не понять, где смотреть конкретную возможность. Выбор
владельца — «отдельным» блоком.

Реестр `apps/tenants/feature_demos.py`: 8 записей «функция → живая страница демо»
(акции → `aktionsmarkt/aktionen/`, запись → `friseur/termin/`, ночёвки →
`hotel/unterkunft/`, Click&Collect → `baeckerei/sortiment/`, размеры/цвета →
`mode/sortiment/`, события → `retreat/veranstaltung/`, заявки/сметы →
`handwerker/anfrage/`, Finder → `baeckerei/finder/`). Ссылка ведёт сразу на нужный
экран, а не на главную. Гейт `onboarding._seeded_demo_hosts()` — незасеянные киты
просто не показываются, мёртвых ссылок не бывает; реестр можно пополнять раньше,
чем прогонят `seed_demo_tenants`. Секция «Beispiele nach Funktion» на `/branchen/`
(4 колонки, `target="_blank"`). 19 msgid × 5 .po; замки — пустой список без демо,
только засеянные хосты, каждая запись указывает на известный кит.

## 2026-08-01 — свободный текст владельца переводится: attributes/FAQ услуги + группы акций

План — `docs/i18n-owner-content-plan-2026-08-01.md`. Закрывает ограничение,
записанное в roadmap после волны HF: витрина умеет N локалей, но три поля шли
мимо обеих i18n-схем и на en/ru/uk/tr оставались немецкими — пункты «Details» и
FAQ богатой карточки услуги (`Service.attributes`/`faq`) и заголовки секций/чипы
групп акций (`Promotion.group`).

**Списочный оверлей.** `I18nMixin.get_overlay` покрывает скаляры; для списков —
новый `apps/core/i18n_seq.py::overlay_seq`: перевод накладывается ПОЭЛЕМЕНТНО по
индексу нормализованного базового списка. Инварианты: база задаёт длину (перевод
не может дорисовать пункт, которого владелец не писал), пустой элемент = фолбэк
на базу (частичный перевод безопасен), любой мусор в оверлее молча даёт базу.
Аксессоры `attributes_localized`/`faq_localized`, миграция `booking/0020`.

**Группа акции — только метка.** `group` служит и ключом фасета `?gruppe=`, и
подписью, поэтому переводится ТОЛЬКО показ: `group_i18n` (миграция
`promotions/0024`), `group_localized`, вьюха отдаёт `(ключ, метка, items)`.
Карта меток строится по НЕотфильтрованной выдаче — иначе при выбранной группе
чипы остальных остались бы без перевода (замок). Поле редактируемое, поэтому
получило и per-locale ввод: динамические `group_<loc>` в `PromotionForm` +
свёрнутый блок «🌐 Translations» под полем (во вкладки языка не кладём — там
full-JSON title/description, другая схема).

Общий `resolve_overlay` вынесен из `I18nMixin.get_overlay` (тот делегирует) —
нужен там, где строки читаются пачкой через `values_list` и объектов нет.

**Демо.** `_fill_seq_overlay` в `translate_tenant_content` (списки строк и
списки словарей по ключам q/a) + `group` в проходе по акциям; 66 строк ×
en/ru/uk/tr дописаны в `demo_i18n_<loc>.json` (attributes/FAQ 51 + 11 меток
групп + 8 строк англоязычного кита). Замок содержания держит выборку из
словарей, чтобы демо не уехало обратно в немецкий.

⚠️ Две аддитивные миграции (`booking/0020`, `promotions/0024`) — деплой
владельцем; после деплоя `seed_demo_tenants --recreate` для переводов демо.

## 2026-08-01 — владелец наполняет богатую карточку услуги (attributes + FAQ)

План — `docs/service-rich-card-editing-plan-2026-08-01.md`. Пункт всплыл при
i18n-волне того же дня: секции «Details» и FAQ детальной страницы услуги рисуются
с UA4-3, но заполнялись ТОЛЬКО демо-китами — формы в кабинете не было. Владелец
услугового архетипа (A3/A7/A9, спа отеля) видел богатую карточку на демо и не мог
повторить её у себя. Миграций нет — поля давно есть.

**Ввод.** Details — textarea «пункт на строку». FAQ — пары полей: разделитель
внутри свободного текста (`Q | A`, «строка вопрос / строка ответ») ломается на
первом же ответе с переносом строки. Пустая пара внизу добавляет вопрос, слоты
дорастают после Save (приём редактора Finder, без JS); очистка вопроса = удаление
пункта. Оба блока — в свёрнутом `<details>` карточки услуги.

**Presence-guard.** `action=update` шлётся и из компактной строки списка, где этих
полей нет, поэтому пишем только при сентинелах `attributes_present`/`faq_present`
— иначе обычный Save длительности стирал бы наполнение (инвариант W0).

**Выравнивание переводов — по ВЫЖИВШИМ пунктам, не по номерам строк формы.**
Базовый парсер выбрасывает пустые строки/пары, значит удаление пункта из середины
сдвинуло бы все переводы ниже него на чужие пункты. FAQ разбирается одним
проходом (база и переводы вместе), Details — по номеру ИСХОДНОЙ строки базовой
textarea. Замок `test_translations_follow_surviving_items_not_row_numbers`.

Семантика оверлея списка вынесена в `i18n_input.apply_seq_overlay`; разбор POST
остался во вьюхе — у списка строк и списка пар разная разметка.

**Грабля (поймана своим же тестом, `'str' object has no attribute 'user'`):**
новые модульные хелперы были вставлены МЕЖДУ `@login_required` и `def
services_view` — декоратор навесился на хелпер, а вьюха осталась БЕЗ авторизации.
Тихая дыра: тесты вьюхи прошли бы, упал только вызов хелпера. Правило: хелперы
только выше блока декораторов; после вставки — `grep -n -B2 "^def <view>"`.

## 2026-08-01 — виды отображения вариантов и опций выбирает владелец (O-волна)

План — `docs/option-display-styles-plan-2026-08-01.md`; решения владельца: вид
выбирается из реестра («варианты, цвет, фото, список, чекбокс…»), модификаторы —
«с фото и чекбоксами-плитками». Вопрос владельца про учёт опций вскрыл мёртвую
функцию: `ProductVariant.images` завели в M4-A, витрина подменяла по нему главное
фото, но загрузить фото было НЕЧЕМ — заполнял только демо-кит.

**O-1:** фото варианта из кабинета (📷 в строке варианта, replace-семантика: одно
фото на вариант, галерея остаётся у товара; W0-замок «Save цены не роняет фото»).
**O-2:** реестр `apps/catalog/option_styles.py` — варианты: список / кнопки /
цветные кружки / фото-плитки / строки с фото / две оси (цвет+размер, недоступные
комбинации гасятся); модификаторы: как раньше / плитки с фото / строка с фото /
пилюли. Хранение: `Product.variant_style` + `ModifierGroup.display_style` +
`ModifierOption.image` (⚠️ миграция `catalog/0022`, аддитив) + дефолт магазина
`site_defaults.variant_style` (presence-minimal, golden целы). Цвет кружка —
ТОЛЬКО по явному словарю немецких названий («Sonnengelb» без словаря стал бы
чёрным); нет в словаре → фото варианта → первая буква.

**Архитектура от замка:** набор имён полей формы корзины пинит
`test_cart_form_variants_and_modifiers_fields` → свотчи НИЧЕГО не шлют сами:
селект остаётся в DOM (прячется CSS-классом `js-opt`), кнопки без `name=`
выставляют его значение с событием change (иначе умерла бы подмена фото). Без JS
— ровно прежний список. Мульти-модификаторы — чекбоксы прямо (они и есть поле);
одиночные держат `select`: радиокнопки с общим `name="mod"` слиплись бы в одну
группу на весь товар. Кабинет: селект вида на товаре (вкладка Marketing) и на
группе модификаторов (сентинел style_present), фото опции, дефолт в теме
конструктора. Демо: mode=axes, cafe=buttons, гастро-модификаторы chips/list.
17 замков `test_option_styles` + 4 `test_variant_photo`; 25 msgid × 5 .po.

## 2026-08-01 — склад в разрезе осей варианта: матрица «размер × цвет»

Вопрос владельца «есть ли в настройках склада учёт количества определённого цвета
и размера отдельно». Разведка: **учёт был** — единица склада = вариант, а вариант
с M4-A несёт обе оси, поэтому «S · Blau» и «M · Blau» уже были независимыми
позициями (свой остаток и anti-oversell, своя строка леджера и реконсиляции, свой
EK/Meldebestand/Warenwert, свой EAN для scan-to-count, свои партии Lot, своя
локация). Не было **экрана в разрезе осей**: везде показывался склеенный `label`,
поэтому «сколько всего синего» или «сколько всего размера M» посмотреть было
негде. Значит правка чисто представленческая — новый `apps/inventory/matrix.py`,
ни модели, ни движения остатка не тронуты, миграций нет.

**Матрица** на `/dashboard/stock/`: строки — размеры, столбцы — цвета, итог строки
= «всего этого размера», итог колонки = «всего этого цвета», ячейка — ссылка в
историю движений. Показывается только у товаров с ОБЕИМИ осями: у легаси-вариантов
одним `label` («100 g») таблица выродилась бы в столбец.

Две вещи, которые нельзя было смешать. **Пустая ячейка ≠ ноль:** отсутствие
комбинации и ноль на складе при инвентаризации значат разное (ноль пересчитывают,
несуществующего варианта нет). **`stock_quantity=None` ≠ ноль:** это «без лимита»,
поэтому в суммы не идёт, а итог показывается прочерком, если считать нечего.

**Фильтры** «Größe»/«Farbe» над плоской таблицей (тот же резолвер оси
`size or label`, что у фасета каталога, — смешанный каталог не теряет позиций).
Товары без вариантов при активном фильтре уходят: показывать их в выдаче «размер
M» было бы враньём. Zählliste следует фильтру — это законная ЧАСТИЧНАЯ
инвентаризация, поэтому над ней встаёт предупреждение. Матрица фильтру НЕ
подчиняется: она и есть обзор.

**CSV-экспорт** движений получил отдельные колонки «Größe»/«Farbe» — склеенный
...
## 2026-08-01 — аудит июня 2026 (мержи, миграции, деплои)

Продолжение аудита по запросу владельца. Клон углублён до 2026-05-28 (bootstrap,
1514 коммитов) — до этого локально не было даже июля целиком, см. граблю в записи
«аудит месяца».

**Мержи 01.06–30.06:** 730 коммитов в main за 24 активных дня; настоящий
merge-коммит один (`81447d33`, 25.06) — остальное FF-push. Авторы: Claude 767,
adaptor2024 6. По неделям: W23 12, W24 200, W25 119, W26 274, W27 168 — пик
приходится на неделю 22–28.06.

**Миграции:** 150 добавлено за июнь (для сравнения: 70 за июль). Это месяц
закладки фундамента — появилось **17 новых Django-приложений**: aggregator,
notifications, publishing (09.06), crm (11.06), booking, finance, orders (12.06),
billing, jobs, stays (13.06), core, events, inbox, secrets, support, telegram
(14.06), loyalty (22.06). Топ по числу миграций: events 21, stays 20, tenants 16,
promotions 15.

**Риск-профиль (открывал каждый файл, не по имени):**
* 6 data-миграций: `aggregator/0011` (листинги stay/event), `promotions/0003`
  (unsubscribe-токены), `stays/0003` (backfill total_cents), `tenants/0012`,
  `tenants/0013` (opt-out существующих из stays/jobs), `tenants/0017` (выключение
  ЛК у шоурумов).
* Вынос loyalty из promotions (22.06) сделан **правильно**: `promotions/0017` и
  `loyalty/0001` — обе `SeparateDatabaseAndState` с `database_operations=[]` и
  `db_table` закреплённым на исходные `promotions_*`. Модели переехали, таблицы
  и данные не тронуты.
* **Единственная реально теряющая данные миграция месяца — `stays/0018`**
  (24.06, G4 multi-tier auto-discounts): шесть `RemoveField` у StaySettings
  (`early_bird_days/percent`, `last_minute_days/percent`, `los_min_nights`,
  `los_discount_percent`) заменены JSON-полем `auto_discount_rules`, БЕЗ
  `RunPython`-переноса старых значений. Настроенные скидки у тенантов при деплое
  обнулились и требовали повторного ввода правилами. На 24.06 в проде были
  фактически только демо-тенанты, так что ущерб нулевой, но приём неверный:
  при замене полей на структуру нужен перенос.

**Невлитая работа — найдено ОДНО реальное расхождение.** Из 16 июньских веток с
неслитыми коммитами пятнадцать содержат либо доки, либо код, попавший в main
через cherry-pick (проверено по темам и по наличию файлов). Исключение —
`claude/m20-retreat-pages` (25.06), два коммита **M22b: индикатор «печатает» через
кэш + статус «бизнес на связи» в треде**: `apps/inbox/public_views.py`,
`views.py`, `urls.py`, `config/urls_tenant.py`, два шаблона, тесты. В main этого
нет — адверсариальная проверка: `thread_typing`, `inbox_typing`, `open_status`
в `apps/inbox` и в `message_thread.html` не встречаются вовсе. Работа не
переизобреталась, просто не долетела. Оживление потребует ручного разрешения ~7
конфликтных мест (с 25.06 inbox прирос на +868 строк: LS-3 композер оффера, LS-6
прямая линия).

**Деплои июня:** явных подтверждений в записях почти нет — июньские строки
build-log в основном помечены «Нужен деплой», а не «задеплоено». Косвенно
закрыто: запись «последний полный деплой 08.07 — применены ВСЕ миграции по
состоянию на тот момент» + `showmigrations` от 01.08 показывает `[X]` до
последних номеров, то есть все июньские миграции на проде применены (проверено
по catalog/stays/booking/inventory/tenants — это 58 из 150 июньских; остальные 92
в 16 приложениях той же проверкой не покрыты).

**Формат записей:** дневниковые заголовки `## ГГГГ-ММ-ДД` в build-log начинаются
только с 08.07; июнь лежит одним разделом «Сделано (хронология)» списком по
спринтам/трекам. Июнь задокументирован, но крупнее — по трекам, а не по
инкрементам, поэтому сверить «инкремент ↔ коммит» для июня нельзя.

## 2026-08-01 — M22b оживлён: «печатает» + «бизнес на связи» в чате

Единственная реально потерянная работа из июньского аудита (два коммита от 25.06
на `claude/m20-retreat-pages`, так и не долетевшие до `main`). Владелец: «давай
отдельным инкрементом и проверим». План — `docs/m22b-revive-plan-2026-08-01.md`.

**Не cherry-pick, а ручной перенос.** Пробный мерж давал ~7 конфликтных мест: с
25.06 `apps/inbox` вырос на +868 строк (LS-3 композер оффера, LS-6 «прямая
линия»), `config/urls_tenant.py` — на +208. Кода в переносе ~60 строк, поэтому
вписали его в ТЕКУЩИЙ код, а не разрешали конфликты пятинедельной давности.

**«Бизнес на связи»** — в публичном треде плашка по часам работы
(`tenant.open_status()`): 🟢 «до HH:MM» / ⚪ «снова Mo 09:00» / ⚪ «сейчас
закрыто». Серверный рендер: статус меняется раз в день, гонять его поллингом
незачем. Часы не заданы → плашки нет вовсе (врать «закрыто» нельзя, замок).

**«Печатает»** — флаг в кэше, TTL 6 с, без модели и миграции: состояние обязано
теряться, а запись на каждое нажатие клавиши в БД была бы вредна. Ключ
`inbox_typing:<conv_pk>:<role>`, где `conv_pk` — UUID, поэтому схемы не
пересекаются даже в общем Redis. Пинг шлётся с троттлингом 3 с при TTL 6 с:
индикатор держится, пока печатают, и гаснет сам через пару секунд после паузы.
Противоположная сторона забирает флаг СВОИМ существующим поллингом (`thread_poll`
отдаёт `typing: bool`) — новых запросов на чтение не появилось.

Отличие от оригинала: импорт `is_typing`/`mark_typing` в `views.py` был локальным
(внутри функций) — поднят на уровень модуля (цикла нет: `public_views` не
импортирует `views`, а локальный импорт в горячем поллинге — лишняя работа на
каждый запрос).

**Замки (42 в `apps/inbox`):** пинг ставит флаг и виден в поллинге другой стороны;
после протухания — `false`; упавший Redis не роняет тред (`cache.get`/`set` под
`try` → индикатор просто не показывается); гейт модуля на новой вьюхе; плашка
open/closed/без часов, причём закрытая ветка проверяет распаковку кортежа `next`
в разметке.

**Стенд (обе стороны вживую, `friseur` на 8010):** клиент → кабинет `typing: true`;
кабинет → клиент `typing: true`, через 7 с (TTL 6) — `false`; плашка
`data-chat-status="open">🟢 Available now — until 23:59`. Грабля: кабинетные
эндпоинты дают 403 не только без CSRF, но и без `Membership` — гейт
`has_cabinet_access` fail-closed, `@login_required` его не заменяет.

i18n: 5 новых msgid × 5 локалей. Третий коммит потерянной ветки (SSE отложен) —
записью в `roadmap-next-sprints.md §Отложено`. Миграций нет.

## 2026-08-01 — `migration_state`: сверка миграций по ВСЕМ схемам

Прямое следствие аудита того же дня: очередь миграций в CLAUDE.md разъехалась с
реальностью на три недели, и разъехалась она не по невнимательности, а потому что
проверить было нечем. `manage.py showmigrations` без tenant-контекста читает
`django_migrations` схемы **public**; у TENANT-приложений своя таблица в КАЖДОЙ
схеме тенанта. То есть привычное «на проде всё [X]» — утверждение только про
public-часть.

Команда `python manage.py migration_state` (read-only) обходит public + все схемы
тенантов и печатает ТОЛЬКО отставание. Вывод сгруппирован **по миграции**, а не по
схеме: у тенантов отстаёт обычно один и тот же хвост, и построчный список на
полсотни схем был бы простынёй. Формат — короткий, его можно целиком вставить в
переписку:

    catalog.0022_… — не применена в 7: aktionsmarkt, baeckerei, friseur и ещё 4
    ИТОГО отстающих миграций: 3 (схем проверено: 8)

`--all` показывает и здоровые схемы, `--schema <имя>` — одну.

**Отдельный диагноз «схемы нет».** Строка `Tenant` может существовать без схемы в
Postgres (провалившийся провижининг, остатки `SignupRequest`). Наивная реализация
отрапортовала бы по такой «миграции не применены» и отправила бы владельца гонять
`migrate` там, где чинить надо провижининг. Такие схемы называются отдельной
строкой `СХЕМЫ ОТСУТСТВУЮТ` и не идут в знаменатель «проверено».

Шаг добавлен в `scripts/deploy.sh` (этап 8, Deploy checks) — теперь каждый деплой
сам печатает вердикт, и очередь в памяти проекта больше не может тихо устареть.

Проверено на локальном стенде: команда честно нашла, что 3 последние миграции не
применены в 7 демо-схемах (public при этом чист) — ровно тот класс расхождения,
который обычный showmigrations не показывает. Замки: чистое состояние, тенант без
схемы, `--all`. Миграций нет.

## 2026-08-03 — M22b доводка: пинг реально не отправлялся + 2 дыры доступа (ревью)

После мержа M22b адверсариальное ревью + браузерный стенд (Playwright, два окна)
нашли то, что curl-стенд показать не мог.

**Пинг «печатает» не отправлялся НИКОГДА.** Скрипт треда стоит ВЫШЕ формы ответа,
а слушатель вешался прямым `querySelector("textarea[name=body]")` — в момент
выполнения элемента ещё нет, `null`, и `if (area)` молча съедал проблему. Сервер
и эндпоинты были исправны (curl-проверки прошли), но из настоящего браузера POST
не уходил: Playwright-прогон дал «POST /typing/ = 0». Фикс — делегирование на
`document` (тот же приём, что в Ф1 #1: «партиал парсился раньше табов»), csrf
ищется в момент отправки. Регресс-замки на ОБА шаблона: в отрендеренном HTML
обязан быть `document.addEventListener("input"` и не должно быть
`area.addEventListener`. Урок: **стенд из curl проверяет сервер, но не страницу;
для фич, живущих в JS, стенд = настоящий браузер.** Второй прогон дал ложную
тревогу «индикатор не горит» — на деле тайминг (печать прекратилась > TTL 6 c до
тика поллинга 5 c); честный сценарий — печатать дольше интервала поллинга.

**Аноним читал кабинетный поллинг и счётчик.** `thread_poll` (тела сообщений +
запись `unread_for_staff=False`) и `unread_count` в кабинете были БЕЗ
`@login_required` — дыра существовала с появления реалтайм-поллинга (не этим
батчем): Membership-гейт middleware анонима сознательно не трогает, полагаясь на
декоратор, которого не было. Подтверждено вживую (аноним → HTTP 200 с телами),
закрыто декоратором, замок на 302→login для обоих. Плюс мелочь той же линзы: оба
`thread_typing` теперь принимают только POST (405 на GET — мутация в обход CSRF).

Стенд после фикса: клиент печатает → в кабинете «Kunde schreibt gerade…»;
сотрудник печатает → у клиента «Typing…»; аноним — 302/405. 47 замков apps/inbox.

## 2026-08-03 — Verkäufe-волна V1–V4: единая страница продаж ЦЕЛИКОМ

По запросу владельца (отель: номера+услуги+товары на одной странице продаж) и
его решениям (вкладки; секции по продажам; Auftragsbuch в волну; reservation в
Marketing; старые страницы пометить «Alte Ansicht»). План —
`docs/unified-sales-page-plan-2026-08-03.md`; разведка — два Explore-агента,
рыночный анализ по 9 классам инструментов (§1 плана).

**V1 каркас.** `/dashboard/verkaeufe/`: вкладки по kind — primary-kind всегда
(у отеля Belegungsplan), прочие появляются с первой продажей
(`transactions.kinds_with_sales` — до 6 exists()-запросов); в каждой вкладке
переключатель Kalender/Board/Liste с persist'ом per-kind (НОВЫЙ ключ
`sales_views`, presence-minimal; легаси `orders_view` остаётся дропаемым).
Переключение вкладок — обычная навигация `?tab=` (неактивные вкладки не
запрашиваются; инлайн-JS календарей исполняется штатно — урок HF-0 про
innerHTML). Календари встроены извлечением тел в партиалы 1:1: 
`stays/_belegungsplan_body.html` (334 строки) + `booking/_tagesplan_body.html`
+ context-функции (`calendar_context`) — страницы стали обёртками, рендер
байт-в-байт (274 stays + 202 booking теста зелёные без правок). Board любого
kind — готовый `_board_body` (в `manage_sections_for` добавлен `only=`).
Generic-Liste (`core/_sales_liste.html`) закрыла давний пробел «списка нет» у
stay/booking/job/ticket. reservation вкладкой не становится — решение
владельца «резервы в Marketing» (у aktionsmarkt появится сам: он primary).
classic_ui редиректит на прежние страницы (страховка ST, замок).

**V3 Auftragsbuch.** Календарь заказов по ДНЯМ выдачи (DACH-традиция книги
заказов пекарни): ось `Order.pickup_slot`, окно 14 дней с навигацией, «сегодня»
подсвечен, активные заказы без слота — янтарный блок «Ohne Termin» (статусы —
`counted_statuses("order")`, кастом-статусы учтены). Только представление.

**V4 навигация.** `orders_view.entry_url_name` в не-classic ведёт на
`verkaeufe` (сайдбар + хаб-плитка + хоум автоматически); на 5 старых страницах
хаба — приписка «Alte Ansicht · 🆕 Alles auf einer Seite →» (в сегмент-контроле,
classic не затронут). Счётчики колонок доски стали ЧЕСТНЫМИ: агрегат по БД
через `status_registry.resolve` (кастом-статусы учтены) вместо подсчёта по 50
загруженным карточкам; `total` — тоже. Осознанно обновлены замки ST-5b/ST-4b:
легаси-маппинг «Verkäufe→Belegungsplan/список» теперь живёт под classic_ui.

Решения владельца на СЛЕДУЮЩУЮ волну зафиксированы: акции переводим ВСЕ сразу
на рельсы «ценовой слой над базовой сущностью»; услуги — с привязкой к
периоду/мастеру/слотам; наборы — расширяем Combo. План-доком следующей сессии.
...
## 2026-08-04 — promo P3+P4: услуги и номера на рельсах «ценового слоя»

**P3 услуги** (`apps/promotions/price_layer.py`, ⚠️ `booking/0021`): матчер
`target_rules` — дни недели, окно часов (в т.ч. «через полночь» 22–02),
конкретный мастер; мусор в правилах = fail-closed. `promo_for_service` отдаёт
клиенту лучшую цену из подходящих акций. `purchase_service` — двойное списание
в одной транзакции: лимит кампании + слот мастера штатным `book()`; занятый
слот откатывает лимит (замок). Правила проверяются в сервисе — POST'ом окно не
обойти. `Booking.promotion` FK: отмена возвращает лимит однократно (BookingSM +
зеркало status_effects); no_show НЕ возвращает (слот потрачен). 7 замков.

**P4 номера** (⚠️ `stays/0031`): акция на номер (percent) — обычный кандидат
`pricing.auto_discount` (новый параметр `extra`; без него байт-в-байт) — «max,
не суммируем», как G4/Lücken. Победа акции списывает лимит в той же транзакции
`book_stay`; исчерпанный лимит откатывает бронь целиком; проигравшая акция
лимит не тратит. Окно проживания {"stay_from","stay_to"} по заезду.
`StayBooking.promotion` FK + возврат при отмене (обе ветки). 5 замков.

**Урок (повтор):** «падения» test_public оказались стейл-ключами
`resv_token`/`rl:*` в **Redis db1** (тест-кэш; мои стендовые пробы насеяли) —
`redis-cli -n 1 --scan --pattern '*resv_token*'`; чистить db1, не db0.

## 2026-08-04 — promo P5: витрина акций на новых рельсах

Деталь акции `/p/<uuid>/` диспатчится по ЦЕЛИ: товар/combo/свободная → форма
«Jetzt kaufen» на новый эндпоинт `/p/<uuid>/kaufen/` (гейты как у
reservation_create: honeypot, ratelimit "resv", одноразовый form_token; успех →
редирект в штатное подтверждение заказа `/bestellung/<code>/`); акция-услуга →
CTA в штатную воронку записи (промо-цена применяется там автоматически, P3);
акция-номер → CTA на деталь номера (скидка через auto_discount, P4). Sold-out →
waitlist без изменений. **Kind reservation новыми акциями больше не порождается**;
/r/<code>/ и открытые резервы доживают (история цела). Обновлены замки
test_buybox_parity (reserve→kaufen) и test_promo_flags (модал → /kaufen/).
2 новых замка (эндпоинт, service-CTA). CI #1826 зелёный → FF-мерж
`5d9a3fed..b78b01b8`.

## 2026-08-04 — promo P6+P7: промо на витрине целей + аналитика по сделкам — ВОЛНА PL ЗАКРЫТА

**P6 «обратная сторона зависимости» — базовая сущность показывает свою акцию.**
Принцип: витрина показывает РОВНО то, что спишет чекаут (те же матчеры
`price_layer`). **Товар:** деталь — rose-баннер «−N % · промо-цена · ~~старая~~ →
Zum Angebot» (ссылка на /p/<uuid>/ — чекаут по промо-цене живёт ТАМ, корзина
продаёт по базовой, поэтому цену в buy-box не подмешиваем); карточки каталога —
бейдж «−N %» приоритетнее ручного (`product_promo_map` — bulk одним запросом,
без N+1). **Услуга:** новые `promo_for_service(promos=)`/`service_promos` —
сетка слотов подсвечивает РОВНО действующие времена (rose-чип + промо-цена под
временем; «Anyone» при правиле resource_id честно не подсвечивается —
fail-closed, сервер может дать сюрприз только в плюс), баннер акции над сеткой,
промо-цена выбранного времени в форме брони (зеркало service_book: только
одиночная бронь). **Номер:** ГЛАВНЫЙ фикс — витринная quote/тарифы/«ab …»
поиска звали `auto_discount` БЕЗ `extra=` → чекаут списывал меньше, чем
показывала витрина; теперь все 4 точки считают акцию тем же кандидатом, что
`book_stay`; бейдж «−N % · титул» до выбора дат (`stay_promo(unit, None)`).
**Демо:** спека акций китов умеет `service`/`stay_unit` (индексы refs, второй
проход ПОСЛЕ `_seed_kit_modules` — цели существуют только там)/`rules`/`limit`;
friseur — модуль promotions включён + «Happy Hours: Herrenschnitt 20 €»
(Mo–Mi 10–14, лимит 20) + товарная «Pflege-Woche −20 %»; hotel — «Frühbucher
10 %» целится в Doppelzimmer Seeblick (демо-брони сеются после акции штатным
book_stay и САМИ списывают лимит — кит-тест держит инвариант «лимит + сделки =
10»). Проверено Playwright на стенде (4 скриншота владельцу). 10 замков
test_price_layer_storefront + кит-ассерты; смежные 1035 зелёных.

**P7 хвосты.** Аналитика по СДЕЛКАМ: `price_layer.deal_counts` (заказы по
маркеру {"promo": id} в OrderItem.modifiers + брони услуг/номеров по FK;
отменённые не считаются — им возвращён лимит); `_promo_stats` += плитка
«Verkäufe», конверсия = (сделки+резервы)/просмотры; `analytics_overview` —
колонка «Verkäufe» bulk без N+1 (2 агрегата по FK + один проход по
promo-маркерам). `services.reserve` помечен ЛЕГАСИ (витрина с P5 резервы не
порождает; /r/<code>/ доживает). Агрегатор/publishing целы — snapshot читает
property. 2 замка. 3 msgid («Zum Angebot», «gilt für die rosa markierten
Zeiten», сноска конверсии) × 5 .po.

## 2026-08-04 — фидбэк владельца: месячный календарь кабинета + артикулы везде + Zusammenführen

**Месячный календарь Termine** (решение «по рекомендации»): сетка месяца над
Tagesplan — число броней под датой (ОДИН запрос на месяц, бакетирование в
локальной TZ — паттерн Auftragsbuch), листание месяцев вкл. историю, клик по
дате разворачивает её брони ниже (прежний список с FSM-действиями). Общий
партиал `booking/_month_grid.html` → работает и на /dashboard/booking/, и во
вкладке Kalender страницы Verkäufe (tab-carry ссылок; замок verkaeufe цел).

**Артикулы «везде»** (решение 2.б + «везде артикул»): (1) SKU варианта
редактируется из кабинета — инпут в строке/добавлении + `variant_update`
сохраняет (раньше — только CSV-импортом); (2) `ModifierOption.sku`
(⚠️ `catalog/0023`) + поле в форме опции — справочный артикул допродаж;
(3) снимок в заказ: `OrderItem.sku` (⚠️ `orders/0017`, вариантный приоритет)
+ sku опций в снимке модификаторов → Art.-Nr. в подтверждении заказа, кабинете,
письмах (created/owner), PDF-документах; (4) витрина-деталь: «Art.-Nr.» под
ценой, следует за выбором варианта (data-sku + общий change-обработчик; фикс:
раньше скрипт умирал без главного фото); (5) Google-фид: mpn варианта
перебивает товарный (как EAN).

**«Zusammenführen»** (решение 2.б): чекбоксы в списке товаров → подтверждение
с выбором главного → прочие товары становятся ВАРИАНТАМИ главного
(`apps/catalog/merge.py`): имя→label (коллизии — суффикс), артикул/EAN/цена/
остаток/EK/первое фото переезжают; старый товар гаснет (metadata.merged_into),
история заказов/леджера остаётся на нём. Склад: перенос остатка двумя
леджер-записями (+N варианту, −N старому) — реконсиляция T1 сходится с обеих
сторон (замок). Отказ (в messages): у сливаемого уже есть варианты/Extras.
Проверено Playwright на стенде: слияние friseur Haaröl→Hitzeschutz-Spray,
Art.-Nr. сквозной (форма → витрина). 6 замков test_merge + месячный замок
календаря; 9+2 msgid × 5 .po; смежные 539+21 зелёные.

## 2026-08-05 — фидбэк владельца: блок дат решением владельца + пикер «специалиста»

**«Не нашёл, где зарезервировать даты»**: движок закрытия дней (ClosedDate)
существовал на «Verfügbarkeit», но (а) владелец вход не находил, (б) закрывать
можно было только по одному дню кликом. Теперь: говорящие входы «🚫 Tage
blockieren» (шапка Kalender + ссылка под месячной сеткой) + форма «Zeitraum
blockieren» (von/bis/причина; ресурс — селектором страницы; идемпотентный
bulk ClosedDate, кап 366 дней) + «Wieder öffnen» снимает диапазон. Блок
ЧАСТИ дня — прежним walk-in («Buchung hinzufügen»).

**«Зачем отелю выбор специалиста?»**: пикер «Choose your specialist» и секция
«Team» показываются только когда ресурсы — ЛЮДИ с профилем (фото/должность/
био, A3). Технические ресурсы (Wellnessbereich, Rezeption, Bühne) гость не
выбирает — назначение автоматическое. Гейт в ДВУХ местах одного правила:
`_service_rich_context._team` (полная страница+секция) и основной ctx
(?box=1-фрагмент) — rich-мерж перекрывает основной ctx (урок: `ctx.update(rich)`
затирал первый вариант гейта). Осознанно обновлён замок паритета секций
(team-ресурсы теста получили профили). 2 новых замка; booking 205 зелёных;
10 msgid × 5 .po.

**Дополнение той же датой — карточка брони услуги** (фидбэк «перекидывает на
непонятную страницу»): клик по сделке услуги (доска/списки Verkäufe) вёл на
ОБЩИЙ календарь сегодняшнего дня — без деталей и даже без даты брони. Новая
карточка `/dashboard/booking/termin/<pk>/` (зеркало stays booking-detail,
FB-11): клиент/услуга/мастер/время/цена/оплата/заметка + действия FSM и
перенос; `_manage_url("booking")` → карточка; reference_code в Tagesplan —
ссылка; `booking_action` уважает внутренний `next=` (open-redirect guard).
Замок test_booking_detail_card_and_manage_url; booking 206 + core 41 зелёные.

## 2026-08-05 — W-v3: полный аудит кабинета (14 агентов) + план унификации W7–W12

Запрос владельца: «полноценная проверка кода и функционала; упрощали кабинет
(Einfach/Experte), а на практике опять каша; выделить базовые настройки, а
дополнительные по типам — в табы; изучить кабинеты TravelLine/Битрикс/CS-Cart».

**Метод:** воркфлоу — 5 аудит-агентов по измерениям (навигация/IA · настройки ·
режимы · продажи · marketing+CRM+website), каждая находка адверсариально
верифицирована отдельным скептиком по коду (CONFIRMED/ADJUSTED/REFUTED), + 4
веб-разведчика (TravelLine, Битрикс24 как антипаттерн-эталон, CS-Cart,
Shopify/Wix/Squarespace/Shopware). ~100 верифицированных находок.

**Главное (7 системных причин «каши»):** 4 несинхронизированных реестра навигации
(+карточки лендингов 5-м слоем); 3 поколения поверхности продаж одновременно;
Простой режим НЕ работает в новом UI (sidebar_nav не вызывает simple_hidden_modules
— требование st4b-плана потеряно, тесты охраняют мёртвый classic-путь);
classic_ui = вторая IA (два неполных кабинета); подсветка живёт на 7 из 44
nav-значений + сломанный поиск меню в компакт-ветке; опасные save-пути настроек
(normalize теряет notify/low_stock/lots_enabled ~в 20 местах записи; голый
tenant.save() в settings_view; notifications затирает выключенные модули; мёртвые
POST-ветки, обнуляющие поля) ; сироты: /dashboard/billing/ (подписка!), Blog,
Newsletter (0 входов), Finanzen/Auswertungen (входы в 404), collections. Плюс
поведенческие дефекты продаж: «Versendet» с доски шлёт письмо без трек-номера,
доска предлагает «Abgeholt» доставочному заказу, резерв с доски теряет таймстемпы,
акцию на услугу/номер нельзя создать из UI (движок PL готов, в форме только product).

**Конкуренты:** канон верхнего уровня совпадает у всех четырёх западных платформ
(объекты бизнеса → рост → расширения → единый Settings-хаб) — наш 7-якорный
сайдбар уже канон, чинить наполнение/подсветку/Settings. Урок Битрикса: «показывать
мало по архетипу и дать включить», один вход на сущность, настройка живёт где её
эффект. TravelLine: календарь = рабочий стол, «Основные/Дополнительные» вкладки;
их болезнь (3 куста настроек) = наша.

**Деливерабл:** `docs/cabinet-audit-2026-08-05.md` (сводка) + папка
`docs/cabinet-audit-2026-08-05/` (5 аудит-отчётов с вердиктами + 4 исследования) +
план `docs/cabinet-unification-plan-2026-08-05.md`: волны W7 (предохранители
data-safety/навигация/продажи — без решений владельца) → W8 (единый реестр
NavEntry: сайдбар/табы/плитки/подсветка/поиск из одного источника + замок
достижимости) → W9 (Settings-хаб: 4 базовых таба + 4 по типам + Erweitert, единый
Save) → W10 (Verkäufe единственной поверхностью: вид «Heute», паритет Liste,
apply_action, редиректы легаси) → W11 (Marketing+Kunden, судьба лендингов,
...
## 2026-08-05 — W8: единый реестр навигации + подсветка «где я» + палитра Ctrl+K

По плану `w8-nav-registry-plan-2026-08-05.md` (после мержа W7+W-CL в main).
**W8-1:** НОВЫЙ `apps/core/nav_registry.py` — один источник правды навигации:
`ANCHORS` (7 якорей сайдбара с гейтами/бейджем/search) + `ENTRIES` (все табы
хабов с hub/module_key/advanced/search). Производные: `cabinet.HUB_TABS` =
`legacy_hub_tabs()` (прежняя форма кортежей — все замки целы), `modules.sidebar_nav`
читает ANCHORS (_sales_nav_item умер), карта `ANCHOR_BY_NAV` (табы → якорь хаба +
явные записи для легаси-страниц продаж/лендингов). **Подсветка:** фильтр
`nav|nav_anchor` в сайдбаре и мобильном таб-баре — «где я» работает на ВСЕХ ~45
nav-значениях, а не на 7 якорных (e2e-замок: страница уведомлений подсвечивает
якорь Einstellungen). **W8-2 (инварианты):** замок «каждый nav-литерал из вьюх
имеет якорь» (скан исходников — новый экран без записи ломает тест; класс ошибок
care/сирота невозможен молча) + «каждая запись реестра reverse'ится».
**W8-4:** палитра Ctrl/Cmd+K (кнопка 🔍 в шапке): оверлей `_nav_palette.html`,
индекс = реестр с гейтами модулей и Простого режима, клиентский фильтр по
label+search, Enter = первый видимый, Esc/фон = закрыть; 2 msgid × 5 .po.
...
## 2026-08-05 — W9-10: «Team & Zugriff» — членства/роли/инвайт + owner-гейт (Р-7)

БЕЗ миграции (вывод разведки плана §2: core.Membership готова). **Ролевой гейт**
— в CabinetOwnerAccessMiddleware: owner-only префиксы /dashboard/billing/ (Abo +
Stripe-Connect OAuth) · /dashboard/recht/ (правовые тексты + налоговые реквизиты
W9-5) · /dashboard/settings/team/. Приглашённый admin/staff получает 403 —
«пригласить сотрудника» ≠ «отдать бизнес» (до этого роль не гейтила НИЧЕГО —
критическая находка аудита). Решение «middleware, не per-view декоратор»
осознанное: десятки существующих тестов зовут billing/legal-вьюхи с
фейк-юзерами без Membership — декоратор их сломал бы; реальный HTTP закрыт
fail-closed, обход возможен только в прямых вызовах тестов. **Экран
`/dashboard/settings/team/`** (прямой таб настроек): список членств, смена роли
селектом, отзыв доступа (User остаётся — история/повторный инвайт), инвайт
email+роль (admin|staff; owner не приглашается). **Инвайт** `apps/core/team.py`:
Redis-токен (sha256-ключ, неймспейс по схеме — паттерн magic-link CA1), TTL 7
дней, одноразовый (расход строго при принятии; слабый пароль НЕ расходует);
письмо fail-safe + ссылка всегда в flash владельцу. **Принятие**
`/team/beitreten/<token>/` (публичный путь, вне CABINET_PREFIXES): новый User —
пароль-форма (≥8, повтор); существующий User с usable-паролем НЕ
перезаписывается (анти-takeover) — только членство; login → dashboard.
Guard'ы: последний owner неудаляем/непонижаем; self-revoke запрещён; AuditEvent
на invite/role/revoke. Ограничение v1: admin ≡ staff (в UI честный хинт).
29 msgid × 5 .po. Замки test_w9_team (8): 403 staff на 3 префиксах / owner
проходит / одноразовость+валидация токена / join создаёт User+Membership+login /
слабый пароль не жжёт токен / guard'ы последнего owner / отзыв второго owner /
рендер. Локально: team+nav+smoke 37 зелёных, полные core+billing — прогон на CI.


## 2026-08-05 — W9-11 (сверка) — ВОЛНА W9 ЗАКРЫТА ЦЕЛИКОМ

W9-11 закрыт сверкой: Finder уже в Einstellungen/Erweitert (W9-1a), site_seo уже
с hub_tabs (W9-4) — кода не потребовалось. Волна W9 (Settings-хаб «базовые + по
типам в табах», W9-1..W9-11) завершена целиком, БЕЗ миграций. Очередь плана
унификации: W10 (Verkäufe единственной поверхностью) → W11 (Marketing+Kunden,
Р-2) → W12 (честные режимы).


## 2026-08-05 — W10-1: одна модель переключения видов продаж + GET-сохранение

Старт волны W10 (план `w10-verkaeufe-plan-2026-08-05.md`). Снята «двойная модель
переключения видов» из аудита: persist `sales_views` (Verkäufe) сосуществовал с
сегментом ST-5b «чистая навигация без персиста» на 5 легаси-страницах.
(1) `verkaeufe_view_set` теперь редиректит на ПОЛНЫЙ исходный путь (hidden
next= в формах переключателя; только внутренний путь) — переключение вида
больше не сбрасывает выбранную дату/поиск/открытую карточку (?von=/?tag=/?q=/
?buchung=). (2) Сегмент ST-5b удалён целиком: тег `orders_view_switch`,
партиал `_orders_view_switch.html`, `switch_options`/`create_option`/VIEWS из
`orders_view.py`; на board/order_list/booking-календаре/stays-календаре/
stay_new остался мостик «Alte Ansicht · 🆕 Alles auf einer Seite →» (0 новых
msgid — строки существующие). Создание не потеряно: walk-in формы живут в
телах календарей (проверено; `stays:stay-new` без входа до «＋» W10-3).
`orders_view.py` сведён к архетип-дефолту `resolve_view` (главная кабинета) +
...
## 2026-08-05 — W11-3: «Über den Betrieb» — портальные отзывы видны владельцу

Экран Bewertungen получил вкладку-чип «Über den Betrieb» (?typ=betrieb):
отзывы о бизнесе с городского портала /entdecken (SHARED BusinessReview,
фильтр по connection.schema_name) — находка аудита «витрина рендерит, владелец
не может ни увидеть, ни ответить» закрыта наполовину: ВИДИТ (v1 read-only,
включая hidden с пометкой). ОТВЕТ владельца требует миграции поля на SHARED-
модели → отдельное решение владельца (честный хинт «Antworten folgen» на
экране). Автор НЕ раскрывается — паритет с порталом (там показываются только
...
## 2026-08-05 — W11-5: разведка Website-свода → код отложен свежей сессией

Разведка §2.6: site.html — функциональная страница (quick-start шаблоны+демо ·
галерея-аплоад multipart · тексты welcome/quick_add · POST-ветки site_view),
а не пустой лендинг-дубль. Полный «слив в Studio» = перенос форм в рейку
site_home.html (3400 строк, двойная буферизация, draft-канал) — по прецеденту
...
## 2026-08-06 — I18N-10: переводы параметров товара (варианты/модификаторы/характеристики)

Запрос владельца: «проверь переводы демо-сайтов, админки, характеристик и
параметров». Аудит + план `i18n-10-parameters-plan-2026-08-06.md`; решение
владельца — «делаем всё, включая миграции». ⚠️ миграция `catalog/0024` (аддитив).

**Что показал аудит.** Хром закрыт: 4014 msgid × 5 локалей, у ru/tr/uk пустых
нет, у en/de пустые записи — identity (msgid уже на нужном языке), то есть
показов «чужого языка» ноль; гейт I18N-9 держит регресс, в allowlist только
Branchen-лендинги. Демо-сущности переводятся с DL-волны. **Дыры были ровно в
параметрах:** метки вариантов (`label/size/color`) и модификаторов
(`ModifierGroup.name`, `ModifierOption.label`) не имели оверлеев вообще;
`material_i18n`/`care_i18n`/`origin_i18n`/`ingredients_i18n` существовали, но
демо-обход их не заполнял; `Category.size_table` без i18n. Отдельно — тот же
класс, что был с акциями в HF-волне: часть переводов меток уже лежала в
демо-словарях мёртвым грузом (применить было некуда).

**P1** оверлеи на `ProductVariant`/`ModifierGroup`/`ModifierOption`/`Category`
+ `*_localized`. Решение зафиксировано планом: **переводится только показ** —
плоские поля остаются ключами учёта (склад-леджер, позиции заказа, CSV-импорт
по `(product,label)`/`(product,size,color)`, оси фасета). **P2** витрина:
пикер (все виды), форма корзины, строка корзины, Anprobe/Warteliste, группы
модификаторов, Größentabelle. **P3** ввод переводов из кабинета — тег
`overlay_i18n_inputs` + партиал (поля неосновных локалей в DOM, скрыты,
показывает свитчер Ф1; presence-guard). **P4** демо: обход дополнен вариантами,
модификаторами, material/care/origin/ingredients и таблицей размеров; словари
+37 строк (материалы/уход/опции/цвета/узоры/размерные таблицы/«Seidentuch
Aurora»).

**Найдено стендом (Playwright, 12/12 на демо `mode` и `cafe`):** составная
метка «S · Blau» производна от осей и своего перевода не имеет → `label_localized`
теперь собирается из ПЕРЕВЕДЁННЫХ осей; в двухосевом пикере подпись свотча
бралась из ключа (`title="Schwarz"`) → разделены `name` (ключ сопоставления) и
`label` (подпись). Замок проекта «без identity-записей в демо-словарях» поймал
13 моих en-записей вида S→S — убраны (фолбэк и так отдаёт базу).

Замки: `test_parameters_i18n` (14) — характеризация «без переводов рендер
прежний», перевод показа, **снимок заказа и ключ оси остаются немецкими**,
ввод из кабинета с presence-guard, осевой свотч. Локально: gettext в песочнице
нет → `.mo` собраны через polib, после чего прогон совпал с CI.


## 2026-08-06 — CI: отмены прогонов = ОЖИДАНИЕ раннера (шардинг откачен)

Диагностическая ошибка, зафиксирована честно. Прогоны отменялись ровно на
15:0x, и это было прочитано как «job упёрся в лимит времени выполнения» →
тесты разбили на два параллельных шарда. **Диагноз неверен:** у всех
отменённых job'ов `runner_id=0` и пустой `runner_name` — раннер НИКОГДА не
назначался; job стоял в очереди и отменялся по таймауту ОЖИДАНИЯ свободного
раннера. Отсюда и «Actions не запускают прогоны» (регистрация run'а есть,
раннера нет) и разброс: прогон #1892 дождался раннеров и прошёл за 8.7 мин.

Следствие: шардинг ВРЕДЕН — он требует два раннера вместо одного, то есть
удваивает шанс не дождаться. Откачен вместе с замком `test_ci_shards`.
Признак для будущих разборов: `cancelled` + `runner_id=0` = очередь раннеров,
а НЕ длительность прогона; смотреть надо `started_at` против назначения
раннера, а не время job'а.


## 2026-08-06/07 — Аудит демо: ST-8 страницы витрины + акции всех типов + починка меню

Запрос владельца (две реплики): «у каждого демо основные модули, меню — выходы
в них, страницы галереи/отзывов/мастеров ОТДЕЛЬНЫМИ страницами, не разделами
главной; проверь на всех языках» и «не везде есть акции, засеять все типы акций
по всем китам». Объём страниц уточнён владельцем: профильным китам, набор —
отзывы/галерея/команда. Полный разбор — `docs/demo-audit-2026-08-06.md`,
план ST-8 — `docs/st8-storefront-pages-plan-2026-08-06.md`. Без миграций.

**ST-8.** Отдельных страниц галереи/отзывов/команды в проекте не было вообще —
только секции главной, а пункты меню отеля/ресторана были ЯКОРЯМИ (`/#galerie`).
Сделаны `/galerie/`, `/team/`, `/bewertungen/`; гейт — по НАЛИЧИЮ контента
(пусто → 404, пункт меню гаснет), архетип-whitelist сознательно не заводили: он
врал бы при пустом разделе и мешал бы владельцу, наполнившему раздел вручную.
Отзывы собирают ДВА источника (портальные `BusinessReview` + кураторские
`testimonials`) — иначе у демо страница была бы пустой. Секции главной остались
тизерами со ссылкой «Все …».

**Акции.** 14 наборов `promotions_spec`: у каждого кита теперь 4–17 акций,
вместе покрывающих все 9 стилей (percent/strikethrough/festpreis/ab/badge/
countdown/mystery/surprise/легаси), оба типа (discount+reservation) и все виды
целей ценового слоя PL (товар/услуга/номер/свободная). `promotions` включён
дополнительно у werkstatt/handwerker/tours.

**Меню — 6 классов дефектов, все от одной причины.** `menu._resolve` отбрасывает
узел без ссылки МОЛЧА, поэтому опечатка/незарегистрированная цель/выключенный
модуль/якорь на отсутствующую секцию не падают нигде. Найдено и исправлено:
у ресторана своего меню не было вовсе (шапка выводилась из легаси-`nav` → ни
галереи, ни отзывов, ни команды); у ретрита из шапки нельзя было попасть в
размещение и заявки (модули активны, узлов нет); у отеля «Buchen» вёл на
`/#buchen`, которого на его главной НЕТ (`hero_widget="stays"` гасит секцию
`stay_search` с этим id); «Kontakt» у туров/Handwerker ссылался на страницу
`contact`, отсутствующую в `_PAGE_URL_NAMES`; у шести китов акции засеяны без
пути к ним; якоря «Aktionen»/«Sale» прокручивали главную вместо страницы
`/aktionen/`, работающей с любой страницы. Плюс `/lookbook/<slug>/` не имел НИ
ОДНОЙ ссылки с витрины (только из кабинета) → ссылка у выбранной подборки с
фото (`collection_chips[].lookbook`).

**Замок, которого не хватало** — `apps/tenants/tests/test_demo_menus.py` (84,
без БД): кит проверяется КАК ДАННЫЕ — каждая `page`-цель есть в реестре; каждая
`archetype`-цель активна у ЭТОГО типа бизнеса; каждый якорь соответствует
секции, включённой у ЭТОГО кита (`_kit_sections`); наполненный модуль выведен в
меню; при наличии галереи/команды/отзывов есть страница ST-8 (исключение
retreat — учителя на своей `/lehrer/`); у каждого кита есть своё меню.

**Грабля активности модулей:** «активно» = всё, кроме погашенного
`default_disabled_for(business_type)`; `enable_modules` кита лишь ВОЗВРАЩАЕТ
погашенное, а не перечисляет активное. Первая версия замка считала иначе и дала
14 ложных падений.

**Исправлен собственный перегиб предыдущего инкремента:** `account` убран из
`NAV_ITEMS` — в шапке уже есть иконка 👤 (`_header_icons.html`), пункт стал бы
вторым входом в то же место у КАЖДОГО тенанта; `combos` получил гейт по наличию
наборов (модуль `orders` активен почти у всех, пустая страница набора в шапке
не нужна). Golden-эталоны `normalize` перегенерированы осознанно — диф ровно три
новых пункта `nav` (loyalty/gift/combos).

**Среда:** локально ставится `gettext` (`apt-get install -y gettext`) — после
этого `scripts/i18n_gap.py` и `msgfmt` работают локально и прогон совпадает с CI
(раньше эти шаги были доступны только на CI, а локальные немецкие ассерты
падали). Новые msgid «Unterkunft» и «Lookbook ansehen» — в 5 каталогах;
`.po` дописываются ТЕКСТОМ, не через polib: polib переформатирует весь файл.

**CLAUDE.md** приведён в соответствие с кодом: §7 помечен архивом (порядок работ
от 01.07 разобран целиком), снята закрытая долговая запись про XSS в карте
агрегатора (`_map.html` строит попап через DOM с проверкой схемы href), статус
языкового модуля от 30.06 свёрнут в историю — он утверждал «`.po` пусты, хром
DE-only», что противоречит коду (5 локалей, ~4100 msgid).

**Тот же класс на первом экране (плитки hero).** Сплошная сверка `HERO_TILE_SETS`
с гейтами вьюх: «Geschenkgutschein» гейтился модулем, а страница требует ещё и
настроенной оплаты; «Rückruf» был без гейта при `_require_jobs_active` во вьюхе;
«Aktuelle Deals» и все `gate="deal"` вели на `/aktionen/`, которая требует модуль
`promotions` — наличие живой акции в БД этого НЕ гарантирует. Замок теперь
проверяет реестр целиком; первая его версия молча пропускала `/aktionen/` —
слабый замок опаснее отсутствующего.

**Побочно — контент-гейт стоил запроса на КАЖДЫЙ рендер меню** (`resolve_menu`
зовётся из шапки, нижнего меню и контекст-процессора кабинета). Поймал чужой
замок: `test_nav_hides_disabled_modules` (юнит-тест без `django_db`) стал красным
на ветке при зелёном main. Мемоизация на объекте тенанта + `.exists()`; пробники
fail-closed — сбой чтения прячет пункт, а не роняет страницу.

**Проверка на живых демо** (14 тенантов, свежий `--recreate`): у каждого меню нет
ни одной мёртвой ссылки (обход всех пунктов), страницы ST-8 отдают 200 там, где
есть контент, и 404 там, где его нет (у aktionsmarkt/shop нет команды — и пункта
меню там тоже нет), акции видны у всех, заголовки страниц переводятся (de ≠ ru).

⚠️ ops после деплоя: `seed_demo_tenants --recreate`.


## 2026-08-07 — Фидбэк витрины: дропдауны шапки, /aktionen/, диеты, каталог, меню, Branchen

Батч по фидбэку владельца (план `docs/storefront-feedback-2026-08-07.md`),
6 коммитов, БЕЗ миграций. Запись дописана 2026-08-10 — батч был влит в main без
строки в build-log (нарушение конвенции памяти, исправлено).

**Дропдауны шапки («Mehr не работает и пустая»).** Разметка прод=локал, дефекты
рантаймовые: (1) панель «Mehr» раскрывалась только по hover/focus-within — на
таче hover нет, iOS не даёт кнопке фокус по тапу; кнопка подключена к тому же
делегированному клик-обработчику, что группы меню (data-nav-group, 2026-07-30);
(2) клик вне панели не закрывал её — обработчик снимал `block`, не возвращая
`hidden`. Замки на оба свойства.

**/aktionen/ «выглядит незаполненной».** Каждая группа рендерилась секцией с
3-колоночной сеткой → группа из ОДНОЙ акции оставляла ⅔ строки пустыми (замер:
у ресторана 6 акций дробились на 5 групп). Теперь секция — от MIN_GROUP_SECTION
(=2) акций, одиночные — общим блоком «Weitere Angebote» в конце; чипы-фильтры
не тронуты. Замок 2026-07-29 обновлён осознанно (охраняемые свойства целы).

**Диета — в панель фильтров** (была отдельной строкой чипов над панелью).
Обычный селект той же GET-формы; `?diet=` прямыми ссылками работает; деталь
гейта — `diet_chips` добавлен в условие показа панели (иначе у гастро без
разброса цен диета исчезла бы вместе с панелью, замок).

**Каталог /sortiment/.** Комбо-тизер стоял ПЕРВЫМ блоком («откуда-то взялось
комбо-меню») → уехал под сетку товаров. Категории — плитки с фото через общий
`_category_tile.html` (главная/каталог/подкатегории; без фото — прежние чипы,
замок на фолбэк). Размер плитки — БЕЗ нового ключа: `SECTION_STYLES
["categories"]` (square/tall/wide, ""=4:3) — механизм presence-minimal, селект
билдера строится из реестра сам, golden целы. Демо-фото категорий засеяны
(иначе владелец правку не увидел бы).

**Меню демо.** «Mehr» появлялась из-за ширины: контейнер отдаёт меню ~788 px, а
меню богатых китов доросло до 9–11 пунктов → «о нас»-разделы свёрнуты под пункт
«Über uns» (остаётся ссылкой, подменю по hover/тапу); считаем ШИРИНУ
(`_menu_row_width`), а не число пунктов; литералы `*_MENUS` не тронуты —
сворачивание производное при сидинге. Попутно пред-существующий баг, ставший
массовым: селект страниц в редакторе меню кабинета был захардкожен {home,
about} — узел с другой целью после Save начинал вести на главную; теперь список
из реестра `menu.PAGE_TARGET_LABELS`.

**Branchen-страницы («добавь среди прочего функционала»).** Сетка модулей
строилась только из `recommended_for` → 4 отраслево-нейтральных модуля
(Auswertung/Veröffentlichung/Finanzen/Telegram-Bot) не показывались нигде.
По решению владельца — СРЕДИ прочих карточек в порядке реестра, не отдельным
блоком «есть у всех»; `recommended_for` не искажён (он управляет пресетами
онбординга). Замки: каждый не-core модуль виден хотя бы на одной странице;
нейтральные не съехали в хвост.


## 2026-08-10 — SM: один функционал для всех + Verkäufe по модулям

Решения владельца одной сессией (план `docs/sm-single-mode-plan-2026-08-10.md`).

**SM-1: режим Простой/Эксперт снесён целиком** (прецедент W-CL/classic_ui).
Удалены: ui_mode/is_simple/simple_hidden_* в modules.py + поля ModuleSpec;
POST-ветка и set_ui_mode_view/ansicht_view; гейты в context/dashboard/hub_tabs/
палитре Ctrl+K; ui_simple формы товара и настроек; тумблер в шапке; экран
Ansicht (роут+шаблон+запись реестра навигации); normalize ДРОПАЕТ ключ ui_mode.
Замки W6-класса «чужой ключ переживает запись» переведены с ui_mode на живой
probe-ключ `notify` (passthrough W7a) — охраняемое свойство не изменилось.
Новый замок test_simple_expert_mode_is_gone. Einfach/Experte РЕДАКТОРА
(UC6-10, свёртка настроек блока) оставлен — решение владельца.

**SM-2a: Verkäufe — вкладки модулей первыми, виды внутри вкладки, Heute на
главной.** Фидбэк: «кнопки идут как будто над верхним уровнем, а должны быть
частью каждого внутри; Heute — часть Übersicht». Раскладка: H1 → вкладки →
тулбар вкладки (виды kind слева, ＋/⚙️ Abläufe?kind= справа) → содержимое.
Сводка Heute рендерится на Übersicht (сетка при делах, слим-строка при пустом
дне); кнопка из ряда видов убрана, ?view=heute жив для deep-links; ссылка из
Belegungsplan → главная, виджет «Anreisen heute» → вкладка stay. Правило
вкладок: КАЖДЫЙ активный модуль сразу (решение владельца; замки W10-2/Р-4
переписаны осознанно).

**SM-2b: паритет настроек статусов всем шести kind.** Доска/списки были generic
изначально — расширены два реестра: _STATUS_LABEL_KINDS (+job/ticket/
reservation; через него валидируются ОБА эндпоинта — имена и переходы) и
_status_kinds_for (вкладки Abläufe). У Reservation модельных choices нет (UD1)
— дефолт-подписи заданы явно, msgid уже были. Замок паритета: коды 1:1 с
status_registry.BUILTIN. Кастом-статусы (FB-3 B) пока order/booking/stay —
второй шаг.

Грабля стенда (повтор): runserver --noreload поднят ДО правок — «дефекты» на
стенде оказались старым кодом; перезапуск обязателен. Всё БЕЗ миграций.


## 2026-08-10 — SM-3: кастом-статусы (FB-3 Вариант B) на всех шести направлениях

Решение владельца «делаем» (второй шаг SM-2b). План
`docs/sm3-custom-status-all-kinds-plan-2026-08-10.md`; разведка — воркфлоу
10 агентов (6 разведчиков + 4 адверсариальные сверки). БЕЗ миграций.

**Главная находка: дверь уже была открыта.** SM-2b расширил
`_STATUS_LABEL_KINDS`, а через этот словарь живёт ВЕСЬ трубопровод кастом-
статусов: normalize_status_defs/edges, гейт вьюх status_manager, вход с
Abläufe. Реестр/эффекты/FSM были 6-kind с Phase 0–6 (kind проставлен у всех
шести SM). То есть владелец МОГ создать статус для Tickets уже вчера — без
гарантий корректности. Инкремент = закрыть дыры, а не «включить фичу».

**Закрытые дыры (замки писались ДО фиксов, красные на дырах):**
- **anti-oversell билетов**: 4 литеральных `Ticket.ACTIVE_STATUSES`
  (guard ёмкости/тира, seats_sold, tier_sold_map) → `active_statuses_for
  ("ticket")` — билет в кастом-active статусе снова держит место (была прямая
  двойная продажа);
- **двойной возврат склада/лимита**: возвраты не идемпотентны (F()+qty), а
  `on_transition` диспатчит по t.dst. Первая версия фикса (дроп рёбер «оба
  конца cancelled») была ОБОЙДЕНА адверсариальной сверкой: двухшаговый путь
  cancelled → кастом-active (un-cancel, счётчики не ре-декрементятся) →
  cancelled. Итоговое правило проще и сильнее: **рёбер ИЗ cancelled-роли не
  бывает** — терминальный статус терминален, ровно как в builtin-графе.
  Слой чтения (лечит сохранённые конфиги) + guard в эффектах + редактор
  (sources/targets раздельно, un-cancel не предлагается) + save;
- **зеркала эффектов**: job done-роль → `commit_stock` (идемпотентен гардом
  stock_committed; иначе кастом-«QK bestanden» закрывал заявку без списания
  Teile); ticket cancel-роль → стоп активной рассрочки (R10e; иначе beat
  продолжал бы off-session списания);
- **асимметрия attended**: `def_from_role` стал kind-aware — done-роль у
  БИЛЕТА получает blocks_capacity=True (паритет с builtin attended; иначе
  свой «завершённый» статус освобождал бы место);
- **varchar(20) vs кламп 40**: у ВСЕХ шести моделей status = varchar(20), а
  normalize клампил код на 40 → «warten_auf_ersatzteile» ронял apply()
  DataError'ом. Кламп 20 согласованно в defs/edges/редакторе (без миграции);
- **re-save терял флаги**: редактор пересобирал каждый деф через def_from_role
  → counts_in_reports (PMS-кейс отеля, ставится через site_config) слетал при
  любом сохранении. Теперь неизменная роль сохраняет деф (label обновляется);
- **правила переходов прятали кастом-кнопки**: правило (Вариант A),
  сохранённое ДО создания кастома, — whitelist целей; плюс
  normalize_transitions вычищал кастом-код при каждом save настроек (класс
  W7a). Разграничение слоёв: правила управляют ТОЛЬКО builtin-целями,
  кастом-цели всегда видимы (их курирует status-manager); панель правил их не
  предлагает и не хранит; danger-флаг кнопки — из дескриптора (кастом-отмена
  красная);
- **литералы «активных»**: PII-purge и max_per_customer резервов (клиент с
  кастом-active резервом терял PII / обходил лимит), дайджест/виджет/Heute
  (сделка выпадала из счётчиков), верификация отзывов (кастом-отменённый
  билет оставался «верифицированным покупателем» — новый
  `cancelled_statuses_for` + 3 файла reviews);
- **сырые слаги** в панелях и per-app экранах: `builtin_status_labels(kind)`
  (у Reservation нет choices) + тег `{% status_label %}` в jobs/list,
  jobs/detail, тикетах event_detail, reservation_list.

**Осознанные квирки (план §2):** beat-протухание резервов — только builtin;
per-app кнопки действий из кастом-статуса — с доски/Verkäufe (W10); drip/
post-event-письма — builtin-пути; публичный акцепт сметы гейтится quoted;
builtin-отмена билета не сторнирует выручку, кастом-cancel сторнирует
(семантика Варианта B, как у живых kind).

Стенд Playwright 13/13 (touren): создать статус из Abläufe → рёбра (un-cancel
...
## 2026-08-11 — SM-4: раскрывающиеся подпункты разделов в сайдбаре

Запрос владельца (скрин «Расширенный» на Angebote): «вынесем настройки в
дополнительные пункты продаж/предложений и т.д. выпадающим списком для каждого
раздела; отчёты — в продажах»; уточнение mid-turn: список — СЛЕВА В МЕНЮ,
раскрытие «слайдером»; у Веб-сайта в подпунктах — и добавление домена. Пункты
вынесены на обсуждение ДО кода; развилки решены AskUserQuestion'ом: Finanzen →
Продажи · Abläufe только в списке · каталог — единый компакт-бар · Website —
фикс SEO. План `docs/sm4-sidebar-children-plan-2026-08-11.md`. БЕЗ миграций.

**Принцип:** подпункты якоря сайдбара = advanced-состав его хабов из
`nav_registry` (единый реестр W8) — гейты модулей, палитра Ctrl+K и подсветка
«где я» достаются даром. Новый `sidebar_children(anchor)`; `sidebar_nav`
отдаёт `children`; в `_base_dashboard.html` — шеврон-кнопка + свёртка
(активный раздел раскрыт с сервера), фильтр поиска меню раскрывает блоки с
совпавшими подпунктами.

**Переезды в реестре:** Auswertungen + Finanzen — из settings-хаба в board
(подпункты «Verkäufe»); `stays:reports` («Berichte») — сирота получил вход;
Abläufe — дубль-запись board (таб Einstellungen жив; прецедент
sellables/catalog). НОВЫЙ хаб `site`: SEO (`site-seo`, nav "site"→"seo") ·
Website & Domains (переезд из main-табов settings) · Medien (из advanced
settings). Настройки слимятся 12→9 main табов. Каталог-компакт:
Lager/Kombi/Import → advanced (таб-бар каталожных страниц = дух скрина
Angebote). «⚙️ Abläufe» убрана из тулбара вкладки Verkäufe. С пяти страниц
(journal/analytics/domains/media_library/site_seo) снят settings-таб-бар —
site_seo рисовал ЧУЖОЙ бар без активного таба ещё с W9.

**Замки:** состав/гейты/рендер подпунктов (test_sidebar_st4b +3), компакт
каталога и карта якорей (test_hub_tabs +2), тулбар без ablaeufe?kind=
(test_verkaeufe, осознанная переписка SM-2-замка), состав settings-хаба
(осознанная переписка W9-замков), W7b-замок «сироты имеют вход» переписан на
board-хаб. Инварианты W8 (reverse всех записей, якорь у каждого nav) — зелёные
без правок.

**Стенд Playwright 25/25** (hotel; finance/analytics временно включались —
у ВСЕХ демо-китов они выключены, гейты честно прячут подпункты): свёртка/
раскрытие шевроном, авто-раскрытие активного раздела, переходы, подсветка
якорей на переехавших страницах, поиск «datev» раскрывает блок и находит
Finanzen, 7 подпунктов Angebote, компакт-бар каталога.

**Грабля (поймана скриншотом стенда, не тестами):** двухстрочный `{# #}`
в verkaeufe.html отрендерился ТЕКСТОМ на странице — `{# #}` у Django строго
однострочный. Замок template_comments её ловит, но гонялся до внесения
...
## 2026-08-12 — GK-15: демо catering в структуре референса goodkarma

Запрос владельца «засеять демо кейтеринга как у исследуемого варианта». План —
`docs/gk15-catering-demo-parity-plan-2026-08-12.md`. **Generic-механизмы кита
(любой архетип наполняет индивидуально):** `DemoKit.home_blocks` — C-блоки
главной по якорю «после секции» (образец spacers, id `demo-block-N`, данные
чистит normalize) · `DemoKit.socials` (whitelist 5 Tenant-полей GK-9; для демо
ТОЛЬКО корневые URL соцсетей — фиктивный handle мог бы указать на ЧУЖОЙ реальный
аккаунт) · `DemoKit.google_rating` (демо-кэш GK-11; place_id пуст → beat/API
демо не трогают) · `enable_categories_section` — фото-плитки категорий на
главной СРАЗУ после hero (референс держит сетку направлений перед «столпами»).
**Наполнение «Grüne Tafel»:** 6 категорий-направлений (+Hochzeit/Business&
Seminar/Private&Messe, пакеты «€ p.P. + ab N Personen»), тиры Fingerfood
Klassik/Plus/Premium, цифры-полоса stats (200+ Events/50.000+ Gerichte/10.000+
Gäste/seit 2012), цитата основателя (данные пресета «Gründer-Zitat» GK-7),
newsletter-блок в конце главной, фото+звёзды у 3 отзывов (аватар-ряд trust),
Google-строка 4,9★/41, социконки футера. Новые товары — ХВОСТОМ категорий,
новые категории — ПЕРЕД Getränke: индексы promotions_spec (0–3) стабильны
(комментарий-страховка в ките). i18n: 5 имён категорий × 4 словаря (включая
2 старых непереведённых); описания позиций — «по спросу» (политика DL).
Проверено на живом сиде (14 маркеров: порядок секций = референс, PDF-кнопка,
Anfrage-CTA детали GK-14). Грабля стенда: маркеры с «&» в HTML — `&amp;`
(ложные FAIL); dev-пробник глушит debug_toolbar через SHOW_TOOLBAR_CALLBACK +
`get_config.cache_clear()`.

## 2026-08-12 — DS-1/DS-2: дизайн-шаблоны — семейства «Fein»/«Natur» + пилот catering/pranasy

Решение владельца: «начнём с кетринга и pranasy; дальше на каждый архетип
несколько дизайнов на выбор, вначале по одному». План —
`docs/ds1-design-templates-plan-2026-08-12.md`. **DS-1 (механика, generic):**
(1) первые self-hosted веб-шрифты — Playfair Display 600 и Nunito 800
(OFL, fontsource; сабсеты latin/latin-ext/cyrillic ≈90 КБ суммарно;
`static/fonts/` + SOURCES.md; @font-face с unicode-range в app.css; грузятся
ЛЕНИВО — только когда семейство в `--font-head`; тело остаётся системным);
`FONTS` += `editorial`/`organic`. (2) `site_defaults.page_bg` — фон СТРАНИЦЫ
(до этого bg-gray-50 захардкожен); presence-minimal hex, правило в _base
скоуплено `html:not(.dark)` — тёмную тему красит `.dark`. (3) `LOOK_FAMILIES`
+= **«Fein»** (Playfair на креме #faf7f2, карточки r20, nav centered) и
**«Natur»** (Nunito на песке #f6f2e7, r24, classic); кортежи
`ARCHETYPE_LOOK_ACCENTS` расширены 3→5 колонок у всех 15 типов (замок «ровно
5» — рассинхрон дал бы чужой акцент молча). 75 Look'ов; идемпотентность
apply→normalize покрыта существующим параметризованным замком бесплатно;
галерея мастера/билдера подхватила 5 карточек сама. **DS-2 (пилот):** кит
catering → `look="fein"` (акцент глубокий лес #166534), pranasy →
`look="natur"` (листовой #4d7c0f). **Фото-качество пилота:** немецкие/
несуществующие ключи давали «леденцовые» SVG-фолбэки → _make_category принял
опциональный 4-й элемент-строку (фото-ключ плитки; слуг остаётся URL/
переводом), ~25 ключей кита catering перемаплены на СУЩЕСТВУЮЩИЕ файлы
static/demo/photos (категории/товары/промо/галерея/обложка каталога/аватары
отзывов — ayurveda-woman/barista-woman/cook-man вместо несуществующих
portrait,*). Замок kit-акцента переписан осознанно (#15803d → #166534).
**Стенд:** Playwright-скриншоты обоих демо (desktop full-page + mobile +
каталог) — вручную отсмотрены; пойманы и закрыты: гигантский «разрыв»
середины страницы (scroll-reveal не стрелял при мгновенном прыжке скролла
скрипта — снимаем с reducedMotion:'reduce', контент виден сразу), непрогруз
lazy-фото (шагающий скролл перед кадром), SVG-обложка каталога. Грабля-повтор:
двухстрочный `{# #}` в _base.html поймал template_comments. Ограничение,
озвученное владельцу: фото-набор не знает catering-ключей (часть фото
повторяется, «свадебный буфет» = сосиски) — новая AI/CC0 фотосессия по
образцу 2026-07-10 закроет; отдельным решением.

## 2026-08-12 — DS-3/DS-4: «Fokus» = виды вывода per-модуль + композиция + сборки + пилот

Решения владельца (AskUserQuestion): модель «Look = кожа · вид вывода per-модуль ·
в мастере готовые СБОРКИ на архетип» · пилот с прайс-листа каталога · композицию
Fokus-макета в ту же волну. План `docs/ds3-fokus-output-views-plan-2026-08-12.md`;
концепт-артефакт «Look Fokus» одобрен («фокус отлично»).

**DS-3a Прайс-лист:** `SECTION_STYLES["products"] += "preisliste"` (группы по
категориям строками «название — состав … цена», отточие, tabular-nums, PDF-чип;
свой запрос только при выбранном стиле; безкатегорийные — «Weitere» в конце) +
страничный пресет `catalog_layout="preisliste"` ПОВЕРХ листинга UB (фасеты/
поиск/пагинация целы — группируется текущая выдача). Пресет валиден ТОЛЬКО
каталогу: НОВЫЙ `PAGE_EXTRA_PRESETS` + `normalize_layout(extra_presets=…)` —
LAYOUT_PRESETS не раздут, пикеры соседних страниц опцию не видят. Урок: у
Product НЕТ плоского description-акцессора с аргументом в шаблоне — имя `{{ p }}`,
описание `p.description_text` (description_localized — это Combo).

**DS-3b Композиция:** `HERO_STYLES += "split"` (текст слева на чистом поле +
строка доверия ★ Google/Seit под CTA, фото справа из hero_image|первого слайда;
слайдер при split не крутится — v1; plain/accent/слайдер байт-в-байт, замок) ·
`nav.cta` presence-minimal — кнопка primary-действия в шапке (метка/URL из
primary_item; мобильно ВМЕСТО второй липкой кнопки акцентируется совпадающий
пункт таб-бара — вторая кнопка при живом таб-баре = перегруз) · секция
**anfrage** в реестре SECTIONS (выкл дефолт; рендер = AF-2 партиал, гейт jobs
fail-closed; ⚠️ осознанная golden-регенерация: +строка секции, формат indent=1
сохранён — дифф только реальный) · `trust += "compact"` (рейтинг + 2 цитаты +
marks одной полосой). 3 счётчика-замка переписаны осознанно (sections-порядок,
trust=5 видов, SECTION_ICONS+anfrage). Грабля: hero в реестре SECTIONS
ВЫКЛЮЧЕН по умолчанию (включает мастер/кит) — тесты главной обязаны включать
его явно.

**DS-3c Сборки:** реестр `sitetemplates.BUNDLES` (v1: «Fokus» для сервисных
архетипов: klar-кожа + split + nav.cta + preisliste секции/страницы + trust
compact + секция anfrage) + `apply_bundle` (apply_look-база = полная копия
конфига, W6; поверх — таргетные оси; идемпотентно, адверсариальный замок
15 архетипов × сборки). Мастер «Stil»: карточки Startpaket НАД Look-галереей
(выбор сборки снимает Look и наоборот — на POST bundle приоритетнее); билдер:
кнопки сборок в области «✨ Look» (серверный action `use_bundle:` — виды
вывода живут вне draft-канала).

**DS-4 Пилот:** кит catering → Fokus (kit-generic: `enable_anfrage_section`,
`config_patch` — shallow-merge осей после look-оверлея; look="klar",
section_styles += preisliste/compact, hero_widget="" — плитки убраны, их работу
несут CTA шапки + форма на главной). «Fein» остаётся семейством в галерее.
...
## 2026-08-12 — DS-5/5b/6: сеточные контролы Studio + листинг-UX (фидбэк владельца)

**DS-5** (план `ds5-grid-controls-plan-2026-08-12.md`): колонки десктопа до 6;
Anzahl у категорий (GRID_SECTION_LIMITS += categories, дефолт 30 = максимум —
раньше выводились ВСЕ, меньший дефолт молча отрезал бы плитки существующим;
потолок 24→30; home-запрос уважает лимит; ⚠️ третья осознанная golden-реген за
день); layout.balance — flex-wrap с центрированием неполного ряда (sf-bal-N);
layout.scroll — горизонтальная лента со snap (sf-scroll-grid, --sf-tile-w);
классы генерит grid_class_string ЦЕНТРАЛЬНО — работает у всех секций-сеток без
правок шаблонов, scroll побеждает balance, характеризация «без ключей прежняя
строка байт-в-байт»; плитка категорий: img_h (80..480, сильнее аспекта, только
фото-плитки) + tile_info {price,count} («ab X €»/«N Produkte», один агрегат);
Studio: чекбоксы/инпуты + live-draft (JS-коллектор + серверный драфт-путь).
Грабля: правочный python-скрипт напечатал «ok», но НЕ записал файл (нет
open(w)) — кламп колонок остался старым, поймано замком; правило: в скриптах
правок последней строкой всегда write+verify. **DS-5b**: стиль
«Preisliste mit Fotos» (строки + превью 40px; секция И страничный пресет).
**DS-6**: панель фасетов → ЛЕВЫЙ ящик (fixed-оффканвас по прежнему тумблеру,
бекдроп+Esc, стартует закрытым — бейдж активных фильтров на кнопке); глобальный
поиск в шапке (details-дропдаун → ?q= каталога, гейт по модулю); вид «список» —
только мобильный (media-скоуп .is-list + md:hidden кнопки); пресеты cols5/cols6
во всех страничных пикерах. **Демо catering**: 8 направлений (+Frühstück&Brunch,
Desserts&Süßes, 4 товара; фидбэк «хотя бы 4, лучше 8 плиток»); стенд поймал
SVG-фолбэк плитки (ключ фото «tarts» не резолвился → cheesecake). 22 msgid × 5
.po суммарно. Уроки среды: фоновый bash-чейн с kill обрывается (exit 144) — сид
не выполнился молча (проверять результат сида пробой БД/HTTP); runserver из
обычного шелла умирает с ним — гонять как managed background task.

## 2026-08-12 — DS-5c: семейство прайс-видов ×5 + засев кафе/ресторана (запрос владельца)

Владелец: «расширь меню до компактного, двухколоночного и т.д. — как печатные
меню кафе/ресторанов; добавь и засей». Пять видов через ОДИН партиал
`_price_list.html` (pl_style): стандарт · с мини-фото 40px · **kompakt** (без
описаний, плотные строки) · **2sp** (md:columns-2, группы break-inside-avoid) ·
**karte** («печатная карта»: центр-заголовок группы с чертой, описание курсивом
ПОД блюдом). Вызовы — префикс-гейт `style|slice:":10" == "preisliste"`; реестры
SECTION_STYLES["products"]/PAGE_EXTRA_PRESETS ×5; пикер Pages ×5. Засев:
restaurant = karte (главная + /sortiment/), cafe = kompakt (главная) +
foto (/sortiment/); catering остаётся на стандарте. **Ultracode-воркфлоу
(4 ревьюера × скептики, 10 агентов) подтвердил и помог закрыть ДО пуша:**
(HIGH, класс W0) канва-селект каталога в билдере не знал preisliste* → браузер
слал первую опцию «list» и ЛЮБОЙ Save молча откатывал прайс-вид (пре-существовало
с DS-3a, дифф утраивал экспозицию) — селект получил расширенные опции из
SECTION_STYLE_LABELS; (2) `apply_page_payload` драфта валидировал пресет только
по LAYOUT_PRESETS → превью показывало сетку при сохранённом прайсе — паритет
extra-пресетов восстановлен; (3) ruff-format нового теста; (4) heads-up:
прайс-строки не несут quick-add/бейджи/промо-бейдж — на cafe это осознанный
trade-off «печатного меню» (фичи живы на детали и /aktionen/). Замки-регрессии
на обе находки; 3 msgid × 5 .po. Грабля-повтор ×2: стенд --noreload кэширует
шаблоны — рестарт после КАЖДОГО шаблонного батча.

## 2026-08-12 — DS-7: лендинги направлений + тумблер цен в меню (запрос владельца)

«Для каждого направления (еда на свадьбу…) — отдельные страницы: описание,
галерея, примеры блюд БЕЗ цен, отзывы, в стилистике Fokus — целевые лендинги
по категориям» + «настройка показывать цену в меню или нет» + засев catering.
План `ds7-category-landings-plan-2026-08-12.md`. **БЕЗ миграции** —
`Category.description` (i18n, витриной не использовался) и `images` уже были.
**DS-7a:** роут `/bereich/<slug>/` + `category_landing` (двойной гейт класса
ST-8: тумблер `category_landings` presence-minimal И контент категории — 404
иначе; дефолт ВЫКЛ — описания у существующих тенантов не должны внезапно
рождать страницы); Fokus-шаблон: брэдкрамб → split-шапка (eyebrow, описание,
CTA `?betreff=<имя>` + ghost «Menu») → галерея (фото категории, ДОБРАННЫЕ фото
блюд до 6 — у категорий обычно одно фото) → «Beispiele aus der Küche» БЕЗ цен →
2 цитаты → зелёная anfrage-полоса; плитки направлений (компакт DS-4b и обычные)
ведут на лендинг при вкл+контенте, иначе прежний фильтр (fail-safe);
`Category.landing_ready`. **DS-7b:** `menu_show_prices` (ключ только при False)
— прячет цены во всех 5 прайс-видах и «ab-ценах» плиток; **PAngV-гейт**:
работает ТОЛЬКО при неактивном orders (browse-only меню; при продаже цены
обязательны — рендер fail-open, чекбокс в кабинете не показывается). Кабинет:
оба тумблера в блоке каталога «Pages» (сентинел cl_present — W0). **Засев:**
8 направлений catering получили описания (5-й элемент кортежа категории кита) +
`category_landings` в config_patch. **Попутный фикс DS-5b:** у Product НЕТ
`image_url` (есть `primary_image`) — мини-фото в «Preisliste mit Fotos» молча
не рендерились (шаблон глотает атрибуты; Python-вьюха галереи упала честно и
...
## 2026-08-13 — DS-9: Fokus для ВСЕХ архетипов — 14 РАЗНЫХ композиций

Запрос владельца: «создай для остальных архетипов, но не однотипные, а
разнообразные — по одному дизайну на каждый; дальше сделаем дополнительные».
Реестр BUNDLES вырос до 14 записей, каждая — свой РИТМ страницы, а не копия:
catering (прайс+заявка) · hotel (поиск дат в баннере → номера) · restaurant
(печатная карта) · cafe (компакт+фото-прайс) · bakery (направления+фото-прайс) ·
**butcher «Theke»** (направления+прайс+Partyservice-заявка) · **friseur
«Termin»** (топ-услуги с кнопкой записи В баннере + МАСТЕРА остаются — у салона
доверие это люди) · **werkstatt «Ablauf»** (фикс-цены + процесс + смета) ·
**handwerker «Referenz»** (работы до/после → timeline процесса → заявка; прайса
нет — цена индивидуальна) · **grocery «Angebote»** (скидки ПЕРВЫМИ) · **retail/
online_shop/other «Sortiment»** (плитки+карточки+полоса преимуществ) ·
**clothing «Lookbook»** (overlay-карточки, вертикальные плитки, галерея
образов) · **events «Programm»** (афиша+ход) · **tour_operator «Touren»**
(даты+фото маршрутов). Оси сборки: += `card_style`, `nav_style` (шапка одной
строкой — правится И легаси `nav`, И дерево `menus.top`, иначе Fokus оставался
двухэтажным) и `hero_widget: "none"` (явное СНЯТИЕ плиток задач — Fokus ведёт
к одному действию; отель/friseur/werkstatt переопределяют функциональным
виджетом). **Единый источник правды:** `apply_bundle_config(config, key)`
выделен из `apply_bundle`, новое поле `DemoKit.bundle` — демо получает ровно
ту композицию, что владелец жмёт кнопкой «Startpaket» (вместо девяти копий
полей в китах). **Разнообразие «кожи»:** Look'и раздали по характеру —
clothing=nacht (свой, сохранён), friseur=warm (свой), butcher/handwerker=warm,
tours/retreat=natur, остальные klar. **Стенд поймал брак ДЕМО-НАБОРА:**
`linen-shirt.webp` содержит ВЫПЕЧКУ (ошибка AI-набора 2026-07-10) — плитка
«Herren» показывала пирожное; заменено на `sweater`, пометка в SOURCES.md.
Плитки направлений clothing/grocery получили реальные фото (были SVG-фолбэки).
Осознанно обновлены замки: акцент handwerker (klar→warm) и состав его главной.

## 2026-08-13 — DS-10: единая строка инструментов листинга; поиск — только в шапке

Запрос владельца: «фильтр на одной строчке с сортировкой и видом отображения;
поиск убрать — он у нас в шапке». Каркас `listing.html`: блок `listing_toolbar`
теперь рисует ОДНУ строку `[Фильтры · Сортировка · Вид]` (маркеры
`data-listing-bar` … `<!--/listing-bar-->`) с двумя слотами —
`listing_bar_filters` (кнопка `_filter_toggle`, только там, где есть панель) и
`listing_bar_view` (переключатель вида; include остался в шаблоне СВОЕЙ сетки —
файловый замок `test_grid_has_matching_view_switcher` цел). Поле поиска и
кнопка «Search» удалены; `?q=` живёт как hidden-carry, чтобы смена сортировки
не сбрасывала запрос.

**Разведка (3 агента) и адверсариальное ревью (4 измерения + скептики, 23
агента) дали 18 подтверждённых находок — все закрыты ДО пуша:**
- (HIGH) поиск в шапке был ЖЁСТКО привязан к каталогу → у отеля/салона/
  организатора он уводил в пустой каталог, а своего поиска на листинге больше
  нет. Теперь 🔍 ведёт в листинг ГЛАВНОГО модуля архетипа
  (`_primary_search_module` + `_SEARCH_ROUTES` в `core/context.py`), фолбэк —
  каталог; замок на все четыре архетипа;
- (HIGH) строка гейтилась `show_listing_toolbar` (= items or q) — при нулевой
  выдаче исчезала кнопка «Фильтры», снять фильтр было НЕЧЕМ. Гейт теперь только
  у формы сортировки;
- (HIGH) `{% block listing_view %}` → `listing_bar_view`: Django МОЛЧА
  отбрасывает блок, которого нет у родителя (проверено мутацией — переключатель
  исчезал без ошибки) → замок на все 4 шаблона;
- панель фасетов отдавалась БЕЗ `hidden`, а CSS DS-6 делает видимую панель
  fixed-ящиком → вспышка открытого ящика до init() и «навсегда открыт» без JS:
  панель рендерится скрытой + `<noscript>`-правило возвращает её в поток;
- без JS сортировка стала неотправляемой (submit-кнопка ушла с поиском) →
  `<noscript>`-кнопка «Apply»;
- в прайс-виде каталога `data-grid` нет вовсе — кнопки вида были мертвы → гейт;
- кнопка-сирота (ключ без панели) плодила бекдроп и лочила скролл → ранний
  выход + скрытие кнопки;
- вёрстка: одинаковая высота h-9 у всех трёх контролов, `ml-auto` у группы
  вида (раньше выравнивание зависело от наличия сортировки), кламп ширины
  селекта на телефоне (иначе строка переносилась), `:not(:has(> *))` убирает
  фантомный отступ пустой строки, тёмная тема получила рамки `border-gray-300`
  и читаемую активную кнопку вида;
- переключатель вида скрыт на десктопе (`md:hidden`): режим «список»
  заскоуплен в CSS до 767px (решение DS-6), на широком экране оставалась одна
  вечно активная кнопка;
- замки первой версии были ВАКУУМНЫ (срез по несуществующим маркерам, `in html`
  ловил инлайн-JS `_grid_view_script`) → переписаны на строгий срез строки +
  count==1 на каждый инструмент.
Демо: плитки направлений shop получили реальные фото (были SVG-фолбэки).

## 2026-08-13 — MEN-1+MEN-2: «Menü-Sets» — радио-фикс конфигуратора + модель набора (⚠️ catalog/0025)

Волна MEN «наборы меню с выбором» (план `docs/menu-sets-plan-2026-08-13.md`;
решения владельца: «Combo, не перемудрить», агрегатор 4/4 — карточка на каждый
набор, ab €/Gast + мин. заказ, блюда в поиске). Разведка (воркфлоу 13 агентов)
подтвердила: НОВЫЕ сущности не нужны — Combo/ComboGroup/ComboOption уже несут
«комплект с выбором», серверная цена и снимок состава существуют.

**MEN-1 (без миграций):** найден и исправлен реальный баг — radio ВСЕХ групп
конфигуратора делили `name="opt"`, браузер схлопывал их в один выбор → набор
с двумя одиночными группами («закуска + горячее») был физически несобираем.
Инпуты теперь `opt-<g.pk>`; `combo_add` собирает `opt` + все `opt-*` (голый
`opt` принимается — бэк-совместимость). Live-итог из `data-delta` — только
показ, цена по-прежнему серверная (`combo_price`). Замки: имена per-группа,
выбор из обеих групп доезжает до корзины/цены (22,50 = 20 + 2,50).

**MEN-2 (⚠️ миграция `catalog/0025`, аддитивная):** Combo += `images` (галерея,
FileRef как у Product) · `category` (FK направления, SET_NULL) ·
`price_per_person` · `min_persons` · `event_types` (словарь общий с AF-1) ·
`free_pool` (режим «свободная сборка», витрина — MEN-4); ComboGroup +=
`included` («Im Set enthalten»: показывается плитками, в валидации/цене НЕ
участвует — id опций из POST не прибавляют надбавку, замок); Product +=
`course` (Gang; реестр `food.COURSES` 7 кодов, lazy). `combos.combo_price_from`
— честная «ab»-цена (фикс + min_select наименьших надбавок обязательных групп).
Кабинет: фото-плитка «＋ Foto» + ★/✕ (паттерн FB-6, эндпоинты
`combo-image-delete/primary`), блок «Menü-Set» в форме набора (категория/
per-person/минимум/поводы-datalist из anfrage/свободная сборка), чекбокс
«Im Set» на группах, селект «Gang» в табе Kennzeichnung формы товара
(ChoiceField валидирует код). 23 msgid × 5 .po (плоский аппенд — polib
переформатировал бы весь файл, диф 3,7k строк; откачено, урок). Замки: 6 новых
в test_combos + course-валидация; 42 зелёных на свежей БД.

## 2026-08-13 — MEN-3: витрина наборов — плитки блюд, попап, per-person, browse-гейт (без миграций)

Деталь набора пересобрана: галерея `_media_gallery` (фото MEN-2), состав
МИНИ-ПЛИТКАМИ (`_combo_groups.html`: фото → generic-попап `#quick-modal`
`/gericht/<pk>/info/` — крупное фото + описание + диеты/аллергены/Zusatzstoffe;
строка с инпутом — выбор, фото клик НЕ переключает инпут), included-группы —
фикс-состав без контролов. Per-person: qty=«Personen», min/value=min_persons,
«ab N Personen», кап qty поднят 50→1000 ТОЛЬКО для per-person. **Гейты:**
`/kombi/*` и попап видны по каталогу (core-модуль → всем; browse-only кейтеринг
видит наборы как Speisekarte; модульный гейт пункта меню снят — держит
контент-гейт ST-8), «в корзину» остаётся за orders; browse-only+jobs → CTA
«Unverbindlich anfragen» с префиллом набора (класс GK-14). **Предохранители
плана §2:** чекаут ревалидирует состав (умершая опция больше НЕ уходит молча
дешевле — редирект с ошибкой) и min_persons (сессию не накрутить); combo_add —
min_persons + per-person-кап. Адаптер `_combo`: фото/галерея, честная «ab»-цена
(combo_price_from), суффикс «/ Person»; несохранённость — `_state.adding`
(UUID-pk назначается в __init__ — гард по pk ходил бы в БД). Цены прячутся
`menu_show_prices=False` (DS-7) целиком: шапка/надбавки/итог. 5 msgid × 5 .po.
Замки: `test_menu_storefront.py` (10) + адаптер-тест (старый лок несохранённого
Combo остался прежним поведением байт-в-байт).

## 2026-08-13 — MEN-4: «свободная сборка» (free_pool) + Menü-Pakete на лендинге + карточки /kombi/ (без миграций)

**Режим 3 целиком.** Деталь free_pool-набора: пул = активные блюда категории,
сгруппированы по Gang'ам (порядок реестра COURSES, без Gang'а → «Weitere»),
чекбокс `name="dish"` + à la carte цена в data-delta (live-итог тем же скриптом
MEN-1), фото → попап блюда. Сервер: `combos.validate_pool` — СТРОГАЯ валидация
(чужой/протухший id = ошибка, не молчаливый дроп; минимум 1 блюдо) +
`pool_price` = базовая цена набора (обычно 0) + Σ base_price — «свободное ≥
наборного» держится ценами блюд без спец-логики. Корзина: ключ `cid|d:p1,p2`
(UUID-опции с "d:" не коллидируют), `_pool_items` держит ИСХОДНЫЙ ключ (✕
попадает и по неполной строке), строки корзины унифицированы полем `labels`.
Чекаут: умершее блюдо → честный редирект пересобрать; заказ идёт
`create_order(custom_lines=)` — цена заморожена сервером, состав снимком в
`OrderItem.modifiers` (item.combo/product = None — кастомная строка). Лендинг
DS-7: блок «Menü-Pakete» (карточки наборов категории: фото/бейдж режима или
...
## 2026-08-13 — MEN-7: выбор состава на browse-only витрине (находка стенда; без миграций)

Playwright-стенд по демо catering (27 проверок) вскрыл дефект, невидимый серверным
тестам: у browse-only витрины (кейтеринг — orders выключен) конфигуратор рендерился
с `show_inputs=False`, то есть **режим «выбрать блюда и увидеть цену» был мёртв
именно там, где он нужен больше всего**. Фикс: browse-ветка получает ТЕ ЖЕ контролы
в обёртке `<div data-combo-form>` (не форма — POST-приёмник за orders, как и был) +
поле «Personen» с минимумом набора; скрипт вынесен из ветки корзины и общий для
обеих: считает живой итог И дописывает выбранный состав с числом персон в ссылку
заявки (`?betreff=`, приёмник AF-1 — новых анонимных эндпоинтов не заводим). Без JS
уезжает только название набора (поведение MEN-3 байт-в-байт).

Стенд подтвердил на живом демо: попап блюда (фото/описание/диеты) открывается и
закрывается, пересчёт 58,00 × 20 персон = 1.160,00 и × 30 = 1.740,00, свободная
сборка группируется по Gang'ам и НЕ предлагает пакеты-буфеты, состав уезжает в
заявку, лендинг направления показывает «Menü-Pakete» с «ab 20 Personen».
Грабли: шаблоны кэшируются и в DEBUG (рестарт runserver); Playwright шлёт
Accept-Language en-US — витрина честно отдавала EN (заодно подтвердило переводы
блюд MEN-6), для проверки немецких строк нужен `locale: 'de-DE'`.

## 2026-08-13 — MEN-8 (строгие диеты) + MEN-9 «корзина = запрос на просчёт» + MEN-10 (демо кейтеринга)

**MEN-8 (решение владельца «строгий набор»).** Чип диеты в агрегаторе теперь
ПЕРЕСЕЧЕНИЕ по составу, а не объединение: «vegan» стоит лишь у набора, где
веганское КАЖДОЕ блюдо — гость по фильтру не получит меню с мясным горячим.
У свободной сборки пересечение считается по пулу (гость волен взять любое
блюдо → обещать диету можно, только если её держит весь пул). Пустой состав →
чипов нет (`diets=None` ≠ `set()`). 2 замка + переписан ассерт смешанного набора.

**MEN-9 (запрос владельца «продавать сразу не нужно, но через корзину прогонять
как заказ на просчёт»).** Разведка показала: готового режима НЕ существовало, а
включение orders «как есть» дало бы §312j-кнопку «Zahlungspflichtig bestellen» —
то есть юридическую покупку. Сделан честный режим: ключ `site_config["quote_cart"]`
(presence-minimal → golden целы). При нём корзина и чекаут работают как обычно, но:
кнопка «Unverbindlich anfragen» + подпись «keine Zahlungspflicht» вместо §312j
(замок паритета: у обычного магазина кнопка прежняя), пикер оплаты не рендерится,
`payment_method=on_site`, `source_channel="quote"` → `Order.is_quote`; письмо
клиенту ремапится на `order_quote_created` (по прецеденту Anprobe — «KEIN
Kaufvertrag, wir melden uns mit einem Angebot»), подтверждение говорит «Ihre
Anfrage ist da» вместо «Your order is in»; AGB-заготовка кита в этом режиме
описывает реальный флоу (запрос → предложение → акцепт), иначе §2 врал бы.
7 замков + 8 msgid × 5 .po.

**MEN-10.** Кит catering: `orders` включён + `quote_cart`; работают ОБА пути —
корзина-просчёт (в т.ч. наборы: выбор состава уезжает позицией заказа) и форма
заявки AF-1. Стенд Playwright 18/18 сквозняком: набор с выбором → пересчёт
58,00 × 20 = 1.160,00 → корзина → «Unverbindlich anfragen» → подтверждение
языком заявки. Грабля стенда: HTML5-валидация не пускает набор в корзину, пока
не заполнена КАЖДАЯ обязательная группа (это правильное поведение, не дефект).

## 2026-08-13 — MEN-11 (заявка в попапе) + MEN-12 (отложенное и сравнение в агрегаторе)

Уточнение владельца: «Корзина не обязательна, можно сделать просто запрос как
сейчас. Но не уводить на дополнительную страницу, а сразу в попапе заполнять.
Но для агрегатора нужно что-то наподобие избранного отложенного или корзина,
где потом можно сравнить варианты».

**MEN-11 (без миграций).** Кит catering возвращён на browse-only «просто запрос»
(orders и quote_cart из кита сняты; сам режим `quote_cart` остаётся в коде
ОПЦИЕЙ для тех, кому корзина нужна — MEN-9 не выброшен). CTA «Unverbindlich
anfragen» на детали набора и в `_buybox` (GK-14) больше НЕ уводит на `/anfrage/`:
новый GET-фрагмент `/anfrage/formular/` (`anfrage_modal`) отдаёт ТУ ЖЕ форму
(`_anfrage_form.html`) в обёртке generic-модалки `#quick-modal`. Приёмник,
honeypot и rate-limit остаются штатными (форма постит в `/anfrage/`) — новых
анонимных эндпоинтов нет. Скрипт MEN-7 теперь дописывает состав и персон в
`data-quick-url`, а не в href, так что попап открывается уже с готовой темой.
4 замка + переписан CTA-замок MEN-3.

**MEN-12 (без миграций).** `apps/aggregator/shortlist.py` — отложенные
предложения БЕЗ аккаунта (сессия, по образцу Merkzettel M4-C: 0 миграций,
DSGVO-чисто); портальное `FavoriteListing` под логином не тронуто, кнопки не
дублируются (`shortlist_enabled` False на порталах). Кнопка «★/☆» на карточках
`_cards.html`, счётчик-вход в шапке (только когда что-то отложено), страница
`/entdecken/merkliste/` — ТАБЛИЦА СРАВНЕНИЯ «характеристика × вариант»
(поставщик/город/цена ab €/Gast/мин. заказ/поводы/диеты/рейтинг); ряды, пустые
у всех вариантов, не рендерятся. `next` в toggle принимается только
относительный (чужой хост → редирект на индекс). 6 замков.

Стенд Playwright 17/17 на живом демо: попап открывается без смены URL, тема
предзаполнена («… — 20 Personen: Vorspeise …»), заявка уходит; в агрегаторе
две карточки откладываются, шапка считает, таблица сравнивает, вариант
убирается. Грабля стенда: в dev-БД не было Domain для public-схемы — `/entdecken/`
отдавал 404, пока не завёл `siteadaptor.de → public`.

**Дефект, найденный гейтом (регресс-класс):** контекст-процессор падал
`AttributeError` на запросе БЕЗ сессии (RequestFactory во вьюх-тестах,
кэш-прогрев) — ложились ВСЕ страницы города. Fail-safe: нет сессии → пустое
состояние. Проверено также, что кэш публичных страниц (`cache_public_page`)
обходит запросы с непустой сессией — значит кэшированная выдача физически не
может унести чужие ★ (утечки состояния между посетителями нет); замок на оба
случая + на скоуп процессора (вне /entdecken/ молчит).

**MEN-11 доводка (красный CI 15a7cb6e → фикс).** CI поймал два УСТАРЕВШИХ замка:
`test_service_detail_buybox_exact_ctas` и `test_orders_module_off_hides_buybox_entirely`
ждали `href="/anfrage/?betreff="`, тогда как CTA стал кнопкой-попапом. Переписаны
осознанно. **Урок процесса:** локальный гейт гонялся по aggregator/jobs/orders, а
правка `_buybox.html` задевает booking и catalog — при изменении ОБЩИХ партиалов
гейт обязан включать всех потребителей (`grep -l` по include). Заодно устранена
непоследовательность самой MEN-11: попап получили только 2 CTA из 6 — остальные
(корпоративный запрос события, лендинг направления, запасной CTA buy-box, страница
слотов) оставались ссылками на отдельную страницу; теперь все шесть открывают
форму попапом. 791 тест по booking/catalog/orders/jobs/events зелёный.

## 2026-08-13 — MEN-13: наборы меню были НЕДОСТИЖИМЫ из навигации демо (фидбэк владельца)

Владелец после деплоя: «где посмотреть варианты блюд в комплекте в кейтеринге».
Диагноз: наборы засеяны и страницы работают, но ни в шапке catering, ни в шапке
restaurant пункта на `/kombi/` НЕ БЫЛО — контент есть, пути к нему нет (тот же
класс, что «наполненный модуль без пункта меню», ST-8). Добавлен пункт «Menüs»
обоим китам + ИНВАРИАНТ `test_kits_with_menu_sets_link_them_in_navigation`:
кит с `combos` обязан иметь пункт на них в меню (иначе фича невидима). Инвариант
сразу поймал второй кит — restaurant.

## 2026-08-13 — MEN-14: виды вывода товара (фотосписок в 2 и 3 колонки)

Запрос владельца: «сделай разные виды вывода товара — список, список с
картинками, список с картинками в 2/3 колонки, плитки по 3–6 колонок, на
мобильном максимум 2 колонки». Ревизия показала, что четыре из пяти видов уже
были: `preisliste` (список), `preisliste_foto` (список с фото), `preisliste_2sp`
(две колонки БЕЗ фото), плитки `cols3…cols6` (DS-5/DS-6), а мобильный кап
architecturally гарантирован (`_clamp(mobile, 1, 2)` + таблица `_GRID_MOBILE`
знает только 1 и 2 колонки).

Дозакрыт ровно недостающий класс — **фотосписок в колонки**:
`preisliste_foto_2sp` / `preisliste_foto_3sp` в `PAGE_EXTRA_PRESETS["catalog_layout"]`
+ `SECTION_STYLES["products"]` + метки → пикеры (страничный селект каталога в
Studio и стиль секции главной) получают опции из реестров, отдельной правки UI не
потребовалось.

**Почему grid, а не CSS-columns.** Двухколоночный `preisliste_2sp` сделан на
`columns-2` — для строк без фото это правильно (группы не рвутся, `break-inside-avoid`).
Со строкой «фото + название + цена» тот же приём рвал бы карточку между колонками
на разрыве потока, поэтому у фото-видов обёртка — `grid md:grid-cols-2` (третий
вид добавляет `xl:grid-cols-3`). Сетка включается только с `md`: на телефоне
такая строка в две колонки нечитаема.

Замки: параметризация всех семи прайс-видов (нормализация + рендер + страничный
пресет), фото не теряются в колоночном виде, и отдельный инвариант
`test_mobile_never_exceeds_two_columns` — требование владельца про мобильный
проверяется на ЛЮБОМ пресете, включая ручной override `mobile: 4`. Осознанно
обновлены два замка семейства прайс-видов (кортеж пресетов и параметризация).
2 msgid × 5 локалей. Миграций нет.

## 2026-08-13 — MEN-16: «меню-книга» (разворот в две колонки с перелистыванием)

Запрос владельца: «было бы классно визуализировать это меню как меню-книгу в
2 столбца с листанием». Сделано ещё одним прайс-видом `preisliste_buch` поверх
существующего `_price_list.html` — отдельной страницы/движка не заводили:
разворот = та же «печатная карта» (`preisliste_karte`), но **страница = группа
блюд**, на md+ видны две страницы рядом, разделённые пунктирным «корешком».

Листание — кнопки ‹ ›, свайп по развороту (порог 60px, чтобы вертикальная
прокрутка не листала) и живой счётчик «Seite 1–2 von 6». **Прогрессивное
улучшение:** в разметке скрыта только навигация, страницы видны все — без JS
меню читается целиком, а скрипт показывает навигацию и берёт листание на себя.
На телефоне разворот вырождается в одну страницу — то же правило мобильного
капа, что у сеток.

Скрипт вынесен в `_price_book_script.html` и подключается ТОЛЬКО этим видом
(на прочих страницах его нет), гард `__sfPriceBookBound` — вид может стоять
одновременно в секции главной и на странице каталога.

Одна несимметричность осознанная: обёртка страниц открывается и закрывается в
разных `{% if %}`-ветках — так один цикл групп служит всем семи видам; дублировать
цикл ради книги дороже. Замки: страницы/навигация в разметке, отсутствие
`hidden` на самих страницах (иначе без JS меню исчезло бы), скрипт не течёт в
другие виды. Миграций нет; 5 msgid × 5 локалей.

## 2026-08-13 — MEN-17: попап блюда крупнее, блюда компактнее (фидбэк владельца)

«Сделай окно просмотра блюда в попапе чуть больше, в блюдах может быть
множественное; немного меньше иконки, а то портянка получается при большом
количестве блюд».

**Попап** (`_dish_info.html`): `max-w-md` → `max-w-3xl`, на sm+ фото слева и
текст справа; у блюда с НЕСКОЛЬКИМИ фото — лента миниатюр, клик подменяет
главное фото. Обработчик подмены живёт в `_base.html` рядом с обработчиком
модалки, а не в самом попапе: разметку кладёт `innerHTML`, а он не исполняет
вложенные `<script>` (грабля, уже стоившая сессии на M22b).

**Блюда** (новый общий `_dish_chip.html`): вместо плитки с фото 4:3 на всю
ширину ячейки — компактная строка «мини-фото 56px + название + цена + контрол»,
сетка `sm:2 / xl:3` колонки. Партиал общий для состава набора и свободной
сборки: вид блюда правится в одном месте, обе поверхности не разъезжаются.
Инвариант MEN-3 сохранён — фото остаётся кнопкой ВНЕ `label`, поэтому клик по
фото открывает попап и не переключает выбор.

**Длинные группы**: видны первые 12 блюд, хвост — в `<details>` («Weitere N
anzeigen»). Все блюда остаются в DOM, поэтому выбор и отправка формы не
страдают (замок считает `data-dish-tile` — их 15 из 15 при пороге 12). Больше
всего это лечит свободную сборку: она выводит весь каталог категории.

**Доводка по скриншотам стенда (та же дата).** Стенд Playwright прошёл 14/14, но
картинки показали два дефекта, которых замки не видят:

1. **Кривой разворот книги.** Страница = группа → слева два блюда, справа
   четырнадцать. Добавлен тег `book_pages` (группа режется на страницы по 8
   позиций, продолжение помечается «Fortsetzung»); форма элемента прежняя
   `{name, items}`, поэтому цикл рендера остался один на все семь видов.
2. **Обрезанные названия блюд.** Сетка чипов была `sm:2 / xl:3` — а брейкпоинты
   Tailwind считают ШИРИНУ ЭКРАНА, тогда как блюда живут в узкой правой колонке
   детали: на широком мониторе выходило три колонки по ~120px и «Kürbiscremes
   uppe». Заменено на container-adaptive `grid-cols-[repeat(auto-fill,minmax(220px,1fr))]`
   — число колонок теперь диктует доступная ширина, а не окно. Плюс `break-words`
   на названии (длинное слово раньше уезжало под `overflow-hidden`).

Демо: у ресторана «Bella Vista» на главной остаётся «печатная карта» тизером, а
полная Speisekarte переведена на книгу — оба вида семейства видны в одном демо.

## 2026-08-13 — MEN-15: категории вторым уровнем меню, с картинками

Запрос владельца: «добавить в меню сайта сверху настройку выводить категории —
во 2-й уровень меню с картинками, и красиво настроить управление». План —
`docs/men15-category-menu-plan-2026-08-13.md` (разведка по коду, развилка A/B,
риски). Реализован вариант A: **список не ведётся руками**.

Новый тип узла `categories` (обычный узел меню, БЕЗ новых ключей — golden-фикстуры
не тронуты, миграций нет): `target` пуст → корневые категории, иначе подкатегории
названной. Резолвер `_category_children` строит детей ОДНИМ запросом и мемоизирует
результат на объекте тенанта — меню резолвится на каждый рендер (шапка, нижнее
меню, контекст кабинета), и без этого 12 категорий стоили бы 12 запросов на
странице (замок `assertNumQueries`-стиля: 1 запрос, второй вызов — 0).

Ссылка ребёнка совпадает с плиткой категории (`_category_tile.html`): лендинг
направления при включённом тумблере `category_landings` и готовом контенте, иначе
фильтр каталога. До этого меню всегда вело на фильтр, а плитка на главной — на
лендинг; замок фиксирует паритет.

Рендер: панель шапки становится сеткой плиток, только если у детей ЕСТЬ фото
(`has_images` считается в резолвере) — иначе прежний текстовый столбец, потому что
у большинства тенантов `Category.images` пуст. В overflow-панели «Mehr» и в
мобильном меню — мини-фото 32px строкой.

Кабинет: тип «Kategorien (automatisch)» в редакторе меню + селект «Alle
Hauptkategorien / конкретная категория», кнопка «+ Categories submenu» одним
кликом и ссылка на управление фото категорий. Попутно исправлено: селект целей
выводил СЫРОЙ JSONField (`{'de': 'Buffets'}`) вместо локализованного имени.
Демо: у кейтеринга пункт «Speisekarte» переведён на новый тип — ширина строки
шапки не изменилась, а путь к направлениям с фото появился.

**Найден и исправлен пре-существующий баг витрины (важнее самой фичи).** Стенд
показал, что подменю не открывается — и «Über uns» тоже, то есть НИ ОДНО
выпадающее меню шапки на десктопе никогда не работало: строка `#sf-nav-desktop`
несёт `overflow-hidden` (страховка от расползания пунктов до отработки fitNav), а
панель позиционирована `top-full`, то есть целиком ВНЕ строки — и клипалась.
Тесты этого не видели: Playwright `:visible` и серверный рендер не знают про
обрезку родителем. Клип теперь снимается на `hover`/`focus-within` — ровно тогда,
когда панель и открывается; без JS страховка сохраняется. Замок — в `test_nav`.
## 2026-08-13 — аудит переводов ДЕМО-витрин + доперевод en/ru/uk/tr (без миграций)

Запрос владельца: «анализ демо-сайтов и процент перевода на основные языки; параллельно
идёт разработка — не помешать деплою; после анализа выполнить перевод». Разбор целиком —
`docs/demo-i18n-audit-2026-08-13.md`, инструмент повторного замера —
**`scripts/demo_i18n_gap.py`** (`uv run python scripts/demo_i18n_gap.py [--missing ru]`).

**Метод.** Демо переводится не gettext'ом, а двумя обходами `demo_i18n` (site_config-оверлей
+ модельные поля) по словарям `demo_i18n_<loc>.json` с ТОЧНЫМ совпадением строки. Скрипт
раскладывает немецкие строки 15 китов на ROUTED (путь перевода есть) / GAP (пути нет) /
не требует перевода (имена собственные, коды размеров) — иначе «процент» врёт в обе стороны.

**Было:** 1299 строк с путём перевода — en 77.8 %, ru 81.3 %, uk 81.3 %, tr 79.7 %; ещё
319 строк без пути вообще. Худшее: акции демо 25 %, кит `catering` 42 %, метки вариантов
en 61 %. Подписи меню (59) — 100 % (идут через gettext-якоря `menu.py`, ложная тревога).

**Стало:** 1359 строк — **en 96.2 %, ru 99.9 %, uk 99.8 %, tr 98.1 %**; остаток —
Espresso/Tiramisu/S-M-L/названия заведений, т.е. то, что переводить нельзя (замок
`test_demo_i18n_maps_have_no_identity_entries` запрещает identity-записи, поэтому такие
строки в словарь не пишутся вовсе).

**Три класса дефектов, а не один** (важный вывод аудита):
* **A — нет записи в словаре**: ~330 строк → ~1150 новых записей в 4 словаря. Валидация
  слияния: ключ байт-в-байт из кита, перевод ≠ оригинала, деньги/проценты сохранены.
  Две ловушки валидатора, которые едва не забраковали ПРАВИЛЬНЫЕ переводы: en переводит
  «ab 17 Uhr» → «from 5 pm» (24ч→12ч — числа законно меняются), tr пишет процент ПЕРЕД
  числом («%25»). Проверять надо деньги/проценты по значению, а не «все числа подряд».
* **B — нет пути перевода** (запись в словаре бесполезна): `anfrage`/`before_after` вне
  `_TRANSLATABLE_CONFIG_KEYS`; C-блоки главной (`sections`) и страниц (`page_blocks`)
  не обходились вовсе; `Category.description` (лендинг `/bereich/<slug>/`, DS-7a) —
  поле i18n есть, обхода не было; `siteui.page_blocks` читал блоки из ГОЛОГО normalize.
  Всё закрыто без миграций. Для C-блоков — точечный обход `_tr_cblock_data` (только
  текстовые поля данных; служебные токены не трогаем) и **индексы из НОРМАЛИЗОВАННОГО
  списка**: оверлей мерджится позиционно, а `normalize_sections` дописывает недостающие
  фикс-секции в конец → иначе перевод сел бы на чужой блок (замок).
* **C — перевод есть, но шаблон печатает базовое поле**: `Combo` (все поверхности),
  имя услуги в buy-box, «похожие номера», подтверждения/отмены броней/билетов/абонементов.
  Переведены на существующие `*_localized`. (Combo параллельно переписала MEN-волна —
  при ребейзе взята её версия, она уже локализованная.)

**Виды мероприятия формы заявки** — отдельный тег `siteui::anfrage_event_choices`:
`<option value="Hochzeit">Свадьба</option>`, т.е. **value остаётся немецким** (вьюха
валидирует его fail-closed и пишет в `Job.event_type` — запись бизнеса живёт на базовом
языке, как снимок заказа в I18N-10), переводится только показ.

**Стенд** (`seed_demo_tenants --kit catering --recreate`, dev-БД, test client): на ru
кириллица 74–85 % на всех страницах; детектор «немецких остатков» на главной оставил 13
латинских слов, и все, кроме `seit 2012` (исправлено — `stats.value` бывает текстом) и
`service_area_note` (нужна миграция), — имена собственные. **Грабля среды:** локально нет
gettext, `.mo` не собраны → стенд показывает немецкий ХРОМ; до `polib`-компиляции это
ложная тревога, а не дефект перевода.

**Осталось за решением владельца** (нужны МИГРАЦИИ — намеренно не делались, чтобы не
конфликтовать с параллельной веткой): ~287 строк у моделей без `*_i18n` — ретрит-лендинг
`Event.details` (~110), отзывы демо (77), заявки/сметы `Job` (46), блог, лояльность,
Extras/ваучеры/абонементы/`ComboGroup.label`, `Tenant.opening_hours_text`/`service_area_note`,
`Teacher`/`Resource`. Попутно найдено вне демо: **de.po** — 137 английских msgid без
немецкого перевода (~36 реальной прозы UI, 32 из них переведены в ru/tr/uk → на НЕМЕЦКОЙ
витрине местами английский); **SEO-мета** (`core/seo_meta.py` DEFAULTS) — немецкие литералы
без gettext, т.е. `<title>`/`description` немецкие на любой локали.

⚠️ ops: миграций нет, но **после деплоя нужен `seed_demo_tenants --recreate`** — перевод
накладывается при сидинге, а не на рендере.

## 2026-08-13 (продолжение) — I18N-12: последние демо-сущности, de.po и SEO-мета

Отмашка владельца после аудита I18N-11: «делай всё и вливай потом с учётом доработок».
Закрыт остаток класса B (контент вообще без i18n-полей) + две находки вне демо.

**Девять аддитивных миграций** (только новые JSONField-оверлеи; существующие колонки
не тронуты): `events/0023` (Event.details/program/questions, Teacher, BlogPost) ·
`booking/0022` (Resource.name/title/bio, PassPlan) · `core/0007` (Extra) ·
`loyalty/0005` (LoyaltyProgram ×2, Voucher) · `catalog/0026` (ComboGroup) ·
`reviews/0004` (Review.comment) · `aggregator/0018` (BusinessReview.comment, SHARED) ·
`jobs/0014` (Job ×2, JobLine) · `tenants/0031` (opening_hours, service_area_note, SHARED).

**Новый слой `apps/core/i18n_json.py`** — третий в семействе: `I18nMixin.get_overlay`
для скаляров, `core.i18n_seq` для плоских списков, теперь `overlay_json` для
ВЛОЖЕННОГО JSON (ретрит-лендинг `Event.details`: скаляры + списки строк + списки
словарей). Инварианты те же: база задаёт форму (перевод не «дорисовывает» блок),
пустой перевод = фолбэк, фото/ссылки/рейтинги не трогаем (`SKIP_KEYS`), списки
мерджатся ПОЗИЦИОННО → оверлей строится по нормализованной форме (`Event.landing`),
иначе перевод сел бы на чужой блок (тот же урок, что с C-блоками в I18N-11).

**Инвариант волны — переводится ПОКАЗ, а не запись.** Отзывы и заявки суть слова
клиента: оверлей заполняет только демо-сидер, у живого тенанта он пуст и витрина
отдаёт подлинный текст. Ключ ответа анкеты (`Ticket.answers`) остаётся БАЗОВЫМ
немецким вопросом — переводится лишь подпись (`questions_localized`), как `value`
у видов мероприятия.

**Перевод:** 241 строка × 4 локали (67 отзывов демо, 45 позиций заявок/смет, 30 строк
ретрит-лендинга, 14 строк часов работы, 12 ресурсов/мастеров, 9 записей блога,
кастом-статусы, зона выезда, шаги наборов). Замер после: **строк «без пути перевода» —
0** (на старте аудита было 319); en 96.6 %, ru 99.7 %, uk 99.7 %, tr 98.2 %; остаток —
непереводимое (Espresso/Tiramisu/S-M-L/названия заведений). Автоматическая QA новых
записей (идентичность, кириллица, немецкие слова-протечки, длина, парность кавычек,
ru≠uk) — 0 проблем.

**de.po (немецкая витрина показывала английский):** 20 английских msgid без немецкого
перевода — «Blocked», «All collections», «Products in this collection», «Check-in is
not possible on this weekday …», подсказка перетаскивания брони в шахматке. Переведены
точечной правкой (форматирование каталога не тронуто, diff 20 строк). Три теста,
ассертивших английскую строку в DE-рендере, переведены на немецкий ассерт. Остальные
~90 «непереведённых» английских msgid — слова, одинаковые в немецком (Team/Status/
Hotel/Newsletter/Dashboard): identity корректна.

**SEO-мета:** `core/seo_meta.py::DEFAULTS` — немецкие литералы без gettext, поэтому
`<title>`/`meta description` ВСЕХ страниц были немецкими на любой локали. Обёрнуты в
lazy; 4 msgid × 5 каталогов, плейсхолдеры сохранены в каждом переводе; `render_template`
приводит lazy к строке. Шаблоны владельца из кабинета остаются его текстом.

⚠️ ops: **миграции требуют деплоя** (`./scripts/deploy.sh single`), затем
`seed_demo_tenants --recreate` — перевод накладывается при сидинге.

- **2026-08-13 · MT-1a «Тур-продукт + маршрут с видимостью» (архетип мото/квадро-туров).**
  Первый инкремент семейства MT (план `moto-tours-archetype-plan-2026-08-04.md`, решения владельца §0b).
  Закрывает давний гэп **T6** («продукт → много дат»): раньше каждый заезд был отдельным `Event`
  с продублированным описанием.
  * **Модель `events.Tour`** (⚠️ миграция `events/0023`, аддитивная): контент (title/summary/
    description + i18n-**оверлеи** Волны L — НЕ старая схема Event, этого ждут адаптеры единого
    слоя), `images`, `details` (та же схема, что у события), `itinerary`, `teachers` M2M, region/
    difficulty, публичные «шапки» `distance_km`/`duration_days` (заданы руками, т.к. маршрут может
    быть закрыт целиком), `is_published`, `sort_order`. **`Event.tour`** (FK, `SET_NULL`, null) —
    событие становится ЗАЕЗДОМ: даты/цены/тиры/депозит/waiver/QR остаются на событии, движок
    продажи не тронут. Событие без тура работает ровно как раньше (замки паритета).
  * **`apps/events/itinerary.py`** — канон маршрута: остановка `{day, time_from, time_to, title,
    text, km, overnight, lat, lng, visibility}` (времена = хронометраж, решение владельца §0b.7),
    нормализация мусора/капы/сортировка, `visible(raw, audience)`, `hidden_count`, `by_day`,
    `map_points`, `from_post`. **Видимость per-остановка, дефолт `private`** (маршрут — IP гида):
    owner видит всё, participant — participants+public, посторонний — только public;
    **незнакомая аудитория трактуется как посторонний (fail-closed)**.
  * **`apps/events/access.py`** — резолвер аудитории (owner по `has_cabinet_access`, participant по
    билету на любой заезд через сессию ЛК). «Есть билет» считается ЧЕРЕЗ РЕЕСТР статусов
    (`cancelled_statuses_for`, SM-3), иначе кастом-статус владельца («Angezahlt») лишал бы участника
    доступа. Шов переиспользуют MT-2/MT-3.
  * **Витрина**: `/touren/` (каркас листинга UB1-1) и `/tour/<slug>/` (каркас `detail.html`) —
    галерея, «шапки», маршрут (`_route.html`: Leaflet/OSM + polyline по порядку + таймлайн по дням,
    попап через DOM как в `_map.html`), тематические блоки ПЕРЕИСПОЛЬЗУЮТ `_event_thematic.html`
    (тур несёт те же images/teachers/landing), список заездов с ценой/местами → штатная страница
    события. Вьюха отдаёт УЖЕ отфильтрованный маршрут — закрытая точка физически не доходит до
    шаблона. Честная плашка «ещё N точек после бронирования».
  * **Кабинет**: `/dashboard/events/touren/` (список + быстрое создание с уникальным slug) и
    страница правки: поля, блоки лендинга, **редактор маршрута строками** (слоты дорастают после
    Save, паттерн FD-3), фото, список заездов. Сентинел `it_present` — частичная форма не стирает
    маршрут (инвариант W0). Пункт «Reisen» в nav-реестре (хаб Verkäufe), Tour.images в
    `media_registry` (замок реестра), туры в sitemap.
  * **Рефактор без изменения поведения**: редактор богатых блоков вынесен из `EventForm` в
    `apps/events/landing.py` (поля добавляются в `__init__` — порядок формы события сохранён,
    т.к. объявленные поля и так шли после модельных) и переиспользован формой тура.
  * Тесты: `test_itinerary` (20) + `test_tours` (16) — замки утечки (гость/участник/полностью
    закрытый маршрут/точки карты), паритет событий, W0-сентинел. 55 msgid × 5 локалей
    (плюралы ru/uk — 4 формы).
  * **Уроки среды:** локально не было `gettext` → ~18 тестов «падали» из-за неподставленных
    немецких строк; после `apt-get install gettext` + компиляции `.mo` — 266/266 зелёные
    (правило: без `.mo` локальный прогон врёт). Переиспользуемая тест-БД накопила строки от
    прогонов с `--create-db`/stash → ложные `MultipleObjectsReturned`; сравнивать надо СПИСКИ
    падений, а не их число, и перепроверять на `--create-db`.

- **2026-08-13 · MT-2 «Досье участника» (SecureDocument) — приватные файлы платформы.**
  Второй инкремент семейства MT (план `moto-tours-archetype-plan-2026-08-04.md §4b`).
  Закрывает не только запрос «зашифрованные документы участника», но и системную дыру:
  до этого ЛЮБОЙ загруженный файл платформы был доступен по прямому URL хранилища.
  * **Новый TENANT-апп `apps/documents`** (⚠️ миграция `documents/0001`). `SecureDocument`:
    `owner`(Customer) · `kind` (паспорт/права/страховка/виза/прочее) · мягкая привязка
    `ref_kind`/`ref_id` (паттерн inbox — переживает удаление билета) · `path`/`original_name`/
    `mime`/`size` · `note` = **EncryptedTextField** · `uploaded_by` · `consent_at` (момент
    согласия — доказательство) · `expires_at`.
  * **Шифрование содержимого:** `crypto.encrypt_bytes`/`decrypt_bytes` (новая бинарная пара
    рядом со строковой) + `storage.save_encrypted` — в бакет уходит ШИФРОТЕКСТ, имя файла
    случайное без расширения. Валидация по СИГНАТУРЕ (не по расширению): фото/PDF/GPX/MP4,
    кап 50 МБ (решение владельца §0b.3).
  * **Приватная выдача:** единственная точка — `document_download`; доступ = владелец
    (сессия ЛК) ИЛИ `has_cabinet_access`; отказ → **404, а не 403** (не подтверждаем
    существование чужого документа); ответ `Cache-Control: private, no-store`.
    `default_storage.url()` для документов не зовётся нигде.
  * **Ретеншн:** `expires_at` = конец заезда + `DOCUMENT_RETENTION_DAYS` (90); beat
    `purge_expired_documents` по всем схемам (паттерн Meldeschein) удаляет запись И blob.
  * **UI:** раздел «Meine Dokumente» в ЛК (загрузка с обязательным согласием, скачивание,
    удаление) + матрица «участник × документ» на заезде в кабинете (кто что сдал).
  * **Побочно закрыт давний пробел:** `stays.GuestRegistration.doc_number` (номер документа
    гостя по §30 BMG) лежал ОТКРЫТЫМ ТЕКСТОМ → переведён на `EncryptedTextField`
    (⚠️ миграция `stays/0032`, ленивая — старые значения читаются как есть). Проверено
    grep'ом, что по полю нигде не фильтруют (Fernet недетерминирован).
  * **Что НЕ стали делать:** ретеншн для `TicketWaiver`. План числил это пробелом, но
    докстринг модели фиксирует обратное — отсутствие авто-чистки там ОСОЗНАННО («срок
    исковой давности»). Удалять подписанный отказ от ответственности раньше срока значит
    лишать организатора юридической защиты; пункт снят, план исправлен.
  * Тесты: 19 (шифротекст в хранилище вместо файла · зашифрованный `note` в БД · сигнатурная
    валидация против «PDF по расширению» · чужой/аноним/залогиненный-без-Membership → 404 ·
    ретеншн удаляет blob · согласие обязательно · легаси-плейнтекст Meldeschein читается).
    24 msgid × 5 локалей.

- **2026-08-14 · MT-3 «Пространство поездки» (Reise-Feed) + двусторонний Telegram-мост.**
  Третий инкремент семейства MT (план `moto-tours-archetype-plan-2026-08-04.md §3.3/3.3b`).
  Отвечает на запрос владельца «сейчас под каждый тур заводим Telegram-группу — хочу вести
  своё»: контент, файлы и состав группы живут у бизнеса, Telegram остаётся каналом.
  * **Новый TENANT-апп `apps/community`** (⚠️ миграция `community/0001`): `FeedSpace`
    (пространство ЗАЕЗДА — мягкая привязка `ref_kind`/`ref_id`, `is_open`,
    `members_can_post`, `tg_chat_id`+`tg_link_code`, `invite_token`), `FeedPost`
    (лента и чат — ОДНА модель через `kind`: общий доступ, общая модерация, общий мост),
    `FeedComment` (плоские, ответ гида выделен). Автор — три поля вместо роли-строки
    (`author_user` | `author_customer` | `author_name` из Telegram): по FK строятся права,
    имя нужно для отправителей, не привязавших бота.
  * **Доступ — `apps/community/access.py`** (один резолвер на все вьюхи, fail-closed):
    гид (`has_cabinet_access`) · участник (непогашенный билет на заезд — ЧЕРЕЗ РЕЕСТР
    статусов, как MT-1) · приглашённый (ссылка-приглашение, факт в сессии — сопровождающий
    без билета). Посторонний получает **404, не 403**.
  * **Витрина** `/reisegruppe/<pk>/`: вкладки «Лента» и «Чат». Чат — поллинг по образцу
    inbox (vanilla fetch + `setInterval(5s)`, сервер отдаёт хвост, клиент дедупит по
    `data-msg-id`, DOM через `textContent`). Курсор сознательно НЕ вводили: он ломается
    при скрытии записей модератором. Скрытое не уезжает ни в HTML, ни в поллинг.
  * **MT-3b мост:** гид добавляет бота в TG-группу тура и пишет там `/link <код>` (код в
    кабинете) → `FeedSpace.tg_chat_id`. Дальше: запись из веба → в группу; сообщение из
    группы → реплика чата. **Эхо гасится источником** (`source=telegram` обратно не шлём),
    **повтор апдейта не двоит** (`external_ref` = message_id + уникальный индекс +
    идемпотентный `add_post`). В `telegram/services.py` добавлен `get_file_url` (getFile),
    `allowed_updates` расширен `my_chat_member`; `webhook.handle_update` получил ветку
    группы (в личном чате поведение прежнее — Mini App). Команда в группе приходит как
    `/link@botname` — учтено. ⚠️ **Ops: у бота в @BotFather нужно выключить privacy mode**,
    иначе Telegram отдаёт боту только команды (записано в кабинете и в докстринге моста).
  * **Уведомления:** домен `community` в матрице каналов UD4-2 (email+Telegram) —
    рассылаем ТОЛЬКО объявления гида; реплики чата не рассылаем (иначе поездка утонет в
    письмах). Регистрация во всех трёх реестрах prefs (в `customer_matrix` обращение к
    `DOMAIN_MODULE[domain]` без `.get` — пропуск ключа уронил бы экран настроек).
  * **Кабинет:** `/dashboard/reisegruppe/<event>/gruppe/` — настройки, код привязки,
    ссылка-приглашение, последние записи; вход со страницы заезда.
  * Тесты: 24 (доступ участник/посторонний/отменённый билет/приглашённый · скрытие
    модератором в ленте и в поллинге · привязка группы, в т.ч. `/link@botname` ·
    идемпотентность повторного апдейта · эхо-петля · приватный чат не считается группой).
    28 msgid × 5 локалей.

- **2026-08-14 · MT-4/5/6: логистика по точкам · расходы и маржа заезда · чек-лист подготовки.**
  Закрывает волну MT в объёме плана (`moto-tours-archetype-plan-2026-08-04.md §3.4–3.6`).
  * **MT-4 `events.SupplierBooking`** (⚠️ миграция `events/0025`): книга закупок гида —
    отель/транспорт/экскурсия/пермит/локальный гид по дню и точке маршрута, статусы
    to_book→requested→confirmed→paid/cancelled, номер подтверждения, ваучер приватным
    файлом MT-2. Справочник поставщиков переиспользован как есть (`inventory.Lieferant`).
    Почему новая модель, а не существующие механизмы: `stays` смотрит ВНУТРЬ (исходящих
    броней нет), закупки склада требуют товар из каталога и двигают остаток, а швы
    `Order.parent_order`/`supplier_tenant_schema` пассивны.
    **Инвариант: закупочная цена не отдаётся наружу ни при каком флаге** —
    `visible_to_participants` открывает участнику факт и статус через отдельный метод
    `participant_view()` (так «цену не показываем» — свойство модели, а не привычка
    вёрстки); замок проверяет отсутствие суммы в выдаче.
  * **MT-5 `finance.ExpenseEntry`** (⚠️ миграция `finance/0007`) — кусок давно
    запланированного M13: до сих пор журнал вёл ТОЛЬКО доходы. Зеркало `RevenueEntry`
    (идемпотентность по `(source, source_ref)`), категории, привязка к заезду, оригинал
    валюты рядом с EUR (решение §0b.6). `tour_finance.sync_expense` заводит расход при
    переводе в «Bezahlt» и СНИМАЕТ при откате оплаты (до оплаты это план, а не расход);
    `event_margin` разделяет оплаченное и плановое — иначе маржа выглядела бы красивее,
    чем есть. Общеплатформенно: учёт расходов теперь есть у всех архетипов.
  * **MT-6 `events.TourTask`** (⚠️ миграция `events/0026`): чек-лист подготовки с
    дедлайном и ответственным; просрочка = срок прошёл И не закрыто. Осознанно НЕ новый
    kind Kanban-доски (доска работает со сделками, у которых FSM и деньги).
  * **Кабинет:** единый экран `/dashboard/events/<pk>/logistik/` — плитки экономики
    (выручка · оплачено · осталось оплатить · маржа после всех расходов), таблица закупок
    со сменой статуса и тумблером видимости, чек-лист. Вход со страницы заезда.
  * Тесты: 15 (приватность закупочной цены · идемпотентность расхода и снятие при откате ·
    категория по виду закупки · маржа с плановым оттоком · отменённая бронь не считается
    планом · немецкая запятая в цене · чек-лист и просрочка). 30 msgid × 5 локалей.

- **2026-08-14 · MT-демо: волны MT-3/4/6 видны в ките `moto`.** `DemoKit.tour_operations`
  (posts / supplier_bookings / tasks) наполняет ПЕРВЫЙ заезд через штатные сервисы
  (`community.services.space_for_event`+`add_post`, `SupplierBooking`, `TourTask`) — демо
  ходит тем же путём, что живой тенант, а не пустыми экранами. Кит «Himalaya Riders»:
  3 объявления гида в ленте, 4 закупки (отель оплачен, отель и транспорт подтверждены,
  пермиты «к бронированию»), 4 задачи подготовки (одна закрыта). Замок проверяет и
  инвариант приватности: закупочная цена не появляется в `participant_view`.
- **2026-08-14 · MT-фикс по красному CI: тур в обходе переводов демо.** Инвариант I18N-12
  (`test_every_overlay_field_is_reachable_by_the_seeder`) поймал ровно то, ради чего написан:
  у `events.Tour` появились оверлеи, а `demo_i18n.translate_tenant_content` про них не знал —
  переводы тура легли бы мёртвым грузом. Добавлен проход по `Tour` (title/summary/description).
  Инвариант приехал из main ВО ВРЕМЯ волны (I18N-12 влился, пока шла разработка), поэтому
  локальные прогоны до ребейза его не видели, а после ребейза гонялись только затронутые
  модули. Урок: после ребейза на ушедший вперёд main прогонять и `apps/tenants` целиком —
  там живут сквозные инварианты платформы.

- **2026-08-14 · MT-D «информативное демо мото-архетипа + переводы» (фидбэк владельца:
  «больше туров и фотографий, разделить по странам, немного про гида, запрос на
  персональное проведение тура» + «проверь переводы на все языки у архетипа»).** План —
  `docs/moto-demo-enrichment-plan-2026-08-14.md`; ⚠️ миграция `events/0027` (аддитивная).
  * **MT-D1 модель.** `Tour` += `country` (ЯВНЫЙ ключ группировки — не парсинг `region`,
    который свободный текст владельца) + оверлеи `region_i18n`/`country_i18n`/
    `details_i18n`/`itinerary_i18n`. Богатая страница и маршрут переводятся по схеме
    I18N-12 (`core.i18n_json`), причём в `route()` порядок операций жёсткий:
    **нормализация → перевод → фильтр видимости**. Наоборот нельзя: оверлей мерджит
    списки ПОЗИЦИОННО, и перевод после фильтра сажает тексты на чужие остановки.
    `visibility`/`time_from`/`time_to`/`lat`/`lng` добавлены в `SKIP_KEYS` — это
    управляющие значения, перевод `public` сломал бы сам фильтр.
  * **MT-D2 витрина.** `/touren/` группируется по странам (секции + якорь-чипы) —
    группировка по БАЗОВОМУ значению, показ `country_localized`, иначе на en/ru одна
    страна распалась бы на несколько групп. Одна страна → плоский список, как было.
    Карточка тура вынесена в партиал `_tour_card.html` (одна разметка на оба режима).
  * **MT-D3 персональный тур.** Модуль `jobs` + `anfrage`-форма (дата/участники/тип
    поездки) у кита; CTA «Individuelle Reise anfragen» на списке и на странице тура
    (`?betreff=<тур>`), плюс блок под списком заездов. Выбран структурированный путь
    (заявка → доска → Sofort-Angebot), а не свободное письмо: демо показывает весь путь
    до продажи. Гейт — активность модуля, без него ссылки нет.
  * **MT-D4 контент.** 8 туров (4 Индия: Manali–Leh, Ладакх, Спити, Раджастхан; 4 Непал:
    Мустанг, Аннапурна, квадро-сафари Читван, Солукхумбу) с «для кого / что входит /
    что взять / FAQ» и маршрутами смешанной видимости; 9 заездов; 3 гида с биографиями
    (страница гида теперь показывает и его поездки); галерея кита; `seed_records` →
    билеты и 2 заявки на приватный выезд лежат на доске.
  * **MT-D5 фото.** 29 файлов CC0/PDM (Openverse, лицензия сверена по метаданным ответа,
    каждый кадр просмотрен глазами: отбракованы водяные знаки, узнаваемые лица, чужая
    география). Покрытие ключей кита — 31/31, все ≤150 КБ, провенанс в `SOURCES.md`.
    Портретов реальных людей не берём и здесь: карточки гидов — мотоциклисты в шлемах.
    Попутно починен `demo_photo_report`: он падал `IndexError` на любом ките с фото-плиткой
    категории (DS-2), то есть отчёт о покрытии не работал вовсе; теперь считает и туры.
  * **MT-D6 переводы.** Обход демо переводит тур целиком (region/country/details/itinerary),
    `scripts/demo_i18n_gap.py` наконец ВИДИТ контент туров (до этого кит был занижен в
    разы) и умеет `--kit`. Кит `moto`: **3 % → en 95 / ru 100 / uk 100 / tr 97 %**
    (~1180 записей в четыре словаря). Топонимы объявлены в `demo_i18n.PROPER_NAMES` —
    их «перевод» на en/tr был бы identity, а такие записи запрещены замком проекта.
    12 новых msgid хрома × 5 каталогов.
  * Замки: группировка стран (порядок, единственная страна, локализованная метка),
    приватный выезд (гейт модуля), переводы тура (фолбэк, только листья, маршрут
    переведён ДО фильтра, `visibility` не переводится), демо (8 туров/4+4 страны/фото/
    3 гида/anfrage/заявки) и рендер витрины демо с реальными файлами фото.
  * **Правки по стенду** (браузерный прогон демо нашёл то, чего не видели серверные
    проверки): страница тура за 1490 € печатала «Kostenlos» — общий партиал секций
    читает `event.price_cents`, а у тура своей цены нет (она на заездах) → ветка
    «ab X €» по `from_price_cents`, событию с тирами это тоже честнее; строка места
    дублировала страну («Ladakh, Indien · Indien»); страница людей у мото-бизнеса
    называлась «Unsere Lehrer» → «Unsere Guides» по ДАННЫМ (есть опубликованные туры),
    а не по списку архетипов. Грабля стенда: `runserver --noreload` + кэш шаблонов
    Django — после правки шаблона рестарт обязателен, иначе «фикс не работает»;
    и headless-скриншот полной страницы не грузит `loading="lazy"`-картинки без
    `--virtual-time-budget` (пустые карточки — артефакт съёмки, а не дефект).

- **2026-08-14 · MT-F «главная в формате Fokus под туры + вход в ленту поездки»** (фидбэк
  владельца: «на главной пусто, нет туров, сделай сайт в формате фокус, специально под туры;
  где посмотреть ленту чат, сделай демоданные если нет»). План —
  `docs/moto-home-fokus-plan-2026-08-14.md`; БЕЗ миграций.
  * **Почему было пусто (разбор, а не догадка):** секции `tours` в реестре `SECTIONS` не
    существовало вовсе (MT-1 отдал туру страницу `/touren/`, на главную не выводил), секция
    `events` у демо-китов не перечислена в `_kit_sections` (normalize дописывает её
    выключенной), а сборки «Fokus» для `tour_operator` в `BUNDLES` не было.
  * **MT-F1 секция `tours`:** реестр + `sections/_tours.html` (реюз карточки MT-D2) + сборка
    в home-вьюхе (гейт: модуль events И есть опубликованные туры) + якорь `#reisen` + лимит
    6 / заголовок / «View all» / иконка реестра. ⚠️ ОСОЗНАННАЯ регенерация четырёх
    golden-эталонов normalize — новая известная секция дописывается в конфиг (прецедент
    FD-2/DS-3b); диффы — только выключенная секция `tours`, формат файлов сохранён (indent=1).
  * **MT-F2/F3 сборка `fokus_touren`:** сплит-баннер, CTA в шапке, компакт-полоса доверия,
    на главной поездки + заявка на приватный выезд; секцию заездов сборка НЕ включает —
    даты видны в карточке поездки, отдельный список был бы теми же датами без маршрута и
    фото. Кит `moto` едет на сборке (`bundle`+`look`+`enable_anfrage_section`), заголовки
    секций свои («Unsere Reisen», «Eigene Gruppe, eigener Termin»); у `DemoKit` появилось
    поле `section_intros`.
  * **MT-F4 «где смотреть ленту»:** вход участника — карточка «Reisegruppe» на странице
    своего билета (`/e/<code>/`); право читать считает тот же fail-closed `access.role_of`,
    без входа в ЛК показываем «Im Konto anmelden», а не ссылку в 404. Владельцу добавлен
    второй вход — с экрана логистики заезда (первый, со страницы заезда, был и раньше).
  * **MT-F5 демо-чат:** `tour_operations` += `chat`/`comments` — пять реплик участников и
    три комментария к объявлениям гида (вкладка «Chat» была пустой). Авторы — имена:
    чат сеется вместе с пространством, то есть до билетов; текст остаётся немецким, у
    `FeedPost` нет i18n-оверлея (закрытая зона участников) — врать переводом не стали.
  * **Найдено стендом, а не тестами:** CTA первого экрана и шапки вели на «Termine»
    (`primary_item` брал лендинг модуля events) → `archetypes.primary_item` подменяет его
    на `/touren/` при наличии туров (признак по данным + мемоизация на тенанте); вторая
    кнопка первого экрана «Menu» вела в ПУСТОЙ каталог у любого не-каталожного архетипа →
    гейт по наличию товаров; секция заявки была озаглавлена «Request a quote / Job address» →
    заголовок и подпись `anfrage` стали настраиваемыми (`SECTION_TITLE_KEYS`).
  * Замки: секция туров (включена → карточки, нет туров → нет пустого блока, черновик не
    показываем, по умолчанию выключена), вход в группу со страницы билета (участник видит
    ссылку, гость — приглашение войти, без пространства блока нет), демо (3 записи + 5 реплик
    + 3 комментария). 4 msgid × 5 каталогов.
  * **Доводка MT-F4b (фидбэк «не вижу чат, в каком он разделе?»):** в ЛК клиента
    (`/konto/`) появился ПОСТОЯННЫЙ раздел «💬 Reisegruppen» — страница билета была
    разовым входом, и чат снова терялся. Критерий списка — тот же, что у
    `community.access._has_ticket` (отменённый билет группы не показывает: раздел не
    должен обещать ссылку, которую вьюха встретит 404-м). Замки в
    `test_account_data`; проверено на демо end-to-end (konto → группа → вкладка Chat).
  * **Доводка MT-F6 (фидбэк «не вижу настройки затрат в админке»):** вход в
    «Logistik & Kosten» существовал, но был мелкой текст-ссылкой, втиснутой в строку
    заголовка «Teilnehmer» на странице заезда. Теперь под шапкой заезда — панель из
    трёх карточек (💶 затраты · 💬 группа · 🗂 документы) с подписями, а на странице
    ТУРА у каждого заезда — прямая ссылка «💶 Kosten» (затраты живут на заезде, и
    искать их через список участников было негде). 4 msgid × 5 каталогов; проверено
    на демо end-to-end (заезд → панель; тур → Kosten; экран считает маржу).

## 2026-08-13 — фиксы по адверсариальному ревью батча MEN-14…17

Перед мержем прогнал ревью-воркфлоу (4 независимых измерения по диффу +
попытка опровергнуть каждую находку по коду). Подтвердились и исправлены:

**HIGH — подменю подкатегорий вело на 404.** `_category_children` строил ссылку
на лендинг `/bereich/<slug>/` для любой категории с контентом, а вьюха
`category_landing` требует КОРНЕВУЮ (`parent__isnull=True`). Инвариант перенесён
в `Category.landing_ready` — теперь он зеркалит гейты вьюхи, и им пользуются оба
потребителя: подменю шапки и плитки категорий. Заодно закрыт тот же
пре-существующий дефект у плиток подкатегорий на /sortiment/.

**MEDIUM — редактор меню терял цели.** Новая сборка списка категорий выводила
только корни и их прямых детей: категория 3-го уровня и активный ребёнок
выключенного родителя исчезали из селекта, и Save молча переставлял такой пункт
на первую опцию (класс W0). Обход стал полным (рекурсия с отступом по уровню,
осиротевшие ветки в конце), замок проверяет и глубину, и сироту.

**MEDIUM — чип блюда потерял два условия старой разметки:** у included-группы
показывалась надбавка, которую с гостя не берут, а свободная сборка перестала
печатать цену 0,00 € (условие `and amount` роняло ноль). Оба восстановлены,
паритет закреплён замком.

**MEDIUM — пагинатор книги собирался из отдельных msgid** «Seite» и «von», а
«von» в каталогах переведено как предлог места: en «from», ru «откуда», tr
«kimden», uk «звідки» → «Страница 1–2 откуда 8». Теперь переводится ЦЕЛАЯ фраза
с плейсхолдерами (`Seite %(r)s von %(n)s`), JS подставляет числа.

**LOW:** дети выключенной/удалённой ветки попадали в подменю (JOIN по родителю
не наследует ни `is_active`, ни soft-delete-менеджер) · пункт с `target` вёл во
весь каталог вместо своей ветки · панель включала плиточный режим при фото у
ОДНОЙ категории, остальные получали серые квадраты (заглушку убрали) · в
фото-сетках `first:mt-0` гасил отступ только у одного элемента, вторая колонка
первой строки съезжала · в overflow-панели «Mehr» классы `flex` были мертвы
(видимость строк переключает общий JS через `block`).

**Попутная находка вне диффа:** блюдо с Gang'ом вне реестра `COURSES` (импорт,
старые данные) молча выпадало из конструктора свободной сборки — `pool_products`
считает его блюдом, а группировка ходила только по реестру. Остаток уезжает в
«Weitere»: конструктор обязан показывать весь пул.

**Второй раунд ревью (тот же день).** Воркфлоу закончил верификацию: 17 находок,
14 подтверждено, 3 опровергнуто (в т.ч. две — как уже исправленные первым
раундом). Дозакрыты оставшиеся шесть:

**HIGH — три новых прайс-вида не попали в пикер экрана «Pages»** (список опций
там был захардкожен): сохранённый `preisliste_buch` не совпадал ни с одной
опцией, браузер слал первую («list»), и Save молча откатывал раскладку каталога
— ровно тот класс, что ловили в DS-5c на канве. Опции теперь строятся из
реестра `PAGE_EXTRA_PRESETS`, замок проверяет и наличие всех видов, и `selected`
у сохранённого.

**MEDIUM — атрибут `hidden` не прятал навигацию книги:** утилита Tailwind
`.flex` идёт в CSS позже правила `[hidden]{display:none}` и побеждает при равной
специфичности, поэтому при единственной группе оставались мёртвые стрелки.
Стартовое состояние задаёт класс, скрипт меняет его на `flex`.

**MEDIUM — панель категорий вылезала за правый край вьюпорта** (фиксированные
420px + `left-0` у пункта справа): правая колонка обрезалась и не нажималась.
Ширина клампится по экрану, а положение при нехватке места подвигает JS.

**MEDIUM — фикс `overflow` не покрывал клик-открытие:** клип снимался только на
`:hover`/`:focus-within`, а на таче панель открывается классом `nav-open` — и
оставалась обрезанной. Теперь при открытой панели клип снимается явно.

**LOW:** переключатель «список/плитка» рисовался и при прайс-видах, где сетки
`data-grid` нет (мёртвые кнопки) · кнопка «+ Categories submenu» писала подпись
на языке КАБИНЕТА, тогда как подписи меню — данные витрины с немецкой базой
(иначе в конфиг уехало бы, например, русское слово для всех языков).

## 2026-08-17 — фидбэк-батч MEN-18/19/20 (виды услуг, гармошка Gänge, меню кейтеринга)

Ветка `claude/goodkarma-catering-analysis-fea071` перед этим перебазирована на
свежий main (за 4 дня туда влились I18N-11/12 и архетип мото-туров) — конфликты
только build-log + app.css, гейт 1423 зелёных.

**MEN-18 — прайс-виды для услуг** («виды нужны везде, где есть товары и услуги;
услугам — список, список с картинками, 2 колонки»). Реестр:
`PAGE_EXTRA_PRESETS["service_index_layout"] = (preisliste, preisliste_foto,
preisliste_2sp, preisliste_foto_2sp)` — гастро-виды каталога (kompakt/karte/buch)
услугам не предлагаем. Рендер — НОВЫЙ партиал `_service_price_list.html`
(строка: имя — описание … цена + длительность, ссылка на деталь услуги):
у Service другие поля (name_localized/price_eur/image_url, валюты нет), а
продуктовый шаблон переплетён с buch/karte/variants — генерализация дороже
дубля строки. Группировки нет: услуги живут без категорий, Collections — уже
фасет-чипы над списком. Пикеры: канва (select услуг знал только сетки — класс
W0), **экран Pages впервые получил блок услуг** (раньше вид /termin/ менялся
ТОЛЬКО с канвы — «не нашёл, где изменить» имел причину); семантика «Standard
удаляет ключ» как у канвы, сентинел присутствия — чужой POST ключ не трогает.
Переключатель «список/плитка» в прайс-виде спрятан (гейт как у каталога).

**MEN-19 — гармошка Gänge** («портянка при большом ассортименте»): в свободной
сборке каждый Gang — `<details>` со счётчиком блюд, открыт только первый.
Чекбоксы в закрытых `<details>` остаются в DOM и отправляются — сворачивание
чисто визуальное. Внутренний порог «12 + Weitere» у pool убран: гармошка
решает то же лучше, а details-в-details нечитаем.

**MEN-20 — меню кейтеринга** («два пункта — оставить один с выпадающим
списком; субменю разнообразить»): пункт «Menüs» из строки шапки демо убран,
наборы — РУЧНЫМ первым ребёнком подменю «Speisekarte» (ручные дети идут перед
авто-категориями; инвариант «кит с combos обязан их линковать» проходит по
детям). Субменю обогащено: панель 420→560px, под названием — описание
категории (`desc` в резолвере, truncatechars в шаблоне). Полная Speisekarte
кейтеринга — «печатной картой» (`preisliste_karte`); тизер главной остаётся
плоским preisliste из одобренного макета Fokus, книга живёт у ресторана.

Замки: 4 прайс-вида услуг (нормализация+рендер+фото+деталь-ссылка), Pages-блок
услуг (save/Standard/чужой POST), гармошка (все блюда в DOM, открыт первый),
desc в резолвере, catering-лок каталога обновлён осознанно. 3 msgid × 5 .po.
Без миграций. ⚠️ ops: `seed_demo_tenants --kit catering --recreate`.

**Доводка по скриншоту стенда (та же дата).** Стенд 16/16 зелёный, но кадр
подменю показал: пункт без фото («Menüs & Pakete», ручной ребёнок) терялся
голой строкой в углу сетки плиток. Теперь такой пункт — полноширинная
акцент-строка (col-span-2, пунктирная рамка, стрелка) над плитками; серых
квадратов-заглушек по-прежнему нет. Правка чисто шаблонная.

## 2026-08-17 — MEN-21/22/23: карточка комбо (отзывы/примеры/CTA/слайдер) + посетительские переключатели вида (фидбэк владельца)

**MEN-21 — карточка набора обросла блоками** («в карточку комбо добавь отзывы,
примеры работ после основного блока, и блок Планируете мероприятие. И так же
другие наборы из этой категории в виде слайдера»). Отзывы: generic-движок
получил kind **"combo"** (`Review.KIND_COMBO`, миграция `reviews/0005` —
choices-only AlterField, DDL нет) + верификатор `has_bought_combo`
(FK `OrderItem.combo`; «свободная сборка» уезжает в заказ custom_lines БЕЗ FK —
её покупатель верификацию не проходит, fail-closed честнее матчинга по
названию) + приёмник `/kombi/<pk>/bewerten/` через общий `handle_review_submit`
+ `_entity_reviews.html` на карточке; loaders кабинета знают Combo. Блоки под
основным: «Beispiele unserer Arbeit» (лента 6 фото из галереи сайта + «Alle
Fotos» → /galerie/; гейт по наличию), CTA-полоса «Sie planen eine
Veranstaltung?» (гейт jobs; кнопка открывает generic-попап заявки
`storefront-anfrage-modal` — негативный замок `/anfrage/?betreff=` цел),
слайдер «Weitere Menüs aus dieser Kategorie» (та же категория, exclude self,
до 8, мини-карточки лентой). Демо: `DemoKit.combo_reviews` +
`refs["combos"]` в сидере + 9 отзывов catering (3×3 набора; тексты отзывов
НЕ переводим — правило DL-волны). Замки: верификатор (покупатель/чужой/
storno/другой набор), приём формы (покупатель создаёт, чужой нет), рендер
блоков и гейтов, слайдер excludes self, демо 3 отзыва на набор; замки
test_reviews переписаны осознанно (combo теперь С верификатором; цикл идёт
по `ENTITY_KINDS` — новый kind без верификатора уронит замок сам).

**MEN-22 — посетительский переключатель прайс-листа** («на главной в блоке
меню товаров товары выводятся списком, без картинок, как покупателю
переключить вид? должно быть с картинками в 2 колонки. и должен быть
переключатель»). У БАЗОВЫХ видов (plain/foto/2sp/foto_2sp/foto_3sp) — три
кнопки «список / с фото / 2 колонки» (`_price_view_toggle.html`), видимые и
на десктопе (в отличие от мобильного DS-10); kompakt/karte/buch — авторские,
без переключателя (фильтр `price_view_state` в siteui). Механика — class-swap
без перезагрузки: целевые классы в `data-cls-*`/`data-gcls-*` (JIT видит
строки в атрибутах — purge-safe), фото ВСЕГДА в DOM (`.plv-img`, в «plain»
скрыты), выбор в localStorage per-ключ (products ≠ catalog — «главная не
перекрашивает каталог», прецедент DS-10). Демо-главная кейтеринга →
`preisliste_foto_2sp` (стартовый вид = запрошенный «с картинками в 2
колонки»). **Стенд поймал дефект, невидимый серверным тестам:** `ml-auto`
партиала внутри `justify-between`-шапки секции съедал всё свободное место —
«Alle anzeigen» прилипала к заголовку; ссылка+переключатель сведены в правую
группу (паттерн шапки `_gallery`). **Урок замков:** голые маркеры
(`data-price-list`, `(4)`, имя набора) в негативных ассертах ловят теперь и
строки СЕЛЕКТОРОВ нового JS в `_grid_view_script.html` / формы отзывов /
`<title>` — четыре замка переведены на маркап-форму
(`data-price-list data-pl-style`, `text-gray-400">(4)</span>`, href-точечные).

**MEN-23 — переключатель галереи** («в галерее так же нужно сделать»):
«⊞ сетка ↔ ▤ крупно» (2 колонки 3:2 — aspect задаёт CSS-правило по
`[data-galv]` в app.css) на дефолт-сетке секции главной и на `/galerie/`
(as_page — всегда дефолт-ветка); авторские стили (strip/large/polaroid/soft)
не переключаем. Тот же class-swap/localStorage. Грабля класса «замок ловит
JS-строку»: цитата селектора `[data-galv="gross"]` в КОММЕНТАРИИ скрипта
роняла негативный замок — комментарий переписан без кавычек.

Стенд Playwright 22/22 на демо catering (все блоки карточки, попап CTA,
plain↔foto↔cols + персист, каталог не перекрашен, галерея gross + aspect
3:2 + персист); первая прогонка стенда «падала» на немецких ассертах —
...
**MEN-25 (та же дата, фидбэк «плитка должна быть от 2-6 + настройка
администратору»):** число колонок Kacheln-вида — из РАСКЛАДКИ секции (пикер
колонок Studio уже умел 2–6 с DS-6, новых ключей ноль): `data-plv-cols` на
контейнере + CSS-блоки вариантов (мобайл всегда 2 — замок). На каталоге
колонки Kacheln и так следуют cols-оверрайду `catalog_layout` (спред в
ansicht-override их сохраняет). Замок: attr из layout.cols, дефолт 4.
Проверено вживую: cols6 → 6 колонок на 1440px, 2 на мобиле.

**MEN-25b (следом, фидбэк «на мобильном может быть одна или 2»):** мобильные
...
## 2026-08-18 — KAT батч 3 (KAT-4+KAT-5): плотность «− N +» посетителю + смена вида каталога без перезагрузки

БЕЗ миграций. **KAT-4** (фидбэк «покупатель не может изменить вид на 2-6
столбцов»): партиал `_density_control.html` + IIFE в `_grid_view_script.html`
(делегирование, localStorage `sf_density:<key>`, кламп 2..6, только десктоп
lg+). Два режима: **plv** — контрол в `_price_view_toggle` (авто-везде с
ключом), пишет `data-plv-cols` прайс-контейнера (CSS-варианты MEN-25),
дефолт читается из атрибута контейнера, видимость ведёт `paint()`
переключателя (осмыслен только в Kacheln); **grid** — контрол в тулбаре
каталога (сервер-гейт «не прайс-вид»), пишет `data-density` сетки
`[data-grid=catalog]` — НОВЫЕ CSS-оверрайды (min-width:1024px, атрибутная
специфичность (0,2,0) бьёт Tailwind-классы владельца; `.is-list` не
конфликтует — он заперт в max-width:767px); равенство дефолту владельца
СНИМАЕТ атрибут (его классы снова правят; `catalog_cols` в контексте).

**KAT-5** (решение владельца — вариант B): клик по `a[data-ansicht]` →
`fetch` полной страницы → DOMParser → подмена `[data-listing-root]`
(атрибут на контейнере listing.html — общий каркас, но перехват только на
каталоге, где есть ansicht-ссылки) + `history.pushState`; сеть/ошибка/
модификаторы клика → обычная навигация; назад после подмены → честный
reload (гард `pushed`). Ноль серверных правок — работает для ВСЕХ видов вкл.
авторские karte/buch. **Сопутствующий рефактор:** инлайн-биндинг автосабмита
сорта в listing.html УМЕР БЫ после первой подмены (урок HF-волны «скрипт в
свапаемой зоне») → делегирован в `_grid_view_script.html`; инициализации
plv/grid_view/density экспортированы (`__sfPlvInit` и др.) и повторяются
после свопа (localStorage-состояние живёт).

Замки: `test_density_view.py` (4: гейты рендера контрола обоих режимов,
дефолт=колонки владельца, data-listing-root единственный + «в swap-зоне нет
инлайн-биндинга», делегированный обработчик в базовом скрипте); смежные
listing parity (catalog/events/booking/stays) целы. Стенд Playwright 20/20
(shop+catering: плотность каталога ± с персистом и снятием атрибута на
дефолте, fetch-своп без перезагрузки с живыми контролами/сортом после свопа,
plv-плотность в Kacheln с реальным изменением колонок, мобильный гейт).
3 msgid × 5 .po. Урок стенда: `waitForURL` может упасть на смене контекста
навигацией form.submit() — проверять факт `page.url()` после catch. Урок
замков (повтор MEN-24): негативный счёт `data-listing-root` ловил строки
JS-селекторов — матчить маркап-форму `data-listing-root>`.

## 2026-08-18 — фидбэк-батч моб-витрины (значки/2 колонки/границы экрана/повтор категорий)

Скриншоты владельца с телефона (демо catering/shop), 4 пункта, всё БЕЗ миграций.
**(1) Порядок значков тулбара каталога:** клиентский «список/плитка»
(`_grid_view`) с каталога УБРАН — на мобильном он стоял ВТОРЫМ переключателем
рядом с серверным `?ansicht=` (Liste/Kacheln это уже покрывают), на десктопе
был скрыт с DS-10; тулбар = Filter + сорт + 3 иконки одной строкой. Замок
`test_filters_sort_and_view_share_one_row` переписан осознанно (вид =
`data-ansicht`). **(2) «2 колонки» скрыта на мобильном** (`hidden md:flex`) в
обоих переключателях — plv-тогглере секции (`data-plv-btn="cols"`) и
серверных ссылках каталога (`preisliste_foto_2sp`): вид `md:columns-2` на
телефоне выглядит как обычный список, кнопка ничего не меняла. Кнопки
остаются в DOM (замки счёта целы), десктоп прежний. **(3) Кнопки за
границей экрана на главной:** ДВЕ причины — (а) шапка секции товаров:
`shrink-0` правой группы + 4 кнопки не влезали → `flex-wrap` (группа
переносится ПОД заголовок); (б) сама шапка сайта: длинное имя бизнеса
(«Grüne Tafel Catering») с `shrink-0` лого выталкивало иконки за 390px →
лого на мобильном сжимается (`min-w-0` + `truncate`), на sm+ прежний
`shrink-0`. **(4) Внутри категории НЕ повторяется верхний список категорий**
(та же сетка плиток была и на /sortiment/, и на каждой странице категории):
в path/GET-режиме категории блок «все категории» (плитки/чипы) не рендерится
— остаются «← Alle» и подкатегории (M20U-3); корень каталога прежний.

Стенд Playwright 14/14 (390px, catering+shop: тулбар/страницы без гориз.
скролла, 2sp невидима на мобиле и видима на десктопе, плитки на корне есть
и в категории нет, hero-шапка kopfbild цела). Грабля контейнера (повтор):
среда пересоздалась со старого снапшота — протухли .mo (Aug 12, LMIV-замок
красный при правильном .po → перекомпилировать polib'ом) и дев-БД стенда
(migrate_schemas + пересев), /etc/hosts очищен. Замер overflow в стенде —
только после settle (reveal-анимации дают транзиентные +24px).
## 2026-08-19 — X0+X7: хотфиксы навигации ЛК + процессные предохранители (план cabinet-cleanup-2026-08-19, утверждён владельцем)

Старт программы «Кабинет-X» (A «Генеральная уборка» → B «Вертикальный кокпит»;
исследование+рынок — `docs/cabinet-ux-research-2026-08-18.md`, воркфлоу 7 агентов
+ живой стенд). **X0:** (1) `integrations_home` был БЕЗ `@login_required` —
декоратор съел вставленный хелпер GK-11 (аноним видел статусы Stripe/Telegram/
домена; карточка Google-рейтинга всегда пуста — обёрнутый хелпер падал в _safe);
фикс + 2 замка. (2) Гейт якоря Marketing: виден при ЛЮБОМ живом модуле его хаба
(`nav_registry.hub_module_keys`) — отель/werkstatt/handwerker/tour_operator
(promotions выкл по умолчанию) вернули вход к Nachrichten/Bewertungen/Kontakte и
бейдж непрочитанных; замки w8/st4b переписаны ОСОЗНАННО. (3) «Nachrichten» —
первый подпункт якоря Marketing со СВОИМ бейджем (скрипт бейджа →
querySelectorAll) + карточка «Nachrichten: N ungelesen» первой на marketing-home
(бейдж вёл на лендинг без сообщений). (4) «Full view» с главной → Verkäufe
(легаси-доска больше не рекламируется; замок wizard переписан). (5)
`NavEntry.owner_only` (Recht/Abo/Team, зеркалит `_OWNER_ONLY` мидлвари W9-10) —
hub_tabs/палитра прячут табы у сотрудников (fail-open без request; роль одним
запросом `roles.role_of`). **X7:** замок состава якорей (мораторий владельца,
квартал) · замок «каждый беспараметрный экран кабинета в реестре/палитре ЛИБО
осознанно в EXPECTED_UNLISTED» (инвентарь URLconf-обходом; 76 текущих исключений
категоризированы [POST]/[302]/[X2]/[X4]/[OK] — волны X2/X4 будут вычищать) ·
`docs/cabinet-screen-dod.md` (DoD нового экрана) + строка в CLAUDE.md §5.
1 msgid × 5 .po. Стенд Playwright 5/5 (hotel: якорь/подпункт/бейджи/Full view/
marketing-home). Грабли среды: `pkill -f <паттерн>` убивает собственный bash
(матчит свою же команду) — гасить по PID; локальный прогон без gettext требует
`polib`-компиляции .mo (3 «падения» wizard-тестов оказались этим классом).

## 2026-08-19 — X1: немецкий хром кабинета (волна «Кабинет-X», без миграций)

Задача по плану `cabinet-cleanup-plan-2026-08-19` формулировалась как «доперевести
de.po». Разведка (воркфлоу 13 агентов: QA всех 4388 переводов по контексту
использования, скан хардкода в 165 шаблонах + AST-обход apps/**, живой стенд с
компиляцией .mo, форматы) **переопределила задачу**: класс «английский текст на
немецком экране» оказался ЧИСТЫМ (ключи de.po = en.po 1:1, дни недели переведены,
identity-записи — интернационализмы), а стендовый «языковой салат» из исследования
2026-08-18 был ложной тревогой среды — в контейнере нет gettext, .mo не собирались,
и Django печатал msgid. **Реальный дефект — смысловые омонимы DeepL.**

**Переводы (217 правок в de.po, все через адверсариальную верификацию немецким
редактором-скептиком: 179 accept + 32 adjust, 5 отклонено как ухудшение).** Классы:
омонимы предметной области — `Jobs→Stellenangebote` (вакансии вместо заказов
ремесленника), `Table→Tabelle` (таблица вместо СТОЛА — на кухонном дисплее и
QR-табличках!), `Quote→Zitat` (цитата вместо сметы, в т.ч. на публичной странице
клиента), `Decline→Rückgang` («спад» на кнопке отклонения), `Withdrawal→Rückzug`
вместо юридического **Widerruf**, `Deposit→Einzahlung` (8 строк), `Passes→Pässe`
(загранпаспорта), `Stamps→Briefmarken` (почтовые марки), `Site→Standort`,
`Theme→Thema`, `Main→Hand` (DeepL принял за французское main), `Tablet→Tablette`
(таблетка), `Padding→Polsterung` (обивка мебели) · машинные ляпы в flash-сообщениях
владельца — «Stay created.»→«Bleib kreativ.», «Teacher saved.»→«Lehrer gerettet.»,
«On trial»→«Vor Gericht» (под судом), «Cabinet language»→«Sprache der Regierung» ·
инверсия смысла — «free from X €»→«kostenlos unter X €», «Dates blocked.» показывал
успех как ошибку · **сломанное форматирование** («Liefer%(f)s€», «Erstellt am%(n)d.»,
«in Höhe v%(p)s %%» — текст вокруг плейсхолдера съеден; набор плейсхолдеров при этом
совпадал, поэтому msgfmt --check-format их НЕ ловит).

**Код (hardcode, 5 файлов):** сырые коды статуса резерва текли в UI — `redeem_detail`
печатал `{{ reservation.status }}`, фильтр списка резервов — голые коды («pending»/
«confirmed»); теперь тег `status_label` и пары (код, подпись) из вьюхи, уважая
собственные имена статусов владельца (FB-4b) · единственная английская flash-фраза
`Cannot {action} a reservation…` и две со СКЛЕЙКОЙ английского кода действия →
словари полных немецких фраз (`_reservation_failed`) · бейджи «Dark»→«Dunkel»,
«Auto»→«Automatisch» (голое «Auto» в немецком = автомобиль) · `_RESERVATION_STATUS_LABELS`
были голыми немецкими строками → `gettext_lazy` (на немецком экране незаметно, но в
кабинете на en/tr/ru/uk выводился немецкий — зеркальный класс дефектов).

**Расщепление омонима msgid:** «Board» обслуживал ДВА смысла — вид доски
(`sales_page.VIEW_LABELS`) и пансион отеля (`units.html`). Перевод одного ломал
другой; в шаблоне взят существующий msgid «Verpflegung», вид доски остался «Board»
(верификатор отклонил «Tafel»: в Германии «die Tafel» — прежде всего
благотворительные продуктовые банки).

10 новых msgid × 5 каталогов. Замок: живой стенд Playwright (friseur+hotel, 8-9
экранов) — ни одной старой ляп-строки. Локально 2122 теста зелёные.

**Грабли среды (повтор + новое):** (1) `.mo` в контейнере собираются только polib —
без этого стенд врёт «английским»; (2) runserver кэширует переводы — после правки
.po нужен рестарт, иначе проверка ложно красная; (3) `rl:*`/`resv_token:*` в Redis
переживают прогоны и роняют чужие тесты (`test_public`) — чистить перед широким
прогоном; (4) многострочные `{# #}` — снова пойманы замком, писать `{% comment %}`.

**Доводка X1 по красному CI (той же датой).** Прогон на git поймал 3 падения, которых
локальный батч не видел: правки тона «du → Sie» меняют строки, на которые ЯВНО
ассертят чужие модули — `apps/aggregator` (magic-link: «Schau in deinen Posteingang»
→ «Prüfen Sie Ihren Posteingang») и `apps/booking` (buy-box: «Wähle einen Zeitpunkt…»
→ «Wählen Sie eine Uhrzeit…»). Ассерты обновлены осознанно. **Урок (новый класс):**
массовая правка `.po` — это правка ПОВЕДЕНИЯ всего репозитория, локальный гейт «по
затронутым модулям» для неё структурно недостаточен, нужен полный прогон; быстрый
предварительный скан — grep старых msgstr из диффа `.po` по `apps/**/test_*.py`.
**Грабля-спутник:** `assert b"…"` с умляутом = SyntaxError («bytes can only contain
ASCII»), в тестах писать `"…".encode()`.


## 2026-08-19 — X2a: уборка дублей кабинета (волна «Кабинет-X», без миграций)

По плану `x2-legacy-surfaces-plan-2026-08-19 §2` (решение владельца: X2a+X2b сейчас,
X2c до X3/X4). Три шага.

**Шаг 1 — мёртвый код.** `dashboard_tiles` (0 прод-импортов, висел с ST-4a) ·
`orders:order-settings` (no-op редирект, 0 ссылок) · маршрут `/willkommen/` (алиас
мастера, 0 ссылок, но ЖИЛ строкой в двух middleware-префиксах). Побочно закрыта дыра
собственного замка X7.3: `/willkommen/` был ВНЕ `_CABINET_PREFIXES`, т.е. инвариант
достижимости его не видел и удаление прошло бы мимо — префиксы расширены.

**Шаг 2 — хаб-плитки главной.** Шесть плиток дублировали якоря сайдбара 1:1 и под
ДРУГИМИ именами («Bestellungen» ↔ якорь «Verkäufe» — один экран под двумя названиями).
Уникален был только вход «Integrationen» (не якорь по Р-3 и не подпункт: запись
`advanced=False`, а `sidebar_children` берёт только advanced) → сперва дан вход в
сайдбар (подпункт «Einstellungen»), затем удалены `hub_tiles` + блок шаблона +
контекст вьюхи. **Исправлено ложное покрытие** (находка разведки X2): ассерты
`"ic-orders" in html` остались бы зелёными без плиток — спрайт подключает базовый
шаблон; заменены на честные.

**Шаг 3 — один механизм прогресса.** Плашка «Setup-Fortschritt N/M» и отдельный
readiness-список AB4 считали «выполнено» по РАЗНЫМ правилам и висели на главной
одновременно. Свёрнуты в `onboarding.setup_checklist()` — источник один (реестр шагов
`steps_with_status`), ссылки ведут на слайд `?step=`; уникальные признаки readiness не
потеряны: «фото/галерея» перенесён внутрь `_check_home` (смягчение: галерея тоже
считается), «часы работы» добавлены отдельным пунктом со ссылкой в настройки (слайда
мастера для них не существует — добавлять шаг без обработчика нельзя).

2 msgid × 5 .po. Стенд Playwright (friseur+hotel): плиток нет, прогресс один, вход
«Integrationen» раскрывается подпунктом, ссылок на легаси-доску с главной нет.
...
**X3 «главная-кокпит» (той же датой, вариант B плана).** Первый экран кабинета
перестал быть набором плиток и стал РАБОЧЕЙ ПЕТЛЁЙ архетипа — как у вертикальных
лидеров (Fresha/TravelLine/Smoobu): отель открывается в Belegungsplan, салон — в
дневной план записей, пекарня — в очередь заказов, Handwerker — в доску заявок.
**X3-1 (рефактор с паритетом):** тело поверхности продаж извлечено в
`core/_sales_body.html`, построение контекста — в `sales_page.body_context()`;
характеризационные замки поставлены ДО извлечения (4 сочетания kind×view), чтобы
петля не форкнулась в копию. **X3-2:** главная зовёт те же функцию и партиал;
виджеты + присутствие сжаты в ОДНУ полосу над петлёй; удалены три блока, которые
показывали те же сделки другими словами (отдельная сводка «Heute», CTA-баннер
календаря, второй канбан «Tasks»). Пустая петля даёт честную подсказку со ссылкой
на «Funktionen», а не мёртвый экран. Стенд Playwright 12/12: работа дня начинается
на 335-м пикселе (было ~2 экрана скролла), плиток-дублей нет.
**Известное ограничение (унаследовано, не введено X3):** petля идёт по
`primary_module` (порядок `_PRIORITY`), поэтому у тенанта с ВСЕМИ включёнными
модулями первым становится events — на реальных архетипных наборах этого не
происходит. Урок тестам: `disabled_modules=[]` — нежизненный набор, для проверок
петли использовать `default_disabled_for(<тип>)`.
**X3-доводка (той же датой):** полоса дня стала отвечать на вопрос, а не просто
показывать число. (1) У выручки — сравнение «▲/▼ N % vs. Vorwoche» (паттерн SumUp);
считается ОДНИМ запросом на 14 дней, вторая неделя — база сравнения. (2) При
выключенном модуле finance виджет раньше просто ИСЧЕЗАЛ — а он выключен по
умолчанию у ВСЕХ 15 типов, то есть свежий владелец не мог узнать, что выручку надо
включить (дефект «сценарий 8 обрывается» из исследования §3). Теперь на его месте
честная карточка «Umsatz — Finanzen aktivieren» со ссылкой в «Funktionen».
Замки: сравнение недель + наличие карточки активации; стенд подтвердил на демо.
**Урок X3 (красный CI дважды подряд):** в X3 строка присутствия была сокращена
(«WhatsApp-Nummer fehlt — jetzt hinterlegen» → «WhatsApp-Nummer fehlt»), а msgid в
каталоги не добавлен — гейт `i18n_gap` уронил ОБА прогона. Локально этот гейт
не запускается (в дев-контейнере нет gettext/xgettext), поэтому класс ловился
только на CI. Добавлен `scripts/i18n_quickcheck.py` — разбирает `{% trans %}`/
`{% blocktrans %}` регулярками и сверяет с `.po` БЕЗ gettext; повторяет
нормализацию Django (одиночный «%» → «%%», `{{ x }}` → `%(x)s`) — на этой детали
он сразу поймал сам себя. Не замена CI-гейта (не видит Python-строки и плюралы),
...
**X5 «Настройки: два ряда, без телепортов, секции» (2026-08-19, план
`docs/x5-settings-plan-2026-08-19.md`, БЕЗ миграций).** Пятая волна программы
cabinet-cleanup. **X5-1:** таб-бар настроек имел ДЕВЯТЬ главных вкладок в
`overflow-x-auto` — «Abo & Rechnung» и «Team & Zugriff» не были видны без
горизонтальной прокрутки. Введено поле `NavEntry.group`; главные вкладки хаба с
группами рендерятся двумя подписанными рядами (**Basis**: Mein Geschäft ·
Sprachen · Zahlung & Lieferung · Benachrichtigungen; **Verwaltung**: Recht &
Steuern · Abläufe · Integrationen · Abo & Rechnung · Team), прочие хабы —
прежней плоской строкой (замок). Попутно поле `sidebar` стало тристейтом
(`None` = «как advanced»), чтобы «Integrationen» был одновременно вкладкой ряда
и подпунктом сайдбара (вход с первого экрана дал X2a — терять нельзя).
**X5-2:** «Google Bewertungen» был ТРЕТЬИМ входом в тот же экран (карточка
Integrationen существует с GK-11) → запись стала `palette_only`. **X5-3:**
«Abläufe» — одна страница с двумя входами, и заход из подпункта «Verkäufe»
телепортировал в настройки (подсветка якоря + чужой таб-бар): причина — обе
записи в реестре, а `ANCHOR_BY_NAV` строится по порядку `ENTRIES` и settings
перезаписывал board. Теперь ссылка подпункта несёт `?from=board`
(поле `NavEntry.query`), вьюха отдаёт `nav_anchor_override="board"`, шаблон
рисует хлебную крошку «← Verkäufe» вместо таб-бара; из настроек — прежний вид.
**X5-4:** «Mein Geschäft» — чипы-якоря секций, соцпрофили выделены из блока
контактов, sticky-Save, и честная подсказка «слоты записи идут от рабочего
времени ресурсов, а не от этих часов» со ссылкой на `booking:resources`
(владелец правил часы и не понимал, почему на витрине «keine freien Termine»).
Инвариант W0 проверен замком: все поля формы по-прежнему в DOM.
Замки: `test_x5_settings` (6) + 5 msgid × 5 каталогов.

**X6 «Консистентные механики» (2026-08-19, план cabinet-cleanup §6.A6, БЕЗ миграций).**
Шестая волна программы. **X6-1 конвенция «＋»:** у услуги и номера своей страницы
создания нет — форма живёт в подвале списка, и «＋ Neu» с «Sortiment» высаживал
владельца в список искать её глазами. Теперь ссылка несёт `?neu=1#neu`, список
раскрывает форму (у услуги появился тот же ＋-раскрыватель, что был у номера).
**X6-2 поиск:** на «Sortiment» — единое поле по ВСЕМ kind сразу (владелец ищет
«Haarschnitt», не зная, товар это или услуга; фильтр в БД по полям из реестра,
i18n-оверлеи через тот же хелпер, что у витринных фасетов UB2-2); в generic-Liste
продаж — поиск по коду сделки и клиенту (был только у заказов, W10-3; у всех шести
kind есть `reference_code` и FK `customer` — условие общее). **X6-3 ловушка
черновика:** статус акции показывался сырым кодом «draft», кнопки переходов были
равнозначными, а подписи переходов вообще голыми английскими литералами
(«Activate») — на немецком экране печатались как есть. Теперь: подписи статусов и
переходов через реестр (13 msgid × 5 локалей), предупреждение «Kunden sehen diese
Aktion noch nicht», ПЕРВИЧНАЯ кнопка «Jetzt aktivieren», в списке — бейдж «nicht
sichtbar» и человеческий фильтр статусов. FSM не тронут. **X6-4 часы ×1:** движки
не сливаем (расписание per-resource by design) — взаимные ссылки «часы бизнеса ↔
рабочее время ресурсов» с обеих сторон. **X6-5 порядок видов:** Kalender · Board ·
Liste одинаково на всех вкладках (раньше order шёл board·liste·kalender, ticket —
liste·board, и переключатель «прыгал» при смене вкладки); дефолтный вид — отдельная
ось, не тронут. **X6-6:** вкладка «Preis & Lager» формы товара наконец СОДЕРЖИТ
цену (жила в «Basics»); из «Basics» ведёт кнопка, поле остаётся в DOM (W0).
Замки: `test_x6_mechanics` (6). **Уроки среды:** (1) красный CI #2060 — гейт
свежести `app.css`: X5 добавил Tailwind-классы, а пересборка не была сделана
(правило CLAUDE.md существует — исполняем в каждом шаблонном батче); (2)
`i18n_quickcheck` шумел на строках с литеральным «%»: и `{% trans %}`, и
Python-строки проходят через templatize, который пишет в .po «%%» — нормализация
добавлена, плюс пропуск plural-блоков.

**X2c «Легаси-поверхности: заявки» (2026-08-19, ревизия плана
`docs/x2-legacy-surfaces-plan-2026-08-19.md §4`, БЕЗ миграций).** Отложенный
владельцем инкремент, разблокирован закрытием X3/X4. **Ревизия по итогам X4:**
волна X4 сделала событие СУЩНОСТЬЮ раздела «Sortiment» (билет — сделка), поэтому
редирект `events:list` → Verkäufe удалил бы единственный экран управления
событием (создание, вместимость, Teacher). Решение: у событий 302 НЕ делаем —
снимаем ложную плашку «Alte Ansicht» и даём честный кросс-вход «Verkäufe:
Tickets»; схлопываем только заявки, где `jobs:list` действительно был второй
поверхностью СДЕЛОК. **Паритет ДО схлопывания (правило W10-6):** (1) фильтр
статуса в generic-Liste — сделан ОБЩИМ для всех шести kind
(`transactions.status_filter_options` поверх реестра статусов: встроенные +
кастомные владельца, с его подписями), раньше он был только на легаси-странице
заявок и в списке заказов; (2) «＋ Neue Anfrage» — новый экран `jobs:new`
(конвенция X6-1), куда переехал ПРИЁМНИК ручной заявки (форма — общий партиал,
разметка 1:1); (3) панель «⚙️ Anfrage-Formular» переехала на «Abläufe»
(настройки процессов продаж) вместе с redirect'ом своего POST. **Затем:**
`jobs:list` → 302 на `?tab=job` с GET-carry, `jobs/list.html` удалён, все входы
перецелены (деталь заявки, удаление, CTA онбординга `_OFFER_CTA` — теперь ведёт
туда, где создание ВОЗМОЖНО). Замки: 3 новых в `test_x6_mechanics` + осознанные
переписки `test_manual_request_creates_job` / `test_list_renders_and_filters`
(→ `test_legacy_list_redirects_to_sales_tab`) / панель-замок переехал в
`ablaeufe_view`; инвариант X7.3 поймал новый экран `jobs:new` — внесён в
категорию [＋]. 2 msgid × 5 каталогов.

**X8 «Ctrl+K по ДАННЫМ» (2026-08-19, план `docs/x8-palette-data-plan-2026-08-19.md`,
БЕЗ миграций) — ПРОГРАММА «КАБИНЕТ-X» ИСПОЛНЕНА ЦЕЛИКОМ.** Сценарий №1
микро-владельца — «найти заказ фрау Мюллер», а не «открыть раздел»; палитра W8
искала только названия разделов. Новый `apps/core/palette_search.py` — ТОНКИЙ слой
поверх уже существующих и гейтнутых поисков (сделки — `_managed_queryset(kind, q=)`
X6-2 по видимым kind; клиенты — фильтр списка CRM; предложения —
`sellable_manage_sections_for(q=)`), новых способов ЧТЕНИЯ данных не заведено, гейты
модулей и изоляция схемой наследуются. Эндпоинт `/dashboard/palette-search/`
(`@login_required` — класс дефектов X0), лимиты 5/секция и 20 всего, запрос < 2
символов не ходит в БД, секция-исключение не валит палитру. Клиент: секция
«Gefunden» НАД пунктами меню, debounce 250 мс, отмена предыдущего запроса
(AbortController), fail-safe — при сбое сети остаётся прежний навигационный поиск.
Замки: `test_x8_palette_data` (7, включая «аноним получает 302, а не данные» и гейт
модуля CRM); инвариант X7.3 поймал новый url — внесён как служебный JSON.
Стенд Playwright (friseur, реальный fetch): «Haarschnitt» → секция Sortiment,
«Klein» → секция Verkäufe с кодом сделки и именем клиента, короткий запрос секцию
не показывает, навигационный поиск жив.

**Пост-программная сверка кабинета (2026-08-19, после X0–X8; БЕЗ миграций).**
Девять волн меняли навигацию, гейты и редиректы, а стенды гонялись в основном на
friseur/hotel. Проверка: локально засеяны 7 демо-архетипов, покрывающих все
primary-модули (friseur=booking, hotel=stays, handwerker=jobs, werkstatt=booking+
jobs, retreat=events, shop=catalog/orders, restaurant-demo=гастро), и по каждому
пройден обход кабинета **ссылками ИЗ DOM** (то, что реально видит владелец:
якоря, подпункты, вкладки хабов, ящик «Erweitert», кнопки экранов), а не по
захардкоженному списку. **Результат: 232 экрана, 0 битых ссылок** (500/404).
Четыре 404 первого черновика оказались: одна — ошибка ПУТИ в самом стенде
(`/reviews/` вместо `/dashboard/reviews/`), три — корректный гейт выключенных у
архетипа модулей. Два прогона упали по таймауту навигации dev-сервера; замер
серверного рендера `/dashboard/` дал 0.04–0.35 с у тех же тенантов — то есть
среда, а не дефект. **Инвариант закреплён в CI** — `test_nav_reachability`: для
каждого из 16 типов бизнеса (реальный пресет модулей архетипа) каждая ВИДИМАЯ
запись навигации (якорь, подпункт, вкладка хаба, запись палитры) обязана
резолвиться И проходить модульный гейт мидлвари. Это ровно тот класс, что раньше
ловился вручную (W3-3 «Handwerker вёл в пустой каталог», W7b «входы-сироты»):
`url_prefixes` спеки разъезжался с `module_key` записи реестра. Урок среды:
`pgrep`-ожидание в bash матчит СВОЮ КОМАНДНУЮ СТРОКУ — цепочка обхода молча
не стартовала (тот же класс, что известная грабля `pkill -f`).

**Сверка кабинета на ВСЕХ архетипах + два дефекта (2026-08-19, БЕЗ миграций).**
Первый обход (браузером) покрыл 7 типов бизнеса из 16 и упирался в скорость: сев
кита и обход конкурировали за дев-сервер. Инструмент заменён на СЕРВЕРНЫЙ обход
(`scratchpad/sweep_server.py`): тот же принцип — ссылки берутся ИЗ DOM отданной
страницы, — но через `TenantClient` django-tenants (настоящие мидлвари и гейты),
без браузера и статики; 14 тенантов за минуты вместо часов. **Итог: 14 архетипов
из 14 (все, у кого есть демо-кит; `online_shop`/`other` кита не имеют), 979
экранов, 0 проблем.** Обход нашёл ДВА реальных дефекта, которых не видели ни
тесты, ни браузерный стенд:
1. **405 из Ctrl+K** — X4 записал в палитру `jobs:anfrage-form-settings`, а это
   `@require_POST`-приёмник, не экран (его UI переехал на «Abläufe» волной X2c):
   клик по «Anfrage-Formular» отдавал 405. Запись убрана, ключевые слова
   («anfrage-formular felder») отданы записи «Abläufe», в инвентаре X7.3 url
   переклассифицирован в [POST]. Замок обновлён осознанно.
2. **404 на карточке клиента** — `orders/order_detail.html` рисовал «Open customer
   card →» всегда, а модуль CRM выключен пресетом у большинства архетипов →
   переход бился о гейт путей. Ссылка гейтится модулем (fail-closed без тенанта);
   замок `test_order_detail_hides_customer_card_when_crm_off` держит обе ветки.
   Это тот же класс, что `test_nav_reachability`, но для ссылки ВНУТРИ страницы.
**Побочное наблюдение (не дефект кода):** свежезасеянный демо-тенант попадает не
в кабинет, а в мастер — AB5-редирект срабатывает на «нетронутом» состоянии
(`onboarding: null` после сидинга). Для стенда состояние помечалось «тронутым»
локально; нужно ли демо-китам пропускать мастер — вопрос владельцу.

**Демо-киты пропускают мастер (2026-08-19, решение владельца, БЕЗ миграций).**
Ответ на вопрос предыдущей записи: да, пропускают. `apply_kit` последним шагом
зовёт новый `onboarding.mark_complete(tenant)` — состояние v2 пишется как
ПРОЙДЕННОЕ целиком (`done` = все видимые шаги, `skipped` пуст, `completed`), а не
как «тронутое и брошенное» (`leave`): у кита сайт наполнен целиком, и честное
состояние — завершённый мастер. Иначе на главной висел бы чек-лист готовности с
пропущенными шагами. Пометка стоит в `apply_kit`, а НЕ в сидере: единственный
не-тестовый вызывающий — сидер, зато любой будущий вызов (кнопка «взять кит»)
получит то же поведение; `demo.load_demo` (демо-контент ВНУТРИ мастера, шаг 1)
пометку не ставит — иначе владельца выбросило бы из мастера на первом шаге
(замок `test_load_demo_does_not_complete_wizard`).
Попутно закрыт остаток того же дефекта: пункт «часы работы» — единственная
строка чек-листа ВНЕ шагов мастера (считается по факту `tenant.opening_hours`),
и кит `moto` был единственным без часов → готовность демо оставалась неполной
даже с пройденным мастером. Часы добавлены (офис Mo–Fr 10–18, туры сезонно;
строка переведена в 4 демо-словаря), класс закрыт замком «у каждого кита есть
часы». Проверено на живом пересеве: `/dashboard/` демо-тенанта отдаёт 200,
состояние `completed: True`, чек-лист готовности не показывается.
⚠️ ops: уже засеянные демо получают пометку только при
`seed_demo_tenants --kit <кит> --recreate` (без `--recreate` сидер существующий
тенант пропускает).

**Починка по адверсариальному ревью «Кабинет-X» (2026-08-19, ⚠️ миграция
`promotions/0026` choices-only).** Ревью 55 агентами (4 измерения → каждая находка
проверена тремя скептиками) дало 14 подтверждённых находок и 3 отклонённых. Перед
починкой каждый факт перепроверен отдельной разведкой (6 агентов, доказательства
файл:строка), замки написаны ДО правок.

**HIGH — форма товара молча не сохранялась.** X6-6 перенёс `base_price` (единственное
обязательное-и-пустое поле) во вкладку «Preis & Lager», у которой `class="hidden"`.
Django рендерит `required`, у формы нет `novalidate` → Chrome валит constraint
validation на НЕФОКУСИРУЕМОМ контроле: сабмит отменяется без единого видимого
сообщения. Проверено на живой форме до/после (до: панель скрыта, страница не
уходит, консоль «An invalid form control with name='base_price' is not focusable»;
после: панель открывается, товар сохраняется). Правка — в ОБЩЕМ партиале
`tenant/_i18n_switch.html`: `invalid` в фазе capture раскрывает и вкладку, и
языковую группу проблемного поля, плюс то же при серверной ошибке (`data-field-error`).
Инвариант W0 цел: поля остаются в DOM, скрытие только CSS. Класс закрыт для всех
форм с панелями (товар, акция и будущие).

**Корректность кабинета.** (1) Главная показывает тело поверхности продаж, но
глотала её fetch-фрагмент: `?buchung=&box=1` возвращал 58 КБ кабинета вместо
карточки брони — теперь не-dict отдаётся как есть. (2) Фильтр в адресе обязан быть
ИСПОЛНЕН: `?status=`/`?q=` понимает только «Liste», board/kalender их игнорировали
молча (легаси-редирект заявок, виджет «Abholbereit») — `sales_page.view_for_filters`
открывает вид, который фильтр исполняет; то, что вид умеет сам (поиск Belegungsplan
по `?q=`), не перебивается. (3) Мёртвый `heute_columns` на главной (5-6 запросов на
каждый заход) снят — X3 поглотил эту полосу. (4) Чипы «Abläufe» несут `?from=board`,
панель «Anfrage-Formular» возвращает на тот же экран (`next`). (5) Хвостовой ряд
`hub_tabs` забирает вкладки с НЕИЗВЕСТНОЙ группой — опечатка в `group` роняла
вкладку из таб-бара молча.

**Ctrl+K.** «Nothing found.» считалось только по пунктам меню: при запросе «Sabine
Lieb» палитра одновременно показывала найденный заказ и надпись «ничего не найдено»,
а Enter уводил на первый пункт МЕНЮ (проверено: было `/dashboard/`, стало
`/dashboard/orders/<uuid>/`). Подпись поля теперь обещает то, что палитра умеет.

**i18n — класс «строка минует оба гейта».** Псевдонимы `gettext as _t` /
`gettext_lazy as _l|_lazy` не извлекаются makemessages (Django передаёт xgettext
только канонические keywords), поэтому 23 строки в 4 файлах молча жили без перевода
— и CI-гейт их не видел ПО ПОСТРОЕНИЮ. Псевдонимы сведены к каноническим именам
(`_` и полное `gettext_lazy`), 23 msgid добавлены в 5 каталогов с переводами,
класс закрыт скан-замком «псевдонимов gettext в исходниках нет». Попутно: счётчик
непрочитанных переведён на СУЩЕСТВУЮЩИЙ plural-msgid (у ru/uk 4 формы), снят дубль
msgid, различавшийся лишь именем плейсхолдера.

**Статусы акций.** Подписи жили словарём во вьюхе (X6-3), поэтому каждый новый экран
печатал сырой код — так и вышло с аналитикой (`{{ r.promo.status }}` → «draft»).
Источник подписей перенесён в модель (`Promotion.STATUSES`, AlterField choices-only,
DDL не порождает), аналитика и тип акции — через `get_status_display`; скан-замок
запрещает печать сырых кодов в шаблонах акций.

**Из отклонённых — одна правка всё же сделана:** скептики верно сняли «открытый
редирект» на двух панелях (не новизна диффа + недостижим без CSRF), но нашли
достижимый вариант того же гейта: у `status_manager_save` значение `next` приходит
ИЗ GET-ссылки, то есть `//evil.tld` срабатывал одним кликом «Save». Гейты в четырёх
местах теперь отбивают протокол-относительный URL, замок держит класс.

Замки: `apps/core/tests/test_cabinet_review_fixes.py` (20) + стенд Playwright
(10 проверок, форма товара и палитра на демо `shop`).

## 2026-08-20 — VK-волна: фидбэк владельца по разделу «Verkäufe» (10 пунктов)

Запрос владельца: «пройдёмся по настройке каждого раздела; берём раздел продажи».
План — `docs/verkaeufe-feedback-plan-2026-08-20.md`. Всё БЕЗ миграций.

**п.1 «Очень много свободного места».** Колонки канбана были фиксированной ширины
(`sm:w-64`): три колонки занимали 768 px на экране 1900 px, дальше пустота. Теперь
тянутся (`sm:flex-1 sm:basis-0`), а `sm:min-w-[15rem]` сохраняет прежнюю
горизонтальную прокрутку при 5+ колонках; мобильный snap-скролл (85vw) не тронут.
Хром страницы: заголовок и вкладки сведены в один ряд, полоса-разделитель над
«Abgeschlossene anzeigen» рисуется только когда в ряду ЕСТЬ табы секций (на
Verkäufe и главной секция одна) — до первой карточки было четыре ряда, стало два.

**п.2 кнопка настроек раздела.** SM-4 увёл «Abläufe» из тулбара в подпункт
сайдбара — и владелец перестал находить настройки. «⚙️ Einstellungen» вернулась в
тулбар вкладки, несёт АКТИВНЫЙ kind и `from=board` (крошка возвращает на ту же
вкладку). Замок SM-4 переписан осознанно.

**пп. 3/4/5 «настройки вообще не понятны» + «Eigene Status запрятан».** Страница
«Abläufe» перестроена: сначала **карта процесса** («So läuft ein Verkauf») —
колонки доски со статусами, которые в них попадают (встроенные ∪ свои, с учётом
своих имён FB-4a и переименования/скрытия колонок W5), затем разделы-карточки с
объяснением. Редактор СВОИХ статусов (был ссылкой внутри свёрнутого `<details>`
внутри панели переходов) вынесен партиалом `tenant/_status_manager_panel.html` и
живёт секцией прямо на странице; отдельный экран `status-manager` остаётся для
deep-link'ов и палитры. Панель переходов с пустым набором правил больше не
исчезает молча (у `reservation` скрывать нечего — экран выглядел сломанным):
честная фраза «здесь нечего скрывать». Подпункт сайдбара переименован в
«Abläufe & Status».

**п.6 «как в Trello».** Колонки настраиваются перетаскиванием: ручка ⠿ (HTML5 DnD,
как в билдере), инлайн-имя, 👁 скрыть. Порядок едет в hidden-поле и
перенумеровывается на drop (было — поле ввода числа 1..9). БЕЗ JS остаются
переименование и скрытие; порядок просто не меняется — потери данных нет.

**п.7 триал-плашка.** «Тест: осталось 13 дней» висела на КАЖДОМ экране. Теперь —
всплывающий тост ОДИН раз за сессию (флаг ставит `billing.context.subscription`) +
постоянная плашка только на «Abo & Rechnung». Два гейта, без которых это стало бы
багом: `fetch`-фрагменты и POST не сжигают единственный показ, а незалогиненный
посетитель не получает записи в сессию — процессор глобальный, и на витрине это
завело бы куку каждому (принцип «без трекинг-кук на витрине»).

**п.9 «в предварительной при указании числа оно пропадает».** Смета
(Kostenvoranschlag): `value="{{ line.qty }}"` в `input[type=number]` — в немецкой
локали Decimal рендерится как «2,50», а `type=number` принимает ТОЛЬКО точку →
браузер показывал поле пустым, и сохранённое количество выглядело потерянным.
Фикс — `floatformat:"-2u"` (неломающееся «1» / «2.5»). Второй способ потерять
число: строка с ценой, но без описания молча отбрасывалась на сервере — теперь
владелец получает сообщение с номером строки.

**п.10 итог позиции.** В строке сметы появилась колонка «Summe» (сервер рендерит
сохранённое, JS считает на вводе) + живой «Zwischensumme (netto)» рядом с Save.
Тот же живой итог добавлен в композер Angebot (inbox) — там суммы не было вовсе.

**п.8 «нужны ли Reservierungen».** Владелец спросил: «разве не должен он попадать
в классический заказ даже по акции?» — разбор кода показал, что УЖЕ попадает: с
волны PL кнопка акции ведёт в `promotions.services.purchase` → обычный `Order`
(услуга → запись, номер → бронь), а форма резерва снята со всех витринных
поверхностей (`storefront-reserve` не упомянут ни в одном шаблоне — эндпоинт
зовут только тесты). Значит `Reservation` — легаси, и вкладка была пустой у всех,
кроме владельцев старых записей. Сделано: вкладка `reservation` — единственное
исключение из правила SM-2a, показывается только при наличии таких записей;
подпись направления «Aktions-Reservierungen (alt)»; продажа по акции ПОМЕЧЕНА
там, где живёт — бейдж «🏷 Aktion» на карточке доски и в списке заказов
(`Order.source_channel="promo"`, без лишних запросов). Замок SM-2 переписан
осознанно.

Замки: `apps/core/tests/test_verkaeufe_feedback.py` (7), `apps/billing/tests/
test_context.py` (+3), `apps/jobs/tests/test_cabinet.py` (+3); осознанно переписаны
`test_verkaeufe::test_toolbar_lives_inside_the_tab_not_above` и
`test_w9_ablaeufe::test_ablaeufe_renders_all_three_panels`. 17 msgid × 5 каталогов.

## 2026-08-20 (вечер) — VS-2: главная рабочая страница продаж

Владелец: «это главная страница админки, наряду с каталогом, должна быть хорошо
продумана». План — `docs/verkaeufe-v2-plan-2026-08-20.md`, решения §2. БЕЗ миграций.

**VS-2a.** У 13 из 16 типов бизнеса направление продаж ровно одно — ряд вкладок из
одного элемента убран, заголовок страницы называет само направление («Aufträge»).
При двух+ (отель, Werkstatt) ряд остаётся и несёт число АКТИВНЫХ сделок (не
терминальные по реестру статусов, один COUNT на направление, fail-safe).

**VS-2b (решение владельца: «колонка = статус, но визуально связать колонки одной
стадии»).** Колонки строятся из `status_registry.all_descriptors` (встроенные ∪
свои, SM-3), стадия стала ГРУППОЙ: полоса «NEU · IN BEARBEITUNG · FERTIG» над
своими колонками; ширина группы = flex-grow по числу колонок (иначе стадия с одной
колонкой забирала треть экрана — поймано на стенде: 421px против 208px). У
кейтеринга «Angebot gesendet» (ждём клиента) и «Beauftragt» (подтверждено) больше
не лежат в одной колонке. Побочно drag стал ОДНОЗНАЧНЫМ: зона несёт целевой статус
(`data-drop-status`), а не стадию — раньше JS выбирал «первый допустимый переход в
стадию». Настройки колонок (W5) продолжают управлять группами — `normalize_board`
и его golden не тронуты; имена колонок = «Status-Namen» (FB-4a). Терминальная
ГРУППА скрывается прежним тумблером «Abgeschlossene anzeigen».

**VS-2c.** Карточка отвечает «когда это и на сколько»: дата события + гости
(`Job.event_date`/`guest_count` волны AF-1 наконец видны), клиент · сумма, срок
сметы бейджем (просрочка — красным, считается НА СЕРВЕРЕ: карточку перерисовывает
`kanban_action` со своим контекстом, переменная `today` из шаблона доски туда бы не
доехала). Доска сортируется по дате события (ближайшее сверху, `nulls_last` —
сделки без даты в конце). В шапке колонки — сумма, но ТОЛЬКО там, где поле суммы в
БД = точный итог сделки (order/job/stay); у booking/ticket итог считается в Python
(цена+extras−скидка), и агрегат по `price_cents` врал бы — там суммы просто нет.

**Замок поймал реальный баг:** `values().annotate()` поверх queryset'а с
`order_by` тащит поля сортировки в GROUP BY — агрегат дробился на строку-на-запись
(счётчики спасало сложение, а сумма перезаписывалась и показывала цену ПОСЛЕДНЕГО
заказа). Фикс — `.order_by()` перед `values()`.

Замки: `apps/core/tests/test_verkaeufe_v2.py` (11); осознанно переписаны
`test_transactions::test_manage_sections_lists_active_transaction_modules`
(колонки = статусы, стадии = группы) и замок ширины колонок VS-2. Стенд Playwright
13/13 на демо кейтеринга (включая drag new→quoted с сохранением на сервере и
отбитый недопустимый переход). 6 msgid × 5 каталогов (в т.ч. plural «%(n)d Gast»).

## 2026-08-20 (ночь) — VS-3: «якорь + прикреплённые сделки» (⚠️ миграция `core/0008`)

Запрос владельца: «к номеру/брони привязываются услуги… они должны двигаться по
колонкам отдельно». Решение по объёму — «сначала кабинет». Спецификация —
`docs/vs3-deal-links-plan-2026-08-20.md` (сценарии по архетипам, обоснование
модели, семь инвариантов, границы v1).

**Чего не было:** Extras («велопрокат» галочкой) — JSON-СНИМОК внутри брони
`[{label, price_cents}]`: ни статуса, ни карточки, ни колонки, двинуть по доске
нельзя. Единственная настоящая связь сделок — `Job.booking` (A7d).

**VS-3a.** `core.DealLink(anchor_kind, anchor_id, child_kind, child_id, note)` —
одна TENANT-таблица вместо полей на шести моделях (одна миграция, симметричные
запросы, ноль правок в движках). Не FK/GFK: якорь и спутник — разные модели, а
GenericForeignKey тянул бы джойн `contenttypes` на каждую карточку доски; приём
generic-ссылки в проекте принят (`reviews`, `inbox`). `unique(child_kind,
child_id)` — у спутника ОДИН якорь, перепривязка = перенос. Сервис
`apps/core/deal_links.py`: attach (идемпотентен; отбивает self-link, цикл A→B→A и
второй уровень), detach, anchor_of, children_of, `for_cards` (батч) и
`block_context` (справочный итог спутников).

**VS-3b.** Карточка доски: у якоря «🔗 N Zusatzleistungen», у спутника
«🔗 gehört zu <якорь>» со ссылкой. Данные приходят в контракте `Transaction`
(`linked_children`/`linked_anchor`) ОДНИМ батчем на секцию — тот же паттерн, что
у problem-полосы LS-6; замок сравнивает число запросов при 1 и 5 связях.

**VS-3c.** Блок «Verknüpfte Leistungen» на четырёх деталях (номер/запись/заказ/
заявка) + экран `/dashboard/verknuepfen/<kind>/<pk>/` с двумя ролями («эта
относится к…» / «прикрепить к этой…»), пометкой «bereits verknüpft» и отвязкой.
Нарушение правила объясняется человеку сообщением, а не 500.

**Контракт += `amount_value`** — сырая сумма рядом с готовой строкой: справочный
итог спутников считается по числам, а не разбором «85,00 €» обратно.

**Стенд вскрыл настоящий пробел (не в связях, а в поиске):** владелец ищет
сделку по тому, что видит на карточке («Fahrrad»), а поиск умел только код и
клиента. `_managed_queryset` расширен полями заголовка per-kind
(`resource__name`/`service__name`/`unit__name`/`title`/`event__title`) —
выигрывают сразу палитра Ctrl+K, список продаж и пикер привязки. Резерв не
трогаем: `Promotion.title` — JSONField, `icontains` по нему в Postgres не работает.

**Демо hotel:** к первой брони номера привязан «Fahrradverleih» со СВОЕЙ ценой
24,00 € (стенд поймал, что бесплатная услуга делает справочный итог пустым и
демо неубедительным).

Замки: `apps/core/tests/test_deal_links.py` (24). Стенд Playwright 9/9 на демо
отеля: привязка из кабинета → обе стороны на доске → «bereits verknüpft» →
отвязка. Уроки стенда: `runserver --noreload` не подхватывает правки Python
(перезапускать); стенд, который МУТИРУЕТ данные, обязан сам создавать своё
состояние, иначе второй прогон красный.

## 2026-08-20 (ночь) — QF: фидбэк по странице заявки/сметы (6 пунктов, без миграций)

**п.1 «зачем так много пустых ячеек».** Смета всегда рисовала 12 пустых строк —
стена полей. Теперь видны заполненные + ОДНА свободная, остальные лежат в DOM
(индексы приёмника не меняются) и раскрываются кнопкой «＋ Position hinzufügen».
Грабля: атрибут `hidden` не сработал — класс `.grid` (display:grid) его
перебивает; скрытие держит инлайновый `display:none`, JS снимает оба. Поймал
стенд («видно 12 из 12»), серверный тест этого не видел.

**п.1 «товар/услугу».** Пикер строки умел только товары/варианты каталога — к нему
добавлены УСЛУГИ (`s:<pk>`, гейт по модулю booking). У услуги нет FK: в смету
едет СНИМОК названия и цены — правка каталога услуг не переписывает отправленную
смету (та же логика, что у товаров).

**п.5 «колонка Teil (Katalog) не заполнена».** Выбор из пикера теперь СРАЗУ
заполняет описание и цену строки (data-атрибуты опции, сервер всё равно делает
свой снимок) — колонка перестала выглядеть пустой и непонятной.

**п.2 «выровни снизу пункты».** Нижний ряд был `flex items-end`: у одного поля
подпись в две строки, у другого подсказка снизу — контролы стояли на разной
высоте. Стал сеткой (до 5 колонок), кнопка сохранения и живой итог — отдельной
строкой под ней. Попутно: «Fahrzeug/Kennzeichen» и «Nächster TÜV» — поля
МАСТЕРСКОЙ, у кейтеринга они были шумом; показываются нужным архетипам и всегда,
если данные уже заполнены. `service_due_date` получил presence-guard — без него
сохранение сметы у кейтеринга стирало бы дату мастерской (класс W0).

**п.3 «информация о клиенте очень мелкая».** Имя — крупно, e-mail и телефон —
кликабельными ссылками (`mailto:`/`tel:`), событие (тип · дата · гости) —
цветной плашкой. Отсутствие контактов названо прямо, а не пустотой.

**п.4 «редактировать инфо о клиенте со страницы заказа».** Форма прямо на
странице (`action=customer`): имя/e-mail/телефон + адрес объекта. Честно
предупреждаем, что правится ОБЩАЯ карточка клиента — та же, что в CRM и в других
его сделках.

**п.6 «сверху есть немного места».** Отступы шапки и карточки поджаты
(заголовок с 156 px до 106 px на чистом заходе).

Замки: `apps/jobs/tests/test_cabinet.py` (+6, включая инвариант W0 для скрытых
полей мастерской). Стенд Playwright 10/10 на демо кейтеринга.

## 2026-08-20 — Волна SH «заказ, клиент, деньги» (фидбэк владельца по shop/retreat)

18 пунктов фидбэка + требование «проверяй остальные архетипы на похожие
проблемы». План и решения — `docs/sh-order-wave-plan-2026-08-20.md`.

### Группа A — быстрые (без миграций)

**п.1 «календарь не нужен, главная страница продаж — список».** Вид «Kalender»
у заказов — это Auftragsbuch по дате выдачи (V3). У магазина слоты выдачи не
используются, и вкладка была вечно пустой. Гейт по ДАННЫМ, а не по архетипу:
`views_for("order")` прячет календарь, пока ни у одного заказа нет `pickup_slot`
(у пекарни C&C слоты есть — там вид остаётся).

**пп.11/12/16 строка списка.** Порядковый номер, покупатель отдельной строкой,
мельче шрифт и меню смены статуса ПРЯМО в строке (`core/_row_status_menu.html`).
Приёмник общий — `board-action` → `transactions.apply_action`, поэтому письма,
склад и деньги идут штатным путём, а правила FSM остаются жёстким полом. Строка
перестала быть одной большой ссылкой (вложенные кнопки в `<a>` недопустимы).
Механика общая для всех направлений: generic-список stay/booking/job/ticket
получил её тем же партиалом, у заказов — свой богатый ряд с теми же элементами.

**п.16 канбан.** Заголовок карточки 13px, детали 11px — на доске помещается
заметно больше сделок.

**п.10.** Кнопки смены статуса переехали в шапку карточки заказа, в одну строку
с номером; нижняя карточка «Actions» удалена как дубль.

**п.13.** На «Abläufe» появилась карточка «Zahlung & Lieferung» со ссылками в
настройки — копию формы не заводим, единственный писатель остаётся один (W9-5).

**п.17 свёрнутое меню.** Было `display:none` — сайдбар исчезал целиком. Стал
полосой иконок (3.5rem): скрываются только подписи, CSS-ом (инвариант W0),
поэтому подсветка «где я» и поиск меню работают без изменений; названия остались
в `title=`. Кнопка свёртки стала тумблером, плавающая «☰» больше не нужна.

**п.18 «хлебные крошки под Проживанием».** В телах Belegungsplan и Tagesplan под
заголовком страницы висел ВТОРОЙ `<h1>` и ряд ссылок-дублей сайдбара. Убраны.
Замки `test_price_nav`/`test_verkaeufe` поймали, что вместе с дублями уезжают два
входа, которых в сайдбаре НЕТ («Heute» и «Preise & Saison» — последний появился
по фидбэку 2026-07-30 «непонятно, где календарь цен»); они оставлены компактной
строкой. Полезный случай, когда замок отличил дубль от единственного входа.

### Группы B/C/D — правка заказа, налоги, оплата (⚠️ 3 аддитивные миграции)

**Правка заказа (пп. 2/3/5/6/7).** Новый `apps/orders/editing.py`: количество
(0 = удалить позицию), добавление позиции из каталога или свободной строкой,
скидка владельца, доставка, контакт клиента. Решение владельца — «править
всегда, склад пересчитывается тем же движком» — исполнено буквально: остаток и
леджер двигаются в одной atomic (правило UD3-2), нехватка остатка бросает
`OutOfStock` и откатывает правку целиком, а ЗАКРЫТЫЙ заказ не правится вовсе —
его остаток уже вернул FSM, и повторное движение задвоило бы склад. Движения
правки пишутся БЕЗ `source_ref`: `record_movement` идемпотентен по
(source, ref, kind), и второе такое же изменение количества молча потерялось бы.

**Налоги (пп. 3/4).** `Product.vat_rate` (форма даёт ВЫБОР 19/7/0 — свободное
число дало бы опечатку «1,9») и СНИМОК `OrderItem.vat_rate`: смена ставки в
каталоге не переписывает уже выставленный документ. Цены в проекте брутто
(PAngV «inkl. MwSt.»), поэтому НДС не доначисляется, а выделяется:
`apps/orders/totals.py` раскладывает итог по ставкам, скидка уменьшает базу
пропорционально долям, доставка идёт по максимальной ставке заказа (правило
Nebenleistung), § 19 Kleinunternehmer обнуляет всё. Один хелпер — карточка,
счёт и будущие письма/PDF считают одинаково.

**Номера и оплата (пп. 8/9).** `Order.external_code` — внешний номер (касса,
маркетплейс, бумажный журнал; решение владельца), ищется поиском сделок наравне
с нашим кодом, поэтому находится и палитрой Ctrl+K. Карточка «Zahlung»: способ
оплаты, плательщик и адрес для счёта (§14 UStG требует реквизиты получателя
СЧЁТА, а не того, кто забирает товар), кнопка «Rechnung aus Auftrag erstellen» →
`finance.services.invoice_from_order` создаёт ЧЕРНОВИК; нумерация и
неизменяемость по-прежнему наступают при выставлении (GoBD цела).

**Клиенты и доставка (пп. 14/15).** Решение владельца — «вкладка в Продажах
всем», а НЕ «включить CRM всем»: вкладка «Kunden» стоит справа от ряда
направлений и в него не входит (иначе одинокое направление снова получило бы ряд
вкладок, VS-2a). Тело вкладки — тот же партиал и тот же контекст, что у экрана
CRM (`crm/_customer_list_body.html` + `customer_list_context`), поэтому
расхождению данных взяться неоткуда. Без модуля CRM список читается, но ссылок в
`/crm/` нет — гейт путей дал бы 404 (класс дефекта из сверки 2026-08-19), вместо
кнопок стоит честная строка «карточка/теги/история — в модуле». «Lieferungen» —
не новый экран, а фильтр списка заказов (`?versand=1`, `fulfillment=delivery`:
`is_delivery` — свойство, не поле), включённый в `_VIEW_FILTERS`, поэтому адрес с
ним ИСПОЛНЯЕТСЯ, а не игнорируется молча. Оба входа — подпункты сайдбара под
«Verkäufe»; дедуп подпунктов стал по паре (url_name, query) — по голому url_name
второй пункт на ту же страницу молча исчезал бы.

**Кросс-архетипная часть.** Правка КЛИЕНТА сделана kind-агностичной
(`deal-customer-edit` + общий партиал) и подключена к карточке брони номера и
карточке записи; у заявки она уже была (QF-4). Правка СОСТАВА осталась заказу:
у брони «состав» — это даты/номер/гости (своя форма), у записи — время и услуга;
общего «состава» у них нет, обобщать было бы вредно.

Пикер позиций вынесен в `apps/catalog/picker.py` — у сметы и у заказа один
список «что можно добавить» (у сметы он появился в QF-1).

**Демо.** У спеки позиции появился ключ `vat`; в ките «Café Morgenrot» еда идёт
7 %, напитки 19 % — владелец видит смешанный чек, как в жизни.

⚠️ Миграции: `catalog/0029` (Product.vat_rate), `orders/0018`
(OrderItem.vat_rate), `orders/0019` (external_code + billing_*). ops после
деплоя: `seed_demo_tenants --kit cafe --recreate`.

Замки: `apps/orders/tests/test_order_editing.py` (23), `apps/core/tests/
test_sh_orders.py` (14). Осознанно переписаны: `test_order_detail_shows_items_
and_actions` (позиции стали редактируемыми — добавлен парный замок «закрытый
заказ read-only»), VS-2a-замок ряда вкладок, состав подпунктов сайдбара.

Уроки: (1) два pytest на одной `--reuse-db` роняют друг друга («relation …
already exists») — прогоны только серийно; (2) новое ОБЯЗАТЕЛЬНОЕ поле формы
товара ломает все чужие POST-и (мастер/импорт/тесты) — поле сделано
необязательным с фолбэком на прежнее значение.
## 2026-08-21 — MX-0: денежная гигиена + id опции (фундамент программы MX, БЕЗ миграций)

Старт исполнения программы MX (стратегия `mx-anchor-strategy` + детализация
`mx-anchor-details` §5b «опция = цена + трекер», классификация проверена владельцем;
мастер-план `mx-execution-plan-2026-08-21.md`). Сделано:

- **id опции в снимках** трёх видов: `extras.snapshot` → `{id, label, price_cents,
  unit_cents, per_night}` (line-total остаётся в price_cents — потребители целы),
  модификаторы заказа и состав набора → `{id, …}`. Немецкая метка перестаёт быть
  учётным ключом — фундамент сводного экрана «Zusatzverkäufe» (MX-2) и закупки от
  опций (MX-4). Легаси-снимки без новых ключей валидны (fail-safe).
- **Три денежных фикса `move_stay(reprice=True)`**: (а) per-night допы пересчитываются
  новым `extras.retotal` — продление 2→5 ночей давало 530 € вместо 575 € (доказано
  тестом до фикса); (б) `auto_discount` зовётся теми же аргументами, что `book_stay`
  (unit/departure/extra из `stay_promo`) — правка дат больше не теряет Lücken-Deal и
  скидку акции при живом FK `promotion`; (в) неоплаченная процентная предоплата (G7,
  `payment_state=PENDING`) следует за новым итогом; оплаченный депозит не трогаем
  (доплата — MX-3).
- **Освобождение койки — единый узел** `events/services.py::release_linked_stay`:
  вызов из `TicketSM.on_transition(cancelled)` + зеркало кастом-статусов в
  `status_effects.restore_stock_for`. До фикса освобождали только ДВА вьюха-хелпера
  (`_cancel_linked_stay` ×2, сырой save мимо FSM) — отмена билета с ЕДИНОЙ ДОСКИ
  (`apply_action`/drag/кастом-статус) оставляла койку занятой (латентный oversell
  ретрита, замок был красным до фикса). Хелперы удалены; письмо гостю от companion-
  брони НЕ шлём осознанно (билет сам шлёт отмену).
- **Карточка записи в кабинете** показывает итог С доп-услугами (`total_eur` вместо
  `price_eur`) + список допов — владелец видел заниженную цену (находка ревью OPT-7).

Замки: `test_mx0_release` (3; board-путь красный до фикса), `test_mx0_money` (5),
обновлены `test_extras`/`test_modifier_flow` (снимок с id). Гейт: stays+events 583,
orders/catalog/booking смежные 51, ruff, i18n_quickcheck, template_comments.
Грабля среды повторилась: 14 «падений» events/storefront — протухшие локальные .mo
(лечится polib-компиляцией; на CI нейтрально).

## 2026-08-21 — MX-1: расходы → раздел ERP (⚠️ миграция `finance/0008`, аддитивная)

- **`ExpenseEntry` += `ref_kind`/`ref_id`** — generic-объект затрат (сделка любого
  kind / опция / пусто = общий расход); `event` FK остаётся легаси-полем туров.
- **Приёмка закупки пишет расход**: `receive_po_line` → `ExpenseEntry(source=purchase,
  source_ref=f"{position.pk}:{qty_received}")` — объявленный и мёртвый
  `SOURCE_PURCHASE` заработал; частичные приёмки = отдельные записи, идемпотентность
  по unique(source, source_ref); нулевой EK записи не плодит.
- **Экран «Ausgaben»** `/dashboard/finance/ausgaben/`: список с фильтрами
  период/категория + ручной ввод (до этого ручного ввода расходов не существовало
  вовсе); удаление — ТОЛЬКО manual-строк (событийные держит идемпотентность).
- **Экран «Ergebnis»**: выручка − расходы за период (первый P&L-срез).
- **Вкладки Finanzen** (Einnahmen · Ausgaben · Ergebnis · Rechnungen) на всех трёх
  страницах — раздел перестал быть «журналом выручки». Якоря сайдбара не тронуты
...
## 2026-08-21 — MX-3 (v1): правка состава доп-услуг из карточки брони и записи (БЕЗ миграций)

Владелец: «опции можно добавить/убрать при оформлении И ДАЛЕЕ ПРИ РЕДАКТИРОВАНИИ».
До MX-3 состав допов не редактировался нигде (OPT-2/3 разведки). Параллельная волна
SH уже закрыла правку ЗАКАЗА (`orders/editing.py`) — объём MX-3 сужен до броней.

- **stays**: чекбокс-блок «Zusatzleistungen» в форме «Buchung bearbeiten»
  (опции ЦЕЛЕВОГО юнита: scope-wide + адресные; выбранные — по id из снимка);
  сентинел `extras_present` (W0 — форма без блока ничего не стирает); изменённый
  состав ПРИНУДИТЕЛЬНО включает пересчёт (иначе итог рассинхронизировался бы).
- **booking (Termin)**: `action=extras` на карточке записи — пересборка снимка
  (адресность по услуге, чужая адресная опция не проходит), итог — свойство
  модели; закрытая сделка не правится.
- **Дельта оплаченной сделки**: PAID + изменившийся итог → warning с суммой
  «±X € — Nachzahlung oder Erstattung mit dem Gast klären» (авто-чарджа нет
  осознанно; частичный Stripe-возврат — external-бэклог). Ticket-допы — follow-up.
- Замки: stays 4 (добавить/убрать/W0-сентинел/дельта) + booking 3 (пересборка/
  чужая адресная/закрытая). 3 msgid × 5 каталогов.

Попутно влита параллельная волна SH (main → ветка, union-мерж .po/build-log):
SH закрыла правку заказа и НДС на товаре — куски плана MX-3/НДС-подосновы.

## 2026-08-21 — MX-4 (v1): закупка от проданных опций — SupplierBooking вне туров (⚠️ миграция `events/0028`)

«Своеобразная ERP» владельца: продали N опций → заказали N у поставщика.

- **`SupplierBooking.event` → nullable + `ref_kind`/`ref_id`** — закупка живёт у
  любой сделки/опции и без сделки (общая); заезды туров работают как раньше.
- **Закупка из сводки Zusatzverkäufe**: у purchase-опции кнопка «→ Beim Anbieter
  buchen» с префиллом qty = продано за окно; опция события цепляется к заезду,
  scope-wide — standalone с ref на опцию. Дедуп: открытая заявка той же опции не
  плодится повторным кликом; закрытая дедуп не держит (новая потребность = новая
  заявка).
- **Секция «Beschaffung»** там же: открытые Anbieter-Buchungen (заезды + общие)
  со сменой статуса; «Bezahlt» пишет расход и вне заезда (`record_expense` несёт
  `ref_kind`/`ref_id` — маржа считается не только у туров), откат статуса расход
  снимает (паритет с логистикой туров).
- Замки: `test_mx4_procurement` (3, свежая БД — standalone/дедуп/расход с ref);
  events-сьют 303 зелёных. 5 msgid × 5 каталогов.

## 2026-08-21 — MX-5 (v1): способ продажи — свойство карточки (⚠️ миграции `booking/0023`* + `catalog/0030`)

Ключевой сдвиг стратегии v3: PURCHASE_MODE остаётся дефолтом модуля, карточка
переопределяет. (*номер booking-миграции — фактический из репо.)

- **`Product.primary_action`** (""=корзина | request): «торт на заказ» — CTA
  заявки вместо корзины у КОНКРЕТНОГО товара, без выключения orders у остальных
  (реюз GK-14-ветки buy-box); поле в форме товара рядом с ценой.
- **`Service.pricing_mode`** (""=за бронь | per_person): режим Tripster «цена ЗА
  ЧЕЛОВЕКА» у Работы — до этого «люди» и «цена» жили в разных ветках движка
  (разведка v2 §2). Витрина: поле «Personen» в форме брони слота, итог = цена ×
  человек × длительность, `party_size` несёт людей; промо-сравнение той же
  формулой; подсунутое поле у «за бронь»-услуги игнорируется (замок). Кабинет:
  селект «Preis gilt: pro Termin / pro Person» в обеих формах услуги
  (presence-guard W0).
- «Private Anfrage» у события — уже покрыто (R6 корп-запрос + moto-CTA);
  зафиксировано без правок.
- Замки: `test_mx5_pricing_mode` (3, свежая БД); booking-сьют + buy-box паритеты
  231 зелёных. 7 msgid × 5 каталогов.

## 2026-08-21 — MX-6 (v1): серия одной точкой входа + свои категории событий (БЕЗ миграций)

- **`series_id` ожил**: писался при клонировании (RT3) и не читался нигде — курс
  из 12 занятий выглядел 12 несвязанными карточками. Деталь события показывает
  блок «Weitere Termine» (будущие опубликованные сиблинги серии, свободные места
  бейджем). Свод листинга в одну карточку серии — follow-up (нужна хирургия
  UB-провайдера с паритет-замками, отдельным слайсом; то же — агрегатор-листинг
  карточкой тура, план §MX-6.3).
- **Своя категория события**: пресет из 9 wellness-тем душил тур-оператора и
  мастерские — поле стало свободным вводом с datalist-подсказками пресетов;
  фасет каталога показывает свои значения как есть (раньше значение вне пресета
  МОЛЧА выпадало из фильтра).
- Замки: `test_mx6_series_categories` (3); events-сьют 300 зелёных;
  2 msgid × 5 каталогов (вкл. плюрал «N frei»).

## 2026-08-21 — MX-7 (v1): цифровые Вещи и Offer в общий учёт (⚠️ миграция `finance/0009`, choices-only) — ПРОГРАММА MX ЗАКРЫТА В ОБЪЁМЕ v1

- **Деньги сертификата и абонемента впервые в журнале**: `RevenueEntry.SOURCES`
  += gift/pass; `mark_gift_voucher_paid` и `purchase_pass` пишут выручку
  идемпотентно (повтор вебхука не плодит запись — замки). До MX-7 оба чекаута
  существовали МИМО «Finanzen» (перепись v2 §1: 7 чекаутов, 5 источников).
- **«Digitale Angebote» на полке Sortiment**: абонементы (PassPlan) и
  сертификат — read-обзор + переход к родным экранам; «тени» кончились.
- **Offer в поиске**: Sofort-Angebot ищется в Ctrl+K-разделе «Verkäufe» по
  имени/почте клиента, вход — тред inbox (был виден ТОЛЬКО внутри переписки).
- Гейт фида: цифровые не в Product-каталоге — в товарный фид не попадают по
  построению (зафиксировано).
- Замки: `test_mx7_digital_goods` (4, свежая БД); loyalty/passes/finance/palette
  73 зелёных. 5 msgid × 5 каталогов.

**Программа MX v1 исполнена целиком: MX-0…MX-7.** Follow-up'ы, объявленные по
ходу: enforcement пула и рецепт списания (2e), ticket-допы (3b), свод листинга/
агрегатора в карточку серии (6c), полный «свод save-блоков» не требуется.

## 2026-08-21 — MX-2e + 3b: enforcement трекеров опций + правка допов билета (БЕЗ миграций)

Первые два follow-up'а программы MX (план `mx-followups-plan-2026-08-21.md`).
Новый `apps/core/option_trackers.py`:

- **Пул** (`tracker=pool`): продажа опции проверяет занятость пула в окне
  ИСПОЛНЕНИЯ сделки (stay — ночи, ticket — дни события, booking — день слота)
  под select_for_update строки Extra; переполнение → PoolFull → честный отказ
  формы с откатом всей сделки. Пул ОБЩИЙ на три kind (аренда байка, занятая
  бронью проживания, видна билету). pool_size=0 = безлимит. Урок: сделка к
  моменту проверки уже создана — обязана исключаться из подсчёта сама (off-by-one
  ловился первым же тестом).
- **Склад** (`tracker=stock`): продажа списывает Вещь опции — движение
  `source="option"` ПЕРВЫМ (идемпотентность (source, source_ref, kind) даром),
  счётчик только при созданном движении; нехватка → OptionOutOfStock → отказ.
  Отмена возвращает через SM-хуки трёх kind + зеркала кастом-статусов
  (двойной вызов безопасен); возврат ТОЛЬКО при существующем sale-движении —
  смена трекера после продажи не доливает склад. source_ref = md5 (составной
  ключ из двух UUID не влезает в varchar(64) — поймано тестом на живой БД).
- Правки состава (MX-3-пути stays/booking) идут через `sync_options` (diff:
  убранное вернуть, добавленное провести с enforcement'ом).
- **3b**: правка допов БИЛЕТА — ветка `action=extras` в `ticket_action` +
  `<details>`-форма в списке билетов события; итог — свойство `Ticket.total_cents`;
  предупреждение о доплате оплаченного — реюз msgid MX-3.

Замки: `test_option_trackers` (7) — пул-отказ/кросс-kind/освобождение отменой
с доски/безлимит; склад-списание/идемпотентный возврат/отказ при нехватке/
не-долив при смене трекера; sync-дифф. Смежные stays+events+booking 799 зелёных.
2 msgid × 5 каталогов. Ограничения v1 объявлены в плане (qty=1, FEFO не трогаем).

## 2026-08-21 — 6c: серия/заезды тура — одной карточкой в листинге и агрегаторе (БЕЗ миграций)

Третий follow-up MX (план `mx-followups-plan-2026-08-21.md`). Закрывает «курс из
12 занятий = 12 карточек»:

- **Витрина** `/veranstaltung/`: события одной группы (tour_id | series_id)
  сворачиваются в одну карточку — представитель = первый в текущем порядке
  (дефолт по дате → ближайший; пользовательский сорт уважается), остальные даты
  — бейдж «+N Termine» (обе раскладки). Обычные события не тронуты — паритет-замки
  индекса зелёные без правок. Свод ПОСЛЕ фасетов/поиска (совпадение любого заезда
  показывает карточку группы).
- **Агрегатор**: `sync_event_group_listing` — заезды тура/серия = ОДНА запись
  (`source_ref = tour:<id> | series:<id>`), payload = ближайший будущий заезд;
  detail_url тура ведёт на страницу тура (там все даты), серии — на деталь
  ближайшего события (MX-6 покажет «Weitere Termine»). `sync_event_listing`
  групповых делегирует и удаляет легаси per-event строку (миграционный путь);
  reconcile строит allowed-refs с учётом групп.
- **Beat** `roll-event-group-listings` (час): протухшие групповые записи
  (ближайший заезд прошёл) перекатываются на следующий или удаляются — иначе
  карточка тура пропадала бы из выдачи при живых будущих заездах (вьюха прячет
  starts_at в прошлом).
...
## 2026-08-21 — v2-опции: рецепт списания, qty>1, FEFO-партии (⚠️ миграция `core/0010`)

Вторая отмашка «Делай» — объявленный v2-остаток stock-трекера (план
`mx-followups-plan-2026-08-21.md §v2`):

- **Рецепт**: `Extra.consume_qty` (аддитивная `core/0010`) — сколько единиц Вещи
  расходует одна продажа опции; списание = consume_qty × ночи у per_night-stay
  (ровно как множатся ДЕНЬГИ снимка), иначе × 1. Guard остатка на полный qty.
- **Точный возврат**: release читает |delta| sale-движения — возвращается ровно
  списанное, даже если consume_qty/даты сменили после продажи (замок).
- **FEFO**: при включённых партиях расход гасит ближайший MHD, отмена доливает
  (`has_lots` → consume_fefo/restore_fefo в той же atomic — зеркало orders).
- **Дыра кабинета закрыта**: форма опций не писала `Extra.product` — stock-трекер
  был незаводим из UI (поле писал только демо-сидер). Добавлены селект
  «Lagerartikel (Verbrauch)» и поле «Verbrauch je Verkauf»; склад без Вещи
  деградирует в честную надбавку.
...
## 2026-08-21 — ERP-волна: зелёный CI и мерж в main (три CI-грабли)

Стек ERP-1..4 доведён до зелёного CI (#2140) и FF-влит в `main` (`88ded7f`).
Три последовательные грабли, каждая ловилась только следующим прогоном:
(1) замок свежести `app.css` — новые finance-шаблоны требуют `npm run
build:css` (повтор урока CI-1121); (2) `msgfmt: duplicate message definition`
— msgid «Einnahmen aus dem Journal…» был добавлен волной MX-1 многострочной
формой, ERP-2-батч добавил его же одной строкой; msgfmt склеивает формы,
grep одной строки дубль НЕ находит → перед пушем .po-батча дуп-чек polib по
всем 5 каталогам (локально теперь есть и настоящий gettext: `apt-get install
gettext` в дев-контейнере работает — `msgfmt --check-format` гоняется 1:1 с
CI); (3) новые записи Ctrl+K-палитры («Offene Posten», «Bankabgleich») — это
текст в ХРОМЕ каждой страницы кабинета: негативный ассерт inbox-теста с
пробной темой «Offen» поймал хром, а не данные (тема заменена на уникальную —
класс MEN «пробные строки уникальными»); CSV-выгрузка `finance:expenses-datev`
— не экран → осознанно в EXPECTED_UNLISTED замка X7 (прецедент export-datev).
Широкий локальный догон: apps/core 1111 + inbox/documents/finance 93 зелёных.

## 2026-08-21 — ERP-5..7: возврат поставщику · маржа Работы · производственный акт

Отмашка владельца «Делай» на остаток гэп-анализа. План
`docs/erp57-plan-2026-08-21.md` (разведка — 3 параллельных агента).

**ERP-5 Rücksendung** (⚠️ миграция `inventory/0005`: `BestellPosition.
qty_returned` + kind'ы `return_supplier`/`production` одной choices-правкой):
`return_po_line` — зеркало приёмки: `apply_manual_movement(−N)` (кламп в 0 →
честно возвращается меньше), FEFO в той же atomic, деньги — СТОРНО-запись
`ExpenseEntry` с отрицательной суммой, идемпотентная по `<строка>:ret:
<накопленный qty>` (зеркало ключа приёмки; записи приёмки целы). Кабинет:
инлайн-форма «↩ Rücksendung» на принятых строках PO. 6 замков.

**ERP-6 маржа Работы** (⚠️ миграция `jobs/0015`): `JobLine.cost_rate` —
плановая себестоимость за единицу (работа = ставка €/Std., Teile = авто-
снимок EK детали при пустом поле — философия снимков ERP-1). EK-инпут под
ценой строки (грид 12 колонок цел, поле в DOM — W0); `Job.plan_calc` → блок
«Kalkulation (intern)»: Plan-Kosten/Plan-Marge (+%) с честной пометкой при
строках без ставки. Главный инвариант замком: cost_rate НЕ течёт в публичную
смету и PDF (HTML-ассерт + скан pdf.py). Ergebnis не тронут (выручку Job
журнал не пишет — invoice-флоу; отдельное решение). 4 замка.

**ERP-7 Produktion buchen** (без новой модели): `services.produce()` — сырьё
−/готовое + в одной atomic; нехватка любой строки → `ProductionError`, откат
всего акта (частичного производства нет); готовое партией `receive_lot`
(kind-параметр добавлен обратно-совместимо — выход акта честный
`production`, MHD хлеба) либо ручным путём; документ = PR-код (паттерн BE) в
note; расход НЕ пишется (сырьё оплачено закупкой — двойной учёт); EK
готового = Σ(qty×EK)/qty при чекбоксе и полностью известном EK (иначе
Warenwert терял бы готовое: stock_value=None → continue). Секция «🏭» на
складе. v2 по спросу: локации, сохранённые рецепты, средневзвешенный EK.
7 замков (вкл. «реконсиляция не меняется» — фабричный стартовый остаток сам
по себе вне леджера, сверяем diff до/после).

Гейты локально 1:1 с CI (gettext поставлен в контейнер): msgfmt+polib-дуп,
i18n_gap, ruff, template_comments, 212 inventory+jobs. 18 msgid × 5.

## 2026-08-24 — Редизайн кабинета, стиль B «Karten & Luft»: дизайн-канвас + волны R1–R3

Запрос владельца (скриншот + «админ-панель неудобна, настройки попрятаны,
нужен новый чистый адаптивный дизайн, мобильная в приоритете, все дизайны
через утверждение»). Дизайн-канвас (skill design, 11 артбордов: IA + 3
стиля + 7 hi-fi экранов; исходники `docs/design/kabinett-redesign-2026-08-24/`)
прошёл ревью-агента и утверждён: структура ок · стиль B · «ок» по экранам.
План волн — `docs/redesign-b-implementation-plan-2026-08-24.md` (R1–R6;
инварианты: только рендер, FSM/вьюхи/URL/реестр целы, W0, CSS-пересборка в
том же коммите). Попутный диагноз скриншота: «Воспоминания»/«Выкупать» —
авто-перевод Chrome поверх немецкого UI (наш ru.po корректен, встроенный
русский — селектор 🗣); реальные правки ru.po: Erweitert→«Дополнительно»,
Care-Zyklus→«Цикл заботы».

**R1 каркас** (`135df71`): холст #F2F4F7; сайдбар md+ — «парящая» карточка
(скругление+тень, border-r снят; sb-min-полоса SH-17 жива — скрытие только
CSS); активный якорь — залитая индиго-пилюля (замок W8-подсветки переписан
осознанно); мобильный таб-бар — плавающая пилюля с SVG-иконками (спрайт
+= ic-nav-dashboard/board/sellables/promotions/more; подписи в title/
aria-label; «⋯» открывает сайдбар); шапка сливается с холстом;
main pb-24 на мобильном (контент не ныряет под таб-бар).

**R2 навигация без дублей** (`5a0f5e3`): ящик «Erweitert» снят со страниц
целиком (тег hub_tabs скипает advanced; details-разметка удалена из
партиала) — advanced-состав живёт подпунктами сайдбара SM-4 и палитрой
Ctrl+K; складская группа (Lager/Einkauf/Kombi/Import) возвращена
подпунктами «Sortiment» (X4-решение пересмотрено по утверждённой IA,
записи реестра целы); активная вкладка X5-рядов — индиго-пилюля. Замки
переписаны осознанно (8: hub_tabs ящик/active-подсчёты, состав подпунктов
Sortiment, w7 blog/newsletter→сайдбар, x4 склад-из-сайдбара). CI #2152
поймал урок: замок свежести app.css ловит и УДАЛЕНИЕ шаблонных классов
(purge выкинул классы снесённого ящика) — пересборка обязательна при любом
изменении классов, включая удаление.

**R3 Verkäufe** (`cfcf9e3`): направления — пилюли-карточки (активная
индиго со счётчиком-пилюлей; мобильный ряд скроллится), виды/тулбар/
фильтры — белые пилюли и карточки с тенью вместо рамок, чип «Nur
Lieferungen» rounded-full (активный — тёмная заливка), строки generic-Liste
и списка заказов — карточки radius 16 (состав SH-11/12/16 цел), «＋» на
мобильном — FAB над таб-баром (та же цель create_target; рендер после тела
— замок «＋ не в шапке» цел), на sm+ — кнопка тулбара. Board/Kalender не
тронуты. Стенд Playwright 10/10 (hotel 2 направления + catering одно;
390 + 1440): FAB только на мобильном, тулбарная «＋» только на десктопе,
Belegungsplan жив. Ложная тревога стенда: «Kunden» на скриншоте выглядела
залитой — вблизи это синий эмодзи 👥 на белой пилюле (computed style
проверен). Всё БЕЗ миграций.

## 2026-08-24 (продолжение) — R5a/b (карточка заказа + формы) и волна R7 (фидбэк владельца)

**R5a/b** (`687e645`): карточка заказа — тени B, кнопки rounded-xl (замок SH-10
цел); формы товара/акции — пилюльные табы (стили .pf-tab в общем партиале
_i18n_switch, механика панелей/invalid-раскрытия цела), поля rounded-xl,
обёртки — тень B.

**Волна R7 — фидбэк владельца по скриншотам (7 пунктов).**

**R7-1 «одно меню»** (`c04129b`): владелец — «по-прежнему на страницах есть
дубль меню; при нажатии на Продажи или Ассортимент должно открываться подменю,
и в нём уже всё, а не сразу на страницу». Таб-бары хабов на страницах СНЕСЕНЫ
целиком (тег `hub_tabs` + партиал `_hub_tabs.html`, 39 шаблонов — прецедент
W-CL): раздел сайдбара стал тумблером, подменю несёт ВЕСЬ состав хабов
(sidebar_children: main+advanced), первым подпунктом — обзорная страница
раздела (дедуп по паре url_name+query — иначе «Kunden» съедала обзор продаж).
Мобильный таб-бар: раздел с подменю выезжает сайдбаром с раскрытым разделом;
свёрнутая полоса (sb-min) при клике разворачивается. **Замок нашёл реальный
дефект:** owner-гейт жил только в снесённом теге и палитре — сотрудник увидел
бы Team/Abo/Recht (мёртвые 403); гейт перенесён в `sidebar_nav(tenant, user)`,
fail-open без user. Замки переписаны осознанно (13 шт. в 6 файлах:
test_hub_tabs переписан целиком на подменю, X5-ряды, X0-owner, W7-сироты,
W8-подсветка — раздел стал `data-nav-toggle`, состав sidebar_children,
«запись не выпадает молча» на новой поверхности).

**R7-2 «вкладки товара сверху, фото — часть Основ»** (`c04129b`): ряд вкладок
формы товара вынесен НАД карточку (владелец искал их как навигацию страницы);
изображения — внутрь вкладки «Grundlagen». Мини-формы ★/✕ — вне главной формы,
кнопки связаны `form=` (вложенные `<form>` браузер выбрасывает — кнопки
умирали бы молча).

**R7-3 «Lieferungen + Zahlungen отдельными страницами»** (`1a6a519`):
- `/dashboard/orders/lieferungen/` — заказы-доставки, адрес/трек-номер/
  Lieferschein-PDF на строке, фильтр Offen/Versendet/Alle; статусы — ТЕМ ЖЕ
  board-action (письмо с Sendungsnummer штатно, W10-5). Запись SH-14
  перенацелена с фильтра списка на страницу.
- `/dashboard/zahlungen/` — обзор денег сделок всех направлений (слой ЧТЕНИЯ
  `apps/core/payments_page.py`): карты «Wartet auf Zahlung»/«Bezahlt (30
  Tage)», чипы состояний + фильтр по способу оплаты (E-7); правило ERP-2
  «none у брони = оплата на месте, не долг»; итоги в Python по контракту
  Transaction (урок VS-2c — SQL-Sum врал бы у booking/ticket). Модуль finance
  не требуется. Обе страницы — подпункты «Verkäufe» + палитра (реестр W8).
  Стенд поймал коллизию nav_key: «payments» занят «Zahlung & Lieferung» —
  подсветка уезжала в настройки; свой ключ «zahlungen».
...
- **2026-08-24 (VF-9b, фидбэк-продолжение по карточке заявки, БЕЗ миграций).** Владелец:
  «зачем вообще выводить селект "Деталь (каталог)" в каждой строке — если нужно, просто
  выбираем из каталога товар или комбо» и «зачем поле закупка на странице заказа? это
  раздел ERP». Смета переложена на модель заказа: (1) строчный селект убран — привязку
  существующей строки держит `<input hidden line_part_i>` (приёмник читает поле; без него
  Save снял бы связь со складом — класс W0), связанные строки помечены подписью
  «📦 Aus dem Sortiment»; (2) выбор из ассортимента — ОДИН селект у «＋ Position hinzufügen»
  (заполняет первую пустую видимую строку текстом+ценой, ставит hidden-привязку; msgid
  реюзится из карточки заказа); (3) пикер сметы += комбо-наборы (`k:<pk>` за флагом
  `include_combos` — приёмник заказа префикса не знает; `_combo_snapshot` — снимок
  имени/цены без FK, как услуга); (4) видимое поле EK убрано — значение едет hidden'ом
  (иначе Save стёр бы введённые ставки), снимок EK для строк каталога делает сервер
  (ERP-1/6), строка «Kalkulation (intern)» осталась справочной; (5) НОВАЯ «✕»-очистка
  строки — раньше привязанную строку было НЕ УДАЛИТЬ (пустой текст сервер восстанавливал
  снимком имени из каталога); (6) `recalc` ищет строку через `closest("[data-qt-row]")`
  (Summe теперь во врапере с «✕» — `parentNode` сломал бы живые суммы, гочча из разведки).
  Замки: test_detail_shows_part_picker переписан осознанно + test_detail_hides_ek_field_
  but_preserves_value + test_combo_in_picker_and_snapshot (jobs 110). Стенд Playwright
  14/14 на catering: пик товара и комбо, 📦-семантика, «✕», Save-круг держит привязки
  (2→3 связанных). msgid «Position leeren» ×5. Примечание: «пусто у кейтеринга» на
  проде — старый код+сид; после деплоя и пересева демо-строки покажут 📦.
- **2026-08-25 (VF-10, фидбэк «Заявки в кейтеринге не нажимается в меню», БЕЗ миграций).**
  Клик технически работал (jobs:list → 302 → verkaeufe?tab=job), но у архетипов с ОДНИМ
  направлением продаж (catering) это та же страница, что «Verkäufe», и подсветка не
  переходила — ни URL-перемены заметной, ни отклика: клик выглядел мёртвым. Три правки:
  (1) пункт «Aufträge» в реестре → прямая ссылка `verkaeufe` + `query="?tab=job"` (без
  редиректа; сам /dashboard/auftraege/ остаётся легаси-302 X2c для старых ссылок →
  jobs:list в EXPECTED_UNLISTED замка X7); (2) специфичность подсветки подпунктов
  (context.py): query-совпадение сильнее голого пути — на ?tab=job горит «Aufträge», а
  не он вместе с обзорным «Verkäufe»; (3) палитра Ctrl+K понимает query-адреса:
  `palette_entries` пропускает query-записи собственными карточками (дедуп по
  (url_name, query); query-запись помечает url_name занятым → дубль-«Abläufe» без query
  по-прежнему схлопнут), тег/шаблон рендерят `{% url %}{{ query }}` — «Aufträge» и
  «Kunden» снова находимы и прыгают прямо на вкладку. Замки: test_child_highlight_query_
  beats_plain_path + переписки w8-палитры (уникальность по адресу, ровно один «Abläufe»)
  и состава подпунктов; nav-свод 82 + core+jobs 1242. Стенд Playwright 5/5 (catering,
  1890px): href прямой, URL меняется, подсветка переезжает, палитра ведёт на вкладку.
- **2026-08-25 (VF-11, фидбэк «в чём разница verkaeufe и ?tab=job — внутри одно и то же» +
  «проверь остальные архетипы», БЕЗ миграций).** Владелец прав: у одно-направленных типов
  вкладка-направление = ТА ЖЕ страница, что обзор «Verkäufe» — подпункт «Aufträge» был
  дублем. Гейт `_is_solo_direction_tab` в `modules.sidebar_nav`: подпункт с query
  `?tab=<kind>` (kind ∈ transactions.KIND_MODULE — «?tab=kunden» не направление) прячется,
  когда `sales_page.visible_kinds(tenant)` ≤ 1; результат memo на объекте тенанта (сайдбар
  рендерится каждой страницей, visible_kinds ходит в БД за легаси-резервами); fail-open —
  ошибка гейта показывает пункт, а не роняет хром. **Свод по запросу владельца: все 16
  типов на честных пресетах** — дублей целей в подменю «Verkäufe» нет НИ У ОДНОГО
  (разведка-скрипт), вкладка-направление осталась ровно у werkstatt (booking+job = 2
  направления); закреплено параметризованным замком
  `test_verkaeufe_children_no_duplicate_targets` (16 типов: цели подпунктов уникальны +
  `?tab=<kind>` только при ≥2 направлениях). VF-10-замок подсветки переведён на werkstatt
  (у catering подпункт теперь спрятан). Стенд Playwright 3/3: подменю catering =
  Verkäufe · Abläufe & Status · Kunden · Zahlungen. core 1133 + sidebar/nav 51 зелёные.
- **2026-08-25 (батч VF-12..22, фидбэк-поток владельца из 11 пунктов; БЕЗ миграций).**
  Разведка 2 агентами (карта узких страниц + карта оплат/CRM/inbox/склада). **VF-12** поле
  «Anzahl» сметы = высота цены (обёртка-ячейка — голый грид-инпут тянулся на высоту строки).
  **VF-14** /dashboard/ablaeufe/: max-w-3xl умер — карта процесса на всю ширину, панели в
  2 колонки на lg+ (Anfrage-панель переехала внутрь правой — жила ПОСЛЕ обёртки и рвала
  раскладку); мобайл 390px без гор. скролла (стенд). Прочие узкие страницы — карта в
  отчёте разведки (max-w-xl/2xl/3xl ×~20 экранов), раскладка по 2 колонки — следующей
  волной по решению владельца. **VF-15** карточка «💳 Zahlung» в рейле заявки: статус
  оплаты/сумма/депозит, НОВОЕ ручное «Als bezahlt markieren» (payment_state писал только
  Stripe-вебхук — оплату на месте было не отметить; идемпотентно, кнопка гаснет после),
  Angebot-ссылка для ручной передачи (quoted/accepted), счёт. **VF-16** «Open customer
  card →» на карточке заявки (гейт crm; прецедент order_detail). **VF-18a** страница
  клиента CRM: брони/номера/билеты стали кликабельными (customer360 _url booking/stays
  detail, билет → экран события; заказы/заявки/счета/треды были). **VF-19** формы акции
  и комбо — рейл-раскладка товара (грид minmax(0,1fr)_340px; акция: контент+форма-табы+
  статистика в main, статус/продвижение/Warteliste/QR/каналы рейлом; комбо: форма+группы
  в main, галерея рейлом — плитка «＋ Foto» прокси-кнопка, перенос безопасен); Kampagnen —
  2 колонки (создание+список | автоматика win-back/ДР). **VF-21** название акции в списке —
  ссылка в форму. **VF-22** блог: кнопки «Ansehen auf der Website ↗» (живой блог витрины),
  «＋ Neuer Beitrag» (якорь к форме), «📣 Teilen» на опубликованных строках (префилл в
  publishing — как на карточке записи; выбор канала на стороне Beiträge/Kanäle).
  Замки: test_payment_card_and_mark_paid (jobs 32); стенд Playwright 18/18 по факту
  (3 «FAIL» = данные/локаль пробы: заявка уже оплачена первым прогоном — бейдж «bezahlt»,
  в демо-блоге catering нет записей). Грабля стенда повторилась: rl:*-ключи Redis (105 шт)
  роняли public-тесты промо — чистка scan_iter. План-док VF-17/18 —
  `docs/deal-comms-plan-2026-08-25.md` (решения владельца §3). msgid ×5: Angebot-Link…,
  Ansehen auf der Website, mit Online-Anzahlung.

## 2026-08-25 (продолжение) — VF-13 резерв склада из сметы · C1/C2/C3 коммуникации · VF-20b · VF-24

Решения владельца: «делай по рекомендации» (VF-13, план коммуникаций C1–C3 по
рекомендациям §3) и «3 — поставить рядом, но не объединять под одной поверхностью»
(Кампании/Акции). Всё БЕЗ миграций.

**VF-13 «резерв склада из сметы при Beauftragt»** (план
`docs/vf13-quote-stock-reserve-plan-2026-08-25.md`). Было: смета списывала склад
только при `done`, между принятием и выполнением витрина могла продать заложенные
детали; возврата при отмене не было вовсе (докстрока утверждала, что `cancelled`
достижим только до `done` — уже неверно, ребро accepted→cancelled есть).
Стало: **резерв = уменьшение `stock_quantity`** (как заказы; витрина получает
«Ausverkauft» бесплатно, реконсиляция «счётчик == Σ леджера» сходится по
построению — новый счётчик «reserved» и новый kind движения НЕ заводили).
`commit_stock` вызывается уже при входе в `accepted` (кламп, не блокировка:
принятие клиентом/вебхуком Anzahlung нельзя валить OutOfStock — детали
докупаются), при `done` повтор идемпотентен (`stock_committed`). НОВОЕ
`release_stock` возвращает по ЛЕДЖЕРУ, а не по строкам (строки пересоздаются
каждым Save сметы — `set_lines` = delete+bulk_create, PK нестабильны):
return-движение пишется ПЕРВЫМ (`ret:<mv.pk>`, дедуп конституционный по
(source, ref, kind)), счётчик двигается conditional UPDATE только при созданном
движении, FEFO — `restore_fefo`; возвращается ровно фактически списанное (вкл.
клампы). `_resync_reserved_stock` в `set_lines`: правка состава при активном
резерве = release + re-commit в одной atomic; после `done`/`invoiced` склад не
трогается (история). Зеркала кастом-статусов: роль `active` c `blocks_capacity`
(дефолт редактора) резервирует, роль `cancelled` возвращает (job-ветка в
`restore_stock_for`; явный «паритет: не делаем» снят). Замки —
`apps/jobs/tests/test_stock_reserve.py` (10, включая кламп-возврат, FEFO,
кастом-статусы); jobs+core+inventory 297 зелёных.

**C1/C2/C3 «общение с клиентом из сделки»** (план `deal-comms-plan-2026-08-25.md §5`).
Канон ref: `ref_kind` + `ref_id = reference_code`; разведка нашла расхождение —
`orders/offers.py` писал UUID заказа, поэтому тред, склеенный принятием Angebot,
не находила ни problem-полоса доски, ни поиск треда сделки (замок переписан
осознанно; `find_thread` держит фолбэк на легаси-UUID). **C1**: новый
`apps/inbox/deal_threads.py` + вьюха `inbox:deal-thread`
(`/dashboard/inbox/deal/<kind>/<pk>/` — под inbox-префиксом, гейт модуля даёт
ModuleGatingMiddleware бесплатно) + партиал `core/_deal_message_button.html` на
карточках заказа/заявки/брони/записи (fail-closed `inbox_active`, как
`crm_active`); существующий тред сделки открывается, второй не заводится; без
e-mail клиента — честный отказ со входом «Kontakt ergänzen». **C2**:
`adopt_open_thread` из `create_order`/`book`/`book_stay`/`create_job` — «спросил
с сайта → купил» остаётся одной беседой (fail-soft, только при РОВНО одном
открытом треде без ref); в customer360 подпись строки стала сделкой (`ref_label`,
без привязки — «Allgemein»), тема ушла во вторую строку. **C3**: в форме ответа
чекбокс «Auch per Telegram senden» (только при живой привязке клиента к боту,
dedupe `inbox:msg:<id>:telegram`) и ссылка «Per WhatsApp schreiben» — первое
применение `wa_link` к номеру КЛИЕНТА (API-отправка остаётся external-backlog);
e-mail дефолт и уходит всегда. Замки `test_deal_comms.py` (13); inbox/orders/crm/
jobs/booking/stays 914 зелёных. 11 msgid × 5 каталогов.

**VF-20b «Кампании рядом с Акциями»**: `Kampagnen` переехали сразу за `Aktionen`
в ENTRIES (замок порядка подпунктов), данные и экраны раздельные со своими
модульными гейтами (Aktionen — promotions, Kampagnen — crm); перекрёстные входы
«🎟 Kampagnen» в шапке акций и «🏷 Aktionen» на кампаниях (экран был тупиком —
ни одной ссылки наружу), оба fail-closed.

**VF-24 «узкие страницы в 2 колонки»** (продолжение VF-14): Domains (работа с
доменом | DNS-справка и входы), Extras (список | форма), Team (участники |
приглашение), Newsletter (создание | список — симметрия с Kampagnen),
Module (группы тумблеров на xl+, одна форма/Save), Finder (настройка | превью;
редактор вопросов на всю ширину — строка это 6 полей маппинга), Stays-Kanäle
(каналы+импорт | импортированные брони), CRM-Firma (форма | клиенты, только в
режиме карточки). Только рендер; мобильный — одна колонка прежним порядком.
Стенд Playwright 17/18 (единственный не-PASS — Finder у catering: без активного
sellable-модуля грид не рендерится по гейту `has_kind`; на hotel 2 колонки).
Грабля повторилась: новый arbitrary-класс `xl:grid-cols-2` требует пересборки
app.css — до неё стенд честно показывал 1 колонку.

## 2026-08-26 — Волна DF: фидбэк владельца по карточке сделки (8 пунктов)

План — `docs/df-deal-card-feedback-plan-2026-08-26.md`. Инвариант волны цел:
правки в общем скелете и общих партиалах, поэтому меняются ВСЕ карточки сразу.

**DF-1a.** Голова: подпись из состава сделки («Hochzeitsmenü Wahl — 20 Personen:
Rote-Bete-Carpaccio, …») убрана — она дублировала тело и обрезалась 200
символами. Остаётся «angelegt <дата>»; текущий статус и так стоит бейджем.

**DF-1b.** Смена статуса переехала из рейла в верхнюю строку выпадающим списком;
вопрос «уведомить клиента/команду» — в `<dialog>` ВНУТРИ той же формы (снятый
чекбокс обязан доехать до приёмника; `showModal()` рисует в top layer, поэтому
stacking-контекст карточки не важен — урок HF). Без JS кнопка остаётся обычным
submit'ом с прежним поведением. Карточка статуса в рейле снята как дубль.

**DF-1c.** Документы — своей секцией в первой колонке сразу после оплаты. У
заказа они были размазаны (оплата + карточка доставки в рейле), у заявки жили в
узком рейле. Секция включается ПО ДАННЫМ: при активном Finanzen, у заказа ещё и
при доставке — иначе была бы пустая рамка (поймано стендом).

**DF-1d.** Скидка — внутри блока позиций, перед итогом. У заявки итог печатается
внутри сметы, поэтому там блок скидки живёт в её шаблоне — но ЗА пределами формы
(вложенные `<form>` браузер разворачивает: «Rabatt speichern» отправил бы
конструктор сметы; замок сканирует разметку всех карточек).

**DF-2.** «В кейтеринге пусто в описании заказа»: конфигуратор набора клеил весь
состав в ТЕМУ заявки, описание оставалось пустым. Тема стала короткой, блюда
едут отдельным параметром в описание, а то, что прислал клиент, получило блок
«Anfrage des Kunden» в теле карточки (событие, текст, адрес, фото).

**DF-3.** Переписка — на карточке, а не отдельной страницей: окно в тот же тред
inbox (второго канала не заводим), ответ возвращает на карточку (`next`, только
свои пути — «//host» отбиваем), новые сообщения подтягивает существующий поллинг
M22b. В CRM непрочитанное помечено видимой меткой: в списке (ОДИН запрос на
страницу, как LTV) и на карточке клиента.

**DF-4.** Сверка с макетом (`docs/design/deal-card-2026-08-25/`): у брони,
записи и билета состав рисовался абзацами, тогда как макет требует таблицу
(№ · Position · MwSt. · Einzel · Menge · Summe) и суммы Zwischensumme → Rabatt →
Netto → MwSt. → Gesamt. Оба блока стали общими партиалами (`core/_deal_lines`,
`core/_deal_totals`), строки берутся из тех же снимков, что и налог — показ не
может разойтись с деньгами (замок сверяет сумму строк с итогом; «pro Nacht» даёт
количество ночей, снимки до MX-0 без `unit_cents` дают одну честную строку).
Ширина рейла приведена к макету (380px).

Замки: `test_deal_card_shell` 24 → 38. Осознанно переписаны замки статуса
(рейл → голова) в shell/sh_orders/jobs. 13 msgid × 5 каталогов. БЕЗ миграций.
Стенд Playwright 14/14 (демо кейтеринга + отеля); он же нашёл три дефекта,
невидимых серверным тестам, — пустую секцию документов, вложенную форму и
осиротевший заголовок в рейле.

## 2026-08-25 — Волна DC «единая карточка сделки» (DC-1 · DC-4 · DC-5)

ТЗ владельца 2026-08-25 (макеты `docs/design/deal-card-2026-08-25/`, артефакт
утверждён) + правки того же дня: «статус во вторую колонку», «номер одной
строкой», «скидка между позициями и суммой и учитывается в общей цене»,
«Menge после Einzel, на месте Menge — порядковый номер», «базовые функции и
блоки должны иметь общие настройки: меняется один — меняются все сразу»,
«если есть календарь — открывается сразу ниже сетки», «все поля и подписи
должны быть переведены». План — `docs/deal-card-implementation-plan-2026-08-25.md`.

**DC-1 «Один скелет» (без миграций).** Новый `templates/core/deal_card_base.html` —
ОДИН шаблон карточки сделки; заказ, заявка, бронь номера и запись услуги
наследуются от него и наполняют только свои блоки. Общие части — партиалы
`core/_deal_head.html` (номер · внешний номер · бейджи · мета одной строкой),
`_deal_status_card.html` (статус в рейле; приёмник generic `board-action` —
тот же путь, что доска и списки; поле трек-номера по `needs_tracking`),
`_deal_customer_card.html` (имя → CRM, кликабельные контакты, «написать
клиенту», правка контакта). Единый контекст — `apps/core/deal_card.py`
(контракт `Transaction` + гейты модулей + календарь + значения скидки и
внешнего номера). Порядок: голова → состав → скидка → суммы → оплата и счёт;
рейл: статус → клиент → контекст направления → связанные услуги.
Календарь — РАЗВЁРНУТЫМ блоком ниже сетки: тот же движок, что на странице
продаж (stay — Belegungsplan, booking — Tagesplan, order — Auftragsbuch при
используемых слотах); формы создания новой сделки в нём скрыты флагом
`deal_calendar_compact`. У заявок движка нет — блок не рисуется.
Колонки позиции заказа: № · Position · MwSt. · Einzel · Menge · Summe.
Панель брони под Belegungsplan (fetch `?box=1`) собрана из ТЕХ ЖЕ партиалов
`stays/_stay_*.html`, что страница брони.

**DC-4 «Внешний номер везде» (⚠️ миграции `booking/0024`, `stays/0033`,
`jobs/0016`).** `external_code` — ручной номер кассы/портала/книги; машинный
`StayBooking.external_ref` (идемпотентность импорта OTA) не трогаем. Приёмник
один: `deal-external-edit/<kind>/<pk>/`; поиск сделок ищет по нему у всех
четырёх видов (`_TITLE_SEARCH`).

**DC-5 «Скидка везде» (без миграций).** Поле `discount_cents` уже было у всех
четырёх видов. Новый `apps/core/deal_discount.py` знает, как пересчитать итог
в каждом домене: заказ — `orders.editing.set_discount`, запись — property
`total_cents`, заявка — `payable_gross`, бронь — пересчёт ХРАНИМОГО
`total_cents` той же формулой, что `reprice` (иначе показанная сумма разошлась
бы с суммой к оплате). Приёмник `deal-discount-edit/<kind>/<pk>/`, форма —
общий партиал; немецкая запятая принимается, мусор не меняет сумму, итог не
уходит ниже нуля (у брони остаётся Kurtaxe).

**Стенд Playwright (демо hotel/catering) нашёл три дефекта, невидимых серверным
тестам:** блок «Aufenthalt» печатал даты дважды и строку-обрубок
«ad i18n cabinet %}» (нарезка партиала захватила лишние строки); «Zahlung»
дублировалась в суммах и в оплате → вынесена в общий `stays/_stay_payment.html`;
календарь на карточке предлагал завести ДРУГУЮ бронь (формы создания скрыты).
Грабля-повтор: Django кэширует шаблоны и в DEBUG — после правки рестарт
runserver, иначе стенд показывает старую страницу.

**Замки:** новый `apps/core/tests/test_deal_card_shell.py` (10) — общие блоки,
статус в рейле, порядок блоков, календарь ниже сетки, единый источник,
внешний номер (форма + сохранение + поиск), скидка (место, влияние на итог,
кламп и мусор). Осознанно переписаны SH-10 (статус был в шапке заказа),
VF-15 (оплата была в рейле заявки), VF-2 (порядок колонок), рейл-замки заявки
и брони. Локально 2021 тест зелёный; 24 msgid × 5 каталогов; гейт
`scripts/i18n_gap.py` зелёный (он строже quickcheck — уронил CI #2196/#2197).

### 2026-08-25 (продолжение) — DC-3 · DC-6 · DC-7 + блог/каналы по фидбэку

**DC-3 «статус списком + вопрос об уведомлении».** По ТЗ («смена статуса
выпадающим списком, при выборе спрашивать — отправить уведомление клиенту и
администратору»): в карточке статуса `<select>` переходов (danger помечен ⚠️),
под ним «Benachrichtigen?» с чекбоксами «Kunde» / «Team» и кнопкой «Status
ändern». Разовое подавление — `notifications.prefs.muted()`: thread-local на
время `SM().apply` (письма ставятся в очередь синхронно в том же потоке),
учитывается существующим `channel_enabled`, настройки тенанта не трогает.
Снятый чекбокс в POST НЕ ПРИХОДИТ вовсе → спрашивающая форма метит себя
`notify_form=1`; без метки (доска, списки, per-app экраны) поведение прежнее.

**DC-6 «счёт из брони и записи»** (⚠️ миграция `booking/0025` — поле связи со
счётом; у брони номера оно уже было). `finance.services.invoice_from_stay`:
проживание ОДНОЙ строкой (цена за ночь округлялась бы дважды — итог счёта
расходился с суммой брони на копейку, поймано замком), допы, скидка, Kurtaxe
отдельной строкой без НДС; ставка 7 %. `invoice_from_booking`: услуга + допы −
скидка, 19 %. Приёмник один — `deal-invoice/<kind>/<pk>/`, черновик
переиспользуется (GoBD: номер только при `issue_invoice`), гейт по модулю
«Finanzen» fail-closed. Кнопка — общий партиал в блоке оплаты.

**DC-7 «карточка билета»** — экрана не было вовсе: доска и списки вели на
страницу СОБЫТИЯ, где билет терялся. Новый `/dashboard/events/ticket/<pk>/` на
общем скелете; своё — мероприятие с датой, тариф и места, допы с правкой
состава (тот же приёмник `events:ticket-action`), ответы анкеты, подписанная
Teilnahmeerklärung, депозит. `transactions.manage_url` для ticket переведён на
карточку. Календарного движка у билетов нет — блок не рисуется.

**Фидбэк того же дня (блог и каналы).** `apps/core/share_links.py` + партиал
`core/_share_menu.html`: у опубликованной статьи меню «📣 Teilen» с выбором
канала — прямые ссылки WhatsApp / Telegram / Facebook / E-Mail, поле со ссылкой
и пункт «во все активные каналы» (контент-календарь publishing с префиллом).
`apps/core/personal_channels.py` + секция «Persönliche Kanäle» на странице
каналов: WhatsApp, Telegram-бот, e-mail, чат сайта и соцпрофили (GK-9) со
статусом «активен / настроить» и входом на нужный экран; слой только читает
состояние, каждая карточка fail-safe. Демо показывает корневые URL соцсетей —
осознанное решение GK-15 (фиктивный handle мог бы указать на чужой аккаунт).

Замки: test_deal_card_shell вырос до 18, новый test_share_and_channels (4);
локально 2333 теста зелёные. 31 msgid × 5 каталогов. Стенд Playwright
(hotel + catering): карточка брони — номер и внешний номер одной строкой,
скидка между составом и суммами, статус списком с вопросом об уведомлении,
календарь развёрнут ниже сетки; страница каналов — личные каналы со статусами;
блог — меню каналов с wa.me. Урок среды: `config/settings/_stand.py`
(development без debug_toolbar) — djdt ломает рендер кабинета в стенде.

### 2026-08-26 — DC-8 честный НДС + DC-9 область действия скидки

Решение владельца («да нужен НДС и да нужна») — остаток утверждённых макетов
карточки сделки. План — `docs/dc8-9-vat-discount-scope-plan-2026-08-26.md`.
⚠️ Миграции: `stays/0034`, `booking/0026`, `events/0029`, `orders/0021`.

**DC-8.** Налог выделялся только у заказа и сметы. Немецкие правила требуют
РАЗНЫХ ставок внутри одной сделки: проживание 7 % (§12 Abs. 2 Nr. 11 UStG),
завтрак и прочие допы 19 % (Aufteilungsgebot), Kurtaxe вне НДС — поэтому ставка
живёт на позиции, а не на сделке: `StayUnit.vat_rate` (7 %), `Service.vat_rate`
(19 %), `Event.vat_rate` (19 %) + СНИМОК на сделке (`StayBooking`/`Booking`/
`Ticket.vat_rate`); снимок допа теперь несёт свою ставку, старые снимки без
ключа идут по ставке сделки (fail-safe). Новый `apps/core/vat.py::deal_vat` —
единый калькулятор (заказ остаётся на своём хелпере, точка входа одна), показ —
общий партиал `core/_deal_vat_rows.html` в секции сумм; счета DC-6 берут ставку
из снимка вместо фиксированных 7/19.

**DC-9.** `discount_scope` на четырёх моделях: вся сделка (дефолт) · позиция ·
доставка (заказ). Область меняет не сумму, а РАСПРЕДЕЛЕНИЕ базы НДС и показ:
«на позицию» снимает скидку с базы основной позиции, «на сделку» —
пропорционально долям ставок. Итог сделки одинаков во всех случаях, поэтому
оплата и письма не тронуты (замок сравнивает итог до/после).

Замки: test_deal_card_shell 18 → 24. Осознанно переписаны два снимок-замка,
сравнивавшие dict целиком (правило CLAUDE.md после MX: сверять ПОЛЯ — снимок
дорастает ключами). Локально 2217 тестов зелёные; 1 msgid × 5 каталогов.
Стенд (демо-отель, смешанная бронь): три строки — 24,00 € по 19 %, 143,20 € по
7 %, 3,00 € Kurtaxe без налога, итог 170,20 € = сумма брони.

**DC-8b (следом).** Разбивка честна ровно настолько, насколько верна ставка на
позиции — а задать её владелец не мог ни у номера, ни у услуги, ни у события.
Поле добавлено в три формы ОДНИМ источником: партиал `core/_vat_select.html` +
тег `{% vat_select %}` (правка markup'а меняет поле сразу везде — принцип общих
блоков). Свободного ввода нет: только три законные ставки DACH из
`vat.RATE_CHOICES` (опечатка «1,9» стоила бы неверного счёта), чужое значение
оставляет прежнюю ставку (`vat.parse_rate`). Событие — ModelForm, поэтому там
`TypedChoiceField` + `clean_vat_rate`: пустое поле не сбрасывает ставку (форму
постят мастер и демо); у номера и услуги ту же роль играет presence-guard во
вьюхе. Замки: сохранение + игнор чужого значения + POST без поля не сбрасывает;
отдельный РЕНДЕР-замок (POST-замок не увидел бы забытого `{% load cabinet %}`).

## 2026-08-25 (продолжение) — Pranasy: кейтеринг-карта владельца (49 блюд из PDF)

Запрос владельца: «для сайта pranasy добавь название, описание блюд из PDF и
сделай для этих блюд картинки; добавь блюда в комбо и сделай раздел кейтеринга
как из архетипа кейтеринг; для блюд — перевод на все языки; разбей по
категориям; параллельно разработка — не сломай». План/решения —
`docs/pranasy-catering-menu-plan-2026-08-25.md`.

**Карта.** 49 блюд PDF (название · «Описание» · «Состав») стали немецкой
Speisekarte демо-кита: контейнер-категория `catering` (page_style «sets») с
шестью подкатегориями-Gäng'ами — Suppen 8 · Beilagen 9 · Ragouts 8 · Saucen 7 ·
warme Vorspeisen 7 · Salate 10. Каждое блюдо несёт состав (LMIV), диет-теги,
аллергены, `course` и 7 % НДС. Категории ДОПИСАНЫ В КОНЕЦ `categories`: индексы
`promotions_spec` считают позицию в общем списке созданных товаров, вставка в
середину увела бы акции на чужие товары. Спека `_p` получила параметр
`ingredients=` (состав раньше заполнялся только в кабинете).

**Комбо (раздел «как в архетипе»).** Четыре набора на одном направлении — те же
три режима, что у кита `catering`: фиксированные «Menü Klassik» и
«Ayurveda-Thali», «Menü nach Wahl» с надбавками у опций и «Freie Auswahl»
(free_pool). Плюс подменю шапки «Catering» (Speisekarte · Menüs & Pakete ·
Anfrage), Anfrage-банда на главной, `menu_labels` в прайсе и 4 отзыва на наборы.

**Побочная правка движка.** `combos.pool_products` брал товары СТРОГО из
`combo.category_id` — у направления с подкатегориями собственных товаров нет,
и «свободная сборка» открывалась ПУСТОЙ. Теперь пул = категория + ПРЯМЫЕ дети,
как у страницы категории и фасета (KAT-1). Два замка, включая «внук в пул не
попадает» и «погашенная подкатегория не протаскивает блюда».

**Честная маркировка вместо слогана.** 24 из 49 блюд содержат сливки, панир,
гхи или йогурт — это вегетарианская аюрведическая кухня, а витрина заявляла
«100 % pflanzlich, ohne Ausnahme». Веганизировать рецепты нельзя: ru-локаль
демо — ДОСЛОВНЫЙ текст PDF, немецкая и русская витрины начали бы противоречить
друг другу в составе. Поэтому блюда помечены честно, а два абсолютных
утверждения смягчены (ответ FAQ + знак доверия «100 % Vegan» → «Vegan &
vegetarisch»); hero/about не тронуты — ресторан и шоп в киte действительно
полностью растительные.

**Что нашло адверсариальное ревью карты** (два скептика по всем 49 блюдам,
39 правок): Pinienkerne — НЕ Schalenfrüchte по LMIV Anhang II (ложная буква «h»
отсекала бы аллергиков без причины) · Gemüsebrühe почти всегда с сельдереем →
«selleriefreie Gemüsebrühe» · розничный Asafötida разбавлен пшеничной мукой →
где стоит `glutenfrei`, там теперь «Asafötida (glutenfrei)» (допущение стало
видимым) · Parmesan делается на телячьем сычуге и не вегетарианский →
«Hartkäse (mikrobielles Lab)» · регулируемое «reich an Ballaststoffen»
(EU-VO 1924/2006) убрано · исчез майонез из салата, где его нет в составе ·
«или» больше не выдаётся за факт («Muskatnuss oder Zimt»).

**Фото.** Пайплайн: Openverse (`license=cc0,pdm`) → кандидаты → КАЖДЫЙ кадр
просматривает агент → webp 800×600 <150 КБ → `SOURCES.md`. Правило этой
витрины: никакого мяса и рыбы в кадре (у «caesar» и «bolognese» в стоках это
норма). Честный кадр нашёлся для 32 блюд из 49; остальным оставлен тематический
плейсхолдер. **Грабля резолвера:** 49 ключей образуют плотные префикс-группы
(`ragout-`, `salad-`, `rice-`…), и ключ без своего файла молча получал фото
СОСЕДНЕГО блюда (тыквенное рагу — фото тыквенного супа). Такие ключи
переименованы так, чтобы падать на плейсхолдер; правило держит замок
`test_catering_dish_keys_never_borrow_another_dishs_photo` (обобщённый фолбэк на
generic-фото библиотеки разрешён, фото ДРУГОГО блюда карты — нет). Двум СТАРЫМ
ключам кита catering, которые новые группы перехватили бы (`eggplant,vegan`,
`gratin,potato`), проставлены явные `_PHOTO_ALIASES`. Проверка на дубли по
хешу поймала ещё один дефект: два блюда получили один и тот же кадр.

**Переводы.** de — база, **ru — дословный текст PDF** (он и есть источник
немецкого), en/uk/tr — перевод. Идентичные пары в словарь не пишутся (замок
проекта), «Catering»/«Extras» закрыты inline-диктом в спеке. Покрытие кита:
en 96 % · ru 100 % · uk 100 % · tr 99 %, строк без пути перевода — 0.

⚠️ ops: миграций нет, но демо получает карту только после
`seed_demo_tenants --kit pranasy --recreate`.

## 2026-08-26 — Pranasy: страница `/sortiment/catering/` как полноценная главная

Фидбэк владельца, четыре пункта. План — `docs/pranasy-catering-page-plan-2026-08-26.md`.
Миграций нет.

**1. Фото у подкатегорий.** Подкатегории рисовались ТЕКСТОВЫМИ карточками при
любом шаблоне страницы категории, кроме «kopfbild» — у направления с шестью
Gängen это шесть серых прямоугольников. Теперь общий партиал `_category_tile`
при том же гейте, что у верхнего списка (`subcategories_have_images`): есть фото
хотя бы у одной — плитки, нет ни у одной — прежняя сетка байт-в-байт (у
большинства тенантов `Category.images` пуст). Ключ фото «Beilagen» проставлен
явно (`couscous,semolina`): раньше он резолвился ПРЕФИКС-ГРУППОЙ и молча брал
`rice-peas.webp` — фото блюда «Gemüsereis» с той же страницы.

**2. Виды отображения наборов меню.** Kacheln / Liste / Kompakt — механикой
MEN-22 (class-swap + localStorage), а НЕ серверным `?ansicht=`: полоса наборов
живёт на странице каталога, где `?ansicht=` уже занят блюдами, и не
пагинируется (кап 6), поэтому ре-рендер не нужен. Карточка `_combo_card` одна на
все виды (паритет разметки цел) — вёрстку списочных видов даёт каскад по
`data-cv` в app.css; «Kompakt» собирает строку через flex, отступы `mt-*`
снимаются атрибутной специфичностью (0,2,0 > 0,1,0), без `!important`. Ключ
localStorage свой, поэтому вид наборов не связан с видом блюд; выбор переживает
переход на `/kombi/` (проверено стендом). Общий контейнер — новый
`_combo_grid.html` (реюз: полоса категории, тизер каталога, `/kombi/`).

**3. Недостающие фото блюд.** Генерации изображений в этой среде нет (ключа
модели не существует). Четыре раунда Openverse `cc0,pdm` на 15 блюд дали ДВА
честных кадра — `rice-plain.webp` и `cucumber-raita.webp`; остальные кандидаты
отбракованы по правилам витрины (мясо/рыба в кадре, узнаваемые лица, рекламные
плакаты 1910–50-х, и чаще всего — чужое блюдо: самосы вместо Minzsauce, дал
вместо Kitchari, красная подлива вместо БЕЛОГО Paneer-Ragouts). Поэтому доведён
сам плейсхолдер: реестр эмодзи знал только английские слова, и немецкие ключи
получали общую «тарелку» 🍽️ или ✨. Добавлены темы (`ragout`→🍲, `kohl`→🥬,
`gurke`→🥒, `aubergine`/`brinjal`→🍆, `bulgur`/`groats`→🌾, `raita`→🥛,
`bratling`→🥟, `kitchari`→🍛, `minz`→🌿, `suppe`→🍲, `reis`→🍚), а ключи
переименованы так, чтобы подпись читалась как название блюда («Gekochter» →
«Reis», «Soy» → «Bolognese», «Mahabrinjal» → «Mahabridschal»). Два новых замка:
у каждого блюда карты своя тема (не 🍽️/✨) и подпись складывается из слов его
собственного названия.

**4. Галерея, отзывы, команда на странице кейтеринга.** `page_blocks` получил
хосты вида `catalog:<slug>` рядом с фикс-whitelist'ом — прямое продолжение KAT-1
«категория = страница». Иначе блоки легли бы на ВЕСЬ каталог: у pranasy это
`/sortiment/` и ещё 12 страниц категорий. Четыре гейта билдера (`add_block`,
`use_block_template`, save-цикл `pb_present`, сборка `page_cblocks`) сведены к
одному предикату `siteconfig.is_page_block_host`; страницы категорий добавлены в
`preview_pages` (группа `catalog`) — без этого Studio не могла их открыть, а «+»
на канве слал бы блок не в тот хост. Сами блоки — ССЫЛОЧНЫЕ (UC2-3b), то есть
галерея/отзывы/команда/FAQ берутся из общего контента сайта: второго набора
текстов у страницы не заводится. Демо pranasy: `DemoKit.page_blocks` →
галерея · отзывы · команда · FAQ · заявка.

**Попутно** (видно ровно на полосе «Menü-Pakete»): карточка свободной сборки
печатала «0,00 € / Person» — базовой цены у неё нет, итог складывается из
выбора. Цену такого набора больше не показываем.

Замки: `apps/catalog/tests/test_category_page_2026_08_26.py` (5), +2 в
`test_menu_storefront`, +2 в `test_demo_images`. Стенд Playwright на живом сиде:
плитки категорий с фото, три вида наборов, persist вида на `/kombi/`, пять
блоков страницы с реальным контентом, отсутствие блоков на других категориях.

⚠️ ops: миграций нет; демо получает фото и блоки после
`seed_demo_tenants --kit pranasy --recreate`.

## 2026-08-26 — Волна VAT: ставка НДС на позиции + перестановки карточки

Фидбэк владельца после деплоя: «сделай отображение налога прямо в каждой позиции и в
итоге рассчитывай налог в зависимости от указанного налога во всех архетипах… если у
товара нет настройки налога — нужно добавить в настройках товара или услуги… Gültig bis
и Angebot schicken перенеси в Dokumente… Nachrichten перенеси в правую колонку под
кнопку написать клиенту». План — `docs/vat-per-line-plan-2026-08-26.md`; разведка 12
агентов (6 тем × разведка + скептик).

**VAT-1 (⚠️ миграция `jobs/0017`, аддитивная).** У сметы ставка была ОДНА на документ, а
в строках печаталась она же — выглядело как ставка позиции. `JobLine.vat_rate` nullable
(NULL = «как весь документ»), поэтому существующие сметы считаются байт-в-байт и бэкфилл
не нужен. Новый `apps/jobs/totals.py::quote_totals` — одна точка истины на четыре
поверхности документа (карточка, PDF, публичная страница принятия, счёт); округление ПО
ГРУППЕ ставки, а не по строке. PDF и публичная смета печатают строку на каждую ставку
(§ 14 Abs. 4 Nr. 8 UStG). Ставка подтягивается из карточки товара при привязке строки и
из карточки услуги через пикер (снимки `_service_snapshot`/`_combo_snapshot` расширены
третьим элементом); явный выбор владельца сильнее каталога.

**VAT-2/VAT-3 (⚠️ миграция `catalog/0031`).** Закрыты места, где ставка молча дефолтилась
на 19 %: `Combo.vat_rate` (у набора ставки не было в модели вообще — меню-сет гастро
уезжал в документ по 19 % вместо 7 %), «свободная сборка» блюд, услуга из пикера в
заказе. `Extra.vat_rate` существовал с DC-8, но ни в одной форме — у 100 % допов он был
пуст, и завтрак с парковкой облагались как проживание (7 %) вместо 19 %
(Aufteilungsgebot); поле добавлено с пунктом «wie die Buchung» (пусто = прежняя
семантика). `custom_lines` получили 7-й элемент — ставку; `parse_rate_optional`
отличает «не задано» от «19 %». Демо-отель: у четырёх допов проставлены 19 %.

**VAT-4 — ДЕЙСТВУЮЩИЙ дефект прода, не будущий риск.** `invoice_from_order` считал ВЕСЬ
счёт по преобладающей ставке: заказ 11,90 € @19 % + 10,70 € @7 % давал счёт 23,80 €
вместо 22,60 € (5 % в пользу бизнеса; при обратном составе — в пользу клиента). Это же
ломало сверку платежа в ERP-2 (сопоставление по равной сумме) и Mahnung. `compute_totals`
получила построчную форму (строки без ставки идут по переданной — старые вызовы
байт-в-байт), строки счёта несут `vat_rate` в снимке (JSONField, миграции не нужно), PDF
счёта печатает разбивку, легаси-счета рисуются по снимку `invoice.vat_rate`. Побочно у
счёта из брони: допы считались по ставке проживания, а Kurtaxe добавлялся брутто и всё
равно облагался 7 % — счёт выходил БОЛЬШЕ самой брони.

**CARD-1/CARD-2.** «Gültig bis» и «Angebot senden» — в блоке «Dokumente» (у срока своя
форма; в приёмнике сметы появился presence-guard, иначе сохранение состава стирало бы
дату — класс W0). «Nachrichten» — в рейл под карточку клиента; правка в общем скелете,
поэтому меняется у всех видов сделок, якорь прежний (скролл DF-6 цел).

**Стенд вскрыл пред-существующий баг, невидимый тестам:** в немецкой локали Decimal
рендерится в атрибут как «19,00», а приёмник сметы сравнивал со строкой «19.00» —
совпадения не было НИКОГДА, и любой выбор ставки документа молча сбрасывался в 19 %.
У кейтеринга это значит, что провести смету по 7 % было нельзя в принципе. Чтение
переведено на `vat.parse_rate`; замок шлёт «7,00» ровно так, как браузер.

Уроки: снимок-замки сверять ПОЛЯМИ, а не dict целиком (замок сметы упал на новом ключе —
повтор урока MX); стенд обязателен там, где значение проходит через локаль.

Отложено осознанно: дробление `RevenueEntry` по ставкам — смена `source_ref` обошла бы
unique-constraint и удвоила оборот, нужен гард (отдельный инкремент).

## 2026-08-27 — Фидбэк-батч: карточка клиента, мобильная вёрстка, витрина кейтеринга

Три пакета правок по фидбэку владельца (скриншоты с телефона и десктопа). Миграций нет.

**Карточка клиента и мобильная вёрстка.** Кнопка «написать клиенту» в заказе дублировала
открытый блок переписки ниже — убрана; строка-раскрывашка «Kundendaten bearbeiten»
заменена карандашом у имени клиента (клик по имени или карандашу открывает `<dialog>`,
ссылка в CRM рядом). Состав заказа на телефоне («выглядит ужасно») раскладывается
переносом: шапка колонок скрыта, ячейки адресуются семантическими классами `dl-*`.
Грабля CSS: `.dl-row { display: flex }` при равной специфичности перебивал
`.dl-head { display: none }` — шапка возвращалась; селектор уточнён до `.dl-row.dl-head`
(поймано стендом). Селектор языка кабинета был скрыт на узком экране — форма видна всегда.
Замок `test_cabinet_language_switcher_is_reachable_on_phones` сам себя подловил: срез по
подстроке `set-cabinet-lang` попадал в комментарий рядом, где процитирован прежний класс —
переписан на срез по тегу формы.

**Витрина кейтеринга (pranasy + кит catering).** Пункт «Menüs & Pakete» убран, наборы едут
плитками с фото в подменю «Catering» (`_combo_children`), нижнее меню ведёт на страницу
кейтеринга, а не на форму запроса. Наборы — 4 в ряд без переключателя вида; блюда внутри
набора компактнее (`auto-fill,minmax(220px)` растягивал ЕДИНСТВЕННОЕ блюдо на всю ширину
группы); попап блюда на телефоне раскрыт на весь экран. Две грабли: `normalize` выбрасывал
новый ключ меню `with_combos` (whitelist — классика проекта; наборы не появлялись, а плитки
категорий рядом выглядели правдоподобно), и подпись плитки набора приходила пустой —
у `Combo` имя плоское поле с оверлеем, а не JSON, `get_i18n` давал "".

**Колонки страницы кейтеринга.** Секция наборов переехала в шапку листинга, заголовок
«Einzelne Gerichte» — в тулбар: строка инструментов (фильтры · сортировка · вид · плотность)
стоит вплотную над сеткой блюд, которой и управляет (в шапке заголовок отрывался от своей
сетки плитками подкатегорий и панелью фильтров). Плитки категорий подчинились настройке
владельца: верхний список звал `{% grid_classes site 'categories' %}`, но `site` эта вьюха в
контекст НЕ кладёт — тег молча брал дефолт секции, а сетка подкатегорий была захардкожена
`lg:grid-cols-4`; оба берут раскладку секции «categories», при дефолте строки классов
прежние байт-в-байт. Кит получил ось `section_layouts` (то же поле `layout`, что правит
владелец в конструкторе); pranasy — категории `cols3`, каталог `cols6`.

Найдено попутно: safelist Tailwind отставал от таблиц движка (`_GRID_SM` 1..4, `_GRID_LG`
1..6) — `sm:grid-cols-4` и `lg:grid-cols-6` в CSS не попадали, то есть выбор «6 в ряд» в
конструкторе молча падал в одну колонку у ЛЮБОГО тенанта. Safelist дополнен, CSS пересобран.

Стенд Playwright на живом сиде pranasy: 6/6 (порядок наборы → заголовок → тулбар → сетка,
3 колонки категорий, 6 колонок блюд). ⚠️ ops: `seed_demo_tenants --kit pranasy|catering
--recreate` после деплоя.

---

## 2026-08-27 — I18N-13: гейт «новых непереведённых параметров» + 56 машинных подписей форм

Запрос владельца: «запусти проверку наличия всех полей админки и параметров в файле для
перевода немецкого на другие языки. Можно ли новые непереведённые параметры сразу проверять
и уведомлять?» План — `docs/i18n-guard-plan-2026-08-27.md`.

**Что показала проверка.** Формальный гейт `scripts/i18n_gap.py` зелёный (4838 msgid, все
покрыты; у пяти каталогов ровно по 4795 msgid, наборы совпадают 1:1, в tr/ru/uk пустых
переводов нет). Но у него слепая зона ПО ПОСТРОЕНИЮ: он сверяет с каталогами то, что извлёк
`makemessages`, поэтому не видит (1) необёрнутую подпись — её xgettext не извлекает вовсе, и
«непокрытой» она не считается; (2) `gettext` внутри f-строки — проверено на xgettext, он не
парсит выражения в f-строках. Строк первого класса нашлось 342 в 68 файлах, второго — 4.

**Главная находка — не в каталогах, а в формах.** Прогон всех форм кабинета под
`translation.override("ru")` (после компиляции .mo — без неё измерение врёт, локально .mo не
было вовсе) дал: **56 подписей из 198 — машинные имена полей Django** («Base price», «Is
featured», «Stock quantity», «Starts at»). Такая подпись не переводится НИ на один язык,
включая немецкий: владелец-немец видел в форме товара английские «Base price» и «Sku».
Отрисовку подтвердил пробник по HTML формы товара (30 подписей рендерятся, 12 машинных).
Причина — поля без `verbose_name`/`Meta.labels`; msgid для них в каталогах не было и быть не
могло. Исправлено формами (без миграций): ProductForm, CategoryForm, EventForm, TeacherForm,
PromotionForm, LoyaltyProgramForm, CompanyForm, CustomerForm, BusinessSettingsForm —
подписи переиспользуют существующие msgid (Category/Price/Unit/Currency/Stock/Origin/
Ingredients/Active/SKU/Title/Description/City/Location/Capacity/Website/Phone/Note/Address/
Email/Opening hours/Teachers/Starts at/Ends at/Link/Latitude/Longitude/Recommended/Content),
новых заведено 34 × 5 локалей.

**Ещё пять дефектов того же класса** (подтверждены адверсариальной проверкой, воркфлоу
«forms»): `events/registration.py` — весь реестр анкеты участника (Land, Geburtsdatum,
Notfallkontakt, Ernährung, Allergien …) был плоскими строками, а он виден НЕ ТОЛЬКО в
кабинете, но и посетителю витрины при покупке билета; переведены только `label`, ключи и
`options` остаются базовыми (инвариант «переводится показ, а не запись» — options уезжают в
`Ticket.answers`). `imports/forms.py` — две ошибки валидации файла. `finance/bank.py` —
`kind_label` счёта в «Offene Posten» (соседние сделки перевод получали, строка счёта нет).
`events/forms.py` + `events/landing.py` — немецкая проза в placeholder'ах.

**Дефект per-locale подписей.** `core/i18n_input.py` собирал подпись динамического поля
f-строкой: `f"{opts['label']} ({loc.upper()})"` — из-за этого «Name (EN)», «Titel (EN)»,
«Beschreibung (EN)» печатались по-немецки, хотя переводы для них в каталогах ЛЕЖАЛИ.
Теперь `format_lazy` + сами заготовки подписи обёрнуты. Тот же класс — четыре `gettext`
внутри f-строк (`core/views.py`, `promotions/forms.py`): msgid у них был только по совпадению
с другими местами, вынесены наружу.

**Мелочь того же аудита:** английский msgid `You enter data per language` (подсказка
переключателя языка в формах) не имел немецкого перевода — немцу показывалась английская
строка; добавлен `msgstr`.

**Предохранители (ответ на «сразу проверять и уведомлять») — три уровня, один сканер
`scripts/i18n_untranslated.py`:** находка = строка в позиции подписи (label/help_text/
verbose_name/choices/ключи label|title|hint|caption реестров/`messages.*`/ValidationError)
ИЛИ с немецкими маркерами, которая не обёрнута И которой нет в `locale/de/…po`; плюс
шаблоны (placeholder/title/aria-label/alt и текстовые узлы вне `{% trans %}`) и правило
f-строк. Пункт про `.po` важен: реестры с рантайм-`gettext(var)` (Finder, меню) переводятся
именно так и ложной тревоги не дают. Долг зафиксирован базовой линией
`locale/i18n-untranslated-baseline.json` (297 строк после правок; классы — план §4), гейт
падает ТОЛЬКО на новом. Уровни: PostToolUse-хук `.claude/hooks/i18n-watch.sh` (сразу после
Edit/Write по apps|templates|config, молчит на чистом файле) → `.githooks/pre-commit`
(ставится `scripts/install-git-hooks.sh`) → шаг CI рядом с прежним i18n coverage.
Сводный отчёт одной командой — `scripts/i18n_status.py` (каталоги, расхождение наборов
msgid, английские msgid без немецкого, строки без пути перевода, f-строки, сверка код⇄.po).

**Замки** `apps/core/tests/test_i18n_guard.py` (5): нет новых строк вне базовой линии ·
правило f-строк живое · английский msgid без немецкого перевода · наборы msgid пяти
каталогов совпадают · **каждая подпись поля формы кабинета переводится на ru** (последний
и поймал остаток: CategoryForm.parent/slug/icon/sort_order, CustomerForm.name/phone/note —
их видно только с БД, скриптом-пробником они не инстанцировались).

Проверено: `i18n_gap` ок (4873 msgid), сканер ок, msgfmt --check-format по пяти каталогам ок,
пробник форм 198/198 переводятся. Без миграций. Воркфлоу-аудит из 12 агентов дошёл до
верификации только измерения «forms» — остальные пять упали по недельному лимиту; их
находки (реестры/шаблоны/сообщения) остаются в базовой линии как размеченный долг.

---

## 2026-08-27 (продолжение) — I18N-13, волна долга: 342 → 120 строк без пути перевода

Отмашка владельца: «мержи… доделывай долг и если Воркфлоу не доделан, выполни его
до конца». Ветка влита в `main` fast-forward (main не двигался — конфликтов по
построению нет), CI на обоих коммитах зелёный.

**Воркфлоу-аудит доведён до конца.** Первый прогон верифицировал только измерение
«forms» (пять агентов упали по недельному лимиту); resume догнал остальные пять из
кэша. Итог: **46 находок подтверждено, 26 отклонено** скептиками. Из отклонённых
поучительно одно: `Stornierung` в `Meta.labels` формы события — msgid в `.po` ЕСТЬ
(из письма о билете), но лейбл не обёрнут, поэтому готовый перевод не подхватывался;
формулировка «никогда не попадёт в .po» для этой строки неверна, а дефект реален.

**Что переведено сверх формы-подписей (первый коммит):** карточки шаблонов витрины
и семейств Look (`sitetemplates.py` — 28 подписей + 28 описаний), подписи пресетов
акций (`promotions/presets.py`) и страниц (`core/page_presets.py`), библиотека
блоков билдера (`core/views.py`), карточки мастера (`core/setup_steps.py`), стили
скидки и повтор акции (`promotions/models.py`), ошибки доменов (`tenants/domains.py`)
и SSRF (`core/ssrf.py`), сообщения вьюх каталога/событий/номеров, `Meta.labels`
CategoryForm/CustomerForm, экраны счетов (`templates/finance/*`), плейсхолдеры
блоков канвы (6 шаблонов), JS-подписи редактора меню, `Extra`-мелочи (Rückrufbitte,
Angebot ·, Lager:, Aktion −%, Art.-Nr., Kurtaxe, Frühbucher/Lücken-Deal).
Итого **+228 msgid × 5 локалей** (переводы ручные: EN/RU/UK/TR).

**Отдельный дефект, найденный по ходу: письма собирались в языке КАБИНЕТА.** Тела
писем, которые строятся в Python (а не шаблоном через `_render`), не имели
override локали — обернуть их в `gettext` «в лоб» означало бы, что владелец с
русским интерфейсом отправит клиенту русское письмо. Введён общий хелпер
`apps/notifications/services.email_locale()` (зеркало `_email_locale` из L4), и
шесть писем собираются под `translation.override(email_locale())`: Mehrfachkarte,
Geschenkgutschein, DOI рассылки, персональный код кампании, «Ihre Website ist
bereit», magic-link ЛК бизнеса. Magic-link портала и письмо о конце триала —
там же, но по локали адресата/тенанта.

**Остаток базовой линии — 120 строк, все с причиной** (разбор — план §4): контент
витрины тенанта (hero-тексты шаблонов, `CBLOCK_DEMO_DATA`, тексты блоков пресетов,
названия акций-пресетов — они становятся ДАННЫМИ тенанта и переводятся словарями
`demo_i18n_*`), правовые страницы платформы и Muster-Widerruf, Django-admin
платформы, имена в коде (классы FSM, модели, переменные окружения), диагностика
конфигурации интеграций, машинные форматы (DATEV, CSV склада), примеры формата в
placeholder, ключи ответов анкеты, вывод dev-команд.

**Инвариант, добавленный по ходу ревью собственной правки: СНИМОК — в языке
бизнеса.** Часть подписей не показывается на лету, а СОХРАНЯЕТСЯ в БД:
`StayBooking.auto_discount_label` («Frühbucher −10 %», «Lücken-Deal −10 %»,
«Last-Minute −…», «Aktion −10 % · …») и строки счёта `stay_to_invoice`
(«Übernachtung …», «Kurtaxe (ohne USt.)»). Обернуть их «в лоб» значило бы взять
язык ДЕЙСТВУЮЩЕГО ЛИЦА: одна и та же бронь получала бы разный текст в зависимости
от того, создал её посетитель витрины (его локаль) или владелец в кабинете (его
язык интерфейса), а в счёт — документ GoBD — попадала бы случайная локаль. Поэтому
такие места собираются под `translation.override(tenant_locale())`; хелпер
`apps/notifications/services.tenant_locale()` (бывш. `email_locale`, псевдоним
сохранён для читаемости почтовых колл-сайтов) — единственный источник «языка
бизнеса». Показ, который не хранится (`OrderItem.modifiers_display` с Art.-Nr.,
сообщения, ошибки), остаётся в локали смотрящего.

**Грабли цикла.** (1) `_` как переменная цикла в `stays/services.py` конфликтует с
импортом `gettext as _` (ruff F402) — служебные переменные переименованы, псевдоним
заводить нельзя (замок `test_no_gettext_aliases_in_sources`). (2) Реестры МОДУЛЬНОГО
уровня требуют `gettext_lazy`, а не `gettext` (в `setup_steps.py` рядом живут оба
импорта). (3) Проверка «английский msgid без немецкого перевода» ловила «Warm» и
«Link (URL)» — короткое слово неотличимо от немецкого; добавлен обязательный
английский маркер. (4) Многострочные тела писем в `.po` пишутся с `\n`-экранированием
— скрипт-помощник это учитывает, иначе каталог не компилируется.

Проверено: `i18n_gap` ок (5066 msgid), сканер ок (0 новых), `msgfmt --check-format`
по пяти каталогам, `scripts/i18n_status.py` — все шесть секций чистые, замки
`test_i18n_guard` 5/5. Без миграций.
### 2026-08-26 (вечер) — фидбэк по странице направления + дефект раскладки

Три правки `/sortiment/catering/`: полоса «Menü-Pakete» ПЕРЕД списком категорий
(шаблон «sets»; у остальных положение прежнее), заголовок «Kategorien» у блока
подкатегорий (msgid уже был в каталогах), сетка блюд демо по умолчанию — шесть
столбцов (`page_layouts["catalog"] = "cols6"`; плотность посетитель по-прежнему
меняет контролом «− N +», мобильный — две). Порядок блоков и шесть столбцов
параллельно сделала сессия волны «витрина кейтеринга» (`77b9401`) — при слиянии
взята её, более полная версия (подложка у наборов, заголовок «Einzelne
Gerichte», категории 3 в ряд); от этой ветки в шаблоне остался заголовок
«Kategorien». Посетительский переключатель видов наборов (виден в записи от
26.08) там же СНЯТ по более позднему фидбэку владельца — возвращать не стали.

**Дефект, найденный стендом (не демо-специфичный):** раскладка «6 столбцов»
рисовала ТРИ у любого тенанта. Классы сетки собирает Python
(`grid_class_string`), в шаблонах их литералов нет, поэтому Tailwind знает о них
только из `safelist` — а `lg:grid-cols-6` туда не попал с волны DS-5 и purge
вырезал правило. Серверные тесты этого не видят: разметка верная, нет только
CSS. Тот же дефект независимо нашла параллельная сессия (`77b9401`, 27.08) и
починила safelist — при слиянии взята её правка. От этой ветки остался ЗАМОК
`test_every_grid_class_the_layout_engine_emits_exists_in_the_built_css`: он
сверяет все комбинации таблиц движка с собранным CSS, чтобы следующий пресет не
повторил ошибку молча.
...
### 2026-08-31 (продолжение) — SF-4b (вариант A): промо-цена в каталоге и корзине

Решение владельца «Вариант A» (план `sparfuchs-ux-tz-plan-2026-08-31 §SF-4b`).
Разведка 3 агентами дала карту: цена строки жила в ТРЁХ независимых формулах
(`_line_price` витрины / `create_order:251` / `editing.add_item`), лимит и
аналитика читают один маркер `{"promo": id}` из `OrderItem.modifiers`, а у
SH-волны нашлись дыры (правка qty промо-строки не двигала лимит — двойная
потеря; удаление строки съедало маркер до отмены).

Сделано (БЕЗ миграций): `price_layer.promo_line_price` (применимость: база
строки = base_price; вариант со своей ценой — по полной; опции поверх) +
`attach_promos` (bulk promo/promo_price/promo_badge/promo_lowest; «%»-бейдж
festpreis → «Aktion»). `create_order` (items-путь): батч акций → `claim_units`
в той же atomic (склад-OutOfStock откатывает лимит) → промо-цена + маркер;
исчерпание на чекауте → «Aktionslimit erreicht…». `editing.set_item_qty`
двигает лимит зеркально складу. Витрина: карточка (промо + зачёркнутая база;
`data-price-edit` остаётся на элементе ТОЛЬКО с базой — канва пишет
base_price), §11 PAngV-строка на промо-карточке, деталь buy-box + buybar,
quick-add, строка корзины (чип «Aktion»), related/upsell/wishlist/главная,
прайс-виды — актуальная цена без зачёркнутой. Минимумы доставки чекаута — по
промо-ценам (пересчёт и заказ зовут один хелпер).

Замки: `test_promo_cart` (8) — паритет «карточка = деталь = quick-add =
корзина = заказ», гонка лимита + atomic-откат, дружелюбный чекаут, вариант со
своей ценой, опции поверх промо, editing-зеркала (вкл. «удаление не задваивает
...
### 2026-09-01 (продолжение) — DL-8: доводка до макетов + демо-переключатель на витрине

Фидбэк владельца: «на дизайнах красивее — проверь все», просмотр/индикация
шаблона в админке, кнопка смены шаблона на демо-сайте без входа; вдогонку —
«карточка Deal der Woche занимает весь экран, сделать горизонтальной с двумя
плитками сбоку». Сверка: утверждённые артборды отрендерены в PNG и сравнены
со стендом — отставали фирменные ДЕТАЛИ вариантов. План §9. Всё БЕЗ миграций.

- **DL-8a** ключ `design` ({look, bundle}, presence-minimal, валидация по
  реестрам; ленивый импорт sitetemplates в normalize — циклов нет) — пишут
  apply_look (только look, bundle остаётся) и apply_bundle (оба); страница
  Design: бейдж «✓ Aktiv» + рамка на активной карточке, кнопка «Vorschau»
  (полный stateless-превью в новой вкладке).
- **DL-8b** body `data-sf-look` + CSS-слой фирменных бейджей/цен по макетам:
  prospekt — жёлтый наклонный стикер-процент с жёсткой тенью, чёрная полоса
  «Endet bald», заголовки секций плашкой в акценте; frisch — пастель-пилюля,
  цены в акценте; neon — лаймовые бейджи/цены (через --accent/--accent-ink);
  blatt — бейдж-бокс с рамкой без заливки, прямые углы, карминовые цены;
  smart — красный бейдж-бокс. Только каскад — шаблоны и паритеты целы.
- **DL-8c** spotlight v2 (фидбэк №2): featured-дил — ГОРИЗОНТАЛЬНАЯ карта
  (общий партиал `_promo_featured.html`, реюз в banner) на 2/3 + ДВЕ компакт-
  плитки сбоку (grid stretch, низ сходится — стенд: 1146==1146), с 4-й акции —
  сетка `grid_classes` владельца; растяжной CSS DL-7c умер вместе со старой
  структурой. Замки spotlight переписаны осознанно.
- **DL-8d** deal_prospekt: hero_style → "accent" (красная плита артборда V1).
- **DL-8e** демо-переключатель: `apps/core/demo_switch.py` — is_demo_tenant
  (слаги демо-китов, lru), роут `/design-testen/?tpl=<bundle|standard>&next=`
  (сессия посетителя, прецедент set_language; //host отбит), overlay_bundle_key
  (GET-превью главнее сессии) питает и processor (кожа), и storefront_home
  (композиция); пилюля «🎨 Design testen» (`<details>`-дропап без JS, по центру
  снизу) — ТОЛЬКО на демо-тенантах, в канве превью скрыта. Конфиг тенанта не
  пишется никогда.

Замки: test_demo_switch (3) + design-key/data-sf-look в test_looks (+2) +
Aktiv/Vorschau в test_design_page (+1) + spotlight v2 переписан. Стенд
Playwright: превью prospekt/neon/blatt с фирменными деталями, пилюля
переключает (body look=frisch) и сбрасывает. 2 msgid × 5 каталогов.

### 2026-09-01 (продолжение) — DL-9: шаблоны различаются структурой + правка демо-фото

Фидбэк владельца: «они выглядят одинаково, только цвет меняется — так и должно
быть?» + «проверь картинки, может нужно более подходящие». Замер подтвердил
жалобу: у пяти дил-сборок был ОДИН набор секций (trust+promotions+categories),
ОДИН порядок (из шаблона архетипа) и почти одни стили — менялась только кожа.
Причина конструктивная: сборки строились через `_fokus()`, чья ДНК («одно
действие на экран») жёстко задаёт состав. План §10. БЕЗ миграций.

- **DL-9a** новая ось `sections_order` (порядок блоков главной; стабильная
  сортировка — незаданные ключи сохраняют относительный порядок и уходят в
  конец). До неё порядок задавал только шаблон архетипа.
- **DL-9b** свой `_deal()` вместо `_fokus()`: гасит всё, кроме явно включённого
  (кроме contact) — иначе шаблоны снова сползутся в одну страницу. Композиции:
  Prospekt = баннер-плита → категории → акции → «как это работает» → полоса
  преимуществ · Frischmarkt = баннер-сплит → категории (square) → акции →
  рассказ (about accent) → отзывы → преимущества · Nachtmarkt = АКЦИИ СРАЗУ →
  категории → FAQ (list) → доверие, карточки overlay · Markthalle = шапка по
  центру → индекс-полоса (usp pillars) → акции → ПРАЙС-ЛИСТ товаров → передовица
  → FAQ (numbered) → контакт с картой · Marktplatz = БЕЗ баннера (hero off,
  minimal-шапка) → акции строками → категории → 3 шага → доверие строкой.
  Проверено на стенде: пять РАЗНЫХ страниц (состав секций и порядок отличаются).
- **DL-9c** демо-фото: технически всё было чисто (19/19 картинок главной —
  реальные файлы, заглушек нет; серое на прежних скриншотах = lazy-loading),
  но три кадра врали содержанием — `bread.webp` (сладкая слойка вместо хлеба),
  `dish-soap.webp` (бутылки с молоком), пробел `food-box.webp`. Заменены на
  CC0 (Openverse, лицензия проверена по метаданным, каждый кадр просмотрен;
  кандидат с водяным знаком автора отклонён). AI-кадр `cola-bottle.webp`
  показывал малиновый напиток — генерации в среде нет, поэтому честнее
  переименована сама позиция: «Cola» → «Limonade» (файл `lemonade-glass.webp`,
  SOURCES + 4 словаря демо; порядок ключей словарей сохранён — diff 2 строки).
  Кит aktionsmarkt: 27/27 фото, покрытие переводов 0 строк без пути.
- Замки: «у каждой пары дил-шаблонов различается набор блоков и/или порядок»
  (иначе сползутся снова), sections_order применяется и идемпотентен ×4 сборки;
  замок DL-3 «categories у всех» переписан осознанно (газета несёт прайс-лист).

### 2026-09-01 (продолжение) — DL-10: форма фото на карточках (круглые / широкие)

Фидбэк владельца: «может ещё фото сделать круглыми и ещё отдельно более
горизонтальными». Форма кадра была зашита в разметку (`aspect-square` у промо
и товарных карточек, `aspect-[3/2]`/`aspect-[4/3]` у sellable и плиток
категорий) — владелец не мог её сменить ничем. План §11. БЕЗ миграций.

- **DL-10a** ось `site_defaults.media_shape` ∈ ("round", "wide"),
  presence-minimal ("" = вид из разметки, golden целы). Эмиссия — body
  `data-sf-media`, потребление — CSS по помеченным боксам `data-sf-media-box`
  (селектор по подстроке Tailwind-класса был бы хрупким). Overlay-карточки
  НЕ помечены: там подпись лежит поверх кадра и круг срезал бы текст.
  Круг: квадрат 62 % ширины по центру + radius 9999px, бейдж скидки
  подтянут к краю кадра; wide: 16/9.
- **DL-10b** управление: селект «Fotoform» в области «Тема» билдера рядом с
  «Kartenform» (Save + префилл + живой draft-канал) и ось сборки `media_shape` —
  Frischmarkt получил круглые кадры, Markthalle широкие; остальные без ключа.
- Стенд подтвердил измеримо: круг 181×181 r=9999px · широкий 398×224 ·
  без ключа 396×396 r=0. Грабля цикла: плитка категории рисуется ДВУМЯ ветками
  шаблона (вторая — с фикс-высотой), пометил сначала одну — круглыми стали
  только карточки, категории остались квадратными; поймано скриншотом.
...
### 2026-09-02 (продолжение) — DL-14: «Verteilen» + авто-колонки + «Alle anzeigen»

Фидбэк владельца по скриншотам (pastell/круглые фото): «если товара мало, к примеру 4, а сетка
на 5 — либо сетку на 4, либо растягивать на всю ширину (увеличивать отступ); так же с последними
товарами в списке»; решение — «по рекомендации: Verteilen + авто-колонки»; следом — «при
скрытии на главной карточек категорий/акций/товаров показывать кнопку «показать все» с
переходом в соответствующий каталог». План `docs/dl14-spread-autocols-plan-2026-09-02.md`.
Продолжение DL-11, БЕЗ миграций.

- **Движок.** `_LAYOUT_TAILS` += `spread`; `auto_cols_triplet(triplet, count)` — колонки окна
  клампятся к числу элементов, но не ниже пола (телефон без изменений, планшет 2, десктоп 3 —
  одна огромная карточка на весь десктоп хуже пустоты); `grid_attr_string(count=, more=)` →
  `data-sf-auto="1"` / `data-sf-more="1"` рядом с прежними атрибутами. Классы Tailwind не
  меняются (паритет-замки строк классов целы) — колонки переопределяет CSS по атрибутам.
- **CSS (генератор `scripts/gen_fill_rows_css.py`, замок свежести).** `spread` = `flex-wrap`
  + `space-evenly`, ширина плитки `(100% − (n−1)·gap)/n` через `--sf-n` (по окну из
  `data-sf-cols`) и `--sf-gap` (из gap-классов); полные ряды выглядят как прежняя сетка,
  неполный последний ряд распределяется по ширине. `[data-sf-auto]` →
  `grid-template-columns: repeat(n)`. Trim-правило разделено на две ветки: без кнопки как в
  DL-11; с `data-sf-more` кнопка `.sf-more` — ПОСЛЕДНИЙ ребёнок сетки — не входит в счёт
  ряда (`:not(.sf-more)` + `nth-last-child` сдвинут на 1) и показывается ТОЛЬКО когда trim
  что-то спрятал (`~ .sf-more {display:flex}` от первого спрятанного); под `spread`/`show`
  кнопка и плитка-подсказка скрыты.
- **Витрина.** Все 13 секций-сеток главной передают `count=`; 8 из них (products/categories/
  promotions/events/services/stay_rooms/tours/blog) — `more=True` + партиал
  `_grid_more.html` («Alle anzeigen →» в цвет акцента, `more_url` = роут листинга через
  `{% url … as more_url %}`). Листинги (каталог/категории/подкатегории, `/aktionen/` группы и
  плоский список, `/kombi/`, «похожие» на детали) — дефолт `spread` вместо плитки-подсказки
  (`tail` раскладки страницы: `fill` → плитка есть, как в DL-11); новый тег `sf_grid_attrs`
  для сеток без секции. Лента «Endet bald»: `lg:justify-evenly`. Studio: «Letzte Reihe» +=
  «verteilen». 1 msgid × 5 каталогов.
- **Замки.** `test_dl14_spread` (10): presence-minimal/пол/кламп, атрибуты с count/more,
  CSS-блок = генератор (spread/auto/more-ветки), главная 2 категории при cols4 → `2/2/3`+auto
  + кнопка последним ребёнком → `/sortiment/`, акции → `/aktionen/`, spread по ключу Studio,
  каталог дефолт spread без плитки / cols5 при 4 товарах → `2/3/4`+auto / `fill` — плитка,
  `/aktionen/` группы spread, Save билдера. DL-11-замки (test_fill_rows) целы, парити
  листингов/акций/дизайна 165 зелёных.
- **Стенд (Playwright, кит + deal_pastell × 1440/1100/820/390 × home/sortiment/kat/aktionen).**
  0 проблем по детектору (ширины плиток равны, ничего не вылезает, высоты в ряду равны,
  кнопка ↔ спрятанные карточки согласованы): главная @820 — 8 товаров при 3 колонках →
  6 + «Alle anzeigen → /sortiment/»; «Getränke» 4 товара при 3 колонках → 3 + одна по центру;
  `/aktionen/` группа из 4 → 3 + 1 по центру, «Räumung» 2 → две по центру; авто-колонки
  проверены в браузере инъекцией `data-sf-cols="2/3/4" data-sf-auto` (5 → 4 колонки при 1440,
  3 при 820, 2 при 390). Демо-данные aktionsmarkt (DL-11: ряды кратны колонкам) авто-кламп не
  задевают — он проявляется у живых тенантов с малым ассортиментом.

### 2026-09-02 (продолжение) — DL-15: неполный ряд «по центру + широкая одиночная», Endet bald в 2 колонки, бейдж крупнее

Фидбэк по скриншотам /aktionen/ (prospekt/monochrom): «2 и 1 выглядит не очень; раньше там
был текстовый блок» (= плитка-подсказка DL-11, которую DL-14 заменил на «Verteilen»). Показаны
5 вариантов на стенде (A сейчас · B по центру · C слева · D широкие карточки · E плитка) →
**решение: B + D, E остаётся режимом `fill`**. Следом: «пересей демо, чтоб всё было по ширине;
то, что заканчивается сегодня — в 2 колонки с растянутым фото; скидку в карточках чуть
больше; проверь адаптацию на мобильном». План `docs/dl15-center-wide-plan-2026-09-02.md`,
БЕЗ миграций.

- **B.** `spread`: `justify-content: center` — остаток ряда по центру с обычным зазором
  (полные ряды байт-в-байт). **D.** Ровно одна карточка в последнем ряду (планшет/десктоп,
  n ≥ 2) — на всю ширину и горизонтальная: фото 38 % слева (3:2), текст справа, у карточки
  товара ряд «цена+кнопка» под текстовой колонкой. Чистый CSS: `:nth-child(Nn+1):last-child`
  (у сетки с кнопкой `.sf-more` — `:nth-last-child(2)`), форма меняется ТОЛЬКО у ссылки
  `a:has(> [data-sf-media-box] + div)` (overlay-карточки без тела и скрытая ✎-ссылка
  редактора не затронуты), блок под `@supports selector(a:has(> b))` — без `:has` остаётся B.
  На телефоне — B. Генератор: `solo_selectors`/`wide_rules` (второй потребитель — класс
  `.sf-wide`). **E** не тронута.
- **«Endet bald»** — вместо ленты 4 × w-56: `grid sm:grid-cols-2`, карточки `sf-wide`
  (та же горизонтальная форма, что D; `_promo_card.html` принимает `wide`).
- **Бейдж скидки на карточках** `text-xs px-2.5` → `text-sm px-3` (акции и промо-бейдж
  товара; детальная `size="lg"` как была; per-Look CSS размер не задаёт → наследует).
- **Демо aktionsmarkt:** +3 акции (Butter MHD-Ware, Bergkäse-Anschnitt → Anti-Food-Waste 6;
  Spülmittel → Räumung 3) — группы /aktionen/ кратны 3, «Endet bald» = 4 (2 × 2); переводы
  ×4 словаря. Studio: подпись «Reihen: zentrieren» (ключ `spread` в хранении не менялся).
- **Замки:** `test_dl15_center_wide` (9): center без space-evenly, solo-правила только для
  планшета/десктопа, guard :has и отсутствие голого `> a {`, свежесть блока, /aktionen/ →
  `data-ending-grid` + 2 `sf-wide`, бейдж, кит кратен 3, **«:has() не вложен в :has()»**;
  парити discount_display переписан осознанно; DL-14/DL-11 замки целы.
- **Стенд (Playwright, кит + pastell + prospekt × 1440/820/390 × home/sortiment/категории/
  aktionen): 0 проблем** (центрирование остатка, одиночная = 100 % ширины сетки и
  горизонтальна, «Endet bald» 2 колонки, бейдж 14 px, без горизонтального скролла, ✎-ссылка
  не оживает). **Два дефекта стенд нашёл мимо серверных тестов:** (1) `:has()` ВНУТРИ `:has()`
  — невалидный селектор, браузер молча выбрасывает всё правило (широкая карточка осталась
  узкой) → условия только цепочкой отдельных `:has()`, замок-сканер; (2) у карточки ТОВАРА
  ряд «цена + кнопка» стоит вне ссылки → название висело посередине, цена внизу → корень
  карточки становится grid 38 %/1fr, ссылка `display: contents`, фото на все ряды,
  «название + цена» блоком по центру (фокус-кольцо рисуется на теле). Урок: правила с
  `:has()` гейтить браузером, серверный тест видит только текст CSS.

### 2026-09-02 (продолжение) — DL-16: анализ форм вывода информации (категория · акции · карточки · детали · слайдеры)

Запрос владельца: «дизайн вывода информации в категории, акциях и карточке товара/акции; слайды и
другие виды — проанализируй и дай на согласование». 4 разведки (категория/каталог; /aktionen/ +
карточка/деталь акции; деталь товара + слайдеры + оси Studio; уже принятые решения по этим поверхностям
в 20+ доках) → `docs/dl16-display-variants-analysis-2026-09-02.md` (§0 что есть, §1 пробелы/дефекты,
§2 варианты K/A/P/D/AK/AD/S с механикой и ценой, §3 пакет v1, §5 вопросы) + канвас «Anzeigeformen»
(15 артбордов, исходники `docs/design/anzeige-formen-2026-09-02/_generate.py`). Главные находки: у
акции не показаны gültig ab / окно happy-hour / max pro Kunde / срок резерва / САМА ЦЕЛЬ акции; на
`/aktionen/` поиск без входа в UI; категория без крошек и лайтбокса; ленты без стрелок/точек;
archetype-cover крутится на телефоне; `menu_show_prices=False` не действует на `/sortiment/`
(нет `site` в контексте). Код — ПОСЛЕ отмашки владельца по §5.

### 2026-09-02 (продолжение) — DL-16.1: S1 слайдер-примитив + S4 фиксы (пакет v1 «Anzeigeformen», инкремент 1/6)

Решения владельца по анализу DL-16: пакет v1 целиком · стрелки при наведении · Preis-zuerst
дефолтом. План — `docs/dl16-implementation-plan-2026-09-02.md`. **S1:** любая горизонтальная
лента с `data-sf-slider` получает делегированный примитив (`_slider_script.html`, подключён в
`_base.html`): JS оборачивает ленту в `.sf-slider`, стрелки ‹ › показываются ТОЛЬКО при наведении и
только на hover-устройствах (`@media (hover: hover) and (pointer: fine)`; на таче — свайп + точки),
точки = «страницы» (≤ 12, активная по scrollLeft), клавиатура ← →, полоса прокрутки прячется,
автопрокрутки нет (R5), идемпотентно + `__sfSliderInit` для KAT-5. `grid_attr_string` при
`scroll` отдаёт `data-sf-slider="1"` → секции главной с «Scrollen» получают стрелки/точки даром;
маркер на strip-галерее и «похожих наборах» (MEN-21). **S4:** archetype-cover — гейт мобайла/
reduced-motion + пауза при наведении (зеркало hero DL-13 C6); лайтбокс на фото Kopfbild; `site`
в контексте `/sortiment/` — гейт `menu_show_prices=False` наконец действует на каталоге (раньше
`site.*` молча давал '' — тег `menu_labels_active` был обходом именно этого). Countdown-sm без
фолбэка даты — не дефект: дату показывает чип флагов. Стенд: 1440 — стрелки opacity 0 → 1 при
наведении, клик листает на 90 % ширины, точка 2 активна; 390 — стрелок нет (display:none), 6 точек,
свайп нативный; горизонтального скролла страницы нет. Замки `test_dl16_slider` (7); `test_fill_rows`
переписан осознанно (scroll → атрибут).

### 2026-09-02 (продолжение) — DL-16.2: /aktionen/ — A3 группы-слайдеры · A4 вид «Liste» · A5 поиск, группа, «Sie sparen» (инкремент 2/6)

**A3.** `site_config["promo_layout"]` ∈ {"", "slider"} (presence-minimal, golden целы; панель
«Aktionsseite gliedern» += селект «Darstellung: Raster / Slider»): в режиме slider каждая секция-
группа — горизонтальная лента `data-promo-strip data-sf-slider` (примитив 16.1: стрелки при
наведении, точки, свайп) + «Alle anzeigen →» = тот же URL, что чип группы (тема) или срока
(время: heute/woche; у länger/dauerhaft/«More offers» ссылки нет). Сетка ("") — байт-в-байт
(парити-замок `test_promotions_parity` цел, h2 прежний). **A4.** `?ansicht=liste` — плоская
таблица `_promo_table.html` (фото · Angebot+флаги · Gruppe · Preis (тем же `_discount_display`,
mystery остаётся скрытой) · Sie sparen · Endet), секции и полоса «Endet bald» в этом виде не
рендерятся; тумблер «Karten | Liste» — серверные ссылки `data-ansicht` → KAT-5 меняет вид без
перезагрузки (стенд: `window`-маркер пережил переключение), узкие столбцы `hidden md:table-cell`.
**A5.** Поле поиска на `/aktionen/` (`?q=` вьюха понимала давно — входа не было); карточка акции
+= «Sie sparen X €» (`part="savings" size="sm"`, mystery молчит) и чип группы (`data-promo-group`
— ВНЕ секций своей группы: главная, плоская выдача; внутри секции заголовок уже её называет).
Замки `test_dl16_aktionen` (6). 9 msgid × 5 каталогов (вкл. «Zurück»/«Weiter» стрелок 16.1).

### 2026-09-02 (продолжение) — DL-16.3: деталь акции — AD1 «Gilt für» · AD2 «Bedingungen» · AD3 «Weitere Aktionen» (инкремент 3/6)

Разведка DL-16 показала: цель акции (товар/услуга/номер/комбо), окно happy-hour
(`target_rules`), лимит на клиента и срок резерва на детали НЕ показывались вовсе. **AD1:** карточка
«Gilt für» с фото/именем/ссылкой через контракт `sellable_for(target_kind, target)` (fail-safe;
mystery — без карточки: раскрыла бы товар, класс SF-1). **AD2:** `apps/promotions/rules_text.py` —
`weekday_span` сжимает дни в диапазоны («Mo–Mi, Fr», «täglich»), `conditions_for` печатает окно
«Mo–Mi 10–14 Uhr»; лимит/срок резерва — ТОЛЬКО у reservation-акций (на пути корзины
`max_per_customer` не enforce'ится — граница SF-4b, обещать нельзя). **AD3:** лента «Weitere
Aktionen» (та же группа, иначе прочие активные; ≤ 8; себя исключает) с `data-sf-slider` — раньше
альтернативы были только на 410-странице. Замки `test_dl16_promo_detail` (4); 7 msgid × 5.

### 2026-09-02 (продолжение) — DL-16.4: карточки — AK1 «Preis zuerst» (дефолт дил-сборок) · P1 «Etikett» · P2 листание фото (инкремент 4/6)

**AK1.** `site_defaults.promo_card` ∈ {"", "preis"}: форма карточки акции «цена сверху» (крупная
новая → зачёркнутая → бейдж inline → Grundpreis → Sie sparen, под ним фото 4:3 и название — ALDI/
HOFER/Marktguru, R3); медиа-бокс и части скидки — те же партиалы (mystery остаётся скрытой);
широкая карточка «Endet bald» сохраняет стандартную форму (CSS `.sf-wide` считает фото первым).
**Решение владельца «Preis-zuerst дефолтом»:** ось `promo_card="preis"` в `_DEAL_BASE` → все
11 дил-сборок; fokus-сборки ключ не трогают; кит aktionsmarkt — `promo_card="preis"`. **P1.**
`card_style` += `etikett`: плашка-ценник в углу фото (цена, зачёркнутая, Grundpreis), строка цены
внизу не дублируется; «"" байт-в-байт» цел (замки test_card_style). **P2.** `site_defaults.
card_slider="on"`: медиа-бокс карточки товара с ≥ 2 фото несёт `data-card-imgs="url|url"` —
JS (второй блок `_slider_script.html`) добавляет стрелки при наведении, точки, свайп; клик по
ним не переходит по ссылке карточки; KAT-5 re-init; кит `mode` (CLOTHING) включает. Studio:
селект «Aktionskarte», «Kartenform» += «Preis-Etikett», чекбокс «Fotos auf der Karte blättern»
(билдер: save + контекст). Замки `test_dl16_cards` (6; вкл. «каждая deal_* → preis, fokus — нет»).
4 msgid × 5.
Стенд (mode, overlay-карточки): реальный клик по стрелке УХОДИЛ на страницу товара —
контролы лежали ПОД градиентом-подписью overlay (elementFromPoint = scrim) → z-index 20 + у
overlay точки наверху (`data-cimg-dots="top"`); после фикса клик листает фото, URL прежний.
Сидер: `img` товара принимает СПИСОК ключей (`_product_images`, первое — главное) — четырём
товарам кита `mode` по 3 кадра; aktionsmarkt на стенде: 21 карточка «Preis zuerst», 4 широкие
«Endet bald» стандартной формы; деталь «Gouda jung −20 %»: «Gilt für → Gouda jung 400 g»,
лента «Weitere Aktionen» ×8 со слайдером.

### 2026-09-02 (продолжение) — DL-16.5: страница категории — K1 крошки + Kopfbild-слайдер · K2 «Regale» · K3 «Tabs» (инкремент 5/6)

**K1.** Видимые хлебные крошки «Sortiment › Родитель › Категория» (`data-breadcrumbs`) + JSON-LD
`BreadcrumbList` (у листинга его не было — только у деталей); при ≥ 2 фото категории Kopfbild
становится слайдером (fade, точки, стрелки при наведении через классы примитива 16.1, БЕЗ
автопрокрутки, лайтбокс по клику) — сетка доп. фото под шапкой этим заменена. **K2.**
`Category.page_style="regale"` (реестр `category_styles`, поле без choices → без миграции):
каждая подкатегория — строка «Название · N → Alle anzeigen» + лента ≤ 8 товаров с
`data-sf-slider` (цены акций через `attach_promos`), прямые товары направления — сеткой ниже;
плитки подкатегорий не дублируются, пустое состояние «Choose a subcategory» молчит. **K3.**
`page_style="tabs"`: подкатегории липкими табами со счётчиком (родитель «Alle · N» + дети),
переход — ссылка `data-listing-nav` → KAT-5 fetch-своп без перезагрузки (селектор расширен);
на странице подкатегории показываются табы родителя. Демо: pranasy `shop` (3 подкатегории) →
regale; у «tabs» демо-потребителя пока нет (единственный второй контейнер — pranasy `catering`,
у которого «sets» по решению владельца). Замки `test_dl16_category` (6). 4 msgid × 5.
Стенд pranasy `/sortiment/shop/`: 3 полки со слайдерами, 12 карточек, крошки «Unsere Produkte ›
Shop», BreadcrumbList в head, плиток подкатегорий нет, горизонтального скролла нет (1440/390).
Урок-повтор (класс MEN): негативные замки на голые строки («(1)», `data-cat-dot`) ловят текст
JS — `go(+1)` в скрипте и regex по тегу в замках.

### 2026-09-02 (продолжение) — DL-16.6: деталь товара — D1 галерея · D3 Vertrauensleiste · D4 ленты · D2 «Tabs» (инкремент 6/6 — пакет v1 «Anzeigeformen» ЗАКРЫТ)

**D1.** `_media_gallery.html` (общий для товара/номера/события): на телефоне — snap-лента
кадров со свайпом и точками (`data-media-strip data-sf-slider`, примитив 16.1), миниатюры экран
не занимают, тап по кадру — прежний лайтбокс с этого кадра; на md+ миниатюры СТОЛБИКОМ слева
от большого фото (`data-media-thumbs`; было — ряд из 5 под фото). Обёртка мобильной ленты
`md:hidden` целиком — иначе `.sf-slider` со стрелками оставался бы на десктопе пустым. В
редакторе столбик по-прежнему всегда (пер-слайд 📷/🗑 + «＋»). **D3.** `_trust_strip.html`
под buy-box — только факты из данных: 🏬 Abholung im Laden (модуль orders; «heute bis HH:MM»
при открытом сейчас по структурным часам) · 🚚 Lieferung (`delivery_enabled`) · 🕒 часы сегодня;
ни одного факта → полосы нет. **D4.** Похожие: до 8; лента `data-related-strip data-sf-slider`
ТОЛЬКО когда их больше колонок ряда (`detail_related_layout.cols`, дефолт 4) — в пределах ряда
прежняя сетка (spread/авто-колонки DL-14), явный scroll-режим Studio отдаёт ленту движка.
«Zuletzt angesehen» — **localStorage, не сессия** (отступление от плана осознанное:
`SESSION_ENGINE=cache` писал бы Redis-ключ на КАЖДЫЙ анонимный просмотр — на каждого бота — и
ставил бы куку витрине без действия посетителя, против UX-принципа владельца «без трекинг-кук»;
localStorage уже держит вид сетки/тему/плотность): ключ `sf_seen_products` (≤8 pk), JS дотягивает
карточки фрагментом `/sortiment/zuletzt/?ids=` (`products_recent`: порядок клиента, только
активные, мусор выпадает, пусто → «») и раскрывает блок при непустом ответе; слаг `zuletzt`
зарезервирован (`RESERVED_SLUGS`, роут до `<slug>`). **D2.** `product_detail.layout` ∈
{"", "tabs"} (presence-minimal, `normalize_product_detail`/`product_detail_layout`; Studio —
селект «Aufbau: Untereinander | Tabs» в инспекторе детали, save под `pd_present`): описание /
Kennzeichnung / отзывы — партиалы `_product_description|_product_info|_product_reviews.html`
(разметка 1:1 с прежними inline-блоками); в «Tabs» они уезжают из правой колонки в
`<details data-pd-panel>` тела — аккордеон на телефоне, на md+ JS строит вкладки поверх тех же
панелей (`.pd-tabs-on`, без JS — аккордеон везде), `#bewertungen` открывает панель отзывов
(hashchange + загрузка), ресайз возвращает аккордеон; скрытые в билдере секции панелями не
становятся, пустая Kennzeichnung панели не даёт. Замки `test_dl16_product_detail` (13); парити
`test_product_reviews` (порядок aside→body→wide) и `test_storefront` (сетка похожих из конфига)
целы. 6 msgid × 5. Заголовок похожих остаётся «Mehr aus dieser Kategorie» (честнее «Passt dazu» —
это та же категория; замок парити цел). Стенд Playwright mode/pranasy (1440/390): мобильная
лента + лайтбокс с кадра, столбик миниатюр, полоса доверия «Abholung · heute bis 22:00 ·
Lieferung · Heute 11:00–22:00», табы (3 вкладки, клик/хэш/ресайз), лента похожих 8 карточек
со стрелкой при наведении, «Zuletzt angesehen» после второго товара, 0 ошибок консоли, без
горизонтального скролла. Попутно: замок `test_dl16_promo_detail` считал голое «W2» — случайный
CSRF-токен дал второе совпадение (класс MEN) → маркер `>W2</h3>`.

### 2026-09-02 (ночь, продолжение) — DL-17: доделка DL-16 по фидбэку владельца (цена, карусели, Vorschau, Studio) + DL-18 (демо и макеты)

Фидбэк владельца после сдачи пакета DL-16: «цена куда-то съехала», «в макете была карусель, но я её
не нашёл в демоките», «проверь, чтобы все блоки редактировались из Studio», плюс доделать отложенное
(A1 «Vorschau», демо-потребитель категории «tabs») и добавить три новые формы карточки + тематические
демо продуктового и одежды. План — `docs/dl17-feedback-plan-2026-09-02.md`.

**17.1 «цена съехала» — измерено, а не угадано.** Стенд показал причину: в форме «Preis zuerst» левый
блок содержал цену, Grundpreis И «Sie sparen», поэтому его ширину задавала самая длинная строка
(«Sie sparen 0,60 €» ≈ 123 px) — бейдж отъезжал вправо на эту ширину, а на карточке 240 px переносился
ПОД неё. По макету AK1 плашка цены собрана заново: строка «НОВАЯ цена · statt старая · бейдж» и под ней
одна мета-строка «Grundpreis · Sie sparen», всё на светлой плашке с нижней границей. Две НОВЫЕ части
`_discount_display.html` — `price_line` (порядок макета: новая цена первой) и `grundpreis` (вынесенный
из `price`); существующие части не тронуты, замки парити целы. Ленты групп акций расширены с 240 до
288 px — только там строка цены помещалась целиком.

**17.2 «карусели нет в демо».** Причина не в коде: у кита `aktionsmarkt` не были включены НИ листание
фото на карточке (`card_slider`), НИ ленты групп (`promo_layout="slider"` терялся — ось сборки его не
ставит). Включены обе, товарам добавлены вторые и третьи кадры (14 карточек с листанием, 4 ленты групп).

**17.3 полнота Studio** — см. отдельную запись инкремента (карта разведки: из конструктора нельзя было
настроить `promo_layout`, `promo_grouping`, `Category.page_style`, `card_chrome`, `page_bg`, `tail=fill`;
у `promo_card`, `card_slider`, `variant_style`, `product_detail.layout`, `tail` не было живого превью).

**17.4 A1 «Vorschau» (снятие v1-ограничения).** Запланированная акция (`status="scheduled"`) не
показывалась нигде, а её деталь отдавала голый 404 — из-за этого карточку «Ab Montag» показать было
нельзя (комментарий в `_time_groups` это и фиксировал). Теперь: лента будущих акций на `/aktionen/`
бакетами по НАЧАЛУ («Ab dieser Woche» · «Ab nächster Woche» · «Demnächst», `_upcoming_buckets`),
карточка помечена чипом «ab Mo 07.09.» (новая часть `part="starts"`) и без счётчика/остатка (`preview`),
деталь будущей акции отдаёт 200 с плашкой «Startet am …» и БЕЗ покупки (кнопка, buybar и модалка брони
скрыты). `draft` по-прежнему 404, «scheduled с прошедшим стартом» (beat не отработал) — тоже.
В демо-спеке появился ключ `starts_in_days`. Замки `test_dl17_vorschau` (6).

**17.5 + 18.2 продуктовое демо как настоящий магазин.** Каталог `aktionsmarkt` пересобран деревом:
«Frische» (полки-ленты, K2 regale) → Obst & Gemüse · Backwaren; «Vorrat & Haushalt» (табы, K3 — вид
наконец получил демо-потребителя) → Getränke · Haushalt; «Überraschungstüten»; «Molkerei & Eier»
(шапка kopfbild). Порядок обхода товаров сохранён, поэтому индексы `promotions_spec.product` целы.
Плитка «Getränke» показывала красное вино — заменена. Попутно исправлен ПЛАТФОРМЕННЫЙ дефект: страница
`/sortiment/` брала любую категорию с товарами, поэтому магазин с полками показывал не четыре
направления, а шесть листьев (иерархия витрины пропадала); теперь — корневые категории с товарами в
поддереве, плоские каталоги не изменились (проверено на трёх демо).

**18.3 демо одежды со скидками.** Sale-витрина: +4 акции (в сумме 6 действующих в двух группах +
предпросмотр «Winter-Sale ab Montag»), группы выводятся лентами. Исправлены битые фото, найденные
разведкой: ключей `scarf-*`, `coat-*`, `blazer-*` в библиотеке нет вовсе (товар уезжал в SVG-заглушку),
`boots,autumn` по префикс-группе доставал `autumn-hair.webp` — фото причёски в лукбуке одежды, а
`linen-shirt.webp` — брак набора (на кадре выпечка) и стоял у мужской льняной рубашки. Также починены
индексы `product_reviews`: отзывы «лучшая базовая футболка» и «меринос не колется» сидели на джинсах и
льняной рубашке. Демо-переводы: 20 новых строк × 4 локали (покрытие aktionsmarkt 98/100/100/100,
clothing 97/100/100/100).

**18.1 три новые формы карточки — на согласование.** Канвас «Kartenformen» (`docs/design/
kartenformen-2026-09-02/`): **N1 «Regal»** — ценник как в зале (жёлтая плашка цены на две трети
карточки, фото-опознавательный квадрат; ALDI/Kaufland, электронные ценники); **N2 «Lookbook»** —
кадр 3:4 во всю плитку, спокойная подпись, размеры прямо в сетке (Zalando/COS/Arket); **N3
«Deal-Kachel»** — горизонтальная плитка, где видны цена, выгода, условие, срок и кнопка
(Marktguru/Lidl-App). Плюс сводка «что уже есть» и напоминание про AK2 «Coupon» и AK3
«Countdown-Ring» из прошлого канваса. Ждут решения владельца.

Уроки: спека категории демо-кита позиционная — у категории БЕЗ детей 4-й элемент это ФОТО-ключ, и
описание, попавшее на его место, уехало в `page_style` (страница молча теряла шаблон); два pytest на
одной `--reuse-db` снова дали DuplicateTable (гонять строго серийно).
