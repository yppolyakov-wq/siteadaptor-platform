...
**⚠️ Зависимость L3 (Волна L, 2026-07-01, PR-1):** `booking.Service`/`stays.StayUnit` сейчас **без i18n**
(плоские DE-строки) → адаптер печёт DE-only, а UC2-4 держит спец-случай «service — плоские строки, fail-closed».
**L3 (`docs/multilanguage-wave-L-plan-2026-07-01.md`) даёт i18n-поля Service/Stay и мёржится ДО/ВМЕСТЕ с UA1-3**
→ адаптер читает `*_i18n` для 5 kind единообразно, i18n-асимметрия снимается. Мультиязычность — N локалей (не DE/EN).
