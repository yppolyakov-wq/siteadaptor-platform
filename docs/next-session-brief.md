...
- **SE-2a ✅** переключатель страницы превью в тулбаре (`preview_pages`); каталог на канве
  (`product_list` draft-aware, `data-sf-section="catalog"`, per-page инспектор раскладки);
  категория покрыта тем же `product_list`/шаблоном.
- **SE-2b-1 ✅** лендинги событий/номеров на канве (draft-aware + маркеры + инспектор раскладки
  в общем fieldset «Landing pages»).
- **Дальше:** **SE-2b-2** (порядок/видимость тематических секций детальной события на канве —
  реестр `event_detail` есть, правится на вкладке Pages; нужно: draft-aware `event_detail_order`,
  превью конкретного события в переключателе, on-canvas инспектор секций) → SE-2c (добавить
  категорию→править) → SE-2d (scope «эта страница/весь сайт») → SE-3 (микрошаблоны/шрифты/
  устройства + фон/отступы SE-3d) → SE-4 → SE-5.
...
## Статус merge / deploy
- `main` сейчас: `a210cea` (SE-1c + SE-2a + SE-2b-1 — всё слито, CI зелёный, полный прогон
  1671 passed). Ранее: SE-1a/SE-1b/SE-1f/SE-3d.
- Всё из ветки `claude/storefront-onsite-editor-5rznbv` смержено в `main` (FF). Pending нет.
