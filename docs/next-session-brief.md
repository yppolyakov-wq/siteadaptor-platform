...
- `main` сейчас: `c949379`. **ВЕСЬ ТРЕК SE-2 ЗАКРЫТ** (SE-2a/2b/2c/2d) + CI-оптимизация.
  ...
  **CI-инкремент** (ci.yml concurrency cancel-in-progress + кэш uv/npm — очередь не копится),
  ...
  Pending нет.
- **Активно: SE-3** (Микрошаблоны/шрифты/устройства + остаток SE-3d фон/отступы). Карта разведки
  и порядок — `docs/storefront-onsite-editor-plan.md` «Дизайн SE-3» + scratchpad se3-plan-skeleton.md.
  Порядок (риск↑): SE-3d (фон/отступы) → SE-3b (размер/начертание шрифта) → SE-3a (микрошаблоны)
  → SE-3c (пер-девайс, последним, план-доком). Миграций нет.
