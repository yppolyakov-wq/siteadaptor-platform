...
- **Custom-домен бизнеса — самообслуживание (вопрос владельца 2026-06-11):**
  механика работает уже сейчас (строка `Domain(домен → тенант)` через админку
  Domains + A-запись у регистратора; verify-domain/Caddy выпустят сертификат
  сами). Не хватает self-service: подключение домена в кабинете бизнеса
  (`/dashboard/settings/`) с проверкой DNS и инструкцией — кандидат на отдельный
  инкремент (вместе с canonical-политикой: какой хост главный — субдомен или
  custom-домен).
