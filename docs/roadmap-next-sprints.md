...
- [ ] **C1 — витрина товаров:** публичный каталог на витрине (`/sortiment/` список
  с фильтром по категориям + страница товара), секция «Sortiment» на главной.
  Каталог уже в БД (Sprint 2) — только публичные страницы, без корзины/заказов
  (покупка офлайн, бронь — только у акций).
