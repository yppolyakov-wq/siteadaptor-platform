...
- [ ] **P2.4b — самообслуживание:** кнопка «Продвинуть» в кабинете акции + Stripe
  one-time Checkout + webhook → featured_until; цена/срок — env
  (PORTAL_FEATURED_PRICE_EUR/_DAYS). Делать после D0 (реестр модулей).
  _Статус D0: D0a (реестр `apps/core/modules.py` + `Tenant.disabled_modules`
  tenants/0007 + nav из реестра + ModuleGatingMiddleware) ✅ в `main` (008a3f7,
  CI run 96); D0b (дефолты по вертикали + /dashboard/modules/) ✅ в `main`
  (8dd12f0, CI run 98); D0c (Onboarding-Wizard /dashboard/setup/ +
  Setup-Fortschritt) ✅ в `main` (4a177b4, CI run 100). **D0 завершён —
  P2.4b разблокирован.** Детали — CLAUDE.md §3 «Track D»._
