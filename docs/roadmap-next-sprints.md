...
**D0 Modul-Framework+Onboarding → D1 CRM-lite → D2 Click&Collect → D3 Booking-
календарь → D4 Light-Finance.** Полное ТЗ — `docs/track-d-business-os-spec.md`.
...
notifications, reportlab). Тяжёлый ERP/маркетплейс/дропшип/биржа — отложены
**по продукту/объёму, не по архитектуре** (ревизия 2026-06-11: кросс-тенантный
граф заказов ложится на SHARED-паттерн агрегатора — мягкие ссылки + событийный
синк; одна Postgres-БД даёт атомарность, пока без мульти-региона; см. ТЗ
§«Архитектурная заметка»). **Полное ТЗ:** `docs/track-d-business-os-spec.md`.

- [ ] **D0 — Modul-Framework + Onboarding** (шов под весь Track D; решения
  2026-06-11: реестр+feature-flags, НЕ плагины; полный D0 до D1). Модули —
  включаемые блоки: реестр `apps/core/modules.py` + `Tenant.disabled_modules`
  (tenants/000x; активно = entitlement∩реестр−disabled) + навигация из реестра
  (заменяет 14 хардкод-ссылок) + `ModuleGatingMiddleware` (зеркало
  billing-гейтинга) + дефолты по `business_type` + страница `/dashboard/modules/`
  + пошаговый Onboarding-Wizard (≤5 шагов, «Setup-Fortschritt», переиспускает
  `presets.py`). D0a реестр+гейтинг → D0b дефолты+страница модулей → D0c мастер.
...
- [ ] **D2 — Click&Collect / Заказы-lite** (vision Модуль 11). Заказ товара из
  ...
  онлайн — позже, связь с P2.5). Переиспользует reserve/redeem-движок; UUID+FSM-хуки
  — задел под будущий кросс-тенантный граф заказов. D2a→D2b→D2c.
...
- [ ] **P2.3d** — настройки уведомлений (централизованная отписка).
