...
- **i18n характеристик и FAQ услуги (`Service.attributes`/`faq`)** — у этих полей
  нет overlay-близнеца (`*_i18n`), поэтому в демо на en/ru/uk/tr богатая карточка
  услуги показывает НЕМЕЦКИЕ пункты и вопросы (описание и название переводятся
  штатно). Замечено 2026-07-31 при наполнении услуг отеля; тот же дефект давно у
  werkstatt/handwerker. Фикс: `attributes_i18n`/`faq_i18n` (аддитивная миграция
  booking) + `*_localized` в контракте `SellableEntity` и партиалах детали +
  проход в `demo_i18n.translate_tenant_content`.
- **Группа акции (`Promotion.group`) не переводится** — заголовки секций на
  `/aktionen/` («Zimmer-Angebote», «Pakete») остаются немецкими на всех локалях:
  поле — плоский CharField без i18n. Фикс: либо `group_i18n`, либо перевод через
  общий словарь витрины.
