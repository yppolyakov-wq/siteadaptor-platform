...
### Realtime-чат: SSE вместо поллинга (отложено 2026-06-25)
Сейчас чат (`apps.inbox`, M22b) живой через **поллинг** (тред 5с, бейдж 20с, опц.
typing через Redis-кэш) — работает, без инфра-рисков. **SSE (Server-Sent Events)**
дал бы мгновенную доставку и меньше запросов, НО на синхронном Django (gunicorn)
каждое открытое SSE-соединение держит воркер → несколько вкладок чата исчерпают
пул воркеров и положат сайт. **Это инфра-задача, не фича-инкремент.** Что нужно:
ASGI-стек (uvicorn/daphne) + async-вьюха стрима ИЛИ отдельный пул воркеров под
长-poll/SSE; настройка Caddy (отключить буферизацию для `text/event-stream`,
таймауты); backpressure/лимит соединений на тенанта; фолбэк на поллинг, если SSE
недоступен. Делать ОТДЕЛЬНО, спланированно (не в автономном цикле). Поллинг —
достаточный baseline до этого.
