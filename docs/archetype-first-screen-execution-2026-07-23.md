...
### E5 — «Слой прогресса»: пошаговое ведение клиента (Schritt N/M) ✅
- **Идея владельца** («ведём клиента последовательно») в honest-варианте: не
  оборачивать всё в визард, а сделать СУЩЕСТВУЮЩИЕ воронки ВЕДОМЫМИ — виден
  прогресс, ясно «сколько осталось». Универсально в плюс, без замедления
  уверенного покупателя (шаги остаются реальными страницами).
- **Механизм:** `apps/core/funnels.py` (реестр шагов FUNNELS: service/stay/event/
  order, плоско, i18n) + inclusion-тег `{% funnel_steps kind N %}` (siteui) +
  партиал `_funnel_steps.html` (степпер done/current/upcoming, акцент темы
  --sf-accent, aria-current, пусто при невалидном шаге).
- **Вшито (флагманы E4/friseur):** воронка **stay** (index→detail→confirm = 1/2/3)
  и **service** (index→slots→confirm = 1/2/3). События — отдельно (у них свой
  in-page 2-step чекаут, гранулярность иная).
- **Замки:** test_funnels (состояния/вне-диапазона/рендер тега), stays+booking
  render (331 зелёных). 8 msgid → 4 .po. Витринный UI-слой — golden/classic/
  миграции не затронуты.
- **Тюнинг:** степпер на листинге (шаг 1) — самый слабый по смыслу; при желании
  владельца легко снять (убрать тег из *_index).

### E5b — «Умные дефолты»: каждый шаг = одно решение ✅
- **Даты предзаполнены** (Anreise=завтра, Abreise=+2 ночи) в hero-виджете отеля,
  секции поиска и на листинге `/unterkunft/` — «просто нажми Suchen». Тег
  `{% default_date N %}` (siteui, `localdate()+N`). ВАЖНО: это лишь значение
  инпута — поиск НЕ запускается автоматически, реальный ?von/?bis в приоритете
  (searched-логика не тронута → без ложных прогонов).
- **Ближайший слот подсвечен** кольцом в слот-пикере записи (глаз идёт туда,
  один тап), пока ничего не выбрано — без авто-выбора (не презумптивно).
