...
**Фикс (вариант A — fail-safe, дополнен по ревью):** не кэшировать ответ, если
`request.META.get("CSRF_COOKIE_NEEDS_UPDATE")` (Django 5.1: `get_token()` ставит
его всегда, `csrf.py:111`) ИЛИ `response.cookies` непуст ИЛИ сессия изменена
за рендер (`session.modified` — куку sessionid ставит middleware ПОЗЖЕ
декоратора, в `response.cookies` её нет) ИЛИ добавлены flash-сообщения ИЛИ
ответ — неотрендеренный `TemplateResponse`. На хите — `Vary: Cookie`
(симметрично промаху). Старый формат записи → промах, не распаковка. Кэш-хит ничего не
...
**Цена (уточнена скептиком):** кэшируемых вьюх всего четыре — `storefront_home`
(витрина) и `portal_home`/`discover_index`/`city_listing` (агрегатор). Из них
после фикса остаются в кэше только те рендеры, где в разметке нет формы с
токеном (главная кита без quick-add/Merkzettel, портал без карточек); страницы
с формами — состояние до SE-5a. Сквозной замок «кэш жив там, где должен, и
мёртв там, где должен» — в `test_portal_pages`.
