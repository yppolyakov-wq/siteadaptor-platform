...
## 9. DL-8 Второй фидбэк-батч (2026-09-01): «на дизайнах красивее» + демо-переключатель

Сверка с утверждёнными артбордами (отрендерены в PNG, бок-о-бок со стендом):
идентичность и композиция сходятся, отстают ДЕТАЛИ — фирменные бейджи/цены
вариантов (V1 жёлтый наклонный стикер · V2 пастель-пилюля и зелёные цены ·
V3 лаймовые бейджи и цены · V4 рамка-бокс без заливки и карминовые цены),
hero V1 = красная плита (у нас сплит с фото), spotlight-карточка крупнее
макетного featured-дила (второй фидбэк: «занимает весь экран, глаз режет»).

- **DL-8a** ключ `design` в normalize (presence-minimal {look, bundle};
  валидация по реестрам) — пишут apply_look/apply_bundle; страница Design
  показывает бейдж «Aktiv» на активной карточке + ссылку «Vorschau öffnen»
  (полный превью в новой вкладке). Golden целы (ключ только заданным).
- **DL-8b** body `data-sf-look="<family>"` (из design.look; в превью — из
  оверлея) + CSS-слой per-Look в app.css: бейджи скидки и цены карточек по
  макетам (prospekt: жёлтый наклонный стикер, чёрная endet-полоса, заголовки
  секций плашкой; frisch: пастель-пилюля + цены акцентом; neon: лайм-бейджи +
  цены акцентом; blatt: бейдж-бокс с рамкой без заливки, прямые углы, цены
  акцентом; smart: скруглённо-квадратные красные). Шаблоны не трогаем —
  только каскад (паритет-замки целы).
- **DL-8c** (фидбэк №2) spotlight v2: featured-дил — ГОРИЗОНТАЛЬНАЯ карта
  (реюз разметки banner-карты, 2/3 ширины) + справа колонка из ДВУХ компакт-
  плиток (фото-строки), остальные — обычной сеткой; растяжной CSS DL-7c
  умирает вместе со старой структурой. Замки spotlight переписываются
  осознанно.
- **DL-8d** сборка deal_prospekt: hero_style "split" → "accent" (красная
  плита V1).
- **DL-8e** демо-переключатель шаблона НА ВИТРИНЕ без входа: session-оверлей
  (тот же read-only механизм превью) + роут `?design-testen=<bundle|standard>`
  и плавающая пилюля «🎨 Design» со списком сборок — ТОЛЬКО на демо-тенантах
  (гейт по слагам демо-китов); конфиг тенанта не пишется, выбор живёт в
  сессии посетителя.
- **DL-8f** стенд: скрин-сравнение всех 5 после доводки + проверка кнопки.

## 10. DL-9 «Шаблоны различаются структурой, а не только цветом» (фидбэк 2026-09-01)

Владелец: «они выглядят одинаково, только цвет меняется — так и должно быть?».
Замер подтвердил: у всех пяти дил-сборок ОДИН набор секций (trust+promotions+
categories), один порядок (из шаблона архетипа) и почти одни стили — различались
hero (3 значения), categories (square/compact), promotions (spotlight/rows),
nav у blatt и catalog_layout. То есть менялась «кожа», а страница оставалась той же.

**Причина конструктивная:** дил-сборки строились через `_fokus()` — ДНК «одно
действие на экран» жёстко ставит trust и гасит остальное. У дил-шаблонов своя
ДНК (акции — главный контент), поэтому им нужен свой базовый набор.

- **DL-9a** новая ось сборки `sections_order` (порядок блоков главной; сортировка
  стабильная — незаданные ключи сохраняют относительный порядок). До неё порядок
  задавал ТОЛЬКО шаблон архетипа, и сборка не могла его изменить.
- **DL-9b** helper `_deal()` вместо `_fokus()` для пяти дил-сборок + разная
  композиция по артбордам (проверено: каждая включаемая секция гейтится по
  контенту, у пустого тенанта пустых блоков не появится):
  - **Prospekt** (плакат): hero-плита → категории → акции → «как это работает»
    (row) → полоса преимуществ (compact). Без about/trust/FAQ.
  - **Frischmarkt** (тёплый рынок): hero-сплит → категории (square) → акции →
    рассказ «о нас» (accent-панель) → отзывы (quotes) → преимущества (plain).
  - **Nachtmarkt** (дил-хантер): АКЦИИ ПЕРВЫМИ (после hero) → категории → FAQ
    (list) → доверие (compact); карточки overlay (фото во всю плитку).
  - **Markthalle** (газета): hero-сплит + centered-шапка → индекс-полоса
    (usp_bar pillars) → акции → прайс-лист товаров (preisliste) → передовица
    (about plain) → FAQ (numbered) → контакт с картой (map_first).
  - **Marktplatz** (маркетплейс): БЕЗ баннера (hero off, minimal-шапка) → акции
    строками → категории → 3 шага (minimal) → доверие (plain); карточки compact.
- **DL-9c** фото: технически чисто (19/19 картинок главной — реальные, заглушек
  нет; серое на скриншотах = lazy-loading), смысловая сверка пар «акция → фото»
  и замена неподходящих — по результату осмотра.
- Замки: «у каждой пары сборок разный набор секций ИЛИ порядок» (иначе шаблоны
  снова сползут в одинаковые), sections_order применяется и переживает normalize.

## 11. DL-10 «Форма фото на карточках» (фидбэк 2026-09-01)

Владелец: «может ещё фото сделать круглыми и ещё отдельно более горизонтальными».
Сейчас форма кадра зашита в разметку: `aspect-square` у промо/товарных карточек,
`aspect-[3/2]`/`aspect-[4/3]` у sellable и плиток категорий — владелец её не
меняет ничем.

- **DL-10a** новая ось `site_defaults.media_shape` ∈ ("round", "wide"),
  presence-minimal ("" = нынешний вид, golden целы). Эмиссия — body
  `data-sf-media`, потребление — CSS-каскад по помеченным медиа-боксам
  (`data-sf-media-box` на обёртках; селектор по подстроке Tailwind-классов был
  бы хрупким). Overlay-карточки исключены: там подпись лежит поверх кадра и
  круглый кроп срезал бы текст.
- **DL-10b** управление: селект «Fotoform» в области «Тема» билдера рядом с
  «Kartenform» (hidden round-trip + live-draft, класс W0/W6), плюс ось сборки
  `media_shape` — Frischmarkt получает круглые кадры (рынок «по-домашнему»),
  Markthalle широкие (газетная полоса), остальные — как было.
- **DL-10c** замки: presence-minimal + рендер атрибута + Save билдера не роняет
  ключ + сборки применяют ось; стенд — три формы бок о бок.
