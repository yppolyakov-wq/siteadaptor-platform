...
- ✅ **AB3** мастер онбординга v2 — прогресс-бар + язык задач уже были; добавлено **живое
  превью** витрины (липкий iframe «/» справа от шагов, same-origin framing; обновляется после
  каждого шага) на контент-шагах 2–6 + кнопка «Website öffnen». Демо-дефолты (load_demo) — на
  шаге 6. Тесты `test_onboarding_wizard`. Без миграций.
- ✅ **AB4** чек-лист готовности сайта на дашборде — `onboarding.completeness(tenant)` считает
  готовность по реальному наполнению (баннер/часы/контакты/первый товар/Impressum) →
  карточка «Dein Site ist zu X% fertig» с прогресс-баром и пунктами-ссылками на действие.
  Тесты `test_onboarding`. Без миграций.
- ✅ **AB5** связать регистрацию → мастер — redirect-gate в самой вьюхе `dashboard` (без правок
  signup/provisioning): нетронутый мастер (шаг 1, без пропусков, не завершён) → редирект на `setup`;
  любое действие в мастере или его завершение снимает гейт, остальная навигация не гейтится.
  Тесты `test_onboarding_wizard` (свежий → 302; тронутый/skipped/completed → 200). Без миграций.
Порядок AB1→AB5; AB1–AB4 низкий риск (UI/контекст/хелперы), AB5 — отдельно с тестами.
**Спринт G (анти-Битрикс кабинет/онбординг) закрыт: AB1–AB5 ✅.**
