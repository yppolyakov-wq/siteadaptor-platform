...
- 🚧 **Визуальный календарь наличия (M–L, сильнейший рычаг)** — план-док готов:
  `docs/hotel-availability-calendar-plan.md` (server-rendered месяц-сетка + HTMX, без
  новых моделей; инкременты C1 данные → C2 вьюха/партиал → C3 выбор диапазона → C4 демо/embed).
...
- Дальше: food-hero пресет (S) · видимость Kombo/Tagesgericht (S) · диет-иконки/фильтр (S–M) ·
