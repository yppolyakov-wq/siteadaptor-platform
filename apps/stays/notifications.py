...
def enqueue_stay_email(booking, event):
    ...
    email_on = channel_enabled(tenant, "customer", "stay", event, "email")
    ...
    if (
        template_base
        and customer
        and channel_enabled(tenant, "customer", "stay", event, "telegram")
    ):
    ...
    if event == "created":
        ...
        if owner and channel_enabled(tenant, "owner", "stay", "created", "email"):
        ...
        if channel_enabled(tenant, "owner", "stay", "created", "telegram"):
            from apps.telegram.notify import send_to_owner

            subj_o, body_o, _h = _render("stay_owner", {**ctx, "unsubscribe_url": ""})
            send_to_owner(
                tenant,
                type="stay_created_owner",
                dedupe_key=f"stay:{booking.id}:created:owner:tg",
