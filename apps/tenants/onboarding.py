...
def mark_complete(tenant) -> dict:
    """Пометить мастер ПРОЙДЕННЫМ целиком (решение владельца 2026-08-19: демо-киты
    мастер пропускают).

    Свежезасеянный демо-тенант попадал не в кабинет, а в мастер: AB5-редирект
    срабатывает на «нетронутом» состоянии, а сидер онбординг не трогал. У кита
    сайт уже наполнен (компания, оформление, предложения, тексты, право), поэтому
    честное состояние — завершённый мастер, а не «тронут и брошен» (`leave`):
    иначе на главной висел бы чек-лист готовности с пропущенным первым шагом.
    """
    state = get_state(tenant)
    state["step"] = _FINAL
    state["done"] = _ordered(set(visible_keys(tenant)) - {_FINAL})
    state["skipped"] = []
    state["completed"] = True
    save_state(tenant, state)
    return state
