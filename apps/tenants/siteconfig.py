...
def apply_legacy_promo_slider(layout, legacy):
    """LAY-7c: легаси-ключ `promo_layout` включает ту же ленту, что ось вывода.

    Решение владельца 2026-09-10: «сеткой или лентой» управляет ОДИН контрол —
    ось вывода. Отдельная настройка `promo_layout` (DL-16.2 A3) убрана из панели,
    но продолжает читаться: её пишут конфиги живых сайтов и демо-киты, а во вьюхе
    её же выставляет композиция «Regale». Здесь она превращается в `scroll`, и
    дальше существует ОДИН путь рендера. Без легаси-ключа раскладка не трогается
    (в т.ч. остаётся пустой, если владелец ничего не задавал).
    """
    if normalize_promo_layout(legacy) != "slider" or (layout or {}).get("scroll"):
        # Возвращаем ВХОД как есть (в т.ч. None): вызывающий отличает «правило не
        # сработало» по тождеству. Подмена None на {} однажды уже стоила паритета —
        # пустая раскладка материализовала дефолтные классы поверх композиции.
        return layout
    return normalize_layout({**(layout or {}), "scroll": True})
