...
#: Листинги, которым композиция объявлена НЕ будет, и почему. Пусто — значит все
#: девять поверхностей волны закрыты (d3 свёл последние пять на каркас
#: `listing.html`; до этого они наследовались от `_base.html`, и хром рисовать
#: было некому). Держим словарём, а не молчаливым отсутствием: причина «страница
#: не на каркасе» через полгода уже не восстанавливается.
PENDING_SURFACES: dict[str, str] = {}
...
EXCLUDED_REASONS = {
    ...
    # STU-18d: страница поездок УЖЕ группирует их по странам (MT-D2: секции +
    # чипы-якоря). Вкладки, полки и указатель дублировали бы встроенную
    # группировку, а «Навигатор» увёл бы в колонку то, что и так есть на странице.
    # Обложка и обычная сетка остаются.
    ("tours", "tabs"): "страница уже группирует поездки по странам (MT-D2)",
    ("tours", "regale"): "страница уже группирует поездки по странам (MT-D2)",
    ("tours", "kompakt"): "страница уже группирует поездки по странам (MT-D2)",
    ("tours", "navigator"): "страница уже группирует поездки по странам (MT-D2)",
}
