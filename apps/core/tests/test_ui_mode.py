...
# --- скрытие продвинутого в Простом (W-CL: носитель — хаб-табы, не classic-сайдбар)
def _tabs_html(hub, tenant, nav="settings"):
    from django.template import Context, Template

    return Template('{% load cabinet %}{% hub_tabs "' + hub + '" %}').render(
        Context({"nav": nav, "request": _SN(tenant=tenant)})
    )


def test_hub_tabs_show_advanced_in_expert(db, settings):
    settings.ROOT_URLCONF = "config.urls_tenant"
    html = _tabs_html("settings", _t())
    assert "Finanzen" in html and "Auswertungen" in html


def test_hub_tabs_hide_advanced_in_simple(db, settings):
    settings.ROOT_URLCONF = "config.urls_tenant"
    html = _tabs_html("settings", _t({"ui_mode": "simple"}))
    assert "Finanzen" not in html
    assert "Auswertungen" not in html
    assert "Zahlung" in html  # обычные табы остаются


# --- S6b: скрытие нерелевантных архетипу каталожных табов в Простом ----------
def test_simple_hides_catalog_tabs_for_service_archetype(db, settings):
    settings.ROOT_URLCONF = "config.urls_tenant"
    html = _tabs_html("sellables", _t({"ui_mode": "simple"}, business_type="friseur"))
    assert "Produkte" not in html and "Lager" not in html
    assert "Angebote" in html  # обзор продаваемого остаётся


def test_expert_keeps_catalog_tabs_for_service_archetype(db, settings):
    settings.ROOT_URLCONF = "config.urls_tenant"
    html = _tabs_html("sellables", _t(business_type="friseur"))
    assert "Produkte" in html


def test_simple_keeps_catalog_tabs_for_werkstatt(db, settings):
    settings.ROOT_URLCONF = "config.urls_tenant"
    html = _tabs_html("sellables", _t({"ui_mode": "simple"}, business_type="werkstatt"))
    assert "Produkte" in html  # Werkstatt продаёт Teile


def test_simple_hidden_modules_registry():
    assert modules.simple_hidden_modules(_t({"ui_mode": "simple"})) == frozenset(
        {"finance", "analytics"}
    )
    assert "catalog" in modules.simple_hidden_modules(
