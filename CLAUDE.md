...
2. **M20 — Site Builder: ✅ аудит проведён (2026-06-23).** Слои M20a–f были готовы;
   аудит критичных багов не выявил (сохранения переживают через `normalize`).
   Добавлено: адаптивный билдер (переключатель ширины предпросмотра + таб
   Editor/Preview для планшета/мобайла) и **переработка кабинета под нативность**
   — мобильный таб-бар (`nav_primary`), поиск по меню в сайдбаре, липкая шапка +
   ссылка «Website». План/детали — **`docs/m20-site-builder-plan.md`**.
