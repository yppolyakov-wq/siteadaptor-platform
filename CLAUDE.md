...
- **Самое свежее (2026-08-04): запрос владельца — архетип «мото/квадро-туры Индия/Непал»
  (семейство MT) — ПЛАН-ДОК на обсуждении.** Разведка 4 агентов: ~70% уже в коде (Event =
  тиры/депозит/рассрочка/waiver/анкета/QR/проживание-к-билету; блог CM-1; magic-link ЛК;
  apps/secrets; CRM 360°; Leaflet). Строить: itinerary по дням+карта, SecureDocument
  (шифрование файлов+proxy-выдача — сейчас ЛЮБОЙ файл публичен по URL), Reise-Feed
  (тур-спейс: inbox строго 1:1, вложений нет), SupplierBooking (исходящих броней нет),
  ExpenseEntry (M13 не начат), чек-лист. Волны MT-1..MT-6 + 8 развилок владельца —
  `docs/moto-tours-archetype-plan-2026-08-04.md §5`. Код НЕ начат — ждём решений.
