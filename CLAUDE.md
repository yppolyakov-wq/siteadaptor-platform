...
- **Самое свежее (2026-09-03, вечер): ВОЛНА STU «переработка Студии» ИСПОЛНЕНА ЦЕЛИКОМ
  ...
  **STU-7 ✅ (2026-09-04):** записанное «ограничение v1» оказалось ДЕФЕКТОМ шире — query
  ...
  страницу без группы — класс `/galerie/`); замер вместо догадки — 70 из 70 проверенных
  страниц (по 40 ссылок с главной двух демо) тип сообщают, исключение только кастомный 404 (standalone НАМЕРЕННО, SF-3) → пустой тип =
  ...
  **Ревью ветки нашло два своих дефекта:** `urlsplit` БРОСАЕТ `ValueError` на `//[`
  ...
  в лимит агентов, покрытие неполное.
