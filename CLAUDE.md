...
- **Самое свежее (2026-07-11): старт трека AB6/AB7 «анти-Битрикс v3» — план-док готов, код НЕ начат.**
  ...
  демо обогащаем (фото+баннер+меню/тексты) · **старт кода только после обсуждения каждого слайда
  (AB6.0 — идёт)**. Ядро: единый реестр `SETUP_STEPS` (питает рельсу мастера, чек-лист AB4-фасадом
  и бейджи плиток AB7), state v2 в opaque-ключе `onboarding` — БЕЗ миграций и БЕЗ правок golden.
  SOURCE OF TRUTH — `docs/master-slides-v3-plan-2026-07-11.md` (карта слайдов §3 — материал
  обсуждения; открытые вопросы помечены ❓).
