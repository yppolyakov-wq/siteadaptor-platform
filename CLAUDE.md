...
- **Самое свежее (2026-08-06/07): АУДИТ ДЕМО-КОНТЕНТА → ST-8 (страницы витрины) + акции
  ...
  Именно этот инвариант ловит весь класс «узел молча выпал». ⚠️ ops: `seed_demo_tenants
  --recreate` после деплоя. **Попутно исправлено в CLAUDE.md:** §7 помечен архивом (порядок
