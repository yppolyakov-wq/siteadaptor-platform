...
# создавая по демо-сайту на субдомене <kit>-demo.<base> (apply_kit). Запуск:
#
#   scripts/seed-demos.sh                 # пересоздать демо по всем китам
...
ARGS="${*:---recreate}"  # по умолчанию --recreate (пересоздать всё)

echo "▶ Seeding ALL demo kits ($ARGS) …"
